...
React + TypeScript + Vite on Cloudflare Pages (`apps/web`), Hono + Zod REST API on Cloudflare Workers (`apps/api`), Cloudflare D1 for storage (`migrations/`, ordered SQL). Shared code: `packages/domain` (types, enums, public IDs), `packages/i18n` (简繁 conversion/folding), `packages/validation` (Zod schemas, kinship + date rules). Maintenance scripts in `scripts/`, Playwright smoke in `e2e/`. No object storage (no images/attachments by policy).
