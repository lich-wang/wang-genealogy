---
schema: wang-person/v1
id: p_Ef5VE9CLFhhmpDq6fdSAuK
status: active
merged_into: null
display_name: 王象艮
cbdb_id: 235529
revision: 3
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_LftJVUv4yM7KDPqGnQ2NFJ
        subject_person_id: p_Ef5VE9CLFhhmpDq6fdSAuK
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王象艮，明人物。籍贯新城，入仕廩生。（中国历代人物传记资料库 CBDB 235529）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 7
      sources:
        - id: cs_TTzJyoEUdalnNhEEIH0_up
          claim_id: c_LftJVUv4yM7KDPqGnQ2NFJ
          source_id: s_QQAM92PBm8hwftGDAqukbH
          stance: supports
          locator: CBDB:235529
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_QQAM92PBm8hwftGDAqukbH
            source_type: api_record
            title: 中国历代人物传记资料库：王象艮（CBDB 235529）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=235529&o=json
            external_identifier: CBDB:235529
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:58.856Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_e3LUEc7sqDKwPnEaog7R1s
        subject_person_id: p_Ef5VE9CLFhhmpDq6fdSAuK
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王象艮
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_1XEPt3wojPPXPaJu9GkYMQ
          claim_id: c_e3LUEc7sqDKwPnEaog7R1s
          source_id: s_QQAM92PBm8hwftGDAqukbH
          stance: supports
          locator: CBDB:235529
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（2801-2900）｜历史性依据：CBDB 朝代 = 明
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_aj_GJWToLMOiQSDwxzYZot
        subject_person_id: p_5r1wESpKqnWgbhR5AChbjE
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_Ef5VE9CLFhhmpDq6fdSAuK
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_WXXqXQqbHL63T34o3rstBm
          claim_id: c_aj_GJWToLMOiQSDwxzYZot
          source_id: s_UVI6nsp6bioER0eTAtdn86
          stance: supports
          locator: CBDB：兄弟 王象春（126731）之父／母 王之猷
          quotation: null
          interpretation_note: 由兄弟关系推断：王象艮 与 王象春 为同胞（CBDB 记「弟」），王象春 之父／母即 王象艮 之父／母。
          source:
            id: s_UVI6nsp6bioER0eTAtdn86
            source_type: api_record
            title: 中国历代人物传记资料库：王象艮（CBDB 235529）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=235529&o=json
            external_identifier: CBDB:235529
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_5r1wESpKqnWgbhR5AChbjE
        status: active
        display_name: 王之猷
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_nHxCCJkIJAWkDyLUew5hpG
        subject_person_id: p_1vwHH8EjSEP1q2oifpwF8P
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_Ef5VE9CLFhhmpDq6fdSAuK
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_xeKjBaL6oGt6qxehwzEyWm
          claim_id: c_nHxCCJkIJAWkDyLUew5hpG
          source_id: s_UVI6nsp6bioER0eTAtdn86
          stance: supports
          locator: CBDB 亲属：弟（KinPerson 126731 王象春）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_UVI6nsp6bioER0eTAtdn86
            source_type: api_record
            title: 中国历代人物传记资料库：王象艮（CBDB 235529）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=235529&o=json
            external_identifier: CBDB:235529
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_1vwHH8EjSEP1q2oifpwF8P
        status: active
        display_name: 王象春
        merged_into_person_id: null
---

# 王象艮

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王象艮，明人物。籍贯新城，入仕廩生。（中国历代人物传记资料库 CBDB 235529） | accepted |
| name.primary | 王象艮 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_5r1wESpKqnWgbhR5AChbjE | 王之猷 | accepted |
| other | p_1vwHH8EjSEP1q2oifpwF8P | 王象春 | accepted |

## 外部来源

- [中国历代人物传记资料库：王象艮（CBDB 235529）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=235529&o=json)
