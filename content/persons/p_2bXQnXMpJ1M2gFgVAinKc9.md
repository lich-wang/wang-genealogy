---
schema: wang-person/v1
id: p_2bXQnXMpJ1M2gFgVAinKc9
status: active
merged_into: null
display_name: 魏氏
revision: 1
cbdb_id: 250839
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_bN1SnI99C908DTNB0IoZJ3
        subject_person_id: p_2bXQnXMpJ1M2gFgVAinKc9
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 魏氏，明人物。成化十四年進士。（中国历代人物传记资料库 CBDB 250839）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_GP6MvO8WEtw9F1uoGrHzOm
          claim_id: c_bN1SnI99C908DTNB0IoZJ3
          source_id: s_ey6WF-xdB5eoQddldr9WiA
          stance: supports
          locator: CBDB:250839
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_ey6WF-xdB5eoQddldr9WiA
            source_type: api_record
            title: 中国历代人物传记资料库：魏氏(王楫妻)（CBDB 250839）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=250839&o=json
            external_identifier: CBDB:250839
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_SKfw_7cVhC9wUg-6j4J3YC
        subject_person_id: p_2bXQnXMpJ1M2gFgVAinKc9
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 魏氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_9xvcqJG6WQ9VlH7lfjZ9-6
          claim_id: c_SKfw_7cVhC9wUg-6j4J3YC
          source_id: s_ey6WF-xdB5eoQddldr9WiA
          stance: supports
          locator: CBDB:250839
          quotation: null
          interpretation_note: CBDB 明确记录的王楫配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_Tug6NPnDj5QKxNLkJkoC7v
        subject_person_id: p_6qn5pH8ZsdNHGV4KR7haUQ
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_2bXQnXMpJ1M2gFgVAinKc9
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_S2ZfTnPuTnMtXLXgTS7lxX
          claim_id: c_Tug6NPnDj5QKxNLkJkoC7v
          source_id: s_ey6WF-xdB5eoQddldr9WiA
          stance: supports
          locator: 成化十四年進士登科錄:一卷，第二甲第十三名：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_6qn5pH8ZsdNHGV4KR7haUQ
        status: active
        display_name: 王楫
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 魏氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 魏氏，明人物。成化十四年進士。（中国历代人物传记资料库 CBDB 250839） | accepted |
| name.primary | 魏氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_6qn5pH8ZsdNHGV4KR7haUQ | 王楫 | accepted |

## 外部来源

- [中国历代人物传记资料库：魏氏(王楫妻)（CBDB 250839）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=250839&o=json)
