---
schema: wang-person/v1
id: p_F7P1m37QAM7LJBGiRdNSb7
status: active
merged_into: null
display_name: 王雄
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_WLL9A6NaoUHZtRRUsdxdMj
        subject_person_id: p_F7P1m37QAM7LJBGiRdNSb7
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王雄
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_qABv5SUrzULj3tp1q2aGKC
          claim_id: c_WLL9A6NaoUHZtRRUsdxdMj
          source_id: s_YzNwZC1JS9cbFP6ugshb34
          stance: supports
          locator: CBDB:208853
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（208853）
          source: &a1
            id: s_YzNwZC1JS9cbFP6ugshb34
            source_type: api_record
            title: 中国历代人物传记资料库：王雄（CBDB 208853）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=208853&o=json
            external_identifier: CBDB:208853
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:16.964Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_VxmLJuZ5MP2JAqzMdfFk7F
        subject_person_id: p_F7P1m37QAM7LJBGiRdNSb7
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王雄，明人物。隆慶五年進士。（中国历代人物传记资料库 CBDB 208853）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_dqt_Bpcu8txdjQQceaX86B
          claim_id: c_VxmLJuZ5MP2JAqzMdfFk7F
          source_id: s_YzNwZC1JS9cbFP6ugshb34
          stance: supports
          locator: CBDB:208853
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_TpIKSXmpNEf4SS6XLDkeEQ
        subject_person_id: p_F7P1m37QAM7LJBGiRdNSb7
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_QLH6TpKgMAWFBq48mvAzJJ
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_HvitPzZQH3X8E_S8Lyanqc
          claim_id: c_TpIKSXmpNEf4SS6XLDkeEQ
          source_id: s_YzNwZC1JS9cbFP6ugshb34
          stance: supports
          locator: 隆慶五年進士登科錄:一卷，第三甲第五名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_QLH6TpKgMAWFBq48mvAzJJ
        status: active
        display_name: 王幼慈
        merged_into_person_id: null
  other: []
---

# 王雄

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王雄 | accepted |
| bio.summary | 王雄，明人物。隆慶五年進士。（中国历代人物传记资料库 CBDB 208853） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_QLH6TpKgMAWFBq48mvAzJJ | 王幼慈 | accepted |

## 外部来源

- [中国历代人物传记资料库：王雄（CBDB 208853）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=208853&o=json)
