---
schema: wang-person/v1
id: p_fLPhak96xTAXhLbiBNKCpn
status: active
merged_into: null
display_name: 劉氏
revision: 1
cbdb_id: 37929
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_6sDkHrqzCKp1WqFk_69JGB
        subject_person_id: p_fLPhak96xTAXhLbiBNKCpn
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉氏（1078—1143），宋人物。籍贯安福。（中国历代人物传记资料库 CBDB 37929）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_FZtUjUskLOVUB_9Sti0woH
          claim_id: c_6sDkHrqzCKp1WqFk_69JGB
          source_id: s_Ccmi9QxWeiliGroC-KTK8V
          stance: supports
          locator: CBDB:37929
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_Ccmi9QxWeiliGroC-KTK8V
            source_type: api_record
            title: 中国历代人物传记资料库：劉氏(王庭珪妻)（CBDB 37929）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=37929&o=json
            external_identifier: CBDB:37929
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_-mj_VsHi3FeZjIEuB0U9fL
        subject_person_id: p_fLPhak96xTAXhLbiBNKCpn
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_cRImUXhTkyPm29Hi0fcTrG
          claim_id: c_-mj_VsHi3FeZjIEuB0U9fL
          source_id: s_Ccmi9QxWeiliGroC-KTK8V
          stance: supports
          locator: CBDB:37929
          quotation: null
          interpretation_note: CBDB 明确记录的王庭珪配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_K4IDMCEcxNyrVNe6nPUmg0
        subject_person_id: p_jGyyBZYE1568dhyVt8RubP
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_fLPhak96xTAXhLbiBNKCpn
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_qY60Mc2iarP8WqLWt6fHos
          claim_id: c_K4IDMCEcxNyrVNe6nPUmg0
          source_id: s_Ccmi9QxWeiliGroC-KTK8V
          stance: supports
          locator: 宋人傳記資料索引(電子版)，1810;1811：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_jGyyBZYE1568dhyVt8RubP
        status: active
        display_name: 王庭珪
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 劉氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 劉氏（1078—1143），宋人物。籍贯安福。（中国历代人物传记资料库 CBDB 37929） | accepted |
| name.primary | 劉氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_jGyyBZYE1568dhyVt8RubP | 王庭珪 | accepted |

## 外部来源

- [中国历代人物传记资料库：劉氏(王庭珪妻)（CBDB 37929）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=37929&o=json)
