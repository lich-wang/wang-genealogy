---
schema: wang-person/v1
id: p_quQXdH6gCbAdm41HTEiYp1
status: active
merged_into: null
display_name: 喬氏
revision: 1
cbdb_id: 263531
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_SjYdzIfyHgtC_5BSuGvKe2
        subject_person_id: p_quQXdH6gCbAdm41HTEiYp1
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 喬氏，明人物。弘治三年進士。（中国历代人物传记资料库 CBDB 263531）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_7BekXXPass1yqKi4SLDGXE
          claim_id: c_SjYdzIfyHgtC_5BSuGvKe2
          source_id: s_hHUzlnGGEY0thenkWHPCsN
          stance: supports
          locator: CBDB:263531
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_hHUzlnGGEY0thenkWHPCsN
            source_type: api_record
            title: 中国历代人物传记资料库：喬氏(王鼎妻)（CBDB 263531）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=263531&o=json
            external_identifier: CBDB:263531
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_J47oUAUlRv3bLnotP2f8QT
        subject_person_id: p_quQXdH6gCbAdm41HTEiYp1
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 喬氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_rWfd03J7a8SOWl041PizPK
          claim_id: c_J47oUAUlRv3bLnotP2f8QT
          source_id: s_hHUzlnGGEY0thenkWHPCsN
          stance: supports
          locator: CBDB:263531
          quotation: null
          interpretation_note: CBDB 明确记录的王鼎配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_dqQo9wYv8Vw-X1VeZ8ZSVX
        subject_person_id: p_3aWXbEScEDV2akyzL58TPm
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_quQXdH6gCbAdm41HTEiYp1
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_j4r_uNfyg_Og0XVCVQL8ct
          claim_id: c_dqQo9wYv8Vw-X1VeZ8ZSVX
          source_id: s_hHUzlnGGEY0thenkWHPCsN
          stance: supports
          locator: 弘治三年進士登科錄:一卷，第三甲第一百八十名：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_3aWXbEScEDV2akyzL58TPm
        status: active
        display_name: 王鼎
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 喬氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 喬氏，明人物。弘治三年進士。（中国历代人物传记资料库 CBDB 263531） | accepted |
| name.primary | 喬氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_3aWXbEScEDV2akyzL58TPm | 王鼎 | accepted |

## 外部来源

- [中国历代人物传记资料库：喬氏(王鼎妻)（CBDB 263531）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=263531&o=json)
