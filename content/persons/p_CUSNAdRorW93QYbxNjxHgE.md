---
schema: wang-person/v1
id: p_CUSNAdRorW93QYbxNjxHgE
status: active
merged_into: null
display_name: 狄氏
revision: 1
cbdb_id: 5133
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_-MHbhOUq9JZbc22c-2SoTf
        subject_person_id: p_CUSNAdRorW93QYbxNjxHgE
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 狄氏（1001—1069），宋人物。籍贯陽翟，入仕封贈，曾任郡君。（中国历代人物传记资料库 CBDB 5133）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_hXrrlY4K_8XHDYSzLstImB
          claim_id: c_-MHbhOUq9JZbc22c-2SoTf
          source_id: s_aawCfIvG_P7Vr5yDtP00HO
          stance: supports
          locator: CBDB:5133
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_aawCfIvG_P7Vr5yDtP00HO
            source_type: api_record
            title: 中国历代人物传记资料库：狄氏(王罕妻)（CBDB 5133）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=5133&o=json
            external_identifier: CBDB:5133
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_leNMQaqrSlRk3OwNqI-jZs
        subject_person_id: p_CUSNAdRorW93QYbxNjxHgE
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 狄氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_XovYLSHRx62zqVUpCAvUe6
          claim_id: c_leNMQaqrSlRk3OwNqI-jZs
          source_id: s_aawCfIvG_P7Vr5yDtP00HO
          stance: supports
          locator: CBDB:5133
          quotation: null
          interpretation_note: CBDB 明确记录的王罕配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_5GlXKvpChv-dhF2fzSTW4b
        subject_person_id: p_egyS5bPtB4BGpQ3ch7iPFq
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_CUSNAdRorW93QYbxNjxHgE
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_t-qsDrsPOyExzNqI5piO4I
          claim_id: c_5GlXKvpChv-dhF2fzSTW4b
          source_id: s_aawCfIvG_P7Vr5yDtP00HO
          stance: supports
          locator: 宋人傳記資料索引(電子版)，648;649：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_egyS5bPtB4BGpQ3ch7iPFq
        status: active
        display_name: 王罕
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 狄氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 狄氏（1001—1069），宋人物。籍贯陽翟，入仕封贈，曾任郡君。（中国历代人物传记资料库 CBDB 5133） | accepted |
| name.primary | 狄氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_egyS5bPtB4BGpQ3ch7iPFq | 王罕 | accepted |

## 外部来源

- [中国历代人物传记资料库：狄氏(王罕妻)（CBDB 5133）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=5133&o=json)
