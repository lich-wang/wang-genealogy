---
schema: wang-person/v1
id: p_yBsYCH8S8WAkxxJc931rVw
status: active
merged_into: null
display_name: 王之翰
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_8JuexGvjVZbMxfx6esbND8
        subject_person_id: p_yBsYCH8S8WAkxxJc931rVw
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王之翰
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_MiWPn8B1AqS252EtgB1aN6
          claim_id: c_8JuexGvjVZbMxfx6esbND8
          source_id: s_WnSUE1vxF3Y8LtPRDc8eie
          stance: supports
          locator: CBDB:572119
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（572119）
          source: &a1
            id: s_WnSUE1vxF3Y8LtPRDc8eie
            source_type: api_record
            title: 中国历代人物传记资料库：王之翰（CBDB 572119）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=572119&o=json
            external_identifier: CBDB:572119
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:25.885Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_UNZ9CWknQVRFoBFsEKoyTG
        subject_person_id: p_yBsYCH8S8WAkxxJc931rVw
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王之翰，明人物。籍贯山陰，入仕鄉貢舉人。（中国历代人物传记资料库 CBDB 572119）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_hHNDmfUanPYfHkFm8U1Xj8
          claim_id: c_UNZ9CWknQVRFoBFsEKoyTG
          source_id: s_WnSUE1vxF3Y8LtPRDc8eie
          stance: supports
          locator: CBDB:572119
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_rWXvCKNqZNeSWLo2Wuo6fh
        subject_person_id: p_iXZtEyPro2rGmw242iDT2E
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_yBsYCH8S8WAkxxJc931rVw
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_r8VeAEpcwlvaN8SUuAPTlf
          claim_id: c_rWXvCKNqZNeSWLo2Wuo6fh
          source_id: s_WnSUE1vxF3Y8LtPRDc8eie
          stance: supports
          locator: 紹興府志:八十卷，lgid=317109：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_iXZtEyPro2rGmw242iDT2E
        status: active
        display_name: 王元輔
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王之翰

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王之翰 | accepted |
| bio.summary | 王之翰，明人物。籍贯山陰，入仕鄉貢舉人。（中国历代人物传记资料库 CBDB 572119） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_iXZtEyPro2rGmw242iDT2E | 王元輔 | accepted |

## 外部来源

- [中国历代人物传记资料库：王之翰（CBDB 572119）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=572119&o=json)
