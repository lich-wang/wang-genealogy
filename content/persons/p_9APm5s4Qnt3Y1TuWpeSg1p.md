---
schema: wang-person/v1
id: p_9APm5s4Qnt3Y1TuWpeSg1p
status: active
merged_into: null
display_name: 王文智
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_BHpChZTqQquuNJuVsN2Pev
        subject_person_id: p_9APm5s4Qnt3Y1TuWpeSg1p
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王文智
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_5Dgu9tWRfdBrMUnp4rNjx3
          claim_id: c_BHpChZTqQquuNJuVsN2Pev
          source_id: s_oYwPkbWkuWCrQ3GYQsxLVj
          stance: supports
          locator: CBDB:227939
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（227939）
          source: &a1
            id: s_oYwPkbWkuWCrQ3GYQsxLVj
            source_type: api_record
            title: 中国历代人物传记资料库：王文智（CBDB 227939）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=227939&o=json
            external_identifier: CBDB:227939
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:17.535Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_rQx5Up3HRMH6wniHVwMZ3x
        subject_person_id: p_9APm5s4Qnt3Y1TuWpeSg1p
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王文智，明人物。成化二年進士。（中国历代人物传记资料库 CBDB 227939）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_9fHfJDGaeSG7yBAqob5ZVC
          claim_id: c_rQx5Up3HRMH6wniHVwMZ3x
          source_id: s_oYwPkbWkuWCrQ3GYQsxLVj
          stance: supports
          locator: CBDB:227939
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_QoEiuq6MpdiOUl2OaAWChW
        subject_person_id: p_9APm5s4Qnt3Y1TuWpeSg1p
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_gYCzF6vom9uRciu3ySukCo
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_DjV-g3jy5UbT8jLGDe6BR3
          claim_id: c_QoEiuq6MpdiOUl2OaAWChW
          source_id: s_oYwPkbWkuWCrQ3GYQsxLVj
          stance: supports
          locator: 成化二年進士登科錄:一卷，第二甲第九十一名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_gYCzF6vom9uRciu3ySukCo
        status: active
        display_name: 王輔
        merged_into_person_id: null
  other: []
---

# 王文智

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王文智 | accepted |
| bio.summary | 王文智，明人物。成化二年進士。（中国历代人物传记资料库 CBDB 227939） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_gYCzF6vom9uRciu3ySukCo | 王輔 | accepted |

## 外部来源

- [中国历代人物传记资料库：王文智（CBDB 227939）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=227939&o=json)
