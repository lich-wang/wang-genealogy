---
schema: wang-person/v1
id: p_51C8Kh4aNymAeDywG3epKL
status: active
merged_into: null
display_name: 王嗣先
cbdb_id: 202354
revision: 4
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_13AHKtaPqA1ahw3BW62SW2
        subject_person_id: p_51C8Kh4aNymAeDywG3epKL
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王嗣先（生于1377年），明人物。中国历代人物传记资料库（CBDB）以人物编号 202354 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 5
      sources:
        - id: cs_NF1x_qYj4KQpoxGD2a71JJ
          claim_id: c_13AHKtaPqA1ahw3BW62SW2
          source_id: s_szwXDrt2sB3Y6MHGLNoizk
          stance: supports
          locator: CBDB:202354
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source:
            id: s_szwXDrt2sB3Y6MHGLNoizk
            source_type: api_record
            title: 中国历代人物传记资料库：王嗣先（CBDB 202354）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=202354&o=json
            external_identifier: CBDB:202354
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:42.210Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_6518voux3uSD3GF4Wthuiw
        subject_person_id: p_51C8Kh4aNymAeDywG3epKL
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1377年
            calendar_note: CBDB 生年字段，精度：年
            earliest: 1377-01-01
            latest: 1377-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_ugk9ddsyk8rwjiZvHntJMp
          claim_id: c_6518voux3uSD3GF4Wthuiw
          source_id: s_szwXDrt2sB3Y6MHGLNoizk
          stance: supports
          locator: CBDB:202354
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（701-800）｜历史性依据：CBDB 生年 = 1377
          source:
            id: s_szwXDrt2sB3Y6MHGLNoizk
            source_type: api_record
            title: 中国历代人物传记资料库：王嗣先（CBDB 202354）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=202354&o=json
            external_identifier: CBDB:202354
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:42.210Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_zHQ4tFVHM4ibcJywDxE3yS
        subject_person_id: p_51C8Kh4aNymAeDywG3epKL
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王嗣先
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_Kq1Qn5weVyBbBjTCVb5ACc
          claim_id: c_zHQ4tFVHM4ibcJywDxE3yS
          source_id: s_szwXDrt2sB3Y6MHGLNoizk
          stance: supports
          locator: CBDB:202354
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（701-800）｜历史性依据：CBDB 生年 = 1377
          source:
            id: s_szwXDrt2sB3Y6MHGLNoizk
            source_type: api_record
            title: 中国历代人物传记资料库：王嗣先（CBDB 202354）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=202354&o=json
            external_identifier: CBDB:202354
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:42.210Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_1uYc2unG7LczVjQ8J0hP3h
        subject_person_id: p_mhv2745cG2mWonF5H2ZCdq
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_51C8Kh4aNymAeDywG3epKL
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_4MMce5aglD2mPk5Am29SsO
          claim_id: c_1uYc2unG7LczVjQ8J0hP3h
          source_id: s_dMf85nhS1Ka74Xis6swYsk
          stance: supports
          locator: 永樂十年進士登科錄:一卷，第三甲第十七名：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_dMf85nhS1Ka74Xis6swYsk
            source_type: api_record
            title: 中国历代人物传记资料库：王古平（CBDB 286917）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=286917&o=json
            external_identifier: CBDB:286917
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:17:02.953Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_mhv2745cG2mWonF5H2ZCdq
        status: active
        display_name: 王古平
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_41gFmoXLLnkIti39yY1qtZ
        subject_person_id: p_Zq7KqpkLmSoe8AXrGGMZL5
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_51C8Kh4aNymAeDywG3epKL
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_jWT57KDlfpO_hGESEy6aca
          claim_id: c_41gFmoXLLnkIti39yY1qtZ
          source_id: s_ioQe7PXr2JHjD9i33LXq2K
          stance: supports
          locator: 永樂十年進士登科錄:一卷，第三甲第十七名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_ioQe7PXr2JHjD9i33LXq2K
            source_type: api_record
            title: 中国历代人物传记资料库：王明德（CBDB 286916）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=286916&o=json
            external_identifier: CBDB:286916
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.242Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_Zq7KqpkLmSoe8AXrGGMZL5
        status: active
        display_name: 王明德
        merged_into_person_id: null
    - claim:
        id: c_9u0ca2Tmrw3nDQC5kS4UV3
        subject_person_id: p_aiJicNJ3TZ1ED5soZKgnCk
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_51C8Kh4aNymAeDywG3epKL
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_0H9ywFxrjpFL1B1mX5XStn
          claim_id: c_9u0ca2Tmrw3nDQC5kS4UV3
          source_id: s_esaG4QePU7fJzzdJkwZ7nP
          stance: supports
          locator: 永樂十年進士登科錄:一卷，第三甲第十七名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_esaG4QePU7fJzzdJkwZ7nP
            source_type: api_record
            title: 中国历代人物传记资料库：王惠軒（CBDB 286915）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=286915&o=json
            external_identifier: CBDB:286915
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:17:02.953Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_aiJicNJ3TZ1ED5soZKgnCk
        status: active
        display_name: 王惠軒
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王嗣先

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王嗣先（生于1377年），明人物。中国历代人物传记资料库（CBDB）以人物编号 202354 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。 | accepted |
| birth.date | 1377年 | accepted |
| name.primary | 王嗣先 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_mhv2745cG2mWonF5H2ZCdq | 王古平 | accepted |
| ancestors | p_Zq7KqpkLmSoe8AXrGGMZL5 | 王明德 | accepted |
| ancestors | p_aiJicNJ3TZ1ED5soZKgnCk | 王惠軒 | accepted |

## 外部来源

- [中国历代人物传记资料库：王古平（CBDB 286917）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=286917&o=json)
- [中国历代人物传记资料库：王惠軒（CBDB 286915）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=286915&o=json)
- [中国历代人物传记资料库：王明德（CBDB 286916）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=286916&o=json)
- [中国历代人物传记资料库：王嗣先（CBDB 202354）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=202354&o=json)
