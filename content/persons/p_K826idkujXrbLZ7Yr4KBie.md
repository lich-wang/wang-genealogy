---
schema: wang-person/v1
id: p_K826idkujXrbLZ7Yr4KBie
status: active
merged_into: null
display_name: 王忱
revision: 3
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_bAPuyygQ2Jv2usuz6QYKBX
        subject_person_id: p_K826idkujXrbLZ7Yr4KBie
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王忱
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_zC9G5zSqADCfQZhWKZ3Vm4
          claim_id: c_bAPuyygQ2Jv2usuz6QYKBX
          source_id: s_aWJEFQLotNNAZsLoEHZy9D
          stance: supports
          locator: CBDB:28730
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（28730）
          source: &a1
            id: s_aWJEFQLotNNAZsLoEHZy9D
            source_type: api_record
            title: 中国历代人物传记资料库：王忱（CBDB 28730）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=28730&o=json
            external_identifier: CBDB:28730
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.976Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_tX7eEEQNYx7BRRHTDF9KcH
        subject_person_id: p_K826idkujXrbLZ7Yr4KBie
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王忱，元人物。籍贯汲縣，曾任竹木場提舉。（中国历代人物传记资料库 CBDB 28730）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_62s74l_Sm4dOMZkf7exykW
          claim_id: c_tX7eEEQNYx7BRRHTDF9KcH
          source_id: s_aWJEFQLotNNAZsLoEHZy9D
          stance: supports
          locator: CBDB:28730
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_Y2YsKKsfzNJm60YaeTYbBc
        subject_person_id: p_VhEE1K9QYVSnK1V1eF11SZ
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_K826idkujXrbLZ7Yr4KBie
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_d1xpdc5fAw2aZIpegR5aAr
          claim_id: c_Y2YsKKsfzNJm60YaeTYbBc
          source_id: s_rRdjfwaqzLh5no1eJANkoO
          stance: supports
          locator: CBDB 亲属：父（KinPerson 28726）
          quotation: null
          interpretation_note: CBDB 直接记录的父／母关系。
          source:
            id: s_rRdjfwaqzLh5no1eJANkoO
            source_type: api_record
            title: 中国历代人物传记资料库：王忱（CBDB 28730）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=28730&o=json
            external_identifier: CBDB:28730
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_VhEE1K9QYVSnK1V1eF11SZ
        status: active
        display_name: 王天鐸
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_L6r-DITgWPxzt4RkuHvNPw
        subject_person_id: p_K826idkujXrbLZ7Yr4KBie
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_ZGiJW67sDSieGJ416En8tR
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs__7SicOxy3fk3mroWpGn6nA
          claim_id: c_L6r-DITgWPxzt4RkuHvNPw
          source_id: s_rRdjfwaqzLh5no1eJANkoO
          stance: supports
          locator: CBDB 亲属：兄（KinPerson 28617 王惲）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_rRdjfwaqzLh5no1eJANkoO
            source_type: api_record
            title: 中国历代人物传记资料库：王忱（CBDB 28730）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=28730&o=json
            external_identifier: CBDB:28730
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_ZGiJW67sDSieGJ416En8tR
        status: active
        display_name: 王惲
        merged_into_person_id: null
---

# 王忱

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王忱 | accepted |
| bio.summary | 王忱，元人物。籍贯汲縣，曾任竹木場提舉。（中国历代人物传记资料库 CBDB 28730） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_VhEE1K9QYVSnK1V1eF11SZ | 王天鐸 | accepted |
| other | p_ZGiJW67sDSieGJ416En8tR | 王惲 | accepted |

## 外部来源

- [中国历代人物传记资料库：王忱（CBDB 28730）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=28730&o=json)
