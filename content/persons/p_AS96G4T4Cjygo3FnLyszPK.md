---
schema: wang-person/v1
id: p_AS96G4T4Cjygo3FnLyszPK
status: active
merged_into: null
display_name: 蔣氏
revision: 1
cbdb_id: 241292
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_zFsmnJ8sjPn99_eLRmiEM4
        subject_person_id: p_AS96G4T4Cjygo3FnLyszPK
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 蔣氏，明人物。正統十三年進士。（中国历代人物传记资料库 CBDB 241292）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_nRyTQMPE04rViHduDDNJtM
          claim_id: c_zFsmnJ8sjPn99_eLRmiEM4
          source_id: s_GMmhSJCqKRKVoqLZEZn3kp
          stance: supports
          locator: CBDB:241292
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_GMmhSJCqKRKVoqLZEZn3kp
            source_type: api_record
            title: 中国历代人物传记资料库：蔣氏(王豪妻)（CBDB 241292）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=241292&o=json
            external_identifier: CBDB:241292
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_OLpklfe9DyqvrDa9Ne-gGw
        subject_person_id: p_AS96G4T4Cjygo3FnLyszPK
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 蔣氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_fL_VJDIfOfAJmJL9v9SlSs
          claim_id: c_OLpklfe9DyqvrDa9Ne-gGw
          source_id: s_GMmhSJCqKRKVoqLZEZn3kp
          stance: supports
          locator: CBDB:241292
          quotation: null
          interpretation_note: CBDB 明确记录的王豪配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_iUZrhg4jH4BO1W9fESWP3E
        subject_person_id: p_CwH6PHR1XzMwocJ8ZwHSPP
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_AS96G4T4Cjygo3FnLyszPK
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Nj2NrNLtI1dKzRZvwSNkij
          claim_id: c_iUZrhg4jH4BO1W9fESWP3E
          source_id: s_GMmhSJCqKRKVoqLZEZn3kp
          stance: supports
          locator: 正統十三年進士登科錄:一卷，第三甲第七十五名：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_CwH6PHR1XzMwocJ8ZwHSPP
        status: active
        display_name: 王豪
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 蔣氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 蔣氏，明人物。正統十三年進士。（中国历代人物传记资料库 CBDB 241292） | accepted |
| name.primary | 蔣氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_CwH6PHR1XzMwocJ8ZwHSPP | 王豪 | accepted |

## 外部来源

- [中国历代人物传记资料库：蔣氏(王豪妻)（CBDB 241292）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=241292&o=json)
