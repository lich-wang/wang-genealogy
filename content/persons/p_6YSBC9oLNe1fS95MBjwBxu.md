---
schema: wang-person/v1
id: p_6YSBC9oLNe1fS95MBjwBxu
status: active
merged_into: null
display_name: 王之模
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_HGvb9dyA2Bsg2AZEXCANJv
        subject_person_id: p_6YSBC9oLNe1fS95MBjwBxu
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王之模
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_BSMH4kj5mBZBNERewixMiA
          claim_id: c_HGvb9dyA2Bsg2AZEXCANJv
          source_id: s_UCb4bwRC8hLvGSDdguiHE7
          stance: supports
          locator: CBDB:692689
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（692689）
          source: &a1
            id: s_UCb4bwRC8hLvGSDdguiHE7
            source_type: api_record
            title: 中国历代人物传记资料库：王之模（CBDB 692689）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=692689&o=json
            external_identifier: CBDB:692689
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:27.638Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_tKgDk25FHsx5entuAvj9r9
        subject_person_id: p_6YSBC9oLNe1fS95MBjwBxu
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王之模，清人物。籍贯海鹽，入仕邑庠生。（中国历代人物传记资料库 CBDB 692689）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs__CMfo0IrtdDuj9NO7OIGZw
          claim_id: c_tKgDk25FHsx5entuAvj9r9
          source_id: s_UCb4bwRC8hLvGSDdguiHE7
          stance: supports
          locator: CBDB:692689
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_JT6XbLi3izj4qu_2_jz0ew
        subject_person_id: p_QSgy6eFERDKo7HPvJEmbdC
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_6YSBC9oLNe1fS95MBjwBxu
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_HcrA-sg8oj6x3U3cv7VpTQ
          claim_id: c_JT6XbLi3izj4qu_2_jz0ew
          source_id: s_UCb4bwRC8hLvGSDdguiHE7
          stance: supports
          locator: "海鹽縣志: 二十二卷(光緒)，lgid=293957：父"
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_QSgy6eFERDKo7HPvJEmbdC
        status: active
        display_name: 王澐
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王之模

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王之模 | accepted |
| bio.summary | 王之模，清人物。籍贯海鹽，入仕邑庠生。（中国历代人物传记资料库 CBDB 692689） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_QSgy6eFERDKo7HPvJEmbdC | 王澐 | accepted |

## 外部来源

- [中国历代人物传记资料库：王之模（CBDB 692689）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=692689&o=json)
