---
schema: wang-person/v1
id: p_WvhMZ8L3PD791uLh9uBESW
status: active
merged_into: null
display_name: 劉氏
revision: 1
cbdb_id: 295492
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_xvONY7o4Weo-67ulWPWpoI
        subject_person_id: p_WvhMZ8L3PD791uLh9uBESW
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉氏，明人物。嘉靖十四年進士。（中国历代人物传记资料库 CBDB 295492）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_t95Li3tFAWPWKJm6BHniVi
          claim_id: c_xvONY7o4Weo-67ulWPWpoI
          source_id: s_0mLhwtS7bQcn1l9qcyf6ak
          stance: supports
          locator: CBDB:295492
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_0mLhwtS7bQcn1l9qcyf6ak
            source_type: api_record
            title: 中国历代人物传记资料库：劉氏(王朝相妻)（CBDB 295492）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=295492&o=json
            external_identifier: CBDB:295492
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_CMTEO2uKe6q9WOUviQaMa9
        subject_person_id: p_WvhMZ8L3PD791uLh9uBESW
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_wWvyPrvKFWfFn5t8xzk_3F
          claim_id: c_CMTEO2uKe6q9WOUviQaMa9
          source_id: s_0mLhwtS7bQcn1l9qcyf6ak
          stance: supports
          locator: CBDB:295492
          quotation: null
          interpretation_note: CBDB 明确记录的王朝相配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c__Soafc8l1RruZA6iRYPHTV
        subject_person_id: p_AQPzHURj5Q55p8A3sruTTG
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_WvhMZ8L3PD791uLh9uBESW
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Al-vaFPWYJbrFgYivmIop6
          claim_id: c__Soafc8l1RruZA6iRYPHTV
          source_id: s_0mLhwtS7bQcn1l9qcyf6ak
          stance: supports
          locator: 嘉靖十四年進士登科錄:一卷，第二甲第四十四名：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_AQPzHURj5Q55p8A3sruTTG
        status: active
        display_name: 王朝相
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 劉氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 劉氏，明人物。嘉靖十四年進士。（中国历代人物传记资料库 CBDB 295492） | accepted |
| name.primary | 劉氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_AQPzHURj5Q55p8A3sruTTG | 王朝相 | accepted |

## 外部来源

- [中国历代人物传记资料库：劉氏(王朝相妻)（CBDB 295492）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=295492&o=json)
