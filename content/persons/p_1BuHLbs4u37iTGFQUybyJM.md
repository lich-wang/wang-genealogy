---
schema: wang-person/v1
id: p_1BuHLbs4u37iTGFQUybyJM
status: active
merged_into: null
display_name: 王宣
revision: 3
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_qynEx4isMvQsrmWchRTynn
        subject_person_id: p_1BuHLbs4u37iTGFQUybyJM
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王宣
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_zjyXfDwUsAi9y3iaAQDDUA
          claim_id: c_qynEx4isMvQsrmWchRTynn
          source_id: s_9tZH2Jfja9WHZ2N9zZXRKc
          stance: supports
          locator: CBDB:265522
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（265522）
          source: &a1
            id: s_9tZH2Jfja9WHZ2N9zZXRKc
            source_type: api_record
            title: 中国历代人物传记资料库：王宣（CBDB 265522）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265522&o=json
            external_identifier: CBDB:265522
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.687Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_5JdTNGU65eEHrT65DFUXNq
        subject_person_id: p_1BuHLbs4u37iTGFQUybyJM
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王宣，明人物。弘治六年進士，籍贯永清。（中国历代人物传记资料库 CBDB 265522）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_HKUhLR657Pk_MPvmHadICt
          claim_id: c_5JdTNGU65eEHrT65DFUXNq
          source_id: s_9tZH2Jfja9WHZ2N9zZXRKc
          stance: supports
          locator: CBDB:265522
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_BeXawCGklFJtuEher9DyV3
        subject_person_id: p_amEGG68EW7aciY6Xv6ntD7
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_1BuHLbs4u37iTGFQUybyJM
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_xGwkbIuPSLH2FLQqbeKVsy
          claim_id: c_BeXawCGklFJtuEher9DyV3
          source_id: s_UgbikxsDgEuEls0ot-poVH
          stance: supports
          locator: CBDB：兄弟 王雄（68045）之父／母 王海
          quotation: null
          interpretation_note: 由兄弟关系推断：王宣 与 王雄 为同胞（CBDB 记「弟」），王雄 之父／母即 王宣 之父／母。
          source:
            id: s_UgbikxsDgEuEls0ot-poVH
            source_type: api_record
            title: 中国历代人物传记资料库：王宣（CBDB 265522）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265522&o=json
            external_identifier: CBDB:265522
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_amEGG68EW7aciY6Xv6ntD7
        status: active
        display_name: 王海
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_OP9XgzaBWttiuhxnm6FKkP
        subject_person_id: p_1BuHLbs4u37iTGFQUybyJM
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_CwNoR4RewoHTE9KBp57Caf
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_RGKouIqBLyEoO_D53GibqD
          claim_id: c_OP9XgzaBWttiuhxnm6FKkP
          source_id: s_UgbikxsDgEuEls0ot-poVH
          stance: supports
          locator: CBDB 亲属：弟（KinPerson 68045 王雄）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_UgbikxsDgEuEls0ot-poVH
            source_type: api_record
            title: 中国历代人物传记资料库：王宣（CBDB 265522）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265522&o=json
            external_identifier: CBDB:265522
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_CwNoR4RewoHTE9KBp57Caf
        status: active
        display_name: 王雄
        merged_into_person_id: null
---

# 王宣

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王宣 | accepted |
| bio.summary | 王宣，明人物。弘治六年進士，籍贯永清。（中国历代人物传记资料库 CBDB 265522） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_amEGG68EW7aciY6Xv6ntD7 | 王海 | accepted |
| other | p_CwNoR4RewoHTE9KBp57Caf | 王雄 | accepted |

## 外部来源

- [中国历代人物传记资料库：王宣（CBDB 265522）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265522&o=json)
