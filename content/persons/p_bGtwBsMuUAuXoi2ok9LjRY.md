---
schema: wang-person/v1
id: p_bGtwBsMuUAuXoi2ok9LjRY
status: active
merged_into: null
display_name: 王武才
cbdb_id: 210018
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_amUP3KMZ4X5R1eBsDaQ1yb
        subject_person_id: p_bGtwBsMuUAuXoi2ok9LjRY
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王武才，史料所见人物。本项目依据《中国历代人物传记资料库：王武才（CBDB 210018）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_Ybytq1TgD7ShOndysI-D5E
          claim_id: c_amUP3KMZ4X5R1eBsDaQ1yb
          source_id: s_BaK1qyg6QggR9879DKj7jz
          stance: supports
          locator: CBDB:210018
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_BaK1qyg6QggR9879DKj7jz
            source_type: api_record
            title: 中国历代人物传记资料库：王武才（CBDB 210018）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=210018&o=json
            external_identifier: CBDB:210018
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:55.593Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_UKJFDSqKq2fFqEc65UjT7p
        subject_person_id: p_bGtwBsMuUAuXoi2ok9LjRY
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王武才
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_VTozvEPqqn2LAhX3oFkFsL
          claim_id: c_UKJFDSqKq2fFqEc65UjT7p
          source_id: s_BaK1qyg6QggR9879DKj7jz
          stance: supports
          locator: CBDB:210018
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（2401-2500）｜历史性依据：CBDB 朝代 = 明
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_AayupScFnO0kIVNNPYIYbL
        subject_person_id: p_bGtwBsMuUAuXoi2ok9LjRY
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_EaFxi6NHrChzwLEA1T89LT
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_FIFnmAj8sck9ZkK_1qN5s2
          claim_id: c_AayupScFnO0kIVNNPYIYbL
          source_id: s_BaK1qyg6QggR9879DKj7jz
          stance: supports
          locator: 隆慶五年進士登科錄:一卷，第三甲第一百零七名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_BaK1qyg6QggR9879DKj7jz
            source_type: api_record
            title: 中国历代人物传记资料库：王武才（CBDB 210018）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=210018&o=json
            external_identifier: CBDB:210018
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:55.593Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_EaFxi6NHrChzwLEA1T89LT
        status: active
        display_name: 王道成
        merged_into_person_id: null
  other: []
---

# 王武才

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王武才，史料所见人物。本项目依据《中国历代人物传记资料库：王武才（CBDB 210018）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 王武才 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_EaFxi6NHrChzwLEA1T89LT | 王道成 | accepted |

## 外部来源

- [中国历代人物传记资料库：王武才（CBDB 210018）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=210018&o=json)
