---
schema: wang-person/v1
id: p_wtbH7xExyKzkv5ZQ6eBmE5
status: active
merged_into: null
display_name: 王敏
revision: 5
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_pB3Te48zcoA7yKr3HPQcKK
        subject_person_id: p_wtbH7xExyKzkv5ZQ6eBmE5
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王敏
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_WjLvnjtdmb1A374CNsJREc
          claim_id: c_pB3Te48zcoA7yKr3HPQcKK
          source_id: s_kFFyHefVzW5sVNR3pyWrFd
          stance: supports
          locator: CBDB:199681
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（199681）
          source: &a1
            id: s_kFFyHefVzW5sVNR3pyWrFd
            source_type: api_record
            title: 中国历代人物传记资料库：王敏（CBDB 199681）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=199681&o=json
            external_identifier: CBDB:199681
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:16.571Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_26AhyxDsYHSwDLJKA5x8yQ
        subject_person_id: p_wtbH7xExyKzkv5ZQ6eBmE5
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1432年
            calendar_note: CBDB 生年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_1MBDTWSFbA3F5DPVJv7tyn
          claim_id: c_26AhyxDsYHSwDLJKA5x8yQ
          source_id: s_kFFyHefVzW5sVNR3pyWrFd
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 生年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_J1gvUyfDiQ7tiSnNfKmtxx
        subject_person_id: p_wtbH7xExyKzkv5ZQ6eBmE5
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王敏（生于1432年），明人物。明清進士進士，籍贯宣府前衛，入仕進士。（中国历代人物传记资料库 CBDB 199681）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_aAW3S2dW-y3tY25NSOZ7bu
          claim_id: c_J1gvUyfDiQ7tiSnNfKmtxx
          source_id: s_kFFyHefVzW5sVNR3pyWrFd
          stance: supports
          locator: CBDB:199681
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_xx-KSAdP9jD6WCfANndTkL
        subject_person_id: p_uTGWZPP3K2AvshF7Q9pjcC
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_wtbH7xExyKzkv5ZQ6eBmE5
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_l-L2Kn6kCHEXi_nuBQRMRH
          claim_id: c_xx-KSAdP9jD6WCfANndTkL
          source_id: s_5J7GFUAmiehHA45GrcwMQF
          stance: supports
          locator: 成化十一年進士登科錄:一卷，第二甲第七十三名：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_5J7GFUAmiehHA45GrcwMQF
            source_type: api_record
            title: 中国历代人物传记资料库：王能（CBDB 248446）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=248446&o=json
            external_identifier: CBDB:248446
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.154Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_uTGWZPP3K2AvshF7Q9pjcC
        status: active
        display_name: 王能
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_1s426we1CrSXRqogHnhdpp
        subject_person_id: p_f3NipocjRy6XK4FGTvrqsn
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_wtbH7xExyKzkv5ZQ6eBmE5
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_mhip9ZqbjBxUPvHvfHB8k4
          claim_id: c_1s426we1CrSXRqogHnhdpp
          source_id: s_hdAAWr2NdMRPnQz1GwYCBA
          stance: supports
          locator: 成化十一年進士登科錄:一卷，第二甲第七十三名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_hdAAWr2NdMRPnQz1GwYCBA
            source_type: api_record
            title: 中国历代人物传记资料库：王成（CBDB 248445）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=248445&o=json
            external_identifier: CBDB:248445
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.153Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_f3NipocjRy6XK4FGTvrqsn
        status: active
        display_name: 王成
        merged_into_person_id: null
    - claim:
        id: c_DBgK2auZgjQ0bwGPmgCs1t
        subject_person_id: p_wSXnKM2BTwAjxUJdAcHBxL
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_wtbH7xExyKzkv5ZQ6eBmE5
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_VHLwyUoNO2z52a609fvLZi
          claim_id: c_DBgK2auZgjQ0bwGPmgCs1t
          source_id: s_35Nwf1Q4e4dnzEL2z5SRw6
          stance: supports
          locator: 成化十一年進士登科錄:一卷，第二甲第七十三名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_35Nwf1Q4e4dnzEL2z5SRw6
            source_type: api_record
            title: 中国历代人物传记资料库：王勝受（CBDB 248444）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=248444&o=json
            external_identifier: CBDB:248444
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:59.761Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_wSXnKM2BTwAjxUJdAcHBxL
        status: active
        display_name: 王勝受
        merged_into_person_id: null
  descendants: []
  other:
    - claim:
        id: c_iNL6xgmxMqdFdLXrLFbdp9
        subject_person_id: p_E7jv7QNWjctPAZKEam7F27
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_wtbH7xExyKzkv5ZQ6eBmE5
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_VyeQgWFsxKb_o0T61lf1TY
          claim_id: c_iNL6xgmxMqdFdLXrLFbdp9
          source_id: s_0mL2Hk_G8mDrbW7nI7bU65
          stance: supports
          locator: CBDB 亲属：兄（KinPerson 199681 王敏）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_0mL2Hk_G8mDrbW7nI7bU65
            source_type: api_record
            title: 中国历代人物传记资料库：王敬（CBDB 248449）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=248449&o=json
            external_identifier: CBDB:248449
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_E7jv7QNWjctPAZKEam7F27
        status: active
        display_name: 王敬
        merged_into_person_id: null
---

# 王敏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王敏 | accepted |
| birth.date | 1432年 | accepted |
| bio.summary | 王敏（生于1432年），明人物。明清進士進士，籍贯宣府前衛，入仕進士。（中国历代人物传记资料库 CBDB 199681） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_uTGWZPP3K2AvshF7Q9pjcC | 王能 | accepted |
| ancestors | p_f3NipocjRy6XK4FGTvrqsn | 王成 | accepted |
| ancestors | p_wSXnKM2BTwAjxUJdAcHBxL | 王勝受 | accepted |
| other | p_E7jv7QNWjctPAZKEam7F27 | 王敬 | accepted |

## 外部来源

- [中国历代人物传记资料库：王成（CBDB 248445）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=248445&o=json)
- [中国历代人物传记资料库：王敬（CBDB 248449）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=248449&o=json)
- [中国历代人物传记资料库：王敏（CBDB 199681）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=199681&o=json)
- [中国历代人物传记资料库：王能（CBDB 248446）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=248446&o=json)
- [中国历代人物传记资料库：王勝受（CBDB 248444）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=248444&o=json)
