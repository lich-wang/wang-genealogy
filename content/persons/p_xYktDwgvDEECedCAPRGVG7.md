---
schema: wang-person/v1
id: p_xYktDwgvDEECedCAPRGVG7
status: active
merged_into: null
display_name: 王基嘉
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_vCsC7m3GFadVwvVZbRPo3H
        subject_person_id: p_xYktDwgvDEECedCAPRGVG7
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王基嘉
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_mLdTa9VUNSEmoSPLC6LZ5c
          claim_id: c_vCsC7m3GFadVwvVZbRPo3H
          source_id: s_AvQ3R8cPK3v6QYDvGwaiFt
          stance: supports
          locator: CBDB:577306
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（577306）
          source: &a1
            id: s_AvQ3R8cPK3v6QYDvGwaiFt
            source_type: api_record
            title: 中国历代人物传记资料库：王基嘉（CBDB 577306）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=577306&o=json
            external_identifier: CBDB:577306
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:04.720Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_6A4NhJGNq87hgtFFQpnftH
        subject_person_id: p_xYktDwgvDEECedCAPRGVG7
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王基嘉，清人物。籍贯歙縣，身份为孝子/孝女。（中国历代人物传记资料库 CBDB 577306）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_aDjBGAWuQmRPtPH9mc5W5c
          claim_id: c_6A4NhJGNq87hgtFFQpnftH
          source_id: s_AvQ3R8cPK3v6QYDvGwaiFt
          stance: supports
          locator: CBDB:577306
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_L1VQZspKA0vKFmfCm1Hueg
        subject_person_id: p_evwUvhEiYMfk4aJf81DhmK
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_xYktDwgvDEECedCAPRGVG7
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_1--kPkNOO-mi8_ZntxWt_u
          claim_id: c_L1VQZspKA0vKFmfCm1Hueg
          source_id: s_v4Jsron3DGMtV41aezGKoi
          stance: supports
          locator: "民國歙縣志: 十六卷，lgid=1163744：子"
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_v4Jsron3DGMtV41aezGKoi
            source_type: api_record
            title: 中国历代人物传记资料库：王崔徵（CBDB 577307）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=577307&o=json
            external_identifier: CBDB:577307
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:04.720Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_evwUvhEiYMfk4aJf81DhmK
        status: active
        display_name: 王崔徵
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王基嘉

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王基嘉 | accepted |
| bio.summary | 王基嘉，清人物。籍贯歙縣，身份为孝子/孝女。（中国历代人物传记资料库 CBDB 577306） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_evwUvhEiYMfk4aJf81DhmK | 王崔徵 | accepted |

## 外部来源

- [中国历代人物传记资料库：王崔徵（CBDB 577307）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=577307&o=json)
- [中国历代人物传记资料库：王基嘉（CBDB 577306）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=577306&o=json)
