---
schema: wang-person/v1
id: p_Ko86R3TKut3FD1aGj75nhU
status: active
merged_into: null
display_name: 王良
cbdb_id: 100680
revision: 3
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_GjMs2pTExLeCid2gLKnqrQ
        subject_person_id: p_Ko86R3TKut3FD1aGj75nhU
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王良，金朝末年、蒙元初年中山唐县（今河北省唐县）人。 金朝末年王良为中山府（治所在今河北定州市）掾，当时百姓遭乱后，多以冤枉入狱，王良前后救活数百人。不久辞去吏职，潜心伊洛之学（程颢、程颐之学），尤其精究天文律历，九十二岁去世。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 5
      sources:
        - id: cs_as3bK6c75Ryj9A9QTKGobC
          claim_id: c_GjMs2pTExLeCid2gLKnqrQ
          source_id: s_JCG85CP74FRaCDN1EnVFGE
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_JCG85CP74FRaCDN1EnVFGE
            source_type: api_record
            title: 维基数据：王良（Q45649173）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45649173
            external_identifier: Q45649173
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:13.205Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%89%AF_(%E5%85%83%E6%9C%9D)
        - id: cs_q05Jk8EiT8e2DxJG77bJPh
          claim_id: c_GjMs2pTExLeCid2gLKnqrQ
          source_id: s_uwzAELWWYnsIk3XwGwFVFO
          stance: supports
          locator: 导言
          quotation: 王良，金朝末年、蒙元初年中山唐县（今河北省唐县）人。 金朝末年王
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 wikipedia。
          source:
            id: s_uwzAELWWYnsIk3XwGwFVFO
            source_type: website
            title: 中文维基百科：王良 (元朝)
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%89%AF_(%E5%85%83%E6%9C%9D)
            external_identifier: Q45649173
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-09-06T22:33:22.403Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_ASMN7q7uvmU1w75mv8asnX
        subject_person_id: p_Ko86R3TKut3FD1aGj75nhU
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王良
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_JvHNs9g6uC4rpYJHV9DSSM
          claim_id: c_ASMN7q7uvmU1w75mv8asnX
          source_id: s_JCG85CP74FRaCDN1EnVFGE
          stance: supports
          locator: Q45649173
          quotation: null
          interpretation_note: null
          source:
            id: s_JCG85CP74FRaCDN1EnVFGE
            source_type: api_record
            title: 维基数据：王良（Q45649173）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45649173
            external_identifier: Q45649173
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:13.205Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%89%AF_(%E5%85%83%E6%9C%9D)
        - id: cs_umaovBmA8j2A5cZRxN7LHp
          claim_id: c_ASMN7q7uvmU1w75mv8asnX
          source_id: s_5Ymd16fJp5zWHvrGyJgQJ5
          stance: supports
          locator: Q45649173
          quotation: null
          interpretation_note: null
          source:
            id: s_5Ymd16fJp5zWHvrGyJgQJ5
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王良（100680）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=100680&o=json
            external_identifier: CBDB:100680
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-05T01:59:13.349Z
            metadata_json: null
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_Bkr3e2gEzXD2KK7SuDN2Dc
        subject_person_id: p_Ko86R3TKut3FD1aGj75nhU
        claim_kind: relationship
        predicate: kinship.parent_of
        object_person_id: p_kSaLSFJv26CRusQkp6QMsj
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_dV4Vqi2t6guPhLKjd7UeQP
          claim_id: c_Bkr3e2gEzXD2KK7SuDN2Dc
          source_id: s_5JsKoH93gordmDjAEdt3gr
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_5JsKoH93gordmDjAEdt3gr
            source_type: api_record
            title: 维基数据：王恂（Q11573072）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q11573072
            external_identifier: Q11573072
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:45:14.157Z
            metadata_json: null
        - id: cs_gX5LuDE1Zi711dSunYg4v6
          claim_id: c_Bkr3e2gEzXD2KK7SuDN2Dc
          source_id: s_JCG85CP74FRaCDN1EnVFGE
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_JCG85CP74FRaCDN1EnVFGE
            source_type: api_record
            title: 维基数据：王良（Q45649173）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45649173
            external_identifier: Q45649173
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:13.205Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%89%AF_(%E5%85%83%E6%9C%9D)
      object_person:
        id: p_kSaLSFJv26CRusQkp6QMsj
        status: active
        display_name: 王恂
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王良

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王良，金朝末年、蒙元初年中山唐县（今河北省唐县）人。 金朝末年王良为中山府（治所在今河北定州市）掾，当时百姓遭乱后，多以冤枉入狱，王良前后救活数百人。不久辞去吏职，潜心伊洛之学（程颢、程颐之学），尤其精究天文律历，九十二岁去世。 | accepted |
| name.primary | 王良 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_kSaLSFJv26CRusQkp6QMsj | 王恂 | accepted |

## 外部来源

- [维基数据：王良（Q45649173）](https://www.wikidata.org/wiki/Q45649173)
- [维基数据：王恂（Q11573072）](https://www.wikidata.org/wiki/Q11573072)
- [中文维基百科：王良 (元朝)](https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%89%AF_(%E5%85%83%E6%9C%9D))
- [CBDB 中国历代人物传记资料库：王良（100680）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=100680&o=json)
