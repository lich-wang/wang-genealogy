---
schema: wang-person/v1
id: p_iC2LMm2r1PH816QULpx83x
status: active
merged_into: null
display_name: 王愷
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_h1G1yCBd9zC87j5fwPN99i
        subject_person_id: p_iC2LMm2r1PH816QULpx83x
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王愷
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_1gJB5bJ7as8uxiav1GKLaX
          claim_id: c_h1G1yCBd9zC87j5fwPN99i
          source_id: s_dAZshWLSiskxDPuLNoEjvp
          stance: supports
          locator: CBDB:67812
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（67812）
          source: &a1
            id: s_dAZshWLSiskxDPuLNoEjvp
            source_type: api_record
            title: 中国历代人物传记资料库：王愷（CBDB 67812）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=67812&o=json
            external_identifier: CBDB:67812
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:13.023Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_LYDUnb4usyrC1XyRgxCAf4
        subject_person_id: p_iC2LMm2r1PH816QULpx83x
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王愷，明人物。明清進士進士，籍贯蒲圻，入仕進士，曾任參議、提刑按察使司僉事、知縣。（中国历代人物传记资料库 CBDB 67812）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_LK2hwuJftWSp9xZBHY6jsU
          claim_id: c_LYDUnb4usyrC1XyRgxCAf4
          source_id: s_dAZshWLSiskxDPuLNoEjvp
          stance: supports
          locator: CBDB:67812
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_v2DHA0-2qJdm-E-2_Oksng
        subject_person_id: p_dqbea7KC6b34QnganHGig3
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_iC2LMm2r1PH816QULpx83x
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_gPDnjr-6CZg-HjReYRc97c
          claim_id: c_v2DHA0-2qJdm-E-2_Oksng
          source_id: s_dAZshWLSiskxDPuLNoEjvp
          stance: supports
          locator: 人名權威資料（中央研究院歷史語言研究所），11449：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_dqbea7KC6b34QnganHGig3
        status: active
        display_name: 王允茂
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王愷

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王愷 | accepted |
| bio.summary | 王愷，明人物。明清進士進士，籍贯蒲圻，入仕進士，曾任參議、提刑按察使司僉事、知縣。（中国历代人物传记资料库 CBDB 67812） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_dqbea7KC6b34QnganHGig3 | 王允茂 | accepted |

## 外部来源

- [中国历代人物传记资料库：王愷（CBDB 67812）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=67812&o=json)
