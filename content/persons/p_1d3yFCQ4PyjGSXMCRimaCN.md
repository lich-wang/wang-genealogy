---
schema: wang-person/v1
id: p_1d3yFCQ4PyjGSXMCRimaCN
status: active
merged_into: null
display_name: 呂氏
revision: 1
cbdb_id: 212088
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_RtnvdgqWJqLurOs_xPxZa0
        subject_person_id: p_1d3yFCQ4PyjGSXMCRimaCN
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 呂氏，明人物。隆慶五年進士。（中国历代人物传记资料库 CBDB 212088）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_gljxMS1JP0Qt4zeGTMCf6g
          claim_id: c_RtnvdgqWJqLurOs_xPxZa0
          source_id: s_Vati1fdeBrTM_28hTbyScT
          stance: supports
          locator: CBDB:212088
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_Vati1fdeBrTM_28hTbyScT
            source_type: api_record
            title: 中国历代人物传记资料库：呂氏(王度妻)（CBDB 212088）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=212088&o=json
            external_identifier: CBDB:212088
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_IByfEL5BZsyzG_0iC6qwwP
        subject_person_id: p_1d3yFCQ4PyjGSXMCRimaCN
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 呂氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_0HD4r7BTCP5DlqiZ2KQ7b9
          claim_id: c_IByfEL5BZsyzG_0iC6qwwP
          source_id: s_Vati1fdeBrTM_28hTbyScT
          stance: supports
          locator: CBDB:212088
          quotation: null
          interpretation_note: CBDB 明确记录的王度配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_oq5KsK3gUJcwSi6V8FWMtA
        subject_person_id: p_feVTPN7HCaS1BxqehN6dAD
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_1d3yFCQ4PyjGSXMCRimaCN
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_xMi4GRfAAiTPzRonui7yFJ
          claim_id: c_oq5KsK3gUJcwSi6V8FWMtA
          source_id: s_Vati1fdeBrTM_28hTbyScT
          stance: supports
          locator: 隆慶五年進士登科錄:一卷，第三甲第二百九十七名：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_feVTPN7HCaS1BxqehN6dAD
        status: active
        display_name: 王度
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 呂氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 呂氏，明人物。隆慶五年進士。（中国历代人物传记资料库 CBDB 212088） | accepted |
| name.primary | 呂氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_feVTPN7HCaS1BxqehN6dAD | 王度 | accepted |

## 外部来源

- [中国历代人物传记资料库：呂氏(王度妻)（CBDB 212088）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=212088&o=json)
