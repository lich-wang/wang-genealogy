---
schema: wang-person/v1
id: p_akpqJMSSGZNHLkzngCE1Qi
status: active
merged_into: null
display_name: 王子成
cbdb_id: 200985
revision: 7
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_eoJTEEhNXNRuz5QuNKEmc5
        subject_person_id: p_akpqJMSSGZNHLkzngCE1Qi
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王子成（生于1460年），明人物。明清進士進士，籍贯咸寧，入仕進士，曾任員外郎。（中国历代人物传记资料库 CBDB 200985）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 7
      sources:
        - id: cs_NMdxMAmgRHOkNLTBbwJbW4
          claim_id: c_eoJTEEhNXNRuz5QuNKEmc5
          source_id: s_Qc3v5M5FxTuwdvP812tJm3
          stance: supports
          locator: CBDB:200985
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_Qc3v5M5FxTuwdvP812tJm3
            source_type: api_record
            title: 中国历代人物传记资料库：王子成（CBDB 200985）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=200985&o=json
            external_identifier: CBDB:200985
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:42.210Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_pJiTghkHGfRXVNv26ekkRs
        subject_person_id: p_akpqJMSSGZNHLkzngCE1Qi
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1460年
            calendar_note: CBDB 生年字段，精度：年
            earliest: 1460-01-01
            latest: 1460-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_HA6o4RoZojQxXHcoA4fL8a
          claim_id: c_pJiTghkHGfRXVNv26ekkRs
          source_id: s_Qc3v5M5FxTuwdvP812tJm3
          stance: supports
          locator: CBDB:200985
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（701-800）｜历史性依据：CBDB 生年 = 1460
          source:
            id: s_Qc3v5M5FxTuwdvP812tJm3
            source_type: api_record
            title: 中国历代人物传记资料库：王子成（CBDB 200985）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=200985&o=json
            external_identifier: CBDB:200985
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:42.210Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_rKFL5YB3eUvr9HJ4FvGs9b
        subject_person_id: p_akpqJMSSGZNHLkzngCE1Qi
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王子成
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_eBtVX8FBYZBVPJQd5WvWNR
          claim_id: c_rKFL5YB3eUvr9HJ4FvGs9b
          source_id: s_Qc3v5M5FxTuwdvP812tJm3
          stance: supports
          locator: CBDB:200985
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（701-800）｜历史性依据：CBDB 生年 = 1460
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_rj8d41foGGtKJkieDZVZxf
        subject_person_id: p_NoQErvZxoB9KT1iHrT84WN
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_akpqJMSSGZNHLkzngCE1Qi
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_IrlVeAgRoD8BybLfkr9_nx
          claim_id: c_rj8d41foGGtKJkieDZVZxf
          source_id: s_gQx6RLAB7kCeVWVmM7d5GD
          stance: supports
          locator: 弘治六年進士登科錄:一卷，第三甲第八十九名：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_gQx6RLAB7kCeVWVmM7d5GD
            source_type: api_record
            title: 中国历代人物传记资料库：王紹（CBDB 265617）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265617&o=json
            external_identifier: CBDB:265617
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.693Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_NoQErvZxoB9KT1iHrT84WN
        status: active
        display_name: 王紹
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_faDgY5_WBs2qxbMzz2fBcj
        subject_person_id: p_CHr9NQtS9eMRbFoCaUAQ6g
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_akpqJMSSGZNHLkzngCE1Qi
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_aB9BheNRDENYDN6_V_eSVb
          claim_id: c_faDgY5_WBs2qxbMzz2fBcj
          source_id: s_wBCHhbKSVqJ61KLLW68ZSG
          stance: supports
          locator: 弘治六年進士登科錄:一卷，第三甲第八十九名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_wBCHhbKSVqJ61KLLW68ZSG
            source_type: api_record
            title: 中国历代人物传记资料库：王本（CBDB 265615）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265615&o=json
            external_identifier: CBDB:265615
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.691Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_CHr9NQtS9eMRbFoCaUAQ6g
        status: active
        display_name: 王本
        merged_into_person_id: null
    - claim:
        id: c_FFNUq6_q-Vth837BWz-avK
        subject_person_id: p_FYFV29vLWqWc2g2kXP26Mo
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_akpqJMSSGZNHLkzngCE1Qi
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_aDDhcz_6x67_9_eZjVjlXx
          claim_id: c_FFNUq6_q-Vth837BWz-avK
          source_id: s_ChEZSY2MgQuwWtZVxi9S3h
          stance: supports
          locator: 弘治六年進士登科錄:一卷，第三甲第八十九名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_ChEZSY2MgQuwWtZVxi9S3h
            source_type: api_record
            title: 中国历代人物传记资料库：王禎（CBDB 265616）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265616&o=json
            external_identifier: CBDB:265616
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.692Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_FYFV29vLWqWc2g2kXP26Mo
        status: active
        display_name: 王禎
        merged_into_person_id: null
  descendants: []
  other:
    - claim:
        id: c_6wpbjAZyQhEyQpY8NW8eGW
        subject_person_id: p_GBf4b1HmrRRj3Ux38Gx5Hv
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_akpqJMSSGZNHLkzngCE1Qi
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_ngGTvjhTYgtaNSQDOOZq2D
          claim_id: c_6wpbjAZyQhEyQpY8NW8eGW
          source_id: s_HFYjsT_SVCvVS4ySQKZTEY
          stance: supports
          locator: CBDB 亲属：兄（KinPerson 200985 王子成）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_HFYjsT_SVCvVS4ySQKZTEY
            source_type: api_record
            title: 中国历代人物传记资料库：王子卿（CBDB 265622）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265622&o=json
            external_identifier: CBDB:265622
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_GBf4b1HmrRRj3Ux38Gx5Hv
        status: active
        display_name: 王子卿
        merged_into_person_id: null
    - claim:
        id: c_OjGKYkzJhnJHCxCDKql5DY
        subject_person_id: p_akpqJMSSGZNHLkzngCE1Qi
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_ggQxCcadxtcfySmVwm5bRK
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_GTPwLUmYDZM6uypCpNl72_
          claim_id: c_OjGKYkzJhnJHCxCDKql5DY
          source_id: s_vNmAFz76L6htHNUQ7Bsmzy
          stance: supports
          locator: CBDB 亲属：兄（KinPerson 200985 王子成）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_vNmAFz76L6htHNUQ7Bsmzy
            source_type: api_record
            title: 中国历代人物传记资料库：王子俊（CBDB 265620）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265620&o=json
            external_identifier: CBDB:265620
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_ggQxCcadxtcfySmVwm5bRK
        status: active
        display_name: 王子俊
        merged_into_person_id: null
    - claim:
        id: c_0-AABRs26W_WZiBFteqRkD
        subject_person_id: p_akpqJMSSGZNHLkzngCE1Qi
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_ifLLBKzA4dDTF7QaxtVFF9
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_0s3H93myUKORZD7Y-6XPlN
          claim_id: c_0-AABRs26W_WZiBFteqRkD
          source_id: s_eO7-5HXyQHGaOvOcJtzefi
          stance: supports
          locator: CBDB 亲属：兄（KinPerson 200985 王子成）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_eO7-5HXyQHGaOvOcJtzefi
            source_type: api_record
            title: 中国历代人物传记资料库：王子英（CBDB 265621）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265621&o=json
            external_identifier: CBDB:265621
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_ifLLBKzA4dDTF7QaxtVFF9
        status: active
        display_name: 王子英
        merged_into_person_id: null
---

# 王子成

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王子成（生于1460年），明人物。明清進士進士，籍贯咸寧，入仕進士，曾任員外郎。（中国历代人物传记资料库 CBDB 200985） | accepted |
| birth.date | 1460年 | accepted |
| name.primary | 王子成 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_NoQErvZxoB9KT1iHrT84WN | 王紹 | accepted |
| ancestors | p_CHr9NQtS9eMRbFoCaUAQ6g | 王本 | accepted |
| ancestors | p_FYFV29vLWqWc2g2kXP26Mo | 王禎 | accepted |
| other | p_GBf4b1HmrRRj3Ux38Gx5Hv | 王子卿 | accepted |
| other | p_ggQxCcadxtcfySmVwm5bRK | 王子俊 | accepted |
| other | p_ifLLBKzA4dDTF7QaxtVFF9 | 王子英 | accepted |

## 外部来源

- [中国历代人物传记资料库：王本（CBDB 265615）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265615&o=json)
- [中国历代人物传记资料库：王紹（CBDB 265617）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265617&o=json)
- [中国历代人物传记资料库：王禎（CBDB 265616）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265616&o=json)
- [中国历代人物传记资料库：王子成（CBDB 200985）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=200985&o=json)
- [中国历代人物传记资料库：王子俊（CBDB 265620）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265620&o=json)
- [中国历代人物传记资料库：王子卿（CBDB 265622）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265622&o=json)
- [中国历代人物传记资料库：王子英（CBDB 265621）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=265621&o=json)
