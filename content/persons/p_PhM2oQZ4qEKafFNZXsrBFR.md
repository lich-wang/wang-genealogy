---
schema: wang-person/v1
id: p_PhM2oQZ4qEKafFNZXsrBFR
status: active
merged_into: null
display_name: 王泰來
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_xNQQGrMyT7rf9KAfD9Yf74
        subject_person_id: p_PhM2oQZ4qEKafFNZXsrBFR
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王泰來
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_b5fiHHx43aFhBs58KS2PD2
          claim_id: c_xNQQGrMyT7rf9KAfD9Yf74
          source_id: s_eQohZGfBJ8LJ4Gn22zb7Y9
          stance: supports
          locator: CBDB:37966
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（37966）
          source: &a1
            id: s_eQohZGfBJ8LJ4Gn22zb7Y9
            source_type: api_record
            title: 中国历代人物传记资料库：王泰來（CBDB 37966）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=37966&o=json
            external_identifier: CBDB:37966
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:12.304Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_haApJob8z4FZVLfrPJaKgr
        subject_person_id: p_PhM2oQZ4qEKafFNZXsrBFR
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王泰來，宋人物。籍贯廬陵，身份为詩人。（中国历代人物传记资料库 CBDB 37966）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_8BS_3LFcRc_P03NBxnvtg3
          claim_id: c_haApJob8z4FZVLfrPJaKgr
          source_id: s_eQohZGfBJ8LJ4Gn22zb7Y9
          stance: supports
          locator: CBDB:37966
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_9VKj8jsm4J3iuw1Eh_JpJZ
        subject_person_id: p_znCpHkj9Wovs2kmQ9yEwwD
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_PhM2oQZ4qEKafFNZXsrBFR
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_3D05i9thpWQ4rd-q5LyfpU
          claim_id: c_9VKj8jsm4J3iuw1Eh_JpJZ
          source_id: s_eQohZGfBJ8LJ4Gn22zb7Y9
          stance: supports
          locator: 宋人傳記資料索引(電子版)：曾祖
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_znCpHkj9Wovs2kmQ9yEwwD
        status: active
        display_name: 王煥之
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王泰來

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王泰來 | accepted |
| bio.summary | 王泰來，宋人物。籍贯廬陵，身份为詩人。（中国历代人物传记资料库 CBDB 37966） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| ancestors | p_znCpHkj9Wovs2kmQ9yEwwD | 王煥之 | accepted |

## 外部来源

- [中国历代人物传记资料库：王泰來（CBDB 37966）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=37966&o=json)
