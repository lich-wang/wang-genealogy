---
schema: wang-person/v1
id: p_Nw8rU459EMKgrDCLjsd6UZ
status: active
merged_into: null
display_name: 翁氏
revision: 1
cbdb_id: 570982
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_Tum-AsIPgIKE_KUN_R1IG7
        subject_person_id: p_Nw8rU459EMKgrDCLjsd6UZ
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 翁氏，清人物。籍贯蕭山，身份为貞婦/節婦。（中国历代人物传记资料库 CBDB 570982）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_I1AHZeJ1XUbWh--1auimHc
          claim_id: c_Tum-AsIPgIKE_KUN_R1IG7
          source_id: s_3VSj6Y0ZlPdI7Ce4YKqJ5v
          stance: supports
          locator: CBDB:570982
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_3VSj6Y0ZlPdI7Ce4YKqJ5v
            source_type: api_record
            title: 中国历代人物传记资料库：翁氏(王家楨妻)（CBDB 570982）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=570982&o=json
            external_identifier: CBDB:570982
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_vzM9ogW4FRfYlQFqWv7SlJ
        subject_person_id: p_Nw8rU459EMKgrDCLjsd6UZ
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 翁氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_03sbax5I5ua42vuV5Ys0to
          claim_id: c_vzM9ogW4FRfYlQFqWv7SlJ
          source_id: s_3VSj6Y0ZlPdI7Ce4YKqJ5v
          stance: supports
          locator: CBDB:570982
          quotation: null
          interpretation_note: CBDB 明确记录的王家楨配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_fyW1TFQZQVb2PpOThghqld
        subject_person_id: p_pP1ZFJ9Eu2YF7pkebSdPnY
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_Nw8rU459EMKgrDCLjsd6UZ
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_KEOGMehDBz394xDwCw9SFk
          claim_id: c_fyW1TFQZQVb2PpOThghqld
          source_id: s_3VSj6Y0ZlPdI7Ce4YKqJ5v
          stance: supports
          locator: 紹興府志:八十卷，lgid=316871：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_pP1ZFJ9Eu2YF7pkebSdPnY
        status: active
        display_name: 王家楨
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 翁氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 翁氏，清人物。籍贯蕭山，身份为貞婦/節婦。（中国历代人物传记资料库 CBDB 570982） | accepted |
| name.primary | 翁氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_pP1ZFJ9Eu2YF7pkebSdPnY | 王家楨 | accepted |

## 外部来源

- [中国历代人物传记资料库：翁氏(王家楨妻)（CBDB 570982）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=570982&o=json)
