---
schema: wang-person/v1
id: p_otsXLm2etPLkQU1NnYKNXq
status: active
merged_into: null
display_name: 尹某
revision: 1
cbdb_id: 150188
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_VkAjRWMAsu_aQZwxj8dJeS
        subject_person_id: p_otsXLm2etPLkQU1NnYKNXq
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 尹某，史料所见人物。本项目依据《中国历代人物传记资料库：尹某(王媛夫)（CBDB 150188）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_moN2yq5al0OY0GYSEBQgbA
          claim_id: c_VkAjRWMAsu_aQZwxj8dJeS
          source_id: s_52w68rmb-Cdp8B7S1vbQwS
          stance: supports
          locator: CBDB:150188
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_52w68rmb-Cdp8B7S1vbQwS
            source_type: api_record
            title: 中国历代人物传记资料库：尹某(王媛夫)（CBDB 150188）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=150188&o=json
            external_identifier: CBDB:150188
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_8FqTcFDkOWl1WwcGr7R1Xh
        subject_person_id: p_otsXLm2etPLkQU1NnYKNXq
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 尹某
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_yqeEbBLk0pEkWHKKUBiw6X
          claim_id: c_8FqTcFDkOWl1WwcGr7R1Xh
          source_id: s_52w68rmb-Cdp8B7S1vbQwS
          stance: supports
          locator: CBDB:150188
          quotation: null
          interpretation_note: CBDB 明确记录的王媛配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_4fB8Sb39-ioxNk72SNvUw_
        subject_person_id: p_2oAxBsuJQKXAAm52SsY22K
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_otsXLm2etPLkQU1NnYKNXq
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_xswMZH6ACHheOnLkhsGeJI
          claim_id: c_4fB8Sb39-ioxNk72SNvUw_
          source_id: s_52w68rmb-Cdp8B7S1vbQwS
          stance: supports
          locator: 唐代墓誌匯編:二卷，Shengli 46：丈夫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_2oAxBsuJQKXAAm52SsY22K
        status: active
        display_name: 王媛
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 尹某

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 尹某，史料所见人物。本项目依据《中国历代人物传记资料库：尹某(王媛夫)（CBDB 150188）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 尹某 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_2oAxBsuJQKXAAm52SsY22K | 王媛 | accepted |

## 外部来源

- [中国历代人物传记资料库：尹某(王媛夫)（CBDB 150188）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=150188&o=json)
