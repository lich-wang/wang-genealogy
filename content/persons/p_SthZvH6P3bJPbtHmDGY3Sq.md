---
schema: wang-person/v1
id: p_SthZvH6P3bJPbtHmDGY3Sq
status: active
merged_into: null
display_name: 王珍
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_3E3y3awPLLGbu4rFzEf7kK
        subject_person_id: p_SthZvH6P3bJPbtHmDGY3Sq
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王珍
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_tEHX2j4VEn5W2KBJm7H5V3
          claim_id: c_3E3y3awPLLGbu4rFzEf7kK
          source_id: s_bAe43a5jBdBknQW7f5t7PK
          stance: supports
          locator: CBDB:100728
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（100728）
          source: &a1
            id: s_bAe43a5jBdBknQW7f5t7PK
            source_type: api_record
            title: 中国历代人物传记资料库：王珍（CBDB 100728）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=100728&o=json
            external_identifier: CBDB:100728
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:14.398Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_yBh9Vf1JXjaGvy2cvZF6Nx
        subject_person_id: p_SthZvH6P3bJPbtHmDGY3Sq
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王珍，元人物。籍贯南樂，曾任諸路總管府治中、同僉行省事、路萬戶。（中国历代人物传记资料库 CBDB 100728）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_QlGmQoEXhk1EBlJWcV5OGD
          claim_id: c_yBh9Vf1JXjaGvy2cvZF6Nx
          source_id: s_bAe43a5jBdBknQW7f5t7PK
          stance: supports
          locator: CBDB:100728
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_wXda1wAOnFjdTJj9C5tHyL
        subject_person_id: p_SthZvH6P3bJPbtHmDGY3Sq
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_dsvNgQDFVWqqs9SGFq9k5p
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Ep27uU8SQ710-bJtZ3Q1Tn
          claim_id: c_wXda1wAOnFjdTJj9C5tHyL
          source_id: s_4BAJxoiAyuMveQqoydEvN3
          stance: supports
          locator: 元人傳記資料索引，935：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_4BAJxoiAyuMveQqoydEvN3
            source_type: api_record
            title: 中国历代人物传记资料库：王文幹（CBDB 101101）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=101101&o=json
            external_identifier: CBDB:101101
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:14.688Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_dsvNgQDFVWqqs9SGFq9k5p
        status: active
        display_name: 王文幹
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王珍

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王珍 | accepted |
| bio.summary | 王珍，元人物。籍贯南樂，曾任諸路總管府治中、同僉行省事、路萬戶。（中国历代人物传记资料库 CBDB 100728） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_dsvNgQDFVWqqs9SGFq9k5p | 王文幹 | accepted |

## 外部来源

- [中国历代人物传记资料库：王文幹（CBDB 101101）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=101101&o=json)
- [中国历代人物传记资料库：王珍（CBDB 100728）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=100728&o=json)
