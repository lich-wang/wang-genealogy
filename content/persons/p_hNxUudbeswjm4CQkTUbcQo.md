---
schema: wang-person/v1
id: p_hNxUudbeswjm4CQkTUbcQo
status: active
merged_into: null
display_name: 王琨
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_MK6KTqipcj4rqLGhWXY78Y
        subject_person_id: p_hNxUudbeswjm4CQkTUbcQo
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王琨
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_kXLC1cnWDZ2BGw1TWZLiUQ
          claim_id: c_MK6KTqipcj4rqLGhWXY78Y
          source_id: s_3P1Ehz7fDt6H1VqN9Ui9w4
          stance: supports
          locator: CBDB:314221
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（314221）
          source: &a1
            id: s_3P1Ehz7fDt6H1VqN9Ui9w4
            source_type: api_record
            title: 中国历代人物传记资料库：王琨（CBDB 314221）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=314221&o=json
            external_identifier: CBDB:314221
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.920Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_PLeuWspukg2nJTd9FbMrYY
        subject_person_id: p_hNxUudbeswjm4CQkTUbcQo
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王琨，明人物。嘉靖二十九年進士，曾任刑部主事。（中国历代人物传记资料库 CBDB 314221）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_EI6pUSlGzfy_BfvqZXCURV
          claim_id: c_PLeuWspukg2nJTd9FbMrYY
          source_id: s_3P1Ehz7fDt6H1VqN9Ui9w4
          stance: supports
          locator: CBDB:314221
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_7SMZIzNyjGvCb1cYlnS4mR
        subject_person_id: p_hNxUudbeswjm4CQkTUbcQo
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_FJ5VCJ2dfQ9YvKZT3bmX4c
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_nKCKhzv8tl70VUPW2Q3gWH
          claim_id: c_7SMZIzNyjGvCb1cYlnS4mR
          source_id: s_3P1Ehz7fDt6H1VqN9Ui9w4
          stance: supports
          locator: 嘉靖二十九年進士登科錄:一卷，第三甲第一百五十四名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_FJ5VCJ2dfQ9YvKZT3bmX4c
        status: active
        display_name: 王汝述
        merged_into_person_id: null
  other: []
---

# 王琨

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王琨 | accepted |
| bio.summary | 王琨，明人物。嘉靖二十九年進士，曾任刑部主事。（中国历代人物传记资料库 CBDB 314221） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_FJ5VCJ2dfQ9YvKZT3bmX4c | 王汝述 | accepted |

## 外部来源

- [中国历代人物传记资料库：王琨（CBDB 314221）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=314221&o=json)
