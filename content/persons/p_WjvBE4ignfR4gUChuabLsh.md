---
schema: wang-person/v1
id: p_WjvBE4ignfR4gUChuabLsh
status: active
merged_into: null
display_name: 王道咸
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_GppqtGsif6XGESt4Dfgzxo
        subject_person_id: p_WjvBE4ignfR4gUChuabLsh
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王道咸
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_DBxVYfiCvPnMwrKcAFBdpG
          claim_id: c_GppqtGsif6XGESt4Dfgzxo
          source_id: s_PthE7R6DNPj8BDTG7Wr3WH
          stance: supports
          locator: CBDB:703575
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（703575）
          source: &a1
            id: s_PthE7R6DNPj8BDTG7Wr3WH
            source_type: api_record
            title: 中国历代人物传记资料库：王道咸（CBDB 703575）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=703575&o=json
            external_identifier: CBDB:703575
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:06.626Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_zFPpGC3XYrksUksRhq47cn
        subject_person_id: p_WjvBE4ignfR4gUChuabLsh
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王道咸，明人物。籍贯鄞縣。（中国历代人物传记资料库 CBDB 703575）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_usgqckZL9TT1kEh_L4yY7x
          claim_id: c_zFPpGC3XYrksUksRhq47cn
          source_id: s_PthE7R6DNPj8BDTG7Wr3WH
          stance: supports
          locator: CBDB:703575
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_5Vb-Itj9inu5tzqD1n3QXk
        subject_person_id: p_WzPiheA3vwptPMZvPCJNwC
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_WjvBE4ignfR4gUChuabLsh
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Hhb8AxJZHHr-a6m2wEdsK8
          claim_id: c_5Vb-Itj9inu5tzqD1n3QXk
          source_id: s_PthE7R6DNPj8BDTG7Wr3WH
          stance: supports
          locator: 鄞縣志，lgid=243323：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_WzPiheA3vwptPMZvPCJNwC
        status: active
        display_name: 王嗣奭
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王道咸

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王道咸 | accepted |
| bio.summary | 王道咸，明人物。籍贯鄞縣。（中国历代人物传记资料库 CBDB 703575） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_WzPiheA3vwptPMZvPCJNwC | 王嗣奭 | accepted |

## 外部来源

- [中国历代人物传记资料库：王道咸（CBDB 703575）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=703575&o=json)
