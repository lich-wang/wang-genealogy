---
schema: wang-person/v1
id: p_7XFSgFj9V9fUQFp9e4eBUF
status: active
merged_into: null
display_name: 王愷
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_nU9TAiZq1B9rPRci92BmHn
        subject_person_id: p_7XFSgFj9V9fUQFp9e4eBUF
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王愷
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_HKLV6q8cp5g9RkUNtyqn4Y
          claim_id: c_nU9TAiZq1B9rPRci92BmHn
          source_id: s_8H9r69eybm93dX8AZB2rp9
          stance: supports
          locator: CBDB:228571
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（228571）
          source: &a1
            id: s_8H9r69eybm93dX8AZB2rp9
            source_type: api_record
            title: 中国历代人物传记资料库：王愷（CBDB 228571）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=228571&o=json
            external_identifier: CBDB:228571
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:17.561Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_7BxQ83SfrysLAJ4H3v19hs
        subject_person_id: p_7XFSgFj9V9fUQFp9e4eBUF
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王愷，明人物。萬曆丙戌科進士進士，曾任指揮。（中国历代人物传记资料库 CBDB 228571）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_Slx32oofCor-bqnQ4SiMwB
          claim_id: c_7BxQ83SfrysLAJ4H3v19hs
          source_id: s_8H9r69eybm93dX8AZB2rp9
          stance: supports
          locator: CBDB:228571
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_xwJgB9iJbK_OlN2Kb2JQNk
        subject_person_id: p_7XFSgFj9V9fUQFp9e4eBUF
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_Z1sXa1thNM94WHiooGYHug
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_ha8tLfHRT-3gWOLKSG7F5p
          claim_id: c_xwJgB9iJbK_OlN2Kb2JQNk
          source_id: s_8H9r69eybm93dX8AZB2rp9
          stance: supports
          locator: 萬曆丙戌科進士同年總錄，第三甲第一百六十五名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_Z1sXa1thNM94WHiooGYHug
        status: active
        display_name: 王孟煦
        merged_into_person_id: null
  other: []
---

# 王愷

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王愷 | accepted |
| bio.summary | 王愷，明人物。萬曆丙戌科進士進士，曾任指揮。（中国历代人物传记资料库 CBDB 228571） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_Z1sXa1thNM94WHiooGYHug | 王孟煦 | accepted |

## 外部来源

- [中国历代人物传记资料库：王愷（CBDB 228571）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=228571&o=json)
