---
schema: wang-person/v1
id: p_Dt54odQN3CFe2bvZC88Thv
status: active
merged_into: null
display_name: 王寅
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_VaQBFvoKvBBxKv9LrNkG4h
        subject_person_id: p_Dt54odQN3CFe2bvZC88Thv
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王寅
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_XaCME2Eawwo74stodR22ge
          claim_id: c_VaQBFvoKvBBxKv9LrNkG4h
          source_id: s_qTVHob728dwJNHD16NpDKS
          stance: supports
          locator: CBDB:318727
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（318727）
          source: &a1
            id: s_qTVHob728dwJNHD16NpDKS
            source_type: api_record
            title: 中国历代人物传记资料库：王寅（CBDB 318727）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=318727&o=json
            external_identifier: CBDB:318727
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:20.020Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_pk16DGorJgzmfroQGczQyh
        subject_person_id: p_Dt54odQN3CFe2bvZC88Thv
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王寅，明人物。嘉靖三十二年進士，曾任知縣。（中国历代人物传记资料库 CBDB 318727）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_9w7ChCh89v82ljLO3kzpQi
          claim_id: c_pk16DGorJgzmfroQGczQyh
          source_id: s_qTVHob728dwJNHD16NpDKS
          stance: supports
          locator: CBDB:318727
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_afhDOfFYtamwVcjDWJGKG6
        subject_person_id: p_Dt54odQN3CFe2bvZC88Thv
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_cDtAvQGzuVZRFeJxenEh2g
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_lGl_JScNO6p9xYAhFqI_WC
          claim_id: c_afhDOfFYtamwVcjDWJGKG6
          source_id: s_qTVHob728dwJNHD16NpDKS
          stance: supports
          locator: 嘉靖三十二年進士登科錄:一卷，第三甲第二百六十九名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_cDtAvQGzuVZRFeJxenEh2g
        status: active
        display_name: 王可信
        merged_into_person_id: null
  other: []
---

# 王寅

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王寅 | accepted |
| bio.summary | 王寅，明人物。嘉靖三十二年進士，曾任知縣。（中国历代人物传记资料库 CBDB 318727） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_cDtAvQGzuVZRFeJxenEh2g | 王可信 | accepted |

## 外部来源

- [中国历代人物传记资料库：王寅（CBDB 318727）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=318727&o=json)
