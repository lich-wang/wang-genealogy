---
schema: wang-person/v1
id: p_9VtJ7v2PnFxabdG54S3Qd2
status: active
merged_into: null
display_name: 王恩
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_jpxRKERPEtmCJ3kqcuovfg
        subject_person_id: p_9VtJ7v2PnFxabdG54S3Qd2
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王恩
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_nL6WCFynaUyCq9bubT65Mx
          claim_id: c_jpxRKERPEtmCJ3kqcuovfg
          source_id: s_iCtF9Qkm3KNJPcyjL6BgPm
          stance: supports
          locator: CBDB:213938
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（213938）
          source: &a1
            id: s_iCtF9Qkm3KNJPcyjL6BgPm
            source_type: api_record
            title: 中国历代人物传记资料库：王恩（CBDB 213938）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=213938&o=json
            external_identifier: CBDB:213938
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:17.126Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_kZSyUE6Q7DMSJbHYVzDxb8
        subject_person_id: p_9VtJ7v2PnFxabdG54S3Qd2
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王恩，明人物。萬曆二年進士，曾任都察院右僉都御史、知縣、中憲大夫。（中国历代人物传记资料库 CBDB 213938）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_TfuEgADo-BY5agYE5jRzOR
          claim_id: c_kZSyUE6Q7DMSJbHYVzDxb8
          source_id: s_iCtF9Qkm3KNJPcyjL6BgPm
          stance: supports
          locator: CBDB:213938
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_Sdqt4dwkYDw3H6cCryhKWp
        subject_person_id: p_9VtJ7v2PnFxabdG54S3Qd2
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_GDsYhHNBBkDcpWQEYco8Kg
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_l-9b0QI-oOQjzL6qxPC18k
          claim_id: c_Sdqt4dwkYDw3H6cCryhKWp
          source_id: s_iCtF9Qkm3KNJPcyjL6BgPm
          stance: supports
          locator: 萬曆二年進士登科錄:一卷，第三甲第八十名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_GDsYhHNBBkDcpWQEYco8Kg
        status: active
        display_name: 王開
        merged_into_person_id: null
  other: []
---

# 王恩

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王恩 | accepted |
| bio.summary | 王恩，明人物。萬曆二年進士，曾任都察院右僉都御史、知縣、中憲大夫。（中国历代人物传记资料库 CBDB 213938） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_GDsYhHNBBkDcpWQEYco8Kg | 王開 | accepted |

## 外部来源

- [中国历代人物传记资料库：王恩（CBDB 213938）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=213938&o=json)
