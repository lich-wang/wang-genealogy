---
schema: wang-person/v1
id: p_AM2s7ac4Vm7QuHGn7oUxv8
status: active
merged_into: null
display_name: 王禎
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_gX2P7LAZPSV2mQB4QFfN6T
        subject_person_id: p_AM2s7ac4Vm7QuHGn7oUxv8
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王禎
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_qRD5fezHMnGTNQg7MgvgXv
          claim_id: c_gX2P7LAZPSV2mQB4QFfN6T
          source_id: s_duDQsCQMvkTQ6nJS4ikRPb
          stance: supports
          locator: CBDB:67862
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（67862）
          source: &a1
            id: s_duDQsCQMvkTQ6nJS4ikRPb
            source_type: api_record
            title: 中国历代人物传记资料库：王禎（CBDB 67862）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=67862&o=json
            external_identifier: CBDB:67862
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:13.025Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_Ve6W6JeJC18grB615H6i4C
        subject_person_id: p_AM2s7ac4Vm7QuHGn7oUxv8
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王禎，明人物。明清進士進士，籍贯乾州，入仕進士，曾任兵科給事中、兵科右給事中、都察院右僉都御史。（中国历代人物传记资料库 CBDB 67862）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_upV2q8-OeREBZmLVW4PaMG
          claim_id: c_Ve6W6JeJC18grB615H6i4C
          source_id: s_duDQsCQMvkTQ6nJS4ikRPb
          stance: supports
          locator: CBDB:67862
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_verc8ntM8fjHGww3N63dxw
        subject_person_id: p_AM2s7ac4Vm7QuHGn7oUxv8
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_m9oVG7UM6Unr739HgWUu1H
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_R-CyqU_8NERCuiEiWRe7na
          claim_id: c_verc8ntM8fjHGww3N63dxw
          source_id: s_duDQsCQMvkTQ6nJS4ikRPb
          stance: supports
          locator: 人名權威資料（中央研究院歷史語言研究所），11507：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_m9oVG7UM6Unr739HgWUu1H
        status: active
        display_name: 王子直
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王禎

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王禎 | accepted |
| bio.summary | 王禎，明人物。明清進士進士，籍贯乾州，入仕進士，曾任兵科給事中、兵科右給事中、都察院右僉都御史。（中国历代人物传记资料库 CBDB 67862） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_m9oVG7UM6Unr739HgWUu1H | 王子直 | accepted |

## 外部来源

- [中国历代人物传记资料库：王禎（CBDB 67862）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=67862&o=json)
