---
schema: wang-person/v1
id: p_QC8gM4R78b4ALUG573C8tw
status: active
merged_into: null
display_name: 王明贞
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_vJcfKzKrH2n8DewCMaFvMo
        subject_person_id: p_QC8gM4R78b4ALUG573C8tw
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王明贞（1906年11月18日—2010年8月28日），史料所见人物。本项目依据《王明贞》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_UWYjqjVROgajSMHCwC0S4F
          claim_id: c_vJcfKzKrH2n8DewCMaFvMo
          source_id: s_tmHCmvEMPsE5UH3YGfsgVW
          stance: supports
          locator: Q56005549
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_tmHCmvEMPsE5UH3YGfsgVW
            source_type: api_record
            title: 维基数据：王明贞（Q56005549）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q56005549
            external_identifier: Q56005549
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:50.151Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%98%8E%E8%B4%9E
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_kVjY14GR5HgSvHh63rKBNS
        subject_person_id: p_QC8gM4R78b4ALUG573C8tw
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1906年11月18日
            calendar_note: 维基数据 P569 结构化日期，精度：日
            earliest: 1906-11-18
            latest: 1906-11-18
            precision: day
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_FnkXKZfVoyC1DVB3kwJzPy
          claim_id: c_kVjY14GR5HgSvHh63rKBNS
          source_id: s_tmHCmvEMPsE5UH3YGfsgVW
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_tmHCmvEMPsE5UH3YGfsgVW
            source_type: api_record
            title: 维基数据：王明贞（Q56005549）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q56005549
            external_identifier: Q56005549
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:50.151Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%98%8E%E8%B4%9E
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_gQ5gbUW31Q8grs15NAXdvT
        subject_person_id: p_QC8gM4R78b4ALUG573C8tw
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 2010年8月28日
            calendar_note: 维基数据 P570 结构化日期，精度：日
            earliest: 2010-08-28
            latest: 2010-08-28
            precision: day
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_rydN6KET4ehGZawp5ZvPSX
          claim_id: c_gQ5gbUW31Q8grs15NAXdvT
          source_id: s_tmHCmvEMPsE5UH3YGfsgVW
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_tmHCmvEMPsE5UH3YGfsgVW
            source_type: api_record
            title: 维基数据：王明贞（Q56005549）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q56005549
            external_identifier: Q56005549
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:50.151Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%98%8E%E8%B4%9E
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_szJHych7TM5UCM3oJNK7jH
        subject_person_id: p_QC8gM4R78b4ALUG573C8tw
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王明贞
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_JHERpP2Pcfsd2eX2dqEoTG
          claim_id: c_szJHych7TM5UCM3oJNK7jH
          source_id: s_tmHCmvEMPsE5UH3YGfsgVW
          stance: supports
          locator: Q56005549
          quotation: null
          interpretation_note: null
          source:
            id: s_tmHCmvEMPsE5UH3YGfsgVW
            source_type: api_record
            title: 维基数据：王明贞（Q56005549）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q56005549
            external_identifier: Q56005549
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:50.151Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%98%8E%E8%B4%9E
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_LoLYPa74L68zMyhm7AZ6Pr
        subject_person_id: p_LXKFHguzJ5DjK8ySHadw6f
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_QC8gM4R78b4ALUG573C8tw
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_7xokBosMopXuUhizi98b7y
          claim_id: c_LoLYPa74L68zMyhm7AZ6Pr
          source_id: s_xmDk28iHW5carA3UyGCGoB
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_xmDk28iHW5carA3UyGCGoB
            source_type: api_record
            title: 维基数据：王季同（Q61306471）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q61306471
            external_identifier: Q61306471
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:37.417Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%AD%A3%E5%90%8C
        - id: cs_QxaNDEp8VRb9MQEq9AkBY9
          claim_id: c_LoLYPa74L68zMyhm7AZ6Pr
          source_id: s_tmHCmvEMPsE5UH3YGfsgVW
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source: *a1
      object_person:
        id: p_LXKFHguzJ5DjK8ySHadw6f
        status: active
        display_name: 王季同
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_ToN7hGNYx2GEWed4aWSYXP
        subject_person_id: p_aYYXCweNVeHc7MXxonEf2p
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_QC8gM4R78b4ALUG573C8tw
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_JBR2zKeub7nho42HRi8MA8
          claim_id: c_ToN7hGNYx2GEWed4aWSYXP
          source_id: s_tmHCmvEMPsE5UH3YGfsgVW
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source: *a1
        - id: cs_B1ArAvtQARJkpPEoJuuX7f
          claim_id: c_ToN7hGNYx2GEWed4aWSYXP
          source_id: s_QeCFrSiuzYMpJLGA5vSMGK
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_QeCFrSiuzYMpJLGA5vSMGK
            source_type: api_record
            title: 维基数据：俞启忠（Q61306665）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q61306665
            external_identifier: Q61306665
            license_code: CC0-1.0
            accessed_at: 2026-09-05T08:06:50.055Z
            metadata_json: null
      object_person:
        id: p_aYYXCweNVeHc7MXxonEf2p
        status: active
        display_name: 俞启忠
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 王明贞

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王明贞（1906年11月18日—2010年8月28日），史料所见人物。本项目依据《王明贞》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| birth.date | 1906年11月18日 | accepted |
| death.date | 2010年8月28日 | accepted |
| name.primary | 王明贞 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_LXKFHguzJ5DjK8ySHadw6f | 王季同 | accepted |
| spouses | p_aYYXCweNVeHc7MXxonEf2p | 俞启忠 | accepted |

## 外部来源

- [维基数据：王季同（Q61306471）](https://www.wikidata.org/wiki/Q61306471)
- [维基数据：王明贞（Q56005549）](https://www.wikidata.org/wiki/Q56005549)
- [维基数据：俞启忠（Q61306665）](https://www.wikidata.org/wiki/Q61306665)
