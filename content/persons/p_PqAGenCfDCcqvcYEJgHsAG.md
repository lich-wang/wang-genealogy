---
schema: wang-person/v1
id: p_PqAGenCfDCcqvcYEJgHsAG
status: active
merged_into: null
display_name: 第五悰
revision: 1
cbdb_id: 146349
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_yooU3a83DPmyc1256iln7m
        subject_person_id: p_PqAGenCfDCcqvcYEJgHsAG
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 第五悰（816—835），唐人物。籍贯河南。（中国历代人物传记资料库 CBDB 146349）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_v14eQI6SbvWcx8zfmY1vtJ
          claim_id: c_yooU3a83DPmyc1256iln7m
          source_id: s_5JCHR9odB3ZrJLIfPJjn2_
          stance: supports
          locator: CBDB:146349
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_5JCHR9odB3ZrJLIfPJjn2_
            source_type: api_record
            title: 中国历代人物传记资料库：第五悰（CBDB 146349）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=146349&o=json
            external_identifier: CBDB:146349
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_R3pM_OX7MUX-9cGDp_8fbC
        subject_person_id: p_PqAGenCfDCcqvcYEJgHsAG
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 第五悰
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_aAy36_XKIsYYgz06Mf8s8O
          claim_id: c_R3pM_OX7MUX-9cGDp_8fbC
          source_id: s_5JCHR9odB3ZrJLIfPJjn2_
          stance: supports
          locator: CBDB:146349
          quotation: null
          interpretation_note: CBDB 明确记录的王滕配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_AKRTtbC4QcGI0hKC9mf_NP
        subject_person_id: p_Heoj3oaQRQiQ5W2ijnMteF
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_PqAGenCfDCcqvcYEJgHsAG
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_XqXOamjJpUjNzMOyeYAMYR
          claim_id: c_AKRTtbC4QcGI0hKC9mf_NP
          source_id: s_5JCHR9odB3ZrJLIfPJjn2_
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_Heoj3oaQRQiQ5W2ijnMteF
        status: active
        display_name: 王滕
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 第五悰

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 第五悰（816—835），唐人物。籍贯河南。（中国历代人物传记资料库 CBDB 146349） | accepted |
| name.primary | 第五悰 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_Heoj3oaQRQiQ5W2ijnMteF | 王滕 | accepted |

## 外部来源

- [中国历代人物传记资料库：第五悰（CBDB 146349）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=146349&o=json)
