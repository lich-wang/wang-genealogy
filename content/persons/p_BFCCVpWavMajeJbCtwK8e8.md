---
schema: wang-person/v1
id: p_BFCCVpWavMajeJbCtwK8e8
status: active
merged_into: null
display_name: 劉氏
revision: 1
cbdb_id: 570744
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_IQd3HCnyOSUbiuV0m4t9MR
        subject_person_id: p_BFCCVpWavMajeJbCtwK8e8
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉氏，清人物。籍贯福建省，身份为貞婦/節婦。（中国历代人物传记资料库 CBDB 570744）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_QiCaNz5nu1C18OWNgmQDR-
          claim_id: c_IQd3HCnyOSUbiuV0m4t9MR
          source_id: s_IqybNQr2nnAA-i3gU2BMmm
          stance: supports
          locator: CBDB:570744
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_IqybNQr2nnAA-i3gU2BMmm
            source_type: api_record
            title: 中国历代人物传记资料库：劉氏(王雍文妾)（CBDB 570744）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=570744&o=json
            external_identifier: CBDB:570744
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_2W1h1t9veBmYhMzHwljdXP
        subject_person_id: p_BFCCVpWavMajeJbCtwK8e8
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_jbT5XEakTVleRioGqr_Moe
          claim_id: c_2W1h1t9veBmYhMzHwljdXP
          source_id: s_IqybNQr2nnAA-i3gU2BMmm
          stance: supports
          locator: CBDB:570744
          quotation: null
          interpretation_note: CBDB 明确记录的王雍文配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_bZforFSrK3yiUkxercAX83
        subject_person_id: p_hF4P8JXPJda96ebQwWdNmF
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_BFCCVpWavMajeJbCtwK8e8
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_6Ppwj_oHvJKnSliCA84ivZ
          claim_id: c_bZforFSrK3yiUkxercAX83
          source_id: s_IqybNQr2nnAA-i3gU2BMmm
          stance: supports
          locator: 紹興府志:八十卷，lgid=316847：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_hF4P8JXPJda96ebQwWdNmF
        status: active
        display_name: 王雍文
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 劉氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 劉氏，清人物。籍贯福建省，身份为貞婦/節婦。（中国历代人物传记资料库 CBDB 570744） | accepted |
| name.primary | 劉氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_hF4P8JXPJda96ebQwWdNmF | 王雍文 | accepted |

## 外部来源

- [中国历代人物传记资料库：劉氏(王雍文妾)（CBDB 570744）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=570744&o=json)
