---
schema: wang-person/v1
id: p_4ies3yZifswwPZ2SViyGoy
status: active
merged_into: null
display_name: 王恭
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_GKs5FQMVbj7abAgSYvmN5D
        subject_person_id: p_4ies3yZifswwPZ2SViyGoy
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王恭
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_H2mcN1rWKS3FW9PFBu4rmJ
          claim_id: c_GKs5FQMVbj7abAgSYvmN5D
          source_id: s_m6uVLk1MfUELHA4M1tnT4s
          stance: supports
          locator: CBDB:147196
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（147196）
          source: &a1
            id: s_m6uVLk1MfUELHA4M1tnT4s
            source_type: api_record
            title: 中国历代人物传记资料库：王恭（CBDB 147196）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=147196&o=json
            external_identifier: CBDB:147196
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.727Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_ZZzHC4ztA7eWu9qeHXY2G5
        subject_person_id: p_4ies3yZifswwPZ2SViyGoy
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王恭，南北朝人物。籍贯太原，曾任州司法參軍。（中国历代人物传记资料库 CBDB 147196）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_4vHokYHSbzY2sP67OHIq9P
          claim_id: c_ZZzHC4ztA7eWu9qeHXY2G5
          source_id: s_m6uVLk1MfUELHA4M1tnT4s
          stance: supports
          locator: CBDB:147196
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_WuQpcEVHhzYyeWWNfkD1v8
        subject_person_id: p_4ies3yZifswwPZ2SViyGoy
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_hexhPbYBH42osz9g6ehbBi
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_YFhV2FA_9XxACfeAMMIJdb
          claim_id: c_WuQpcEVHhzYyeWWNfkD1v8
          source_id: s_m6uVLk1MfUELHA4M1tnT4s
          stance: supports
          locator: 唐代墓誌匯編:二卷，Yonghui 121：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_hexhPbYBH42osz9g6ehbBi
        status: active
        display_name: 王寬
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王恭

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王恭 | accepted |
| bio.summary | 王恭，南北朝人物。籍贯太原，曾任州司法參軍。（中国历代人物传记资料库 CBDB 147196） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_hexhPbYBH42osz9g6ehbBi | 王寬 | accepted |

## 外部来源

- [中国历代人物传记资料库：王恭（CBDB 147196）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=147196&o=json)
