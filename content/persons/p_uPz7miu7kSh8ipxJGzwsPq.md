---
schema: wang-person/v1
id: p_uPz7miu7kSh8ipxJGzwsPq
status: active
merged_into: null
display_name: 王承僎
cbdb_id: 17754
revision: 1
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_yZv4FE9BqKTxnxJyoBPH11
        subject_person_id: p_uPz7miu7kSh8ipxJGzwsPq
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王承僎，宋人物。CBDB 记录其籍贯记录为海北州，曾任尚書左右丞。中国历代人物传记资料库（CBDB）以人物编号 17754 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_bjNFCpuPLUKtuQbsWDWGKX
          claim_id: c_yZv4FE9BqKTxnxJyoBPH11
          source_id: s_4KJkEEeYfKjCKRRJ1eij2q
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_4KJkEEeYfKjCKRRJ1eij2q
            source_type: api_record
            title: 维基数据：王承僎（Q45387121）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45387121
            external_identifier: Q45387121
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:00:03.152Z
            metadata_json: null
        - id: cs_JYpDYpvZB26VnirZ2DlGI-
          claim_id: c_yZv4FE9BqKTxnxJyoBPH11
          source_id: s_Wxq9iXUdTB7ZTa8abynSPo
          stance: supports
          locator: CBDB:17754
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_Wxq9iXUdTB7ZTa8abynSPo
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王承僎（17754）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=17754&o=json
            external_identifier: CBDB:17754
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-05T02:00:03.309Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_YYoq2seXfKaM82jDX7B84A
        subject_person_id: p_uPz7miu7kSh8ipxJGzwsPq
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王承僎
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_9MqHt1GxdTNG3F3qaMoxqj
          claim_id: c_YYoq2seXfKaM82jDX7B84A
          source_id: s_4KJkEEeYfKjCKRRJ1eij2q
          stance: supports
          locator: Q45387121
          quotation: null
          interpretation_note: null
          source:
            id: s_4KJkEEeYfKjCKRRJ1eij2q
            source_type: api_record
            title: 维基数据：王承僎（Q45387121）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45387121
            external_identifier: Q45387121
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:00:03.152Z
            metadata_json: null
        - id: cs_LT2Ab6BnA6RK9N4wREx4xo
          claim_id: c_YYoq2seXfKaM82jDX7B84A
          source_id: s_Wxq9iXUdTB7ZTa8abynSPo
          stance: supports
          locator: Q45387121
          quotation: null
          interpretation_note: null
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_bwniFvu42yEC3uYF6ZeDN5
        subject_person_id: p_jhAXJEb1U9Ldt73n9uUeLS
        claim_kind: relationship
        predicate: kinship.parent_of
        object_person_id: p_uPz7miu7kSh8ipxJGzwsPq
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_4Z2vMQuxmjEgYuRDAXzucs
          claim_id: c_bwniFvu42yEC3uYF6ZeDN5
          source_id: s_DD7i8QKPn7xPVwKmB24Dfi
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_DD7i8QKPn7xPVwKmB24Dfi
            source_type: api_record
            title: 维基数据：王审琦（Q15940340）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15940340
            external_identifier: Q15940340
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:48:40.526Z
            metadata_json: null
        - id: cs_V8rQ1NBdB69k9fX9c4fXQy
          claim_id: c_bwniFvu42yEC3uYF6ZeDN5
          source_id: s_xENpuVFCdYJGng5v6VfekN
          stance: supports
          locator: 亲属关系：子
          quotation: null
          interpretation_note: CBDB 注明此条来源：宋人傳記資料索引(電子版)
          source:
            id: s_xENpuVFCdYJGng5v6VfekN
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王審琦（3976）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=3976&o=json
            external_identifier: CBDB:3976
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-05T02:01:41.447Z
            metadata_json: null
        - id: cs_xCt5FkS2gdRjevwVTC8Eoc
          claim_id: c_bwniFvu42yEC3uYF6ZeDN5
          source_id: s_4KJkEEeYfKjCKRRJ1eij2q
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_4KJkEEeYfKjCKRRJ1eij2q
            source_type: api_record
            title: 维基数据：王承僎（Q45387121）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45387121
            external_identifier: Q45387121
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:00:03.152Z
            metadata_json: null
      object_person:
        id: p_jhAXJEb1U9Ldt73n9uUeLS
        status: active
        display_name: 王审琦
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王承僎

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王承僎，宋人物。CBDB 记录其籍贯记录为海北州，曾任尚書左右丞。中国历代人物传记资料库（CBDB）以人物编号 17754 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。 | accepted |
| name.primary | 王承僎 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_jhAXJEb1U9Ldt73n9uUeLS | 王审琦 | accepted |

## 外部来源

- [维基数据：王承僎（Q45387121）](https://www.wikidata.org/wiki/Q45387121)
- [维基数据：王审琦（Q15940340）](https://www.wikidata.org/wiki/Q15940340)
- [CBDB 中国历代人物传记资料库：王承僎（17754）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=17754&o=json)
- [CBDB 中国历代人物传记资料库：王審琦（3976）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=3976&o=json)
