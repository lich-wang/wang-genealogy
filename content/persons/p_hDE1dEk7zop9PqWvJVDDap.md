---
schema: wang-person/v1
id: p_hDE1dEk7zop9PqWvJVDDap
status: active
merged_into: null
display_name: 王光逢
cbdb_id: 26616
revision: 3
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_6YqLhKwDvhA6P2NE3dzPZF
        subject_person_id: p_hDE1dEk7zop9PqWvJVDDap
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王光逢（1153年—1229年），史料所见人物。本项目依据《中国历代人物传记资料库：王光逢（CBDB 26616）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_op1-syGcuAFP2ieNen-nIJ
          claim_id: c_6YqLhKwDvhA6P2NE3dzPZF
          source_id: s_VdoY9CdigiKgYKXx3QmikP
          stance: supports
          locator: CBDB:26616
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_VdoY9CdigiKgYKXx3QmikP
            source_type: api_record
            title: 中国历代人物传记资料库：王光逢（CBDB 26616）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=26616&o=json
            external_identifier: CBDB:26616
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:05.401Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_CSZJbwkbr21NDpHG3NmWTw
        subject_person_id: p_hDE1dEk7zop9PqWvJVDDap
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1153年
            calendar_note: CBDB 生年字段，精度：年
            earliest: 1153-01-01
            latest: 1153-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_Ww7LcJJLm4Gc83YPWvAVwf
          claim_id: c_CSZJbwkbr21NDpHG3NmWTw
          source_id: s_VdoY9CdigiKgYKXx3QmikP
          stance: supports
          locator: CBDB:26616
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（1-100）｜历史性依据：CBDB 卒年 = 1229
          source:
            id: s_VdoY9CdigiKgYKXx3QmikP
            source_type: api_record
            title: 中国历代人物传记资料库：王光逢（CBDB 26616）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=26616&o=json
            external_identifier: CBDB:26616
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:05.401Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_1oP6FMiEbYNXWvN6MBF9nG
        subject_person_id: p_hDE1dEk7zop9PqWvJVDDap
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1229年
            calendar_note: CBDB 卒年字段，精度：年
            earliest: 1229-01-01
            latest: 1229-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_B2taAV7k83WKUKXC5iTH8P
          claim_id: c_1oP6FMiEbYNXWvN6MBF9nG
          source_id: s_VdoY9CdigiKgYKXx3QmikP
          stance: supports
          locator: CBDB:26616
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（1-100）｜历史性依据：CBDB 卒年 = 1229
          source:
            id: s_VdoY9CdigiKgYKXx3QmikP
            source_type: api_record
            title: 中国历代人物传记资料库：王光逢（CBDB 26616）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=26616&o=json
            external_identifier: CBDB:26616
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:05.401Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_JYEk4QK4TD4fT2YbizJD2w
        subject_person_id: p_hDE1dEk7zop9PqWvJVDDap
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王光逢
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_EVu8JqxY3tbi2wJWE8CGGo
          claim_id: c_JYEk4QK4TD4fT2YbizJD2w
          source_id: s_VdoY9CdigiKgYKXx3QmikP
          stance: supports
          locator: CBDB:26616
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（1-100）｜历史性依据：CBDB 卒年 = 1229
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_JtJDDwHKzwNwyKqYI7pGBe
        subject_person_id: p_J7D7mZY2Hh6FDLAfcyMWb1
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_hDE1dEk7zop9PqWvJVDDap
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_o2atXUPXGMbCTwb70hJzau
          claim_id: c_JtJDDwHKzwNwyKqYI7pGBe
          source_id: s_NK12e9rjC2BQt5rJSZNZqp
          stance: supports
          locator: CBDB 双向互证（子 王光逢 ⇄ 父 王楹）
          quotation: null
          interpretation_note: Luna 在线复核：CBDB 底本未知，经 Wikidata 双向核对后采纳。
          source:
            id: s_NK12e9rjC2BQt5rJSZNZqp
            source_type: api_record
            title: 中国历代人物传记资料库：王楹（CBDB 26615）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=26615&o=json
            external_identifier: CBDB:26615
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.942Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_J7D7mZY2Hh6FDLAfcyMWb1
        status: active
        display_name: 王楹
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_Fl6x5T31hV92GUtfUGLffn
        subject_person_id: p_7ZkuLBZx4sM3DaTb2XEViR
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_hDE1dEk7zop9PqWvJVDDap
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_CcA78Vit42l2rVn7-wyIHY
          claim_id: c_Fl6x5T31hV92GUtfUGLffn
          source_id: s_4wBHe44bd6siK9D6AUEfUk
          stance: supports
          locator: 宋人傳記資料索引(電子版)，1563：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_4wBHe44bd6siK9D6AUEfUk
            source_type: api_record
            title: 中国历代人物传记资料库：王渙之（CBDB 26612）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=26612&o=json
            external_identifier: CBDB:26612
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.941Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_7ZkuLBZx4sM3DaTb2XEViR
        status: active
        display_name: 王渙之
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王光逢

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王光逢（1153年—1229年），史料所见人物。本项目依据《中国历代人物传记资料库：王光逢（CBDB 26616）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| birth.date | 1153年 | accepted |
| death.date | 1229年 | accepted |
| name.primary | 王光逢 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_J7D7mZY2Hh6FDLAfcyMWb1 | 王楹 | accepted |
| ancestors | p_7ZkuLBZx4sM3DaTb2XEViR | 王渙之 | accepted |

## 外部来源

- [中国历代人物传记资料库：王光逢（CBDB 26616）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=26616&o=json)
- [中国历代人物传记资料库：王渙之（CBDB 26612）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=26612&o=json)
- [中国历代人物传记资料库：王楹（CBDB 26615）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=26615&o=json)
