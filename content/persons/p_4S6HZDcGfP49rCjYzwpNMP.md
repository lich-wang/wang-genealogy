---
schema: wang-person/v1
id: p_4S6HZDcGfP49rCjYzwpNMP
status: active
merged_into: null
display_name: 王正序
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_FRGowNdg_CpZaVNf4a1G6n
        subject_person_id: p_4S6HZDcGfP49rCjYzwpNMP
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王正序（1893年—1984年），中国银行家，英文名：Cheng Hsu Wang。字儒奇，号子章。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_Knx1qTAfXKOxdny3CjjXkP
          claim_id: c_FRGowNdg_CpZaVNf4a1G6n
          source_id: s_TMZGZOACH2uX8h1JMejfoC
          stance: supports
          locator: 导言
          quotation: 王正序（1893年—1984年），中国银行家，英文名：Cheng
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 wikipedia。
          source: &a1
            id: s_TMZGZOACH2uX8h1JMejfoC
            source_type: website
            title: 中文维基百科：王正序
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%AD%A3%E5%BA%8F
            external_identifier: Q20687916
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-09-06T22:33:22.403Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_8pX3dM575JyhzNFYJ9bwgn
        subject_person_id: p_4S6HZDcGfP49rCjYzwpNMP
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1893年
            calendar_note: 维基数据 P569 结构化日期，精度：年
            earliest: 1893-01-01
            latest: 1893-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_ZP9aext5U73sctabBoGJqi
          claim_id: c_8pX3dM575JyhzNFYJ9bwgn
          source_id: s_Lp7wDUqiEh8sFqzwa95P53
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_Lp7wDUqiEh8sFqzwa95P53
            source_type: api_record
            title: 维基数据：王正序（Q20687916）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q20687916
            external_identifier: Q20687916
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:08.871Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%AD%A3%E5%BA%8F
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_EJqX5Yr4JpnB7si1h9pyxB
        subject_person_id: p_4S6HZDcGfP49rCjYzwpNMP
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1984年
            calendar_note: 维基数据 P570 结构化日期，精度：年
            earliest: 1984-01-01
            latest: 1984-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_ADjkN7LpebXoDJWA154ae4
          claim_id: c_EJqX5Yr4JpnB7si1h9pyxB
          source_id: s_Lp7wDUqiEh8sFqzwa95P53
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_Lp7wDUqiEh8sFqzwa95P53
            source_type: api_record
            title: 维基数据：王正序（Q20687916）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q20687916
            external_identifier: Q20687916
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:08.871Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%AD%A3%E5%BA%8F
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_MYHFzVUTSipPFBLDh8P7RU
        subject_person_id: p_4S6HZDcGfP49rCjYzwpNMP
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王正序
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_oZbJBmZKc7ifyjAszP7UWQ
          claim_id: c_MYHFzVUTSipPFBLDh8P7RU
          source_id: s_Lp7wDUqiEh8sFqzwa95P53
          stance: supports
          locator: Q20687916
          quotation: null
          interpretation_note: null
          source:
            id: s_Lp7wDUqiEh8sFqzwa95P53
            source_type: api_record
            title: 维基数据：王正序（Q20687916）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q20687916
            external_identifier: Q20687916
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:08.871Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%AD%A3%E5%BA%8F
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_T2MDd2NsJ2yMv6z8A3vEu5
        subject_person_id: p_Mu15E56wyrNWUKHP8qFxt8
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_4S6HZDcGfP49rCjYzwpNMP
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_y9Vify8K46XG2JEs8dWVBo
          claim_id: c_T2MDd2NsJ2yMv6z8A3vEu5
          source_id: s_TMZGZOACH2uX8h1JMejfoC
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source: *a1
        - id: cs_ch1MZYwkY3fMbsMpJU25F6
          claim_id: c_T2MDd2NsJ2yMv6z8A3vEu5
          source_id: s_Vu8MRc3pp9WwKYcye94xWa
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_Vu8MRc3pp9WwKYcye94xWa
            source_type: api_record
            title: 维基数据：王际唐（Q30949751）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q30949751
            external_identifier: Q30949751
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:09.701Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%9C%89%E5%85%89_(%E8%8B%B1%E5%9C%8B%E5%82%B3%E6%95%99%E5%A3%AB)
      object_person:
        id: p_Mu15E56wyrNWUKHP8qFxt8
        status: active
        display_name: 王际唐
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王正序

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王正序（1893年—1984年），中国银行家，英文名：Cheng Hsu Wang。字儒奇，号子章。 | accepted |
| birth.date | 1893年 | accepted |
| death.date | 1984年 | accepted |
| name.primary | 王正序 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_Mu15E56wyrNWUKHP8qFxt8 | 王际唐 | accepted |

## 外部来源

- [维基数据：王际唐（Q30949751）](https://www.wikidata.org/wiki/Q30949751)
- [维基数据：王正序（Q20687916）](https://www.wikidata.org/wiki/Q20687916)
- [中文维基百科：王正序](https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%AD%A3%E5%BA%8F)
