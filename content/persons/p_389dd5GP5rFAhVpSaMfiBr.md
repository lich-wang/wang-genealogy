---
schema: wang-person/v1
id: p_389dd5GP5rFAhVpSaMfiBr
status: active
merged_into: null
display_name: 王思溫
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_z6k2fe7vF3nw2GNHu6UPUf
        subject_person_id: p_389dd5GP5rFAhVpSaMfiBr
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王思溫
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_1K8KdL5NnCqAiWw6KMbdY7
          claim_id: c_z6k2fe7vF3nw2GNHu6UPUf
          source_id: s_HBLydA1DYMvZqNEjFjgUKz
          stance: supports
          locator: CBDB:153900
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（153900）
          source: &a1
            id: s_HBLydA1DYMvZqNEjFjgUKz
            source_type: api_record
            title: 中国历代人物传记资料库：王思溫（CBDB 153900）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=153900&o=json
            external_identifier: CBDB:153900
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.866Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_hUzEaZpvcVEXL5fCMDGp9K
        subject_person_id: p_389dd5GP5rFAhVpSaMfiBr
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王思溫，唐人物。籍贯洛陽。（中国历代人物传记资料库 CBDB 153900）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_CYpNqr_Jn-bTZceBFj9MEa
          claim_id: c_hUzEaZpvcVEXL5fCMDGp9K
          source_id: s_HBLydA1DYMvZqNEjFjgUKz
          stance: supports
          locator: CBDB:153900
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_U38CdTTMYM0p4YkalLtC8f
        subject_person_id: p_389dd5GP5rFAhVpSaMfiBr
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_rtWrRgJEgo23nh8Eimw1Ru
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_HJpJo85yDHfO8n69FmY_fG
          claim_id: c_U38CdTTMYM0p4YkalLtC8f
          source_id: s_HBLydA1DYMvZqNEjFjgUKz
          stance: supports
          locator: 唐代墓誌匯編:二卷，Tianbao 138：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_rtWrRgJEgo23nh8Eimw1Ru
        status: active
        display_name: 王同福
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王思溫

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王思溫 | accepted |
| bio.summary | 王思溫，唐人物。籍贯洛陽。（中国历代人物传记资料库 CBDB 153900） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_rtWrRgJEgo23nh8Eimw1Ru | 王同福 | accepted |

## 外部来源

- [中国历代人物传记资料库：王思溫（CBDB 153900）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=153900&o=json)
