---
schema: wang-person/v1
id: p_2dN4ALZANWDT3o6bn8ajjf
status: active
merged_into: null
display_name: 王琚
revision: 3
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_gKh1L2ym4ZDoCjSwv1raCQ
        subject_person_id: p_2dN4ALZANWDT3o6bn8ajjf
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王琚
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_yy6kykKNnWNpd5RTU7MsXt
          claim_id: c_gKh1L2ym4ZDoCjSwv1raCQ
          source_id: s_5oVokW7ZaSixYu1MQCBwBY
          stance: supports
          locator: CBDB:291581
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（291581）
          source: &a1
            id: s_5oVokW7ZaSixYu1MQCBwBY
            source_type: api_record
            title: 中国历代人物传记资料库：王琚（CBDB 291581）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=291581&o=json
            external_identifier: CBDB:291581
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.360Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_waHy5yt4sQP3SYgFHSaytM
        subject_person_id: p_2dN4ALZANWDT3o6bn8ajjf
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王琚，明人物。嘉靖十一年進士，籍贯交河。（中国历代人物传记资料库 CBDB 291581）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_tOrUuKCvvGKACFLhnEfuUB
          claim_id: c_waHy5yt4sQP3SYgFHSaytM
          source_id: s_5oVokW7ZaSixYu1MQCBwBY
          stance: supports
          locator: CBDB:291581
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_xPrqua4dxi7mcTOQfR5l82
        subject_person_id: p_hm7Wj97RnwGDWxbzPtoAnt
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_2dN4ALZANWDT3o6bn8ajjf
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_37skyqIoZ32ymwZwdE7oCI
          claim_id: c_xPrqua4dxi7mcTOQfR5l82
          source_id: s_-otJrQoKHIuXjfM1YDEopl
          stance: supports
          locator: CBDB：兄弟 王珩（202648）之父／母 王浩
          quotation: null
          interpretation_note: 由兄弟关系推断：王琚 与 王珩 为同胞（CBDB 记「弟」），王珩 之父／母即 王琚 之父／母。
          source:
            id: s_-otJrQoKHIuXjfM1YDEopl
            source_type: api_record
            title: 中国历代人物传记资料库：王琚（CBDB 291581）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=291581&o=json
            external_identifier: CBDB:291581
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_hm7Wj97RnwGDWxbzPtoAnt
        status: active
        display_name: 王浩
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_ii26Pa67tK5-mcSobS2FLn
        subject_person_id: p_2dN4ALZANWDT3o6bn8ajjf
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_znAcPph3Ny2FkUGcQdE5go
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_3Wu6mIYrd4VWRPY-IopEjC
          claim_id: c_ii26Pa67tK5-mcSobS2FLn
          source_id: s_-otJrQoKHIuXjfM1YDEopl
          stance: supports
          locator: CBDB 亲属：弟（KinPerson 202648 王珩）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_-otJrQoKHIuXjfM1YDEopl
            source_type: api_record
            title: 中国历代人物传记资料库：王琚（CBDB 291581）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=291581&o=json
            external_identifier: CBDB:291581
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_znAcPph3Ny2FkUGcQdE5go
        status: active
        display_name: 王珩
        merged_into_person_id: null
---

# 王琚

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王琚 | accepted |
| bio.summary | 王琚，明人物。嘉靖十一年進士，籍贯交河。（中国历代人物传记资料库 CBDB 291581） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_hm7Wj97RnwGDWxbzPtoAnt | 王浩 | accepted |
| other | p_znAcPph3Ny2FkUGcQdE5go | 王珩 | accepted |

## 外部来源

- [中国历代人物传记资料库：王琚（CBDB 291581）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=291581&o=json)
