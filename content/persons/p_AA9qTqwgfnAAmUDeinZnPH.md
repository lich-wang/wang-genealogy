---
schema: wang-person/v1
id: p_AA9qTqwgfnAAmUDeinZnPH
status: active
merged_into: null
display_name: 王紀
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_E6WN22XAaqKPQhjEVbNzQH
        subject_person_id: p_AA9qTqwgfnAAmUDeinZnPH
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王紀
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_C98YDgAzrzFNMJExmvNyrK
          claim_id: c_E6WN22XAaqKPQhjEVbNzQH
          source_id: s_GUSoYgcmdyNNMFz11TFrQZ
          stance: supports
          locator: CBDB:561539
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（561539）
          source: &a1
            id: s_GUSoYgcmdyNNMFz11TFrQZ
            source_type: api_record
            title: 中国历代人物传记资料库：王紀（CBDB 561539）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=561539&o=json
            external_identifier: CBDB:561539
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:25.746Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_ThAUVP4zPEBfZ6gXzxrHME
        subject_person_id: p_AA9qTqwgfnAAmUDeinZnPH
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: "王紀，清人物。籍贯山陰，入仕學校: 生員(庠生)，曾任國子監典籍、知縣、知州。（中国历代人物传记资料库 CBDB 561539）"
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_MoZLgabCld8NOtfYMd7F69
          claim_id: c_ThAUVP4zPEBfZ6gXzxrHME
          source_id: s_GUSoYgcmdyNNMFz11TFrQZ
          stance: supports
          locator: CBDB:561539
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_tb3oO6KBvlg_Rvq6hvEyo9
        subject_person_id: p_BbWngpSB9EAqEnSXLPWm75
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_AA9qTqwgfnAAmUDeinZnPH
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_xnYVgcdSGlUpQSj5ByVR6L
          claim_id: c_tb3oO6KBvlg_Rvq6hvEyo9
          source_id: s_GUSoYgcmdyNNMFz11TFrQZ
          stance: supports
          locator: 紹興府志:八十卷，Igid=316436：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_BbWngpSB9EAqEnSXLPWm75
        status: active
        display_name: 王之鼎
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王紀

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王紀 | accepted |
| bio.summary | 王紀，清人物。籍贯山陰，入仕學校: 生員(庠生)，曾任國子監典籍、知縣、知州。（中国历代人物传记资料库 CBDB 561539） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_BbWngpSB9EAqEnSXLPWm75 | 王之鼎 | accepted |

## 外部来源

- [中国历代人物传记资料库：王紀（CBDB 561539）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=561539&o=json)
