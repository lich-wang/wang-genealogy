---
schema: wang-person/v1
id: p_2bAFPA2B6z7Y79GVcB1qC2
status: active
merged_into: null
display_name: 王緒隆
cbdb_id: 236273
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_UTMi26YeXodDcDf56FcJ3E
        subject_person_id: p_2bAFPA2B6z7Y79GVcB1qC2
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王緒隆，史料所见人物。本项目依据《中国历代人物传记资料库：王緒隆（CBDB 236273）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_q7QYbUSXvMQT0-GZk2ivHU
          claim_id: c_UTMi26YeXodDcDf56FcJ3E
          source_id: s_e6V5HTFN1c2PLgfFztmn1w
          stance: supports
          locator: CBDB:236273
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_e6V5HTFN1c2PLgfFztmn1w
            source_type: api_record
            title: 中国历代人物传记资料库：王緒隆（CBDB 236273）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=236273&o=json
            external_identifier: CBDB:236273
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:58.856Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_oRiPs5K8oEMrFrQ5qZrgv1
        subject_person_id: p_2bAFPA2B6z7Y79GVcB1qC2
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王緒隆
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_ZyfvpD77ZpVDbTN2JDXZ1G
          claim_id: c_oRiPs5K8oEMrFrQ5qZrgv1
          source_id: s_e6V5HTFN1c2PLgfFztmn1w
          stance: supports
          locator: CBDB:236273
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（2801-2900）｜历史性依据：CBDB 朝代 = 明
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_vWW_djJY38J2Rli0G_U72a
        subject_person_id: p_Q4mq6fygocp26TN7xUYfs3
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_2bAFPA2B6z7Y79GVcB1qC2
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_E-NGB6DxgpdsZ1UT75k9sX
          claim_id: c_vWW_djJY38J2Rli0G_U72a
          source_id: s_e6V5HTFN1c2PLgfFztmn1w
          stance: supports
          locator: 萬曆三十八年庚戌科序齒錄:一卷，第三甲第十二名：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_e6V5HTFN1c2PLgfFztmn1w
            source_type: api_record
            title: 中国历代人物传记资料库：王緒隆（CBDB 236273）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=236273&o=json
            external_identifier: CBDB:236273
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:58.856Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_Q4mq6fygocp26TN7xUYfs3
        status: active
        display_name: 王弘祖
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王緒隆

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王緒隆，史料所见人物。本项目依据《中国历代人物传记资料库：王緒隆（CBDB 236273）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 王緒隆 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_Q4mq6fygocp26TN7xUYfs3 | 王弘祖 | accepted |

## 外部来源

- [中国历代人物传记资料库：王緒隆（CBDB 236273）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=236273&o=json)
