---
schema: wang-person/v1
id: p_TemfgwrpFYqJXJgrCQQV3E
status: active
merged_into: null
display_name: 曹氏
revision: 1
cbdb_id: 238055
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_0rA4ZuQlTAIJNokEze0mIF
        subject_person_id: p_TemfgwrpFYqJXJgrCQQV3E
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 曹氏，明人物。正统七年進士。（中国历代人物传记资料库 CBDB 238055）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_a-x2k5xpbeP1JcEwlrzDzT
          claim_id: c_0rA4ZuQlTAIJNokEze0mIF
          source_id: s_tXr19Xoo72PTD1blJotUI5
          stance: supports
          locator: CBDB:238055
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_tXr19Xoo72PTD1blJotUI5
            source_type: api_record
            title: 中国历代人物传记资料库：曹氏(王詔妻)（CBDB 238055）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=238055&o=json
            external_identifier: CBDB:238055
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_uCA7G0koRg2QCDAoCECyKC
        subject_person_id: p_TemfgwrpFYqJXJgrCQQV3E
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 曹氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_lfhms70KCj076O5ipcN3nZ
          claim_id: c_uCA7G0koRg2QCDAoCECyKC
          source_id: s_tXr19Xoo72PTD1blJotUI5
          stance: supports
          locator: CBDB:238055
          quotation: null
          interpretation_note: CBDB 明确记录的王詔配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_-3np-FQRQ8IKp-AJKY7zy7
        subject_person_id: p_8Sfy82F6BmihArAucn9Vuw
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_TemfgwrpFYqJXJgrCQQV3E
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_bsq4f5H1mVTK8ggF2uL1Rc
          claim_id: c_-3np-FQRQ8IKp-AJKY7zy7
          source_id: s_tXr19Xoo72PTD1blJotUI5
          stance: supports
          locator: 正统七年進士登科錄:一卷，第三甲第十三名：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_8Sfy82F6BmihArAucn9Vuw
        status: active
        display_name: 王詔
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 曹氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 曹氏，明人物。正统七年進士。（中国历代人物传记资料库 CBDB 238055） | accepted |
| name.primary | 曹氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_8Sfy82F6BmihArAucn9Vuw | 王詔 | accepted |

## 外部来源

- [中国历代人物传记资料库：曹氏(王詔妻)（CBDB 238055）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=238055&o=json)
