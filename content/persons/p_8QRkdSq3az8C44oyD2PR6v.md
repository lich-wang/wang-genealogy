---
schema: wang-person/v1
id: p_8QRkdSq3az8C44oyD2PR6v
status: active
merged_into: null
display_name: 王洽
revision: 1
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_T1oJxtFvfs2AWaZmjprGFQ
        subject_person_id: p_8QRkdSq3az8C44oyD2PR6v
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王洽（323年—358年），东晋大臣。维基数据以独立条目 Q15915744 收录该人物；当前资料页据此确认其身份，其他生平细节仍待可靠史料补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 5
      sources:
        - id: cs_HdbjIFn1bMR3O_PiiEth4h
          claim_id: c_T1oJxtFvfs2AWaZmjprGFQ
          source_id: s_cEaTt9MJJL6TnPNmyJ8rAC
          stance: supports
          locator: Q15915744
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 wikidata。
          source: &a1
            id: s_cEaTt9MJJL6TnPNmyJ8rAC
            source_type: api_record
            title: 維基數據：王洽（Q15915744）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15915744
            external_identifier: Q15915744
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:03.233Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%B4%BD_(%E6%9D%B1%E6%99%89)
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_82TzySr2yt8BJeQVFE3jUQ
        subject_person_id: p_8QRkdSq3az8C44oyD2PR6v
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 323年
            calendar_note: 維基數據 P569 結構化日期，精度：年
            earliest: 0323-01-01
            latest: 0323-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_R64dpGAmUtNec51Nm7G53K
          claim_id: c_82TzySr2yt8BJeQVFE3jUQ
          source_id: s_cEaTt9MJJL6TnPNmyJ8rAC
          stance: supports
          locator: Q15915744
          quotation: null
          interpretation_note: null
          source:
            id: s_cEaTt9MJJL6TnPNmyJ8rAC
            source_type: api_record
            title: 維基數據：王洽（Q15915744）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15915744
            external_identifier: Q15915744
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:03.233Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%B4%BD_(%E6%9D%B1%E6%99%89)
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_GYjcd2tu1XPN5CGgxf5kzW
        subject_person_id: p_8QRkdSq3az8C44oyD2PR6v
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 358年
            calendar_note: 維基數據 P570 結構化日期，精度：年
            earliest: 0358-01-01
            latest: 0358-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_E8MZ42FS23hHzzDY1Dhpcs
          claim_id: c_GYjcd2tu1XPN5CGgxf5kzW
          source_id: s_cEaTt9MJJL6TnPNmyJ8rAC
          stance: supports
          locator: Q15915744
          quotation: null
          interpretation_note: null
          source:
            id: s_cEaTt9MJJL6TnPNmyJ8rAC
            source_type: api_record
            title: 維基數據：王洽（Q15915744）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15915744
            external_identifier: Q15915744
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:03.233Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%B4%BD_(%E6%9D%B1%E6%99%89)
      object_person: null
    alternatives: []
  - predicate: historical.period
    recommended:
      claim:
        id: c_u27qyDKDwBC8Qjg34JRuNd
        subject_person_id: p_8QRkdSq3az8C44oyD2PR6v
        claim_kind: property
        predicate: historical.period
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 东晋
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_KiN731dzMD54zTvi2AM9oY
          claim_id: c_u27qyDKDwBC8Qjg34JRuNd
          source_id: s_cEaTt9MJJL6TnPNmyJ8rAC
          stance: supports
          locator: 人物简介中的朝代／时期表述
          quotation: null
          interpretation_note: 从同一来源支持的简介摘取：东晋大臣
          source:
            id: s_cEaTt9MJJL6TnPNmyJ8rAC
            source_type: api_record
            title: 維基數據：王洽（Q15915744）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15915744
            external_identifier: Q15915744
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:03.233Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%B4%BD_(%E6%9D%B1%E6%99%89)
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_UJA181fx7f289XUY6ofPri
        subject_person_id: p_8QRkdSq3az8C44oyD2PR6v
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王洽
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_9dCQBn7V9mFmi6ZoiR9hAc
          claim_id: c_UJA181fx7f289XUY6ofPri
          source_id: s_cEaTt9MJJL6TnPNmyJ8rAC
          stance: supports
          locator: Q15915744
          quotation: null
          interpretation_note: null
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_XfXNgdp7PjdGKbovxj2aUJ
        subject_person_id: p_Xbn65bXVofZnsi76cqAQCP
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_8QRkdSq3az8C44oyD2PR6v
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_vAw7WZ5arT62YXpASJuFsP
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_ZAm91q8ts6LtYjrmAiQGNG
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_ZAm91q8ts6LtYjrmAiQGNG
            source_type: website
            title: 中文维基百科：王導
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王導
            external_identifier: Q3274914
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_drcb7mYkd3xD6JrBdqQQRZ
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_FRo67ZrNmc5pka5csQaFUo
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_FRo67ZrNmc5pka5csQaFUo
            source_type: api_record
            title: 維基數據：王導（Q3274914）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q3274914
            external_identifier: Q3274914
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:40.794Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%B0%8E
        - id: cs_3A4J4Lg89qq44icvJo6MoL
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_cEaTt9MJJL6TnPNmyJ8rAC
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_cEaTt9MJJL6TnPNmyJ8rAC
            source_type: api_record
            title: 維基數據：王洽（Q15915744）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15915744
            external_identifier: Q15915744
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:03.233Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%B4%BD_(%E6%9D%B1%E6%99%89)
        - id: cs_QT7FFXeVTnNgK3CD9f3Cm7
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_rCP6CJoM7Pq41uB8uHSbXK
          stance: supports
          locator: 亲属关系：三子
          quotation: null
          interpretation_note: CBDB 注明此条来源：未知
          source:
            id: s_rCP6CJoM7Pq41uB8uHSbXK
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王導（25788）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25788&o=json
            external_identifier: CBDB:25788
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:39.033Z
            metadata_json: null
        - id: cs_Ax3tcFdoBccDEkNMAm1x7Y
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_ZJmD4B38XLin7a8LxP99Q1
          stance: supports
          locator: 亲属关系：父
          quotation: null
          interpretation_note: CBDB 注明此条来源：未知
          source:
            id: s_ZJmD4B38XLin7a8LxP99Q1
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王洽（25790）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25790&o=json
            external_identifier: CBDB:25790
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:34.484Z
            metadata_json: null
        - id: cs_2q65kfrsP76y7nbjWzQXf9
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_EuhopAKPrGRPYfhK6LWGqg
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王洽
          interpretation_note: null
          source:
            id: s_EuhopAKPrGRPYfhK6LWGqg
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-24T12:08:06.893Z
            metadata_json: null
        - id: cs_Csoixvqh4SgnuNGsCYQUME
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_UT2fnx3qtzoieQ2DR3uegK
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王洽
          interpretation_note: null
          source:
            id: s_UT2fnx3qtzoieQ2DR3uegK
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-24T12:12:42.934Z
            metadata_json: null
        - id: cs_wgw6jPdJPRGVHeJ6R2xna3
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_Hu48xKLWbTYqemmfCh8psU
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王洽
          interpretation_note: null
          source:
            id: s_Hu48xKLWbTYqemmfCh8psU
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T01:50:46.620Z
            metadata_json: null
        - id: cs_ZpexFXsDFhgZzzTYrCqBP6
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_am7Fw6MoGrjn6MQaPht7Mr
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王洽
          interpretation_note: null
          source:
            id: s_am7Fw6MoGrjn6MQaPht7Mr
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:24:33.641Z
            metadata_json: null
        - id: cs_CfXvTPXQjmKraqoH4R1iyV
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_MGBoig692jprej3f8uiDi2
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王导 → 王洽
          interpretation_note: null
          source:
            id: s_MGBoig692jprej3f8uiDi2
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:58:59.171Z
            metadata_json: null
        - id: cs_pBwExDoN5SWbtNALSLc5Qg
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_xEBYVW3xjFzLnp9P7pHZ2L
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王导 → 王洽
          interpretation_note: null
          source:
            id: s_xEBYVW3xjFzLnp9P7pHZ2L
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:02:22.297Z
            metadata_json: null
        - id: cs_8YKdAr14poiXgFrXrdc5rB
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_2Vkz5GQzEDauSJDkgPos6m
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王洽
          interpretation_note: null
          source:
            id: s_2Vkz5GQzEDauSJDkgPos6m
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:22:13.683Z
            metadata_json: null
        - id: cs_aFXqoRG9rFrj6Fsy2s1zbC
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_WUGDBD9YgdJyH7S1ZJAkW2
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王洽
          interpretation_note: null
          source:
            id: s_WUGDBD9YgdJyH7S1ZJAkW2
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:53:57.308Z
            metadata_json: null
        - id: cs_yk8yLNsQKVCepvUJ6vquRR
          claim_id: c_XfXNgdp7PjdGKbovxj2aUJ
          source_id: s_XmK4uXKaoBAKcwvzzk8Dzr
          stance: supports
          locator: 条文：条文识读（子）
          quotation: 六子：悅、恬、劭、洽、協、薈。
          interpretation_note: null
          source:
            id: s_XmK4uXKaoBAKcwvzzk8Dzr
            source_type: book
            title: 中文维基文库：新唐書/卷072中
            creator: null
            publisher: 维基文库
            published_at_text: null
            canonical_url: https://zh.wikisource.org/wiki/%E6%96%B0%E5%94%90%E6%9B%B8%2F%E5%8D%B7072%E4%B8%AD
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T10:12:55.026Z
            metadata_json: null
      object_person:
        id: p_Xbn65bXVofZnsi76cqAQCP
        status: active
        display_name: 王導
        merged_into_person_id: null
  children:
    - claim:
        id: c_xKQkRKMsXYAU5T4S6KcjJT
        subject_person_id: p_8QRkdSq3az8C44oyD2PR6v
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_ojGchAZHmRHY7mAfMcQCu4
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_DqG8snAeHuC2zReN2yCer5
          claim_id: c_xKQkRKMsXYAU5T4S6KcjJT
          source_id: s_ZJmD4B38XLin7a8LxP99Q1
          stance: supports
          locator: 亲属关系：子
          quotation: null
          interpretation_note: CBDB 注明此条来源：明清婦女著作數據庫
          source:
            id: s_ZJmD4B38XLin7a8LxP99Q1
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王洽（25790）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25790&o=json
            external_identifier: CBDB:25790
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:34.484Z
            metadata_json: null
        - id: cs_rPPDpBmdki3eT1LSz6bqnc
          claim_id: c_xKQkRKMsXYAU5T4S6KcjJT
          source_id: s_cEaTt9MJJL6TnPNmyJ8rAC
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_cEaTt9MJJL6TnPNmyJ8rAC
            source_type: api_record
            title: 維基數據：王洽（Q15915744）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15915744
            external_identifier: Q15915744
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:03.233Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%B4%BD_(%E6%9D%B1%E6%99%89)
        - id: cs_rmAR18G9G1Q6epKdGAGxoi
          claim_id: c_xKQkRKMsXYAU5T4S6KcjJT
          source_id: s_F2EKb33rEMMM2gS8nVwxMC
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_F2EKb33rEMMM2gS8nVwxMC
            source_type: api_record
            title: 维基数据：王珣（Q15902305）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15902305
            external_identifier: Q15902305
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:48:46.289Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%8F%A3
        - id: cs_6jymTrEEQTtS8zibkwfHL4
          claim_id: c_xKQkRKMsXYAU5T4S6KcjJT
          source_id: s_zP4QiwVoLPd8vMyh4DsJ8L
          stance: supports
          locator: 亲属关系：父
          quotation: null
          interpretation_note: CBDB 注明此条来源：明清婦女著作數據庫
          source:
            id: s_zP4QiwVoLPd8vMyh4DsJ8L
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王珣（134773）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=134773&o=json
            external_identifier: CBDB:134773
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:48:46.431Z
            metadata_json: null
        - id: cs_TxQ5N2E4UjPsSderriQpiM
          claim_id: c_xKQkRKMsXYAU5T4S6KcjJT
          source_id: s_MGBoig692jprej3f8uiDi2
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王洽 → 王珣
          interpretation_note: null
          source:
            id: s_MGBoig692jprej3f8uiDi2
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:58:59.171Z
            metadata_json: null
        - id: cs_3qRNFgVsiLqMKk9A7P85Uy
          claim_id: c_xKQkRKMsXYAU5T4S6KcjJT
          source_id: s_xEBYVW3xjFzLnp9P7pHZ2L
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王洽 → 王珣
          interpretation_note: null
          source:
            id: s_xEBYVW3xjFzLnp9P7pHZ2L
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:02:22.297Z
            metadata_json: null
        - id: cs_CqbarvEFrBDfYE7j3sfn1V
          claim_id: c_xKQkRKMsXYAU5T4S6KcjJT
          source_id: s_XmK4uXKaoBAKcwvzzk8Dzr
          stance: supports
          locator: 条文：条文识读（子）
          quotation: 二子：珣、珉。
          interpretation_note: null
          source:
            id: s_XmK4uXKaoBAKcwvzzk8Dzr
            source_type: book
            title: 中文维基文库：新唐書/卷072中
            creator: null
            publisher: 维基文库
            published_at_text: null
            canonical_url: https://zh.wikisource.org/wiki/%E6%96%B0%E5%94%90%E6%9B%B8%2F%E5%8D%B7072%E4%B8%AD
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T10:12:55.026Z
            metadata_json: null
      object_person:
        id: p_ojGchAZHmRHY7mAfMcQCu4
        status: active
        display_name: 王珣
        merged_into_person_id: null
    - claim:
        id: c_uWJuLBeMkVK71gSSQteLgF
        subject_person_id: p_8QRkdSq3az8C44oyD2PR6v
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_tHbVGq9sycrykvp4emuxp6
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_bcY4TU1pJAX6CuH5bHvnf9
          claim_id: c_uWJuLBeMkVK71gSSQteLgF
          source_id: s_cEaTt9MJJL6TnPNmyJ8rAC
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_cEaTt9MJJL6TnPNmyJ8rAC
            source_type: api_record
            title: 維基數據：王洽（Q15915744）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15915744
            external_identifier: Q15915744
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:03.233Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%B4%BD_(%E6%9D%B1%E6%99%89)
        - id: cs_bCFnb9uFBYDUk8LpxMXRcQ
          claim_id: c_uWJuLBeMkVK71gSSQteLgF
          source_id: s_Bgg1PQFKhmwF8dZVqQKj43
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_Bgg1PQFKhmwF8dZVqQKj43
            source_type: api_record
            title: 维基数据：王珉（Q15925683）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15925683
            external_identifier: Q15925683
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:48:54.326Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%8F%89_(%E6%9D%B1%E6%99%89)
        - id: cs_pF7q49SQubr9hMZVBZ7jfC
          claim_id: c_uWJuLBeMkVK71gSSQteLgF
          source_id: s_MGBoig692jprej3f8uiDi2
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王洽 → 王珉
          interpretation_note: null
          source:
            id: s_MGBoig692jprej3f8uiDi2
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:58:59.171Z
            metadata_json: null
        - id: cs_HKLdpCeKCguacSmbA5tCPQ
          claim_id: c_uWJuLBeMkVK71gSSQteLgF
          source_id: s_xEBYVW3xjFzLnp9P7pHZ2L
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王洽 → 王珉
          interpretation_note: null
          source:
            id: s_xEBYVW3xjFzLnp9P7pHZ2L
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:02:22.297Z
            metadata_json: null
        - id: cs_gyQPdDpGHMJC98WF39gbiq
          claim_id: c_uWJuLBeMkVK71gSSQteLgF
          source_id: s_XmK4uXKaoBAKcwvzzk8Dzr
          stance: supports
          locator: 条文：条文识读（子）
          quotation: 二子：珣、珉。
          interpretation_note: null
          source:
            id: s_XmK4uXKaoBAKcwvzzk8Dzr
            source_type: book
            title: 中文维基文库：新唐書/卷072中
            creator: null
            publisher: 维基文库
            published_at_text: null
            canonical_url: https://zh.wikisource.org/wiki/%E6%96%B0%E5%94%90%E6%9B%B8%2F%E5%8D%B7072%E4%B8%AD
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T10:12:55.026Z
            metadata_json: null
      object_person:
        id: p_tHbVGq9sycrykvp4emuxp6
        status: active
        display_name: 王珉
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王洽

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王洽（323年—358年），东晋大臣。维基数据以独立条目 Q15915744 收录该人物；当前资料页据此确认其身份，其他生平细节仍待可靠史料补充。 | accepted |
| birth.date | 323年 | accepted |
| death.date | 358年 | accepted |
| historical.period | 东晋 | accepted |
| name.primary | 王洽 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_Xbn65bXVofZnsi76cqAQCP | 王導 | accepted |
| children | p_ojGchAZHmRHY7mAfMcQCu4 | 王珣 | accepted |
| children | p_tHbVGq9sycrykvp4emuxp6 | 王珉 | accepted |

## 外部来源

- [维基数据：王珉（Q15925683）](https://www.wikidata.org/wiki/Q15925683)
- [维基数据：王珣（Q15902305）](https://www.wikidata.org/wiki/Q15902305)
- [維基數據：王導（Q3274914）](https://www.wikidata.org/wiki/Q3274914)
- [維基數據：王洽（Q15915744）](https://www.wikidata.org/wiki/Q15915744)
- [中文维基百科：琅邪王氏](https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F)
- [中文维基百科：琅邪王氏世系圖](https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96)
- [中文维基百科：王導](https://zh.wikipedia.org/wiki/王導)
- [中文维基文库：新唐書/卷072中](https://zh.wikisource.org/wiki/%E6%96%B0%E5%94%90%E6%9B%B8%2F%E5%8D%B7072%E4%B8%AD)
- [CBDB 中国历代人物传记资料库：王導（25788）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25788&o=json)
- [CBDB 中国历代人物传记资料库：王洽（25790）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25790&o=json)
- [CBDB 中国历代人物传记资料库：王珣（134773）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=134773&o=json)
