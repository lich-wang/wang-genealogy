---
schema: wang-person/v1
id: p_M2wUJoj3fjg23MLrQcpHh9
status: active
merged_into: null
display_name: 劉徹
cbdb_id: 16626
revision: 1
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_ss681PP5TTp6baG3YPFRai
        subject_person_id: p_M2wUJoj3fjg23MLrQcpHh9
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉徹（前155年6月7日—前86年3月27日），西漢人物。籍贯長安，入仕繼位。（中国历代人物传记资料库 CBDB 16626）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 7
      sources:
        - id: cs_8wy7P9CPrUvrf6llrlFRRy
          claim_id: c_ss681PP5TTp6baG3YPFRai
          source_id: s_U5nHZRsqxmfZe2AUJHC84U
          stance: supports
          locator: CBDB:16626
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_U5nHZRsqxmfZe2AUJHC84U
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：劉徹（16626）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=16626&o=json
            external_identifier: CBDB:16626
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T03:25:11.356Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_YygjCtJHqGDM4roKc59qvH
        subject_person_id: p_M2wUJoj3fjg23MLrQcpHh9
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 前155年6月7日
            calendar_note: 维基数据 P569 结构化日期，精度：日
            earliest: -0155-06-07
            latest: -0155-06-07
            precision: day
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_wgXfEXgCQXmtPDe2QmjvKi
          claim_id: c_YygjCtJHqGDM4roKc59qvH
          source_id: s_JWomymJEpcvT2hLi58R4Ta
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_JWomymJEpcvT2hLi58R4Ta
            source_type: api_record
            title: 维基数据：汉武帝（Q7225）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7225
            external_identifier: Q7225
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:25:11.210Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%AD%A6%E5%B8%9D
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_SwAwLPEuPgcnrcGAFD4AGA
        subject_person_id: p_M2wUJoj3fjg23MLrQcpHh9
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 前86年3月27日
            calendar_note: 维基数据 P570 结构化日期，精度：日
            earliest: -0086-03-27
            latest: -0086-03-27
            precision: day
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_9Uag3djAqRxj6ArzmHbNeG
          claim_id: c_SwAwLPEuPgcnrcGAFD4AGA
          source_id: s_JWomymJEpcvT2hLi58R4Ta
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_JWomymJEpcvT2hLi58R4Ta
            source_type: api_record
            title: 维基数据：汉武帝（Q7225）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7225
            external_identifier: Q7225
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:25:11.210Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%AD%A6%E5%B8%9D
      object_person: null
    alternatives: []
  - predicate: name.alias
    recommended:
      claim:
        id: c_o8sdCCQXG9qnZZArm4nH6h
        subject_person_id: p_M2wUJoj3fjg23MLrQcpHh9
        claim_kind: property
        predicate: name.alias
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 汉武帝
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_JnaM81HamDqRxR2FDTZLX5
          claim_id: c_o8sdCCQXG9qnZZArm4nH6h
          source_id: s_U5nHZRsqxmfZe2AUJHC84U
          stance: supports
          locator: Q7225
          quotation: null
          interpretation_note: null
          source:
            id: s_U5nHZRsqxmfZe2AUJHC84U
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：劉徹（16626）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=16626&o=json
            external_identifier: CBDB:16626
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T03:25:11.356Z
            metadata_json: null
        - id: cs_fqM1PH5KPax3qJQKCFZ2Zi
          claim_id: c_o8sdCCQXG9qnZZArm4nH6h
          source_id: s_JWomymJEpcvT2hLi58R4Ta
          stance: supports
          locator: Q7225
          quotation: null
          interpretation_note: null
          source:
            id: s_JWomymJEpcvT2hLi58R4Ta
            source_type: api_record
            title: 维基数据：汉武帝（Q7225）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7225
            external_identifier: Q7225
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:25:11.210Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%AD%A6%E5%B8%9D
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_6S3LEQ939ZNpLvL716tZTB
        subject_person_id: p_M2wUJoj3fjg23MLrQcpHh9
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉徹
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 3
      sources:
        - id: cs_8CjVPV2D3rEiH6Vgw2WykZ
          claim_id: c_6S3LEQ939ZNpLvL716tZTB
          source_id: s_U5nHZRsqxmfZe2AUJHC84U
          stance: supports
          locator: Q7225
          quotation: null
          interpretation_note: null
          source: *a1
        - id: cs_vh93XPNm2F6gAfz1Ct9MkH
          claim_id: c_6S3LEQ939ZNpLvL716tZTB
          source_id: s_JWomymJEpcvT2hLi58R4Ta
          stance: supports
          locator: Q7225
          quotation: null
          interpretation_note: null
          source:
            id: s_JWomymJEpcvT2hLi58R4Ta
            source_type: api_record
            title: 维基数据：汉武帝（Q7225）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7225
            external_identifier: Q7225
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:25:11.210Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%AD%A6%E5%B8%9D
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_AdN4mfHDtreN1gFAUoDYUd
        subject_person_id: p_M2wUJoj3fjg23MLrQcpHh9
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_TvYmJqHhUUMuhW1evEFF8c
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 4
      sources:
        - id: cs_GPBpj9Du5Nt3Fc8JyJ1H1L
          claim_id: c_AdN4mfHDtreN1gFAUoDYUd
          source_id: s_JWomymJEpcvT2hLi58R4Ta
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_JWomymJEpcvT2hLi58R4Ta
            source_type: api_record
            title: 维基数据：汉武帝（Q7225）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7225
            external_identifier: Q7225
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:25:11.210Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%AD%A6%E5%B8%9D
        - id: cs_fCag22Gms2qXzwmiFMPxm7
          claim_id: c_AdN4mfHDtreN1gFAUoDYUd
          source_id: s_YjR6JQ4MC3jHAxJ32yKPVE
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_YjR6JQ4MC3jHAxJ32yKPVE
            source_type: api_record
            title: 维基数据：王夫人（Q8253173）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q8253173
            external_identifier: Q8253173
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:31:40.921Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%A4%AB%E4%BA%BA_(%E6%B1%89%E6%AD%A6%E5%B8%9D)
      object_person:
        id: p_TvYmJqHhUUMuhW1evEFF8c
        status: active
        display_name: 王夫人
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 劉徹

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 劉徹（前155年6月7日—前86年3月27日），西漢人物。籍贯長安，入仕繼位。（中国历代人物传记资料库 CBDB 16626） | accepted |
| birth.date | 前155年6月7日 | accepted |
| death.date | 前86年3月27日 | accepted |
| name.alias | 汉武帝 | accepted |
| name.primary | 劉徹 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_TvYmJqHhUUMuhW1evEFF8c | 王夫人 | accepted |

## 外部来源

- [维基数据：汉武帝（Q7225）](https://www.wikidata.org/wiki/Q7225)
- [维基数据：王夫人（Q8253173）](https://www.wikidata.org/wiki/Q8253173)
- [CBDB 中国历代人物传记资料库：劉徹（16626）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=16626&o=json)
