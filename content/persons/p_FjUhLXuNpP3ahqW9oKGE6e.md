---
schema: wang-person/v1
id: p_FjUhLXuNpP3ahqW9oKGE6e
status: active
merged_into: null
display_name: 王仁
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_urNxFHCWFj4gwm7iqg8ScZ
        subject_person_id: p_FjUhLXuNpP3ahqW9oKGE6e
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王仁
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_B9KuZVVnWAAZsHZ2usFBHi
          claim_id: c_urNxFHCWFj4gwm7iqg8ScZ
          source_id: s_zDyQohuKuvz5f1oe12BK5F
          stance: supports
          locator: CBDB:29411
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（29411）
          source: &a1
            id: s_zDyQohuKuvz5f1oe12BK5F
            source_type: api_record
            title: 中国历代人物传记资料库：王仁（CBDB 29411）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=29411&o=json
            external_identifier: CBDB:29411
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:12.009Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_ncfMG4mgzJQr1pQPpRrPk6
        subject_person_id: p_FjUhLXuNpP3ahqW9oKGE6e
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王仁，元人物。籍贯安喜。（中国历代人物传记资料库 CBDB 29411）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_-tT-wCZf-lncFy4I1FFXh9
          claim_id: c_ncfMG4mgzJQr1pQPpRrPk6
          source_id: s_zDyQohuKuvz5f1oe12BK5F
          stance: supports
          locator: CBDB:29411
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_-nZ2SPsH38i-1h_aqDzWI9
        subject_person_id: p_4r434FXidoHPAJv6kFC7qF
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_FjUhLXuNpP3ahqW9oKGE6e
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_55F_j15SzkH7Mi1AUO24wC
          claim_id: c_-nZ2SPsH38i-1h_aqDzWI9
          source_id: s_nh--4UOZI3iFtevWAbsyfb
          stance: supports
          locator: CBDB 亲属：父（KinPerson 29408）
          quotation: null
          interpretation_note: CBDB 直接记录的父／母关系。
          source:
            id: s_nh--4UOZI3iFtevWAbsyfb
            source_type: api_record
            title: 中国历代人物传记资料库：王仁（CBDB 29411）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=29411&o=json
            external_identifier: CBDB:29411
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_4r434FXidoHPAJv6kFC7qF
        status: active
        display_name: 王惟賢
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王仁

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王仁 | accepted |
| bio.summary | 王仁，元人物。籍贯安喜。（中国历代人物传记资料库 CBDB 29411） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_4r434FXidoHPAJv6kFC7qF | 王惟賢 | accepted |

## 外部来源

- [中国历代人物传记资料库：王仁（CBDB 29411）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=29411&o=json)
