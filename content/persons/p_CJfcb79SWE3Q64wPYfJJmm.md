---
schema: wang-person/v1
id: p_CJfcb79SWE3Q64wPYfJJmm
status: active
merged_into: null
display_name: 王鈺
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_NdE9L4aEeqWnQRQGMDvqPt
        subject_person_id: p_CJfcb79SWE3Q64wPYfJJmm
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王鈺
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_DjjmAUExRDEhbeWWEYH6bp
          claim_id: c_NdE9L4aEeqWnQRQGMDvqPt
          source_id: s_vy2yKLMj4NA3zt32rYtxvX
          stance: supports
          locator: CBDB:221658
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（221658）
          source: &a1
            id: s_vy2yKLMj4NA3zt32rYtxvX
            source_type: api_record
            title: 中国历代人物传记资料库：王鈺（CBDB 221658）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=221658&o=json
            external_identifier: CBDB:221658
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:17.324Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_K6To5mF6NGdVxECEBZqQnL
        subject_person_id: p_CJfcb79SWE3Q64wPYfJJmm
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王鈺，明人物。萬曆八年進士。（中国历代人物传记资料库 CBDB 221658）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_pp0ug5VBWnQpERQMqOBm2X
          claim_id: c_K6To5mF6NGdVxECEBZqQnL
          source_id: s_vy2yKLMj4NA3zt32rYtxvX
          stance: supports
          locator: CBDB:221658
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_DTgDHxFs35QGcaxFtp3DcD
        subject_person_id: p_CJfcb79SWE3Q64wPYfJJmm
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_DtN8Z5QdtH34dfH2KX2i1q
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_rv4aTknuLyPDEkiKbL3M6H
          claim_id: c_DTgDHxFs35QGcaxFtp3DcD
          source_id: s_vy2yKLMj4NA3zt32rYtxvX
          stance: supports
          locator: 萬曆八年進士登科錄:一卷，第三甲第二百零七名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_DtN8Z5QdtH34dfH2KX2i1q
        status: active
        display_name: 王夢暘
        merged_into_person_id: null
  other: []
---

# 王鈺

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王鈺 | accepted |
| bio.summary | 王鈺，明人物。萬曆八年進士。（中国历代人物传记资料库 CBDB 221658） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_DtN8Z5QdtH34dfH2KX2i1q | 王夢暘 | accepted |

## 外部来源

- [中国历代人物传记资料库：王鈺（CBDB 221658）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=221658&o=json)
