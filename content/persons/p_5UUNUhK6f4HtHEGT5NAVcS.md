---
schema: wang-person/v1
id: p_5UUNUhK6f4HtHEGT5NAVcS
status: active
merged_into: null
display_name: 范景姒
revision: 1
cbdb_id: 95455
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_lzHc1QCBH7ioel8HMZ4JE2
        subject_person_id: p_5UUNUhK6f4HtHEGT5NAVcS
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 范景姒，明人物。籍贯秀水，身份为畫家。（中国历代人物传记资料库 CBDB 95455）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_epnSzYvArifYD8dsBW2Hwk
          claim_id: c_lzHc1QCBH7ioel8HMZ4JE2
          source_id: s_IoRkcuF26Zl90Cg6CUfLy1
          stance: supports
          locator: CBDB:95455
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_IoRkcuF26Zl90Cg6CUfLy1
            source_type: api_record
            title: 中国历代人物传记资料库：范景姒（CBDB 95455）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=95455&o=json
            external_identifier: CBDB:95455
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_1YIEXKL_oUQDwe-6X1lPLS
        subject_person_id: p_5UUNUhK6f4HtHEGT5NAVcS
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 范景姒
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_RoGclUKNoBYk4xUzpTQrn1
          claim_id: c_1YIEXKL_oUQDwe-6X1lPLS
          source_id: s_IoRkcuF26Zl90Cg6CUfLy1
          stance: supports
          locator: CBDB:95455
          quotation: null
          interpretation_note: CBDB 明确记录的王世德配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_VvBAwTgEjMKVNxPxqoannq
        subject_person_id: p_ccMLFYfTxtFBjS85FiC5rC
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_5UUNUhK6f4HtHEGT5NAVcS
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Pgx3o3vhwYOpt6f34_a3Bk
          claim_id: c_VvBAwTgEjMKVNxPxqoannq
          source_id: s_IoRkcuF26Zl90Cg6CUfLy1
          stance: supports
          locator: "明清婦女著作數據庫，MQWW PoetID #1678, HuWenKai #133：妻子"
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_ccMLFYfTxtFBjS85FiC5rC
        status: active
        display_name: 王世德
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 范景姒

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 范景姒，明人物。籍贯秀水，身份为畫家。（中国历代人物传记资料库 CBDB 95455） | accepted |
| name.primary | 范景姒 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_ccMLFYfTxtFBjS85FiC5rC | 王世德 | accepted |

## 外部来源

- [中国历代人物传记资料库：范景姒（CBDB 95455）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=95455&o=json)
