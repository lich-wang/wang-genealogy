---
schema: wang-person/v1
id: p_44AWrjz5RmMvPNw5hwzVKf
status: active
merged_into: null
display_name: 王端
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_AMmciJtxEKooWH5KXBSdcR
        subject_person_id: p_44AWrjz5RmMvPNw5hwzVKf
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王端
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_nJFu6S5WUdJrqAh1b9j3et
          claim_id: c_AMmciJtxEKooWH5KXBSdcR
          source_id: s_THWnVa5JTBbLat9749Xi1E
          stance: supports
          locator: CBDB:268856
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（268856）
          source: &a1
            id: s_THWnVa5JTBbLat9749Xi1E
            source_type: api_record
            title: 中国历代人物传记资料库：王端（CBDB 268856）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=268856&o=json
            external_identifier: CBDB:268856
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.799Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_mnf3S1iaCepntX8xPu2RH9
        subject_person_id: p_44AWrjz5RmMvPNw5hwzVKf
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王端，明人物。弘治九年進士，籍贯武進。（中国历代人物传记资料库 CBDB 268856）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_DLuc5vvOjD8kxtwPyHPTtw
          claim_id: c_mnf3S1iaCepntX8xPu2RH9
          source_id: s_THWnVa5JTBbLat9749Xi1E
          stance: supports
          locator: CBDB:268856
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_XC7Zyy_B0Q8zAahz1JPYPp
        subject_person_id: p_44AWrjz5RmMvPNw5hwzVKf
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_XA3uBQQ21RdALjw9VGgLBS
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_v2q6DiEOhjkChRkmfDaUjh
          claim_id: c_XC7Zyy_B0Q8zAahz1JPYPp
          source_id: s_THWnVa5JTBbLat9749Xi1E
          stance: supports
          locator: 弘治九年進士登科錄:一卷，第三甲第一百零五名：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_XA3uBQQ21RdALjw9VGgLBS
        status: active
        display_name: 王禾
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王端

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王端 | accepted |
| bio.summary | 王端，明人物。弘治九年進士，籍贯武進。（中国历代人物传记资料库 CBDB 268856） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_XA3uBQQ21RdALjw9VGgLBS | 王禾 | accepted |

## 外部来源

- [中国历代人物传记资料库：王端（CBDB 268856）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=268856&o=json)
