---
schema: wang-person/v1
id: p_PjGqvu9o7MybC5QxmLcjhr
status: active
merged_into: null
display_name: 王柄
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_Tv3UCHomK2AUGpRZe6v9sM
        subject_person_id: p_PjGqvu9o7MybC5QxmLcjhr
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王柄
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_VPPK6AgQFAj7rPN349X4SU
          claim_id: c_Tv3UCHomK2AUGpRZe6v9sM
          source_id: s_sZhSLMdd31VDU78gLiFgJq
          stance: supports
          locator: CBDB:57161
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（57161）
          source: &a1
            id: s_sZhSLMdd31VDU78gLiFgJq
            source_type: api_record
            title: 中国历代人物传记资料库：王柄（CBDB 57161）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=57161&o=json
            external_identifier: CBDB:57161
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:12.792Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_Y16ZcG8fu3foKogjTkeJa3
        subject_person_id: p_PjGqvu9o7MybC5QxmLcjhr
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1810年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_4k1s5now2x7qFBc6n3Vbo3
          claim_id: c_Y16ZcG8fu3foKogjTkeJa3
          source_id: s_sZhSLMdd31VDU78gLiFgJq
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_KUqLraQdA4UABm7HuQLN2P
        subject_person_id: p_PjGqvu9o7MybC5QxmLcjhr
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王柄（卒于1810年），清人物。籍贯漢軍鑲白旗，曾任冠軍使、關監督、鑾儀衛整儀尉。（中国历代人物传记资料库 CBDB 57161）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_dhEateM4XtRUf7CfVEQUYn
          claim_id: c_KUqLraQdA4UABm7HuQLN2P
          source_id: s_sZhSLMdd31VDU78gLiFgJq
          stance: supports
          locator: CBDB:57161
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_s_6uIwYMAX9iFTdxukmTFn
        subject_person_id: p_PjGqvu9o7MybC5QxmLcjhr
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_5g4tSrSZXTmsQ1dfUpGmMk
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_EHCpNiTY57OfUb1GIJxT8_
          claim_id: c_s_6uIwYMAX9iFTdxukmTFn
          source_id: s_sZhSLMdd31VDU78gLiFgJq
          stance: supports
          locator: 人名權威資料（中央研究院歷史語言研究所），380：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_5g4tSrSZXTmsQ1dfUpGmMk
        status: active
        display_name: 王保舜
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王柄

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王柄 | accepted |
| death.date | 1810年 | accepted |
| bio.summary | 王柄（卒于1810年），清人物。籍贯漢軍鑲白旗，曾任冠軍使、關監督、鑾儀衛整儀尉。（中国历代人物传记资料库 CBDB 57161） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_5g4tSrSZXTmsQ1dfUpGmMk | 王保舜 | accepted |

## 外部来源

- [中国历代人物传记资料库：王柄（CBDB 57161）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=57161&o=json)
