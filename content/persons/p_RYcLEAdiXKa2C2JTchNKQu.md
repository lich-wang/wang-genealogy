---
schema: wang-person/v1
id: p_RYcLEAdiXKa2C2JTchNKQu
status: active
merged_into: null
display_name: 王師剪
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_EZFDPkmPTE53Q9MK8qmabu
        subject_person_id: p_RYcLEAdiXKa2C2JTchNKQu
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王師剪
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_hArXLAA9YRqS3i4QkB4yqD
          claim_id: c_EZFDPkmPTE53Q9MK8qmabu
          source_id: s_RiPwL9BhYF8HCY18uFALXo
          stance: supports
          locator: CBDB:555354
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（555354）
          source: &a1
            id: s_RiPwL9BhYF8HCY18uFALXo
            source_type: api_record
            title: 中国历代人物传记资料库：王師剪（CBDB 555354）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=555354&o=json
            external_identifier: CBDB:555354
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:04.520Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_6YaGJRubUeeGsJp1jiBCF3
        subject_person_id: p_RYcLEAdiXKa2C2JTchNKQu
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王師剪，清人物。籍贯南召，入仕邑庠生。（中国历代人物传记资料库 CBDB 555354）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_TkeXCpK_-NmX5u7bXePkxV
          claim_id: c_6YaGJRubUeeGsJp1jiBCF3
          source_id: s_RiPwL9BhYF8HCY18uFALXo
          stance: supports
          locator: CBDB:555354
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_GD4o-9evDCMfA3FbG6aHMD
        subject_person_id: p_w1GiHCCDCJVq4Cbb8yEdBZ
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_RYcLEAdiXKa2C2JTchNKQu
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Xr8XdHVWdVe6hCU8xwsuVU
          claim_id: c_GD4o-9evDCMfA3FbG6aHMD
          source_id: s_RiPwL9BhYF8HCY18uFALXo
          stance: supports
          locator: 南陽府志，lgid=878691-878692：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_w1GiHCCDCJVq4Cbb8yEdBZ
        status: active
        display_name: 王之政
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王師剪

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王師剪 | accepted |
| bio.summary | 王師剪，清人物。籍贯南召，入仕邑庠生。（中国历代人物传记资料库 CBDB 555354） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_w1GiHCCDCJVq4Cbb8yEdBZ | 王之政 | accepted |

## 外部来源

- [中国历代人物传记资料库：王師剪（CBDB 555354）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=555354&o=json)
