---
schema: wang-person/v1
id: p_exVPUvM5pJEY8oaS9fKMdm
status: active
merged_into: null
display_name: 王德明
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_J1BeSvz6BZa55tP16szGxv
        subject_person_id: p_exVPUvM5pJEY8oaS9fKMdm
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王德明
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_65ZQ4yyy1zJptqEvkshKYe
          claim_id: c_J1BeSvz6BZa55tP16szGxv
          source_id: s_uCAtthFE7bYG5fGxm2mirz
          stance: supports
          locator: CBDB:694289
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（694289）
          source: &a1
            id: s_uCAtthFE7bYG5fGxm2mirz
            source_type: api_record
            title: 中国历代人物传记资料库：王德明（CBDB 694289）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=694289&o=json
            external_identifier: CBDB:694289
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:27.669Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_7crhV6a7DQnbYM6g25BJcs
        subject_person_id: p_exVPUvM5pJEY8oaS9fKMdm
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: "王德明，清人物。籍贯海鹽，入仕學校: 生員(庠生)。（中国历代人物传记资料库 CBDB 694289）"
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_vZKFSb3_cH4bt0bXKjJ8oK
          claim_id: c_7crhV6a7DQnbYM6g25BJcs
          source_id: s_uCAtthFE7bYG5fGxm2mirz
          stance: supports
          locator: CBDB:694289
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_PCY3pn1yV3oVIzfUEaZ5hg
        subject_person_id: p_exVPUvM5pJEY8oaS9fKMdm
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_qzRDrqjW6BFLYhMP3xVCXP
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_PpWeVw0MDTM-fBU4JcK-OT
          claim_id: c_PCY3pn1yV3oVIzfUEaZ5hg
          source_id: s_vR6lM5739rO2xkgHKseic4
          stance: supports
          locator: CBDB 亲属：弟（KinPerson 694288 王尙志）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_vR6lM5739rO2xkgHKseic4
            source_type: api_record
            title: 中国历代人物传记资料库：王德明（CBDB 694289）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=694289&o=json
            external_identifier: CBDB:694289
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_qzRDrqjW6BFLYhMP3xVCXP
        status: active
        display_name: 王尙志
        merged_into_person_id: null
---

# 王德明

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王德明 | accepted |
| bio.summary | 王德明，清人物。籍贯海鹽，入仕學校: 生員(庠生)。（中国历代人物传记资料库 CBDB 694289） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| other | p_qzRDrqjW6BFLYhMP3xVCXP | 王尙志 | accepted |

## 外部来源

- [中国历代人物传记资料库：王德明（CBDB 694289）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=694289&o=json)
