---
schema: wang-person/v1
id: p_v8HEjP8Bg5CEkMJA5fVvpw
status: active
merged_into: null
display_name: 崔暟
revision: 1
cbdb_id: 141213
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_lVx-o639-k0CYoDevYGuIH
        subject_person_id: p_v8HEjP8Bg5CEkMJA5fVvpw
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 崔暟（632—705），周人物。籍贯醴泉。（中国历代人物传记资料库 CBDB 141213）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_lcqRs3pZ8igaOh6WeJ_4DR
          claim_id: c_lVx-o639-k0CYoDevYGuIH
          source_id: s_xjKouNs6J5_JWRBR1E3UjZ
          stance: supports
          locator: CBDB:141213
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_xjKouNs6J5_JWRBR1E3UjZ
            source_type: api_record
            title: 中国历代人物传记资料库：崔暟（CBDB 141213）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=141213&o=json
            external_identifier: CBDB:141213
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_BXN9vgle61s3kMkHztERPC
        subject_person_id: p_v8HEjP8Bg5CEkMJA5fVvpw
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 崔暟
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_j_QxkaXcM5Xf4rh456IYxq
          claim_id: c_BXN9vgle61s3kMkHztERPC
          source_id: s_xjKouNs6J5_JWRBR1E3UjZ
          stance: supports
          locator: CBDB:141213
          quotation: null
          interpretation_note: CBDB 明确记录的王媛配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_rz3GspZEeEhXOebYD437r4
        subject_person_id: p_fWhHLZZ5pFkeikFbBAN6W2
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_v8HEjP8Bg5CEkMJA5fVvpw
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_SYVR2awrLMDSVMvHcVMmvm
          claim_id: c_rz3GspZEeEhXOebYD437r4
          source_id: s_xjKouNs6J5_JWRBR1E3UjZ
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：丈夫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_fWhHLZZ5pFkeikFbBAN6W2
        status: active
        display_name: 王媛
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 崔暟

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 崔暟（632—705），周人物。籍贯醴泉。（中国历代人物传记资料库 CBDB 141213） | accepted |
| name.primary | 崔暟 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_fWhHLZZ5pFkeikFbBAN6W2 | 王媛 | accepted |

## 外部来源

- [中国历代人物传记资料库：崔暟（CBDB 141213）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=141213&o=json)
