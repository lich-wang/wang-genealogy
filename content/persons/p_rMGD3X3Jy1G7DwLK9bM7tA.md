---
schema: wang-person/v1
id: p_rMGD3X3Jy1G7DwLK9bM7tA
status: active
merged_into: null
display_name: 王愛景
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_S84Jv5KBXAzxKo467rrJ1B
        subject_person_id: p_rMGD3X3Jy1G7DwLK9bM7tA
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王愛景
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_3rZXwBpMdhbjgCZ86S95nj
          claim_id: c_S84Jv5KBXAzxKo467rrJ1B
          source_id: s_qo1JYQyZDHttCzzUABDvwp
          stance: supports
          locator: CBDB:175749
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（175749）
          source: &a1
            id: s_qo1JYQyZDHttCzzUABDvwp
            source_type: api_record
            title: 中国历代人物传记资料库：王愛景（CBDB 175749）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175749&o=json
            external_identifier: CBDB:175749
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:04.254Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_KyoFncqJJ8UuM5825PiH3K
        subject_person_id: p_rMGD3X3Jy1G7DwLK9bM7tA
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 743年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Eg1kutU7YyyVaF8n7wUFfv
          claim_id: c_KyoFncqJJ8UuM5825PiH3K
          source_id: s_qo1JYQyZDHttCzzUABDvwp
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_uiuzpX2Ru85FTPVLScUC5r
        subject_person_id: p_rMGD3X3Jy1G7DwLK9bM7tA
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王愛景（卒于743年），唐人物。籍贯河南，曾任參軍。（中国历代人物传记资料库 CBDB 175749）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_JA8pqidCFnIaUR1xraaAqs
          claim_id: c_uiuzpX2Ru85FTPVLScUC5r
          source_id: s_qo1JYQyZDHttCzzUABDvwp
          stance: supports
          locator: CBDB:175749
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_KNxwxz9w4u0bGtDaJouQkP
        subject_person_id: p_T2JshioRPfxgoYHCxfDYxt
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_rMGD3X3Jy1G7DwLK9bM7tA
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_XlHRT7vKSyNTfOxydx1jTE
          claim_id: c_KNxwxz9w4u0bGtDaJouQkP
          source_id: s_qo1JYQyZDHttCzzUABDvwp
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_T2JshioRPfxgoYHCxfDYxt
        status: active
        display_name: 王守忠
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王愛景

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王愛景 | accepted |
| death.date | 743年 | accepted |
| bio.summary | 王愛景（卒于743年），唐人物。籍贯河南，曾任參軍。（中国历代人物传记资料库 CBDB 175749） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_T2JshioRPfxgoYHCxfDYxt | 王守忠 | accepted |

## 外部来源

- [中国历代人物传记资料库：王愛景（CBDB 175749）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175749&o=json)
