---
schema: wang-person/v1
id: p_jWhyM89PP9d3eMCukjP7Hb
status: active
merged_into: null
display_name: 楊氏
revision: 1
cbdb_id: 295403
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_cI2zHd-iCFsVRshIAa3SCR
        subject_person_id: p_jWhyM89PP9d3eMCukjP7Hb
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 楊氏，明人物。嘉靖十四年進士。（中国历代人物传记资料库 CBDB 295403）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_KzfezHD5_1r5Pul1ajQQov
          claim_id: c_cI2zHd-iCFsVRshIAa3SCR
          source_id: s_nVrhIMPmgIm2IdKyiKAGc8
          stance: supports
          locator: CBDB:295403
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_nVrhIMPmgIm2IdKyiKAGc8
            source_type: api_record
            title: 中国历代人物传记资料库：楊氏(王珉妻)（CBDB 295403）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=295403&o=json
            external_identifier: CBDB:295403
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_L2xt8YWgpdC0j_IcwEUYjg
        subject_person_id: p_jWhyM89PP9d3eMCukjP7Hb
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 楊氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_flPZgy8BOoblC_UefT4LfH
          claim_id: c_L2xt8YWgpdC0j_IcwEUYjg
          source_id: s_nVrhIMPmgIm2IdKyiKAGc8
          stance: supports
          locator: CBDB:295403
          quotation: null
          interpretation_note: CBDB 明确记录的王珉配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_HUCpFfP4nOuZ14d-ze-m0w
        subject_person_id: p_fJbtRZx7s1XqSoTR8tLmBu
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_jWhyM89PP9d3eMCukjP7Hb
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_UVQEVcuwn8xU87vL0mRHsm
          claim_id: c_HUCpFfP4nOuZ14d-ze-m0w
          source_id: s_nVrhIMPmgIm2IdKyiKAGc8
          stance: supports
          locator: 嘉靖十四年進士登科錄:一卷，第二甲第三十七名：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_fJbtRZx7s1XqSoTR8tLmBu
        status: active
        display_name: 王珉
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 楊氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 楊氏，明人物。嘉靖十四年進士。（中国历代人物传记资料库 CBDB 295403） | accepted |
| name.primary | 楊氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_fJbtRZx7s1XqSoTR8tLmBu | 王珉 | accepted |

## 外部来源

- [中国历代人物传记资料库：楊氏(王珉妻)（CBDB 295403）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=295403&o=json)
