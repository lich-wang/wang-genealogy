---
schema: wang-person/v1
id: p_BphiRB1rQnM8AK6CrBfD3c
status: active
merged_into: null
display_name: 王曾
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_mYotFyn7r8SnGSJjWeinUu
        subject_person_id: p_BphiRB1rQnM8AK6CrBfD3c
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王曾
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_ZxtrYo4K6AnPj4aYyNZFKP
          claim_id: c_mYotFyn7r8SnGSJjWeinUu
          source_id: s_D4da3zKEcUJwBoozcr87Lj
          stance: supports
          locator: CBDB:194512
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（194512）
          source: &a1
            id: s_D4da3zKEcUJwBoozcr87Lj
            source_type: api_record
            title: 中国历代人物传记资料库：王曾（CBDB 194512）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=194512&o=json
            external_identifier: CBDB:194512
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:16.450Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_oV7WMEVFVcKH4XH461czZp
        subject_person_id: p_BphiRB1rQnM8AK6CrBfD3c
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 798年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Gwx8AL8k49nBquGcg3GAVS
          claim_id: c_oV7WMEVFVcKH4XH461czZp
          source_id: s_D4da3zKEcUJwBoozcr87Lj
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_AEM16Q1LypUexeDfzXDsw3
        subject_person_id: p_BphiRB1rQnM8AK6CrBfD3c
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王曾（卒于798年），唐人物。籍贯長安，曾任州別駕。（中国历代人物传记资料库 CBDB 194512）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_RrEyIvfKBgCgUnRGe2zEWe
          claim_id: c_AEM16Q1LypUexeDfzXDsw3
          source_id: s_D4da3zKEcUJwBoozcr87Lj
          stance: supports
          locator: CBDB:194512
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_bhgQUjjx3QmRtt1KzHTTA5
        subject_person_id: p_Jes5f5Kh67Z67c3E4f6FNQ
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_BphiRB1rQnM8AK6CrBfD3c
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Al5EiiMDqtqgbmp5bhp2hG
          claim_id: c_bhgQUjjx3QmRtt1KzHTTA5
          source_id: s_D4da3zKEcUJwBoozcr87Lj
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_Jes5f5Kh67Z67c3E4f6FNQ
        status: active
        display_name: 王倩
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王曾

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王曾 | accepted |
| death.date | 798年 | accepted |
| bio.summary | 王曾（卒于798年），唐人物。籍贯長安，曾任州別駕。（中国历代人物传记资料库 CBDB 194512） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_Jes5f5Kh67Z67c3E4f6FNQ | 王倩 | accepted |

## 外部来源

- [中国历代人物传记资料库：王曾（CBDB 194512）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=194512&o=json)
