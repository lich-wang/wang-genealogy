---
schema: wang-person/v1
id: p_KeQFoyHScM56Wpxk7FspZa
status: active
merged_into: null
display_name: 王純
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_Ai1K1899vEHm8hibJAaczA
        subject_person_id: p_KeQFoyHScM56Wpxk7FspZa
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王純
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_XqtCJAwdg4FmVg7q7Mudkt
          claim_id: c_Ai1K1899vEHm8hibJAaczA
          source_id: s_L2XHUgzR4kJPG1ydMUHMy8
          stance: supports
          locator: CBDB:138195
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（138195）
          source: &a1
            id: s_L2XHUgzR4kJPG1ydMUHMy8
            source_type: api_record
            title: 中国历代人物传记资料库：王純（CBDB 138195）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=138195&o=json
            external_identifier: CBDB:138195
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.358Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_z5mC7EYsqxK8dDjhEPpso8
        subject_person_id: p_KeQFoyHScM56Wpxk7FspZa
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王純，宋人物。寶祐進士。（中国历代人物传记资料库 CBDB 138195）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_ekm-7K3XxsOi6lD2RLQud5
          claim_id: c_z5mC7EYsqxK8dDjhEPpso8
          source_id: s_L2XHUgzR4kJPG1ydMUHMy8
          stance: supports
          locator: CBDB:138195
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_ua1OVo8i5qekeF315XAfdw
        subject_person_id: p_KeQFoyHScM56Wpxk7FspZa
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_jmLNJ8GNCMVo18MJZjNJp9
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_DHZqaT62_g-LDrPslkP07O
          claim_id: c_ua1OVo8i5qekeF315XAfdw
          source_id: s_L2XHUgzR4kJPG1ydMUHMy8
          stance: supports
          locator: 寶祐登科錄：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_jmLNJ8GNCMVo18MJZjNJp9
        status: active
        display_name: 王巽申
        merged_into_person_id: null
  other: []
---

# 王純

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王純 | accepted |
| bio.summary | 王純，宋人物。寶祐進士。（中国历代人物传记资料库 CBDB 138195） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_jmLNJ8GNCMVo18MJZjNJp9 | 王巽申 | accepted |

## 外部来源

- [中国历代人物传记资料库：王純（CBDB 138195）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=138195&o=json)
