---
schema: wang-person/v1
id: p_RhztFNKY4NjCU9wusho1it
status: active
merged_into: null
display_name: 王翊
revision: 1
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_LLgEiPVBdFbn9ED7uyG3Y5
        subject_person_id: p_RhztFNKY4NjCU9wusho1it
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王翊（生于485年），史料所见人物。本项目依据《王翊》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_28jCTjsXMWUsZiSLlGme5Z
          claim_id: c_LLgEiPVBdFbn9ED7uyG3Y5
          source_id: s_sEwGyjct8Du6MKHAr73uNu
          stance: supports
          locator: Q22079108
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_sEwGyjct8Du6MKHAr73uNu
            source_type: api_record
            title: 维基数据：王翊（Q22079108）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q22079108
            external_identifier: Q22079108
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:30:58.603Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%BF%8A_(%E5%8D%97%E5%8C%97%E6%9C%9D)
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_fgQBRMgbNHmMbCwG1PRuyJ
        subject_person_id: p_RhztFNKY4NjCU9wusho1it
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 485年
            calendar_note: 维基数据 P569 结构化日期，精度：年
            earliest: 0485-01-01
            latest: 0485-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_eurSP3RR3mAqHXZXKXZmJP
          claim_id: c_fgQBRMgbNHmMbCwG1PRuyJ
          source_id: s_sEwGyjct8Du6MKHAr73uNu
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_sEwGyjct8Du6MKHAr73uNu
            source_type: api_record
            title: 维基数据：王翊（Q22079108）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q22079108
            external_identifier: Q22079108
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:30:58.603Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%BF%8A_(%E5%8D%97%E5%8C%97%E6%9C%9D)
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_CLLo82pBmJpu92Nz7qmDyX
        subject_person_id: p_RhztFNKY4NjCU9wusho1it
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王翊
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_WG1FXu2e1q539LwPFXvE7c
          claim_id: c_CLLo82pBmJpu92Nz7qmDyX
          source_id: s_sEwGyjct8Du6MKHAr73uNu
          stance: supports
          locator: Q22079108
          quotation: null
          interpretation_note: null
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_xj9biwghTANR5M45yNEWBt
        subject_person_id: p_LSr2FsGq3jMfSxr6puXDCR
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_RhztFNKY4NjCU9wusho1it
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_tCsr44sRYCfAjXGM4k7qsE
          claim_id: c_xj9biwghTANR5M45yNEWBt
          source_id: s_wxS2wnZsRoxd2eKTrDMVs9
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_wxS2wnZsRoxd2eKTrDMVs9
            source_type: api_record
            title: 维基数据：王琛（Q22814923）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q22814923
            external_identifier: Q22814923
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:24:48.145Z
            metadata_json: null
        - id: cs_e9oeQoGU5MbTwEYsjsyVmB
          claim_id: c_xj9biwghTANR5M45yNEWBt
          source_id: s_sEwGyjct8Du6MKHAr73uNu
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_sEwGyjct8Du6MKHAr73uNu
            source_type: api_record
            title: 维基数据：王翊（Q22079108）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q22079108
            external_identifier: Q22079108
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:30:58.603Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%BF%8A_(%E5%8D%97%E5%8C%97%E6%9C%9D)
      object_person:
        id: p_LSr2FsGq3jMfSxr6puXDCR
        status: active
        display_name: 王琛
        merged_into_person_id: null
  children:
    - claim:
        id: c_2bWefe767X3mQN9xV7ztGk
        subject_person_id: p_RhztFNKY4NjCU9wusho1it
        claim_kind: relationship
        predicate: kinship.parent_of
        object_person_id: p_mPrN2vZZXVCq7iVmZzDF7H
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_LbSv56ZvFFd27js6C9KLSS
          claim_id: c_2bWefe767X3mQN9xV7ztGk
          source_id: s_MGBoig692jprej3f8uiDi2
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王翊 → 王渊
          interpretation_note: null
          source:
            id: s_MGBoig692jprej3f8uiDi2
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:58:59.171Z
            metadata_json: null
        - id: cs_E5CNvLUoFEYU1xwemu4n8d
          claim_id: c_2bWefe767X3mQN9xV7ztGk
          source_id: s_xEBYVW3xjFzLnp9P7pHZ2L
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王翊 → 王渊
          interpretation_note: null
          source:
            id: s_xEBYVW3xjFzLnp9P7pHZ2L
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:02:22.297Z
            metadata_json: null
      object_person:
        id: p_mPrN2vZZXVCq7iVmZzDF7H
        status: active
        display_name: 王渊
        merged_into_person_id: null
    - claim:
        id: c_XLjCMDECs6noawyDBVManz
        subject_person_id: p_RhztFNKY4NjCU9wusho1it
        claim_kind: relationship
        predicate: kinship.parent_of
        object_person_id: p_JDNX3z8pfGtKBk9YQzJ1Ab
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_Ad1V5YgM6FQqZJgyQkXAAV
          claim_id: c_XLjCMDECs6noawyDBVManz
          source_id: s_MGBoig692jprej3f8uiDi2
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王翊 → 王淡
          interpretation_note: null
          source:
            id: s_MGBoig692jprej3f8uiDi2
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:58:59.171Z
            metadata_json: null
        - id: cs_biSEfPti4bhsZmaoKcNQN7
          claim_id: c_XLjCMDECs6noawyDBVManz
          source_id: s_xEBYVW3xjFzLnp9P7pHZ2L
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王翊 → 王淡
          interpretation_note: null
          source:
            id: s_xEBYVW3xjFzLnp9P7pHZ2L
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:02:22.297Z
            metadata_json: null
      object_person:
        id: p_JDNX3z8pfGtKBk9YQzJ1Ab
        status: active
        display_name: 王淡
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王翊

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王翊（生于485年），史料所见人物。本项目依据《王翊》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| birth.date | 485年 | accepted |
| name.primary | 王翊 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_LSr2FsGq3jMfSxr6puXDCR | 王琛 | accepted |
| children | p_mPrN2vZZXVCq7iVmZzDF7H | 王渊 | accepted |
| children | p_JDNX3z8pfGtKBk9YQzJ1Ab | 王淡 | accepted |

## 外部来源

- [维基数据：王琛（Q22814923）](https://www.wikidata.org/wiki/Q22814923)
- [维基数据：王翊（Q22079108）](https://www.wikidata.org/wiki/Q22079108)
- [中文维基百科：琅邪王氏世系圖](https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96)
