---
schema: wang-person/v1
id: p_q3ZkDJwcYneEdHqF1jDPv8
status: active
merged_into: null
display_name: 王洋
revision: 1
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_wozu7ZBoIoWmzRCrt2dl-B
        subject_person_id: p_q3ZkDJwcYneEdHqF1jDPv8
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王洋（1087年—1154年），字元渤，山阳人。 原籍东牟（今山东蓬莱），省試第二名，宣和六年（1124年）甲辰科进士。绍兴元年（1131年），秘书省正字，歷官校书郎、吏部员外郎、起居舍人，知制诰。
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_ycS4F5Hrbz5KNkNHrfZLdR
          claim_id: c_wozu7ZBoIoWmzRCrt2dl-B
          source_id: s_tWscQNWA9_IS-SlkfApRcb
          stance: supports
          locator: 导言
          quotation: 王洋（1087年—1154年），字元渤，山阳人。 原籍东牟（今山
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 wikipedia。
          source:
            id: s_tWscQNWA9_IS-SlkfApRcb
            source_type: website
            title: 中文维基百科：王洋 (宋朝)
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%B4%8B_(%E5%AE%8B%E6%9C%9D)
            external_identifier: Q16077398
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-09-06T22:33:22.403Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_Gad3qRPDfjWfDFDEvvxeLZ
        subject_person_id: p_q3ZkDJwcYneEdHqF1jDPv8
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1087年
            calendar_note: 维基数据 P569 结构化日期，精度：年
            earliest: 1087-01-01
            latest: 1087-12-31
            precision: year
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_ovn98atCmMx7kRZLeXECDR
          claim_id: c_Gad3qRPDfjWfDFDEvvxeLZ
          source_id: s_5tQFeQ9M1zdY2LF4Jo7pY2
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_5tQFeQ9M1zdY2LF4Jo7pY2
            source_type: api_record
            title: 维基数据：王洋（Q16077398）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q16077398
            external_identifier: Q16077398
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:48:56.804Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_y8WrtE8WXfVR9TDhfPf62d
        subject_person_id: p_q3ZkDJwcYneEdHqF1jDPv8
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1153年
            calendar_note: 维基数据 P570 结构化日期，精度：年
            earliest: 1153-01-01
            latest: 1153-12-31
            precision: year
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_CVtHDhYD5a2uayG4SFNUqy
          claim_id: c_y8WrtE8WXfVR9TDhfPf62d
          source_id: s_5tQFeQ9M1zdY2LF4Jo7pY2
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_5tQFeQ9M1zdY2LF4Jo7pY2
            source_type: api_record
            title: 维基数据：王洋（Q16077398）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q16077398
            external_identifier: Q16077398
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:48:56.804Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_euQeNBGokSSfh8aWqgV6Ym
        subject_person_id: p_q3ZkDJwcYneEdHqF1jDPv8
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王洋
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_X4xkdGjoVYwFp1CV7XBS9R
          claim_id: c_euQeNBGokSSfh8aWqgV6Ym
          source_id: s_5tQFeQ9M1zdY2LF4Jo7pY2
          stance: supports
          locator: Q16077398
          quotation: null
          interpretation_note: null
          source:
            id: s_5tQFeQ9M1zdY2LF4Jo7pY2
            source_type: api_record
            title: 维基数据：王洋（Q16077398）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q16077398
            external_identifier: Q16077398
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:48:56.804Z
            metadata_json: null
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_y2iwqLVEnozptMzQuaquDZ
        subject_person_id: p_4i3jHPAhfhAhxjuPATCoyQ
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_q3ZkDJwcYneEdHqF1jDPv8
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_65UvPybWyQCoSdJSngP5yi
          claim_id: c_y2iwqLVEnozptMzQuaquDZ
          source_id: s_5tQFeQ9M1zdY2LF4Jo7pY2
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_5tQFeQ9M1zdY2LF4Jo7pY2
            source_type: api_record
            title: 维基数据：王洋（Q16077398）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q16077398
            external_identifier: Q16077398
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:48:56.804Z
            metadata_json: null
        - id: cs_U658HjnV47V5pTUEWNfQkq
          claim_id: c_y2iwqLVEnozptMzQuaquDZ
          source_id: s_LJAjcAHudFkvGGAcFvPYDB
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_LJAjcAHudFkvGGAcFvPYDB
            source_type: api_record
            title: 维基数据：李珏（Q45364417）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45364417
            external_identifier: Q45364417
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:00:17.572Z
            metadata_json: null
        - id: cs_sFGLLs3x5BQ626X4n4eZ4q
          claim_id: c_y2iwqLVEnozptMzQuaquDZ
          source_id: s_929j9mT6VcY5z94MFbSCY3
          stance: supports
          locator: 亲属关系：妻子
          quotation: null
          interpretation_note: CBDB 注明此条来源：未知
          source:
            id: s_929j9mT6VcY5z94MFbSCY3
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王洋（15443）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=15443&o=json
            external_identifier: CBDB:15443
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-05T02:02:10.849Z
            metadata_json: null
      object_person:
        id: p_4i3jHPAhfhAhxjuPATCoyQ
        status: active
        display_name: 李珏
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 王洋

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王洋（1087年—1154年），字元渤，山阳人。 原籍东牟（今山东蓬莱），省試第二名，宣和六年（1124年）甲辰科进士。绍兴元年（1131年），秘书省正字，歷官校书郎、吏部员外郎、起居舍人，知制诰。 | accepted |
| birth.date | 1087年 | accepted |
| death.date | 1153年 | accepted |
| name.primary | 王洋 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_4i3jHPAhfhAhxjuPATCoyQ | 李珏 | accepted |

## 外部来源

- [维基数据：李珏（Q45364417）](https://www.wikidata.org/wiki/Q45364417)
- [维基数据：王洋（Q16077398）](https://www.wikidata.org/wiki/Q16077398)
- [中文维基百科：王洋 (宋朝)](https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%B4%8B_(%E5%AE%8B%E6%9C%9D))
- [CBDB 中国历代人物传记资料库：王洋（15443）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=15443&o=json)
