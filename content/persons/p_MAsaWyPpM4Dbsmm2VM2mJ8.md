---
schema: wang-person/v1
id: p_MAsaWyPpM4Dbsmm2VM2mJ8
status: active
merged_into: null
display_name: 趙汝議
revision: 1
cbdb_id: 5324
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_vBGCoZGTFa0iLwRj9P1knO
        subject_person_id: p_MAsaWyPpM4Dbsmm2VM2mJ8
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 趙汝議，宋人物。籍贯餘杭，曾任孺人。（中国历代人物传记资料库 CBDB 5324）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_TlGuebffFiBywBiGfywYsR
          claim_id: c_vBGCoZGTFa0iLwRj9P1knO
          source_id: s_8tXUe_giyEECga336NGDOq
          stance: supports
          locator: CBDB:5324
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_8tXUe_giyEECga336NGDOq
            source_type: api_record
            title: 中国历代人物传记资料库：趙汝議（CBDB 5324）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=5324&o=json
            external_identifier: CBDB:5324
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_i-4fsKvFpEucfjeJ6ar5SK
        subject_person_id: p_MAsaWyPpM4Dbsmm2VM2mJ8
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 趙汝議
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_BI8tH1uXTjYh3PAcoYIYuW
          claim_id: c_i-4fsKvFpEucfjeJ6ar5SK
          source_id: s_8tXUe_giyEECga336NGDOq
          stance: supports
          locator: CBDB:5324
          quotation: null
          interpretation_note: CBDB 明确记录的王夢龍配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_SI-lPhJhNjdJ2IKhcDXBt5
        subject_person_id: p_PfgA5W1G2Fv9NYWMemcshd
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_MAsaWyPpM4Dbsmm2VM2mJ8
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_9Xng9euZZLG1V97jrRhegk
          claim_id: c_SI-lPhJhNjdJ2IKhcDXBt5
          source_id: s_8tXUe_giyEECga336NGDOq
          stance: supports
          locator: 宋人傳記資料索引(電子版)，2014;2015：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_PfgA5W1G2Fv9NYWMemcshd
        status: active
        display_name: 王夢龍
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 趙汝議

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 趙汝議，宋人物。籍贯餘杭，曾任孺人。（中国历代人物传记资料库 CBDB 5324） | accepted |
| name.primary | 趙汝議 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_PfgA5W1G2Fv9NYWMemcshd | 王夢龍 | accepted |

## 外部来源

- [中国历代人物传记资料库：趙汝議（CBDB 5324）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=5324&o=json)
