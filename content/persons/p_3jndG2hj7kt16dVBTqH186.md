---
schema: wang-person/v1
id: p_3jndG2hj7kt16dVBTqH186
status: active
merged_into: null
display_name: 王雱
revision: 1
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_W4nAkzgZ7HaFiHzmCLjpZM
        subject_person_id: p_3jndG2hj7kt16dVBTqH186
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王雱（1044年—1076年），字元澤，臨川（今江西撫州）人，名臣王安石的长子。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 4
      sources:
        - id: cs_R52AuPvZADnSAVK2q8BDvy
          claim_id: c_W4nAkzgZ7HaFiHzmCLjpZM
          source_id: s_VdmXrxM2y5tkGjKCX4GHbD
          stance: supports
          locator: Q15936430
          quotation: null
          interpretation_note: null
          source:
            id: s_VdmXrxM2y5tkGjKCX4GHbD
            source_type: api_record
            title: 維基數據：王雱（Q15936430）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15936430
            external_identifier: Q15936430
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:09:58.250Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E9%9B%B1
        - id: cs_bT2qVc9eAX6SFX1d7Yqtql
          claim_id: c_W4nAkzgZ7HaFiHzmCLjpZM
          source_id: s_7IPcOR4WeGaEWxuCr2uJPS
          stance: supports
          locator: 导言
          quotation: 王雱（1044年—1076年），字元澤，臨川（今江西撫州）人，名
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 wikipedia。
          source:
            id: s_7IPcOR4WeGaEWxuCr2uJPS
            source_type: website
            title: 中文维基百科：王雱
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E9%9B%B1
            external_identifier: Q15936430
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-09-06T14:32:33.958Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_gsaCa3tjNkMeAjGj9RPRPR
        subject_person_id: p_3jndG2hj7kt16dVBTqH186
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1044年
            calendar_note: 維基數據 P569 結構化日期，精度：年
            earliest: 1044-01-01
            latest: 1044-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_M9pUAAMbbjXs8sFC4dDDiR
          claim_id: c_gsaCa3tjNkMeAjGj9RPRPR
          source_id: s_VdmXrxM2y5tkGjKCX4GHbD
          stance: supports
          locator: Q15936430
          quotation: null
          interpretation_note: null
          source:
            id: s_VdmXrxM2y5tkGjKCX4GHbD
            source_type: api_record
            title: 維基數據：王雱（Q15936430）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15936430
            external_identifier: Q15936430
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:09:58.250Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E9%9B%B1
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_vZEzMiCBmjtBQCXYZtRq6k
        subject_person_id: p_3jndG2hj7kt16dVBTqH186
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1076年
            calendar_note: 維基數據 P570 結構化日期，精度：年
            earliest: 1076-01-01
            latest: 1076-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_CBw4YBNoW8iwnQrehagFjx
          claim_id: c_vZEzMiCBmjtBQCXYZtRq6k
          source_id: s_VdmXrxM2y5tkGjKCX4GHbD
          stance: supports
          locator: Q15936430
          quotation: null
          interpretation_note: null
          source:
            id: s_VdmXrxM2y5tkGjKCX4GHbD
            source_type: api_record
            title: 維基數據：王雱（Q15936430）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15936430
            external_identifier: Q15936430
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:09:58.250Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E9%9B%B1
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_Ejs2Nra9LbUWZYrM7E7Bzt
        subject_person_id: p_3jndG2hj7kt16dVBTqH186
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王雱
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_DAXK3o4tYzNzmf4rzbyQ2V
          claim_id: c_Ejs2Nra9LbUWZYrM7E7Bzt
          source_id: s_VdmXrxM2y5tkGjKCX4GHbD
          stance: supports
          locator: Q15936430
          quotation: null
          interpretation_note: null
          source:
            id: s_VdmXrxM2y5tkGjKCX4GHbD
            source_type: api_record
            title: 維基數據：王雱（Q15936430）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15936430
            external_identifier: Q15936430
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:09:58.250Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E9%9B%B1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_PAr9kobL69WD1Szgcy1etX
        subject_person_id: p_fuv1kE3UCBeTFNiA6c9Tuw
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_3jndG2hj7kt16dVBTqH186
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_9HWPPQBvGJ34jij1zK1dLT
          claim_id: c_PAr9kobL69WD1Szgcy1etX
          source_id: s_fLAce8PGeb2eAaVFYTSBka
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_fLAce8PGeb2eAaVFYTSBka
            source_type: api_record
            title: 維基數據：王安石（Q319618）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q319618
            external_identifier: Q319618
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:55.261Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%AE%89%E7%9F%B3
        - id: cs_49y4hYFyMC3jLrQTWykJpf
          claim_id: c_PAr9kobL69WD1Szgcy1etX
          source_id: s_VdmXrxM2y5tkGjKCX4GHbD
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_VdmXrxM2y5tkGjKCX4GHbD
            source_type: api_record
            title: 維基數據：王雱（Q15936430）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15936430
            external_identifier: Q15936430
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:09:58.250Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E9%9B%B1
        - id: cs_vv5nNRPrqjvdXvtGAC4oiS
          claim_id: c_PAr9kobL69WD1Szgcy1etX
          source_id: s_Gh6euCr2RKfuAXB8kPrVBR
          stance: supports
          locator: 亲属关系：子
          quotation: null
          interpretation_note: CBDB 注明此条来源：宋人傳記資料索引(電子版)
          source:
            id: s_Gh6euCr2RKfuAXB8kPrVBR
            source_type: api_record
            title: CBDB 中国历代人物传记资料库
            creator: null
            publisher: null
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu
            external_identifier: CBDB:1762
            license_code: CC-BY-NC-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_ArgDdp3M5ow3ve6mDNgHjw
          claim_id: c_PAr9kobL69WD1Szgcy1etX
          source_id: s_N8eFyA9jxYjP3URGB4QvGz
          stance: supports
          locator: 亲属关系：父
          quotation: null
          interpretation_note: CBDB 注明此条来源：未知
          source:
            id: s_N8eFyA9jxYjP3URGB4QvGz
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王雱（3968）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=3968&o=json
            external_identifier: CBDB:3968
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:20.279Z
            metadata_json: null
        - id: cs_6iBgQbWyg4CDN4UxmQgJPE
          claim_id: c_PAr9kobL69WD1Szgcy1etX
          source_id: s_5g7WxJuDRVzW6i37RBszym
          stance: supports
          locator: 条文：信息框 children
          quotation: 王雱
          interpretation_note: null
          source:
            id: s_5g7WxJuDRVzW6i37RBszym
            source_type: website
            title: 中文维基百科：王安石
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%AE%89%E7%9F%B3
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:53:03.947Z
            metadata_json: null
        - id: cs_q192tAA463HKJ9uZPzgZgZ
          claim_id: c_PAr9kobL69WD1Szgcy1etX
          source_id: s_G22w545KjS5SJCHy35YdSq
          stance: supports
          locator: 条文：之子/之女
          quotation: 王雱，字元澤，臨川（今江西撫州）人，名臣王安石的长子
          interpretation_note: null
          source:
            id: s_G22w545KjS5SJCHy35YdSq
            source_type: website
            title: 中文维基百科：王雱
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E9%9B%B1
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:53:10.921Z
            metadata_json: null
      object_person:
        id: p_fuv1kE3UCBeTFNiA6c9Tuw
        status: active
        display_name: 王安石
        merged_into_person_id: null
  children:
    - claim:
        id: c_6JpLR5vaSs2JrbcLmBPGaB
        subject_person_id: p_3jndG2hj7kt16dVBTqH186
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_u4eFkZcAi2Wj3khZrTyMM4
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_XETUEftJf9U3Z7mHjS3VrF
          claim_id: c_6JpLR5vaSs2JrbcLmBPGaB
          source_id: s_N8eFyA9jxYjP3URGB4QvGz
          stance: supports
          locator: 亲属关系：子
          quotation: null
          interpretation_note: CBDB 注明此条来源：宋人傳記資料索引(電子版)
          source:
            id: s_N8eFyA9jxYjP3URGB4QvGz
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王雱（3968）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=3968&o=json
            external_identifier: CBDB:3968
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:20.279Z
            metadata_json: null
        - id: cs_ZeWGirjKuxV227JcyyGPjH
          claim_id: c_6JpLR5vaSs2JrbcLmBPGaB
          source_id: s_VdmXrxM2y5tkGjKCX4GHbD
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_VdmXrxM2y5tkGjKCX4GHbD
            source_type: api_record
            title: 維基數據：王雱（Q15936430）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15936430
            external_identifier: Q15936430
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:09:58.250Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E9%9B%B1
        - id: cs_J9VZgRmSM2ppzD61w7Z5o9
          claim_id: c_6JpLR5vaSs2JrbcLmBPGaB
          source_id: s_uYseLrzAvUDGJLA7cNfC2e
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_uYseLrzAvUDGJLA7cNfC2e
            source_type: api_record
            title: 维基数据：王棣（Q45402041）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45402041
            external_identifier: Q45402041
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:49:22.443Z
            metadata_json: null
        - id: cs_UQRN3bWGtsfRpa8bnMSg6r
          claim_id: c_6JpLR5vaSs2JrbcLmBPGaB
          source_id: s_hyn3RBownq9NTiG26mHH8p
          stance: supports
          locator: 亲属关系：父
          quotation: null
          interpretation_note: CBDB 注明此条来源：未知
          source:
            id: s_hyn3RBownq9NTiG26mHH8p
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王棣（22210）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=22210&o=json
            external_identifier: CBDB:22210
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:49:22.578Z
            metadata_json: null
      object_person:
        id: p_u4eFkZcAi2Wj3khZrTyMM4
        status: active
        display_name: 王棣
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_AaNrR9xFV7Cv4rXfry2rHT
        subject_person_id: p_3jndG2hj7kt16dVBTqH186
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_mP4eY1Vv1A5GcUoNeqNM9S
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_ZYUcDYxV2nxyyW2uKFYhKe
          claim_id: c_AaNrR9xFV7Cv4rXfry2rHT
          source_id: s_G22w545KjS5SJCHy35YdSq
          stance: supports
          locator: 条文：条文识读（妻子）
          quotation: 王安石長子王雱與妻子蕭氏，只有生一位女兒，
          interpretation_note: null
          source:
            id: s_G22w545KjS5SJCHy35YdSq
            source_type: website
            title: 中文维基百科：王雱
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E9%9B%B1
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:53:10.921Z
            metadata_json: null
      object_person:
        id: p_mP4eY1Vv1A5GcUoNeqNM9S
        status: active
        display_name: 蕭氏
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 王雱

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王雱（1044年—1076年），字元澤，臨川（今江西撫州）人，名臣王安石的长子。 | accepted |
| birth.date | 1044年 | accepted |
| death.date | 1076年 | accepted |
| name.primary | 王雱 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_fuv1kE3UCBeTFNiA6c9Tuw | 王安石 | accepted |
| children | p_u4eFkZcAi2Wj3khZrTyMM4 | 王棣 | accepted |
| spouses | p_mP4eY1Vv1A5GcUoNeqNM9S | 蕭氏 | accepted |

## 外部来源

- [维基数据：王棣（Q45402041）](https://www.wikidata.org/wiki/Q45402041)
- [維基數據：王安石（Q319618）](https://www.wikidata.org/wiki/Q319618)
- [維基數據：王雱（Q15936430）](https://www.wikidata.org/wiki/Q15936430)
- [中文维基百科：王安石](https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%AE%89%E7%9F%B3)
- [中文维基百科：王雱](https://zh.wikipedia.org/wiki/%E7%8E%8B%E9%9B%B1)
- [CBDB 中国历代人物传记资料库](https://cbdb.fas.harvard.edu)
- [CBDB 中国历代人物传记资料库：王棣（22210）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=22210&o=json)
- [CBDB 中国历代人物传记资料库：王雱（3968）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=3968&o=json)
