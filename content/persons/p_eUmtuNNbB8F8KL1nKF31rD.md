---
schema: wang-person/v1
id: p_eUmtuNNbB8F8KL1nKF31rD
status: active
merged_into: null
display_name: 王大成
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_y6rz65VgwMSnsEcC1ZCaQ2
        subject_person_id: p_eUmtuNNbB8F8KL1nKF31rD
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王大成
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_Gbgy9s2sap2pAV42thR99w
          claim_id: c_y6rz65VgwMSnsEcC1ZCaQ2
          source_id: s_2T8zpJWX5dQNPTQi99QWzz
          stance: supports
          locator: CBDB:691540
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（691540）
          source: &a1
            id: s_2T8zpJWX5dQNPTQi99QWzz
            source_type: api_record
            title: 中国历代人物传记资料库：王大成（CBDB 691540）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=691540&o=json
            external_identifier: CBDB:691540
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:27.612Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_55xGtdFPHQtS3WDnpRJy6x
        subject_person_id: p_eUmtuNNbB8F8KL1nKF31rD
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王大成，清人物。籍贯婺源，入仕監生，曾任州同知。（中国历代人物传记资料库 CBDB 691540）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_PoYikBU09ETpQu-8ivbWpR
          claim_id: c_55xGtdFPHQtS3WDnpRJy6x
          source_id: s_2T8zpJWX5dQNPTQi99QWzz
          stance: supports
          locator: CBDB:691540
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_71pzSJCZamLBogxzWXw69F
        subject_person_id: p_MCutbyvi1BeB1uNuEEF1Tp
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_eUmtuNNbB8F8KL1nKF31rD
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_IMzyK0ndrUWdlkRp4cvcNO
          claim_id: c_71pzSJCZamLBogxzWXw69F
          source_id: s_2T8zpJWX5dQNPTQi99QWzz
          stance: supports
          locator: (道光)徽州府志:十六卷卷首一卷，lgid=1160833：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_MCutbyvi1BeB1uNuEEF1Tp
        status: active
        display_name: 王廷陞
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王大成

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王大成 | accepted |
| bio.summary | 王大成，清人物。籍贯婺源，入仕監生，曾任州同知。（中国历代人物传记资料库 CBDB 691540） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_MCutbyvi1BeB1uNuEEF1Tp | 王廷陞 | accepted |

## 外部来源

- [中国历代人物传记资料库：王大成（CBDB 691540）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=691540&o=json)
