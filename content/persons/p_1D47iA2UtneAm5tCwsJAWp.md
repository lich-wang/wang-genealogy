---
schema: wang-person/v1
id: p_1D47iA2UtneAm5tCwsJAWp
status: active
merged_into: null
display_name: 竇含
revision: 1
cbdb_id: 140944
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_hSXQdidDwiULcjdVPDoFSP
        subject_person_id: p_1D47iA2UtneAm5tCwsJAWp
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 竇含（717—748），唐人物。籍贯扶風。（中国历代人物传记资料库 CBDB 140944）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_bnt01JbdGpl-WJXuvfvJtU
          claim_id: c_hSXQdidDwiULcjdVPDoFSP
          source_id: s_fA6YH8gw5wWPc51HdfYQBs
          stance: supports
          locator: CBDB:140944
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_fA6YH8gw5wWPc51HdfYQBs
            source_type: api_record
            title: 中国历代人物传记资料库：竇含（CBDB 140944）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=140944&o=json
            external_identifier: CBDB:140944
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_MoJhN5-8-yDK79LA1Bzn0X
        subject_person_id: p_1D47iA2UtneAm5tCwsJAWp
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 竇含
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_T1gOB58HGmISea3pjnAjEx
          claim_id: c_MoJhN5-8-yDK79LA1Bzn0X
          source_id: s_fA6YH8gw5wWPc51HdfYQBs
          stance: supports
          locator: CBDB:140944
          quotation: null
          interpretation_note: CBDB 明确记录的王昔配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_2_rXWYLHE71yyjF7dPea7E
        subject_person_id: p_opy5i84MG6yAt4FcbfBzbm
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_1D47iA2UtneAm5tCwsJAWp
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_xNzlbvVWWV6JxsflVvEe1b
          claim_id: c_2_rXWYLHE71yyjF7dPea7E
          source_id: s_fA6YH8gw5wWPc51HdfYQBs
          stance: supports
          locator: 唐代墓誌匯編:二卷，Tianbao 140：丈夫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_opy5i84MG6yAt4FcbfBzbm
        status: active
        display_name: 王昔
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 竇含

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 竇含（717—748），唐人物。籍贯扶風。（中国历代人物传记资料库 CBDB 140944） | accepted |
| name.primary | 竇含 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_opy5i84MG6yAt4FcbfBzbm | 王昔 | accepted |

## 外部来源

- [中国历代人物传记资料库：竇含（CBDB 140944）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=140944&o=json)
