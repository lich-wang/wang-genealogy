---
schema: wang-person/v1
id: p_r3Dq9mpAQb1qJo3FSkUuFi
status: active
merged_into: null
display_name: 王勸
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_H3CjeY6s6dwH24F47SYSS2
        subject_person_id: p_r3Dq9mpAQb1qJo3FSkUuFi
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王勸
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_SBfBwAv81m6x1Mw9ub7TT4
          claim_id: c_H3CjeY6s6dwH24F47SYSS2
          source_id: s_fAnQR1pckGbgkLZAtVsHon
          stance: supports
          locator: CBDB:185512
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（185512）
          source: &a1
            id: s_fAnQR1pckGbgkLZAtVsHon
            source_type: api_record
            title: 中国历代人物传记资料库：王勸（CBDB 185512）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=185512&o=json
            external_identifier: CBDB:185512
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:16.246Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_d5exkaPYTHF2a2TMKjcCXY
        subject_person_id: p_r3Dq9mpAQb1qJo3FSkUuFi
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 841年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_LrZPc67FVFrC9PjMpwwUnn
          claim_id: c_d5exkaPYTHF2a2TMKjcCXY
          source_id: s_fAnQR1pckGbgkLZAtVsHon
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_PRR4BnBvPjEBSngGXxxB6F
        subject_person_id: p_r3Dq9mpAQb1qJo3FSkUuFi
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王勸（卒于841年），唐人物。籍贯武進。（中国历代人物传记资料库 CBDB 185512）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_cSHGK3N_wrG0ZfZzfhYqmE
          claim_id: c_PRR4BnBvPjEBSngGXxxB6F
          source_id: s_fAnQR1pckGbgkLZAtVsHon
          stance: supports
          locator: CBDB:185512
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_xY_7dFcKIsJOUNjgvzCSCc
        subject_person_id: p_UHQ83RNHQcEz6ALsbjLwK3
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_r3Dq9mpAQb1qJo3FSkUuFi
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Ra8kP9QYNZayE74U2p8d3W
          claim_id: c_xY_7dFcKIsJOUNjgvzCSCc
          source_id: s_fAnQR1pckGbgkLZAtVsHon
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_UHQ83RNHQcEz6ALsbjLwK3
        status: active
        display_name: 王甫
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王勸

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王勸 | accepted |
| death.date | 841年 | accepted |
| bio.summary | 王勸（卒于841年），唐人物。籍贯武進。（中国历代人物传记资料库 CBDB 185512） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_UHQ83RNHQcEz6ALsbjLwK3 | 王甫 | accepted |

## 外部来源

- [中国历代人物传记资料库：王勸（CBDB 185512）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=185512&o=json)
