---
schema: wang-person/v1
id: p_GCUbRsVW8TQC8UPFJ4RVue
status: active
merged_into: null
display_name: 王海朝
cbdb_id: 143290
revision: 5
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_abr4JSjZpKKi38QygMw6xB
        subject_person_id: p_GCUbRsVW8TQC8UPFJ4RVue
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王海朝（726年—807年），史料所见人物。本项目依据《中国历代人物传记资料库：王海朝（CBDB 143290）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_3i5Ll93cAaYQjiPSOD1nvA
          claim_id: c_abr4JSjZpKKi38QygMw6xB
          source_id: s_9EYx3hp1N67jvRbRMWvB82
          stance: supports
          locator: CBDB:143290
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_9EYx3hp1N67jvRbRMWvB82
            source_type: api_record
            title: 中国历代人物传记资料库：王海朝（CBDB 143290）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=143290&o=json
            external_identifier: CBDB:143290
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_fuHRho1t8xmgbHiq6kf6FV
        subject_person_id: p_GCUbRsVW8TQC8UPFJ4RVue
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 726年
            calendar_note: CBDB 生年字段，精度：年
            earliest: 0726-01-01
            latest: 0726-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_LXCY3qZmC7RH5fFRKShjoC
          claim_id: c_fuHRho1t8xmgbHiq6kf6FV
          source_id: s_9EYx3hp1N67jvRbRMWvB82
          stance: supports
          locator: CBDB:143290
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 807
          source:
            id: s_9EYx3hp1N67jvRbRMWvB82
            source_type: api_record
            title: 中国历代人物传记资料库：王海朝（CBDB 143290）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=143290&o=json
            external_identifier: CBDB:143290
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_hieguRwEpLqnUBFQ4PKYHr
        subject_person_id: p_GCUbRsVW8TQC8UPFJ4RVue
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 807年
            calendar_note: CBDB 卒年字段，精度：年
            earliest: 0807-01-01
            latest: 0807-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_4DQM6uwaRLC5B6ykWMRG11
          claim_id: c_hieguRwEpLqnUBFQ4PKYHr
          source_id: s_9EYx3hp1N67jvRbRMWvB82
          stance: supports
          locator: CBDB:143290
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 807
          source:
            id: s_9EYx3hp1N67jvRbRMWvB82
            source_type: api_record
            title: 中国历代人物传记资料库：王海朝（CBDB 143290）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=143290&o=json
            external_identifier: CBDB:143290
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_6BS6ozeYBWUZZ3NrEof2Gi
        subject_person_id: p_GCUbRsVW8TQC8UPFJ4RVue
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王海朝
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_LF1PXGNogXAhxmNhZdokUS
          claim_id: c_6BS6ozeYBWUZZ3NrEof2Gi
          source_id: s_9EYx3hp1N67jvRbRMWvB82
          stance: supports
          locator: CBDB:143290
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 807
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_mv0hyCdy19TSO8NO1yZR1H
        subject_person_id: p_GCUbRsVW8TQC8UPFJ4RVue
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_DxoGAcXvj7GVXEUMHUUA5z
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_vVCLEa466d-cZkKTbMH5uA
          claim_id: c_mv0hyCdy19TSO8NO1yZR1H
          source_id: s_WDPuAkPQHj6yHpVubA4QjP
          stance: supports
          locator: 唐代墓誌彙編續集，(XJ)Yuanhe65：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_WDPuAkPQHj6yHpVubA4QjP
            source_type: api_record
            title: 中国历代人物传记资料库：王詮（CBDB 166233）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=166233&o=json
            external_identifier: CBDB:166233
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.981Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_DxoGAcXvj7GVXEUMHUUA5z
        status: active
        display_name: 王詮
        merged_into_person_id: null
    - claim:
        id: c_Hkhg-r6_psoawpEgTuhhA0
        subject_person_id: p_GCUbRsVW8TQC8UPFJ4RVue
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_mfUAiE5928e8unf41fEhwH
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_u8q4ItyaGbOfno67J_dLrY
          claim_id: c_Hkhg-r6_psoawpEgTuhhA0
          source_id: s_XmYU3XvGAMoVzdECzViSBb
          stance: supports
          locator: 唐代墓誌彙編續集，(XJ)Yuanhe65：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_XmYU3XvGAMoVzdECzViSBb
            source_type: api_record
            title: 中国历代人物传记资料库：王均（CBDB 166234）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=166234&o=json
            external_identifier: CBDB:166234
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.982Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_mfUAiE5928e8unf41fEhwH
        status: active
        display_name: 王均
        merged_into_person_id: null
    - claim:
        id: c_yqmRJI9dnNipWYYin-LLUn
        subject_person_id: p_GCUbRsVW8TQC8UPFJ4RVue
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_tubeRxV3sBCQdXoc3GnJ6g
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_mjmTEN7QkX0JUdGKrMhiIu
          claim_id: c_yqmRJI9dnNipWYYin-LLUn
          source_id: s_drGRqcJFSAqFdK6ApUJP2q
          stance: supports
          locator: 唐代墓誌彙編續集，(XJ)Yuanhe65：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_drGRqcJFSAqFdK6ApUJP2q
            source_type: api_record
            title: 中国历代人物传记资料库：王建（CBDB 166235）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=166235&o=json
            external_identifier: CBDB:166235
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.983Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_tubeRxV3sBCQdXoc3GnJ6g
        status: active
        display_name: 王建
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_kt1rajZ1dR422_MdH7QTcN
        subject_person_id: p_GCUbRsVW8TQC8UPFJ4RVue
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_NsuLYWGMDN4Ho6Lw73DB78
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_SGtWhMLu8ll6dkZD3WgPHd
          claim_id: c_kt1rajZ1dR422_MdH7QTcN
          source_id: s_-4w4i5PxYz1jZK6cFSJDA3
          stance: supports
          locator: 唐代墓誌彙編續集，(XJ)Yuanhe65：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_-4w4i5PxYz1jZK6cFSJDA3
            source_type: api_record
            title: 中国历代人物传记资料库：揚氏(王海朝妻)（CBDB 166232）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=166232&o=json
            external_identifier: CBDB:166232
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_NsuLYWGMDN4Ho6Lw73DB78
        status: active
        display_name: 揚氏
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 王海朝

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王海朝（726年—807年），史料所见人物。本项目依据《中国历代人物传记资料库：王海朝（CBDB 143290）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| birth.date | 726年 | accepted |
| death.date | 807年 | accepted |
| name.primary | 王海朝 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_DxoGAcXvj7GVXEUMHUUA5z | 王詮 | accepted |
| children | p_mfUAiE5928e8unf41fEhwH | 王均 | accepted |
| children | p_tubeRxV3sBCQdXoc3GnJ6g | 王建 | accepted |
| spouses | p_NsuLYWGMDN4Ho6Lw73DB78 | 揚氏 | accepted |

## 外部来源

- [中国历代人物传记资料库：王海朝（CBDB 143290）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=143290&o=json)
- [中国历代人物传记资料库：王建（CBDB 166235）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=166235&o=json)
- [中国历代人物传记资料库：王均（CBDB 166234）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=166234&o=json)
- [中国历代人物传记资料库：王詮（CBDB 166233）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=166233&o=json)
- [中国历代人物传记资料库：揚氏(王海朝妻)（CBDB 166232）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=166232&o=json)
