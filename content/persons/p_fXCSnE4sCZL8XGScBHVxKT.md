---
schema: wang-person/v1
id: p_fXCSnE4sCZL8XGScBHVxKT
status: active
merged_into: null
display_name: 王東裏
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_63ENPANLnAYRhvLJv14JuZ
        subject_person_id: p_fXCSnE4sCZL8XGScBHVxKT
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王東裏
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_pd1Xf3QZqH6Z8GFnWQ86eA
          claim_id: c_63ENPANLnAYRhvLJv14JuZ
          source_id: s_xpHjc9SLTB85r9T2gyP6VK
          stance: supports
          locator: CBDB:97927
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（97927）
          source: &a1
            id: s_xpHjc9SLTB85r9T2gyP6VK
            source_type: api_record
            title: 中国历代人物传记资料库：王東裏（CBDB 97927）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=97927&o=json
            external_identifier: CBDB:97927
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:14.238Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_jUQBDAW8YPPHdU4fUb357M
        subject_person_id: p_fXCSnE4sCZL8XGScBHVxKT
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1130年
            calendar_note: CBDB 生年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_7XeZRvAHBUEbK1RWHEYkS5
          claim_id: c_jUQBDAW8YPPHdU4fUb357M
          source_id: s_xpHjc9SLTB85r9T2gyP6VK
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 生年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_oGig92YbDgLEH96RyaB86p
        subject_person_id: p_fXCSnE4sCZL8XGScBHVxKT
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王東裏（生于1130年），宋人物。籍贯麗水，入仕制舉。（中国历代人物传记资料库 CBDB 97927）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_-a6Hys19LfFCSMU0c5oxZX
          claim_id: c_oGig92YbDgLEH96RyaB86p
          source_id: s_xpHjc9SLTB85r9T2gyP6VK
          stance: supports
          locator: CBDB:97927
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_ztP6aouOG5HgJS9St9TI5e
        subject_person_id: p_DDp1tgWGCtANTjEPxEciDQ
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_fXCSnE4sCZL8XGScBHVxKT
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_uR1GHpCQ7KqWOVA2tBwktm
          claim_id: c_ztP6aouOG5HgJS9St9TI5e
          source_id: s_xpHjc9SLTB85r9T2gyP6VK
          stance: supports
          locator: 紹興十八年同年小錄，270：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_DDp1tgWGCtANTjEPxEciDQ
        status: active
        display_name: 王世修
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王東裏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王東裏 | accepted |
| birth.date | 1130年 | accepted |
| bio.summary | 王東裏（生于1130年），宋人物。籍贯麗水，入仕制舉。（中国历代人物传记资料库 CBDB 97927） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_DDp1tgWGCtANTjEPxEciDQ | 王世修 | accepted |

## 外部来源

- [中国历代人物传记资料库：王東裏（CBDB 97927）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=97927&o=json)
