---
schema: wang-person/v1
id: p_NiDwYtQeM1mw9R2BpSH12J
status: active
merged_into: null
display_name: 王尚辭
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_HjNkDRYvpTwT3TKgDzH2eJ
        subject_person_id: p_NiDwYtQeM1mw9R2BpSH12J
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王尚辭
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_bPVvjCu8XeYLRDFAgf15N8
          claim_id: c_HjNkDRYvpTwT3TKgDzH2eJ
          source_id: s_8bE4ZM3wpKGR5QKmEHmx7Q
          stance: supports
          locator: CBDB:696765
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（696765）
          source: &a1
            id: s_8bE4ZM3wpKGR5QKmEHmx7Q
            source_type: api_record
            title: 中国历代人物传记资料库：王尚辭（CBDB 696765）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=696765&o=json
            external_identifier: CBDB:696765
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:06.552Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_oA9iTrqHdCgnypTRQzHCGq
        subject_person_id: p_NiDwYtQeM1mw9R2BpSH12J
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王尚辭，史料所见人物。本项目依据《中国历代人物传记资料库：王尚辭（CBDB 696765）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_M2bqzLnZZO3TioTPZ5DPVi
          claim_id: c_oA9iTrqHdCgnypTRQzHCGq
          source_id: s_8bE4ZM3wpKGR5QKmEHmx7Q
          stance: supports
          locator: CBDB:696765
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_VGn8YYgVYI-0_2pK0i9zzH
        subject_person_id: p_HFQK5L8YrnKx1CYJmv1MtZ
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_NiDwYtQeM1mw9R2BpSH12J
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_STvI5YvgaRIADQ3uiwXWmp
          claim_id: c_VGn8YYgVYI-0_2pK0i9zzH
          source_id: s_8bE4ZM3wpKGR5QKmEHmx7Q
          stance: supports
          locator: 新出宋代墓志碑刻輯錄，一二四 王汲墓誌：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_HFQK5L8YrnKx1CYJmv1MtZ
        status: active
        display_name: 王汲
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王尚辭

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王尚辭 | accepted |
| bio.summary | 王尚辭，史料所见人物。本项目依据《中国历代人物传记资料库：王尚辭（CBDB 696765）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_HFQK5L8YrnKx1CYJmv1MtZ | 王汲 | accepted |

## 外部来源

- [中国历代人物传记资料库：王尚辭（CBDB 696765）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=696765&o=json)
