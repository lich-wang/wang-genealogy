---
schema: wang-person/v1
id: p_AKY9fS2Voe2u51T5iwbtN7
status: active
merged_into: null
display_name: 王德宣
revision: 3
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_6V7FTdi1qYu4LYbJAXBGQE
        subject_person_id: p_AKY9fS2Voe2u51T5iwbtN7
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王德宣
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_pFWDbdPSHxQCtJqUC276mh
          claim_id: c_6V7FTdi1qYu4LYbJAXBGQE
          source_id: s_WfrAkXP3XbCfMK1rcJxDUi
          stance: supports
          locator: CBDB:261603
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（261603）
          source: &a1
            id: s_WfrAkXP3XbCfMK1rcJxDUi
            source_type: api_record
            title: 中国历代人物传记资料库：王德宣（CBDB 261603）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=261603&o=json
            external_identifier: CBDB:261603
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.552Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_zxVRGpo7kLwL1dSKJRPN48
        subject_person_id: p_AKY9fS2Voe2u51T5iwbtN7
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王德宣，明人物。景泰五年進士，籍贯平夷長官司。（中国历代人物传记资料库 CBDB 261603）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_aNPYlWazZnLG9kTN5AOmbW
          claim_id: c_zxVRGpo7kLwL1dSKJRPN48
          source_id: s_WfrAkXP3XbCfMK1rcJxDUi
          stance: supports
          locator: CBDB:261603
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_t0mumWNjqhrHHNKhyvB7Js
        subject_person_id: p_SeER6gzsJFyEjnyJm77Ynj
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_AKY9fS2Voe2u51T5iwbtN7
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_cmFQWnuCLLPxdloAJAfUNh
          claim_id: c_t0mumWNjqhrHHNKhyvB7Js
          source_id: s_DLPHW4qYjP3NuMaN0MXbqL
          stance: supports
          locator: CBDB：兄弟 王常（198354）之父／母 王有問
          quotation: null
          interpretation_note: 由兄弟关系推断：王德宣 与 王常 为同胞（CBDB 记「兄」），王常 之父／母即 王德宣 之父／母。
          source:
            id: s_DLPHW4qYjP3NuMaN0MXbqL
            source_type: api_record
            title: 中国历代人物传记资料库：王德宣（CBDB 261603）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=261603&o=json
            external_identifier: CBDB:261603
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_SeER6gzsJFyEjnyJm77Ynj
        status: active
        display_name: 王有問
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_C11gQ97RZvAgoM4s56Naz7
        subject_person_id: p_99gNXAHSy7CHUSg6MDVZmM
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_AKY9fS2Voe2u51T5iwbtN7
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_GzWXM8SiqaAJQUadpcAMSF
          claim_id: c_C11gQ97RZvAgoM4s56Naz7
          source_id: s_DLPHW4qYjP3NuMaN0MXbqL
          stance: supports
          locator: CBDB 亲属：兄（KinPerson 198354 王常）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_DLPHW4qYjP3NuMaN0MXbqL
            source_type: api_record
            title: 中国历代人物传记资料库：王德宣（CBDB 261603）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=261603&o=json
            external_identifier: CBDB:261603
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_99gNXAHSy7CHUSg6MDVZmM
        status: active
        display_name: 王常
        merged_into_person_id: null
---

# 王德宣

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王德宣 | accepted |
| bio.summary | 王德宣，明人物。景泰五年進士，籍贯平夷長官司。（中国历代人物传记资料库 CBDB 261603） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_SeER6gzsJFyEjnyJm77Ynj | 王有問 | accepted |
| other | p_99gNXAHSy7CHUSg6MDVZmM | 王常 | accepted |

## 外部来源

- [中国历代人物传记资料库：王德宣（CBDB 261603）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=261603&o=json)
