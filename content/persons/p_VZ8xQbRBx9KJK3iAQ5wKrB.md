---
schema: wang-person/v1
id: p_VZ8xQbRBx9KJK3iAQ5wKrB
status: active
merged_into: null
display_name: 王壽山
cbdb_id: 522136
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_YLaENALxJCfvXFbp8zEFnu
        subject_person_id: p_VZ8xQbRBx9KJK3iAQ5wKrB
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王壽山，史料所见人物。本项目依据《中国历代人物传记资料库：王壽山（CBDB 522136）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs__NmOX3XR7i96r22QT3G-ln
          claim_id: c_YLaENALxJCfvXFbp8zEFnu
          source_id: s_3ek8kbR2tcBgDdUD1ved23
          stance: supports
          locator: CBDB:522136
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_3ek8kbR2tcBgDdUD1ved23
            source_type: api_record
            title: 中国历代人物传记资料库：王壽山（CBDB 522136）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=522136&o=json
            external_identifier: CBDB:522136
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:18:17.868Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_FaHYYREZu31Qjc8Vgsk83s
        subject_person_id: p_VZ8xQbRBx9KJK3iAQ5wKrB
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王壽山
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_GHYxUXPKA763kQ9E523jBX
          claim_id: c_FaHYYREZu31Qjc8Vgsk83s
          source_id: s_3ek8kbR2tcBgDdUD1ved23
          stance: supports
          locator: CBDB:522136
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（7001-7100）｜历史性依据：CBDB 朝代 = 清
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_qso-DKD8PNYgRK1gkJWeKg
        subject_person_id: p_7ZLkhM6fzxrjvBYHUXQTJQ
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_VZ8xQbRBx9KJK3iAQ5wKrB
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_gX-VcOlU0S6PEy0Oeb0M43
          claim_id: c_qso-DKD8PNYgRK1gkJWeKg
          source_id: s_3ek8kbR2tcBgDdUD1ved23
          stance: supports
          locator: 人名權威資料（中央研究院歷史語言研究所），9165：祖父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_3ek8kbR2tcBgDdUD1ved23
            source_type: api_record
            title: 中国历代人物传记资料库：王壽山（CBDB 522136）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=522136&o=json
            external_identifier: CBDB:522136
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:18:17.868Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_7ZLkhM6fzxrjvBYHUXQTJQ
        status: active
        display_name: 王念孙
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王壽山

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王壽山，史料所见人物。本项目依据《中国历代人物传记资料库：王壽山（CBDB 522136）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 王壽山 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| ancestors | p_7ZLkhM6fzxrjvBYHUXQTJQ | 王念孙 | accepted |

## 外部来源

- [中国历代人物传记资料库：王壽山（CBDB 522136）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=522136&o=json)
