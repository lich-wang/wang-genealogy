---
schema: wang-person/v1
id: p_4BFbF2D5CjTVVGnKZRvPjH
status: active
merged_into: null
display_name: 王脩福
cbdb_id: 140405
revision: 4
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_r3by9EuyVAsfDMdT3fLHB4
        subject_person_id: p_4BFbF2D5CjTVVGnKZRvPjH
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王脩福（662年—716年），史料所见人物。本项目依据《中国历代人物传记资料库：王脩福（CBDB 140405）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_vK-r2nBWWemoffmMz0c7Kx
          claim_id: c_r3by9EuyVAsfDMdT3fLHB4
          source_id: s_qHRkoxRnpfqKZD7DVUSvS9
          stance: supports
          locator: CBDB:140405
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_qHRkoxRnpfqKZD7DVUSvS9
            source_type: api_record
            title: 中国历代人物传记资料库：王脩福（CBDB 140405）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=140405&o=json
            external_identifier: CBDB:140405
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_dH6yQLx7NXAvWpW8w5yVsM
        subject_person_id: p_4BFbF2D5CjTVVGnKZRvPjH
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 662年
            calendar_note: CBDB 生年字段，精度：年
            earliest: 0662-01-01
            latest: 0662-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_LcxNqwyxtB25oUL5SEVF5a
          claim_id: c_dH6yQLx7NXAvWpW8w5yVsM
          source_id: s_qHRkoxRnpfqKZD7DVUSvS9
          stance: supports
          locator: CBDB:140405
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 716
          source:
            id: s_qHRkoxRnpfqKZD7DVUSvS9
            source_type: api_record
            title: 中国历代人物传记资料库：王脩福（CBDB 140405）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=140405&o=json
            external_identifier: CBDB:140405
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_MKZHHnjKeCSMamyzgMTDpc
        subject_person_id: p_4BFbF2D5CjTVVGnKZRvPjH
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 716年
            calendar_note: CBDB 卒年字段，精度：年
            earliest: 0716-01-01
            latest: 0716-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_uJL7CApnXd4UGtFo6AABny
          claim_id: c_MKZHHnjKeCSMamyzgMTDpc
          source_id: s_qHRkoxRnpfqKZD7DVUSvS9
          stance: supports
          locator: CBDB:140405
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 716
          source:
            id: s_qHRkoxRnpfqKZD7DVUSvS9
            source_type: api_record
            title: 中国历代人物传记资料库：王脩福（CBDB 140405）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=140405&o=json
            external_identifier: CBDB:140405
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_3qjG1bPFSb9XUnmAVmf811
        subject_person_id: p_4BFbF2D5CjTVVGnKZRvPjH
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王脩福
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_aAGAvKg8Y2h6i9Cc5Lydxx
          claim_id: c_3qjG1bPFSb9XUnmAVmf811
          source_id: s_qHRkoxRnpfqKZD7DVUSvS9
          stance: supports
          locator: CBDB:140405
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 716
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_9RnpBoTfPG9WxtLHdP3YhW
        subject_person_id: p_tWRDzk8caSQDv9HKGQpndD
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_4BFbF2D5CjTVVGnKZRvPjH
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_eSNPUiRJlYaGU5BaH0dp0x
          claim_id: c_9RnpBoTfPG9WxtLHdP3YhW
          source_id: s_eJfJ74fi83YiwN7NMmDP72
          stance: supports
          locator: 唐代墓誌匯編:二卷，Kaiyuan 131：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_eJfJ74fi83YiwN7NMmDP72
            source_type: api_record
            title: 中国历代人物传记资料库：王朗（CBDB 151604）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=151604&o=json
            external_identifier: CBDB:151604
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.840Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_tWRDzk8caSQDv9HKGQpndD
        status: active
        display_name: 王朗
        merged_into_person_id: null
  children:
    - claim:
        id: c_x6aiZSn_VpUJMtSN2z25qb
        subject_person_id: p_4BFbF2D5CjTVVGnKZRvPjH
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_6LS7HSLNuHSKm1QNRy3hLS
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_H1Kzx_3FzphLnR3p01-XNZ
          claim_id: c_x6aiZSn_VpUJMtSN2z25qb
          source_id: s_P5ai82ceGdyXmuanGWijhj
          stance: supports
          locator: 唐代墓誌匯編:二卷，Kaiyuan 131：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_P5ai82ceGdyXmuanGWijhj
            source_type: api_record
            title: 中国历代人物传记资料库：王景陽（CBDB 151606）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=151606&o=json
            external_identifier: CBDB:151606
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.840Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_6LS7HSLNuHSKm1QNRy3hLS
        status: active
        display_name: 王景陽
        merged_into_person_id: null
    - claim:
        id: c_nMYNk8pMXQpCnTqsrHIS-t
        subject_person_id: p_4BFbF2D5CjTVVGnKZRvPjH
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_CPwGMA1QZhWbQyip5boHEM
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs__5c4ku_gjbAMm_8-l4V9CU
          claim_id: c_nMYNk8pMXQpCnTqsrHIS-t
          source_id: s_WYKNANnB9cdXwokUbKmJR6
          stance: supports
          locator: 唐代墓誌匯編:二卷，Kaiyuan 131：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_WYKNANnB9cdXwokUbKmJR6
            source_type: api_record
            title: 中国历代人物传记资料库：王景珍（CBDB 151605）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=151605&o=json
            external_identifier: CBDB:151605
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:53.368Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_CPwGMA1QZhWbQyip5boHEM
        status: active
        display_name: 王景珍
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王脩福

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王脩福（662年—716年），史料所见人物。本项目依据《中国历代人物传记资料库：王脩福（CBDB 140405）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| birth.date | 662年 | accepted |
| death.date | 716年 | accepted |
| name.primary | 王脩福 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_tWRDzk8caSQDv9HKGQpndD | 王朗 | accepted |
| children | p_6LS7HSLNuHSKm1QNRy3hLS | 王景陽 | accepted |
| children | p_CPwGMA1QZhWbQyip5boHEM | 王景珍 | accepted |

## 外部来源

- [中国历代人物传记资料库：王景陽（CBDB 151606）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=151606&o=json)
- [中国历代人物传记资料库：王景珍（CBDB 151605）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=151605&o=json)
- [中国历代人物传记资料库：王朗（CBDB 151604）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=151604&o=json)
- [中国历代人物传记资料库：王脩福（CBDB 140405）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=140405&o=json)
