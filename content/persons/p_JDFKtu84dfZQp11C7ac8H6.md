---
schema: wang-person/v1
id: p_JDFKtu84dfZQp11C7ac8H6
status: active
merged_into: null
display_name: 王泰文
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_QgR6z849bYpeHHFVCNQ35v
        subject_person_id: p_JDFKtu84dfZQp11C7ac8H6
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王泰文
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_M1WkR8nuUshEr8QS4drv22
          claim_id: c_QgR6z849bYpeHHFVCNQ35v
          source_id: s_2T8nE6MKy7Fnsp2eoXb1BX
          stance: supports
          locator: CBDB:245287
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（245287）
          source: &a1
            id: s_2T8nE6MKy7Fnsp2eoXb1BX
            source_type: api_record
            title: 中国历代人物传记资料库：王泰文（CBDB 245287）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=245287&o=json
            external_identifier: CBDB:245287
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.057Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_mftp2RdnTmZDCe8S5ESqkv
        subject_person_id: p_JDFKtu84dfZQp11C7ac8H6
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王泰文，明人物。成化八年進士。（中国历代人物传记资料库 CBDB 245287）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_Rx84CzYjq2vYgvhVR_i-Hx
          claim_id: c_mftp2RdnTmZDCe8S5ESqkv
          source_id: s_2T8nE6MKy7Fnsp2eoXb1BX
          stance: supports
          locator: CBDB:245287
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_vB58PXZiDzjWi5SgKyP0qJ
        subject_person_id: p_JDFKtu84dfZQp11C7ac8H6
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_nMp3bRUZX5b7h1zfUaXGei
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_32j7ZDZXQZjp00ujAbjnoZ
          claim_id: c_vB58PXZiDzjWi5SgKyP0qJ
          source_id: s_2T8nE6MKy7Fnsp2eoXb1BX
          stance: supports
          locator: 成化八年進士登科錄:一卷，第二甲第二十名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_nMp3bRUZX5b7h1zfUaXGei
        status: active
        display_name: 王參
        merged_into_person_id: null
  other: []
---

# 王泰文

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王泰文 | accepted |
| bio.summary | 王泰文，明人物。成化八年進士。（中国历代人物传记资料库 CBDB 245287） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_nMp3bRUZX5b7h1zfUaXGei | 王參 | accepted |

## 外部来源

- [中国历代人物传记资料库：王泰文（CBDB 245287）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=245287&o=json)
