---
schema: wang-person/v1
id: p_N1C3MtgJB3M71G28PGdcAW
status: active
merged_into: null
display_name: 劉啟
cbdb_id: 16625
revision: 1
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_8sUsWZokh6e2A8MDzaKiNx
        subject_person_id: p_N1C3MtgJB3M71G28PGdcAW
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉啟（前187年—前140年3月6日），中国汉朝皇帝。维基数据以独立条目 Q7224 收录该人物；当前资料页据此确认其身份，其他生平细节仍待可靠史料补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_ePt4M9P2vSYiywXVJhfJAe
          claim_id: c_8sUsWZokh6e2A8MDzaKiNx
          source_id: s_xKz9UBNSopYgDsfwuwRFm8
          stance: supports
          locator: Q7224
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 wikidata。
          source: &a1
            id: s_xKz9UBNSopYgDsfwuwRFm8
            source_type: api_record
            title: 维基数据：汉景帝（Q7224）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7224
            external_identifier: Q7224
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:31:31.982Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%99%AF%E5%B8%9D
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_RxQ265D5gTNuVAFiscZrZ6
        subject_person_id: p_N1C3MtgJB3M71G28PGdcAW
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 前187年
            calendar_note: 维基数据 P569 结构化日期，精度：年
            earliest: -0187-01-01
            latest: -0187-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_2T7TcefGotiFmybrDTAdEY
          claim_id: c_RxQ265D5gTNuVAFiscZrZ6
          source_id: s_xKz9UBNSopYgDsfwuwRFm8
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_xKz9UBNSopYgDsfwuwRFm8
            source_type: api_record
            title: 维基数据：汉景帝（Q7224）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7224
            external_identifier: Q7224
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:31:31.982Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%99%AF%E5%B8%9D
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_vvbwb5956Rk9x4XqiD9UCL
        subject_person_id: p_N1C3MtgJB3M71G28PGdcAW
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 前140年3月6日
            calendar_note: 维基数据 P570 结构化日期，精度：日
            earliest: -0140-03-06
            latest: -0140-03-06
            precision: day
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_Pg2p4JzcrqoFDSmgtai669
          claim_id: c_vvbwb5956Rk9x4XqiD9UCL
          source_id: s_xKz9UBNSopYgDsfwuwRFm8
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_xKz9UBNSopYgDsfwuwRFm8
            source_type: api_record
            title: 维基数据：汉景帝（Q7224）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7224
            external_identifier: Q7224
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:31:31.982Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%99%AF%E5%B8%9D
      object_person: null
    alternatives: []
  - predicate: name.alias
    recommended:
      claim:
        id: c_wnL9x9BMB1gBQ8bV3tUV91
        subject_person_id: p_N1C3MtgJB3M71G28PGdcAW
        claim_kind: property
        predicate: name.alias
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 汉景帝
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_1LTr5YwqHiq2KrBjkYbH3S
          claim_id: c_wnL9x9BMB1gBQ8bV3tUV91
          source_id: s_tx8SMFTtbsbWuV4B3tptRv
          stance: supports
          locator: Q7224
          quotation: null
          interpretation_note: null
          source:
            id: s_tx8SMFTtbsbWuV4B3tptRv
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：劉啟（16625）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=16625&o=json
            external_identifier: CBDB:16625
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T03:31:32.143Z
            metadata_json: null
        - id: cs_F2iXz2R6G91j3mvgcVHkFN
          claim_id: c_wnL9x9BMB1gBQ8bV3tUV91
          source_id: s_xKz9UBNSopYgDsfwuwRFm8
          stance: supports
          locator: Q7224
          quotation: null
          interpretation_note: null
          source:
            id: s_xKz9UBNSopYgDsfwuwRFm8
            source_type: api_record
            title: 维基数据：汉景帝（Q7224）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7224
            external_identifier: Q7224
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:31:31.982Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%99%AF%E5%B8%9D
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_BwSewQxZvTPRxPyH6Xfbpx
        subject_person_id: p_N1C3MtgJB3M71G28PGdcAW
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉啟
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 3
      sources:
        - id: cs_jhdKctF75EFCvKYaHeTxTJ
          claim_id: c_BwSewQxZvTPRxPyH6Xfbpx
          source_id: s_tx8SMFTtbsbWuV4B3tptRv
          stance: supports
          locator: Q7224
          quotation: null
          interpretation_note: null
          source:
            id: s_tx8SMFTtbsbWuV4B3tptRv
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：劉啟（16625）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=16625&o=json
            external_identifier: CBDB:16625
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T03:31:32.143Z
            metadata_json: null
        - id: cs_z3objM6CzrDaKY6CDakQVk
          claim_id: c_BwSewQxZvTPRxPyH6Xfbpx
          source_id: s_xKz9UBNSopYgDsfwuwRFm8
          stance: supports
          locator: Q7224
          quotation: null
          interpretation_note: null
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_4kLPQViJerb59QJmdAHMfP
        subject_person_id: p_8rmLAjjNeK1vUUwZgEJckW
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_N1C3MtgJB3M71G28PGdcAW
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 4
      sources:
        - id: cs_4StHnKxpiVPP6sJBRnJz4b
          claim_id: c_4kLPQViJerb59QJmdAHMfP
          source_id: s_2cEcyZRMgcs4AGUHbs86Gz
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_2cEcyZRMgcs4AGUHbs86Gz
            source_type: api_record
            title: 维基数据：孝景王皇后（Q448012）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q448012
            external_identifier: Q448012
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:31:31.982Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E5%AD%9D%E6%99%AF%E7%8E%8B%E7%9A%87%E5%90%8E
        - id: cs_cR8MCpMRjsZh7mzsZY1vti
          claim_id: c_4kLPQViJerb59QJmdAHMfP
          source_id: s_xKz9UBNSopYgDsfwuwRFm8
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_xKz9UBNSopYgDsfwuwRFm8
            source_type: api_record
            title: 维基数据：汉景帝（Q7224）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7224
            external_identifier: Q7224
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:31:31.982Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%99%AF%E5%B8%9D
      object_person:
        id: p_8rmLAjjNeK1vUUwZgEJckW
        status: active
        display_name: 王娡
        merged_into_person_id: null
    - claim:
        id: c_kp7ZXQjoySSHfWk9cQ88aV
        subject_person_id: p_N1C3MtgJB3M71G28PGdcAW
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_dRM2nNMR4Y4tMEwG8AxGcm
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 4
      sources:
        - id: cs_43hN5bNX1yUBYtHERVxRhH
          claim_id: c_kp7ZXQjoySSHfWk9cQ88aV
          source_id: s_xKz9UBNSopYgDsfwuwRFm8
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_xKz9UBNSopYgDsfwuwRFm8
            source_type: api_record
            title: 维基数据：汉景帝（Q7224）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7224
            external_identifier: Q7224
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:31:31.982Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E6%99%AF%E5%B8%9D
        - id: cs_bP8exvYvkoJGggseyj2Fnp
          claim_id: c_kp7ZXQjoySSHfWk9cQ88aV
          source_id: s_XaJ4zmShinLkdVNBs8CQR9
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_XaJ4zmShinLkdVNBs8CQR9
            source_type: api_record
            title: 维基数据：王兒姁（Q8253153）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q8253153
            external_identifier: Q8253153
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:40:02.167Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%85%92%E5%A7%81
      object_person:
        id: p_dRM2nNMR4Y4tMEwG8AxGcm
        status: active
        display_name: 王兒姁
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 劉啟

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 劉啟（前187年—前140年3月6日），中国汉朝皇帝。维基数据以独立条目 Q7224 收录该人物；当前资料页据此确认其身份，其他生平细节仍待可靠史料补充。 | accepted |
| birth.date | 前187年 | accepted |
| death.date | 前140年3月6日 | accepted |
| name.alias | 汉景帝 | accepted |
| name.primary | 劉啟 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_8rmLAjjNeK1vUUwZgEJckW | 王娡 | accepted |
| spouses | p_dRM2nNMR4Y4tMEwG8AxGcm | 王兒姁 | accepted |

## 外部来源

- [维基数据：汉景帝（Q7224）](https://www.wikidata.org/wiki/Q7224)
- [维基数据：王兒姁（Q8253153）](https://www.wikidata.org/wiki/Q8253153)
- [维基数据：孝景王皇后（Q448012）](https://www.wikidata.org/wiki/Q448012)
- [CBDB 中国历代人物传记资料库：劉啟（16625）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=16625&o=json)
