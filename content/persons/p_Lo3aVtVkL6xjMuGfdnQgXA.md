---
schema: wang-person/v1
id: p_Lo3aVtVkL6xjMuGfdnQgXA
status: active
merged_into: null
display_name: 郭氏
revision: 1
cbdb_id: 683943
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_2a3EK3aq-2lK7WucGIdMjD
        subject_person_id: p_Lo3aVtVkL6xjMuGfdnQgXA
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 郭氏，宋人物。籍贯長子。（中国历代人物传记资料库 CBDB 683943）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_dWbOuzFfCbVgyAFztP3eQx
          claim_id: c_2a3EK3aq-2lK7WucGIdMjD
          source_id: s_LO7xDzyGOarrAFz3lpRp-k
          stance: supports
          locator: CBDB:683943
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_LO7xDzyGOarrAFz3lpRp-k
            source_type: api_record
            title: 中国历代人物传记资料库：郭氏(王蘇七妻)（CBDB 683943）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=683943&o=json
            external_identifier: CBDB:683943
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_GtdDwfEL05SbHedrf0jnJ1
        subject_person_id: p_Lo3aVtVkL6xjMuGfdnQgXA
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 郭氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs__W4UsceUpcKWUcZzko_RcX
          claim_id: c_GtdDwfEL05SbHedrf0jnJ1
          source_id: s_LO7xDzyGOarrAFz3lpRp-k
          stance: supports
          locator: CBDB:683943
          quotation: null
          interpretation_note: CBDB 明确记录的王蘇七配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_x_G1pemyAu82V07VScNWdU
        subject_person_id: p_PKez8fDzEkcxaF3twbojXm
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_Lo3aVtVkL6xjMuGfdnQgXA
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_FMsk1MXHUWmxBeZw-d1TPZ
          claim_id: c_x_G1pemyAu82V07VScNWdU
          source_id: s_LO7xDzyGOarrAFz3lpRp-k
          stance: supports
          locator: 新出宋代墓志碑刻輯錄，王章、王虔、王珣墓誌：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_PKez8fDzEkcxaF3twbojXm
        status: active
        display_name: 王蘇七
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 郭氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 郭氏，宋人物。籍贯長子。（中国历代人物传记资料库 CBDB 683943） | accepted |
| name.primary | 郭氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_PKez8fDzEkcxaF3twbojXm | 王蘇七 | accepted |

## 外部来源

- [中国历代人物传记资料库：郭氏(王蘇七妻)（CBDB 683943）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=683943&o=json)
