---
schema: wang-person/v1
id: p_RQeyyQ8v759GU56Ye8JJst
status: active
merged_into: null
display_name: 王崇冠
cbdb_id: 202901
revision: 4
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_M6rDKMHT4PA72KMFooQpnW
        subject_person_id: p_RQeyyQ8v759GU56Ye8JJst
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王崇冠（生于1508年），史料所见人物。本项目依据《中国历代人物传记资料库：王崇冠（CBDB 202901）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_eqh0K-GY7u38khzR8CuC_3
          claim_id: c_M6rDKMHT4PA72KMFooQpnW
          source_id: s_qyLhWhL8HbT1wKAnyJ4B43
          stance: supports
          locator: CBDB:202901
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_qyLhWhL8HbT1wKAnyJ4B43
            source_type: api_record
            title: 中国历代人物传记资料库：王崇冠（CBDB 202901）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=202901&o=json
            external_identifier: CBDB:202901
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:42.210Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_32Lr2BZFZTB73z2mmNG2K8
        subject_person_id: p_RQeyyQ8v759GU56Ye8JJst
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1508年
            calendar_note: CBDB 生年字段，精度：年
            earliest: 1508-01-01
            latest: 1508-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_ABHS4N1JDEoTKmBCdRkYcj
          claim_id: c_32Lr2BZFZTB73z2mmNG2K8
          source_id: s_qyLhWhL8HbT1wKAnyJ4B43
          stance: supports
          locator: CBDB:202901
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（701-800）｜历史性依据：CBDB 生年 = 1508
          source:
            id: s_qyLhWhL8HbT1wKAnyJ4B43
            source_type: api_record
            title: 中国历代人物传记资料库：王崇冠（CBDB 202901）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=202901&o=json
            external_identifier: CBDB:202901
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:42.210Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_sx9SmzDh5W4U4SdvWA8Qn9
        subject_person_id: p_RQeyyQ8v759GU56Ye8JJst
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王崇冠
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_NAoZgbpuRgwEuPNWjULJcX
          claim_id: c_sx9SmzDh5W4U4SdvWA8Qn9
          source_id: s_qyLhWhL8HbT1wKAnyJ4B43
          stance: supports
          locator: CBDB:202901
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（701-800）｜历史性依据：CBDB 生年 = 1508
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_sUxuxslERglUuVFNBmqRgQ
        subject_person_id: p_WKi2gp1aMEpd772NtPQKTC
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_RQeyyQ8v759GU56Ye8JJst
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_CRacrpML693T1Vy0wqGhLo
          claim_id: c_sUxuxslERglUuVFNBmqRgQ
          source_id: s_1Xr8FkY9LaZLxDdRbWSXJH
          stance: supports
          locator: 嘉靖十四年進士登科錄:一卷，第三甲第三十二名：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_1Xr8FkY9LaZLxDdRbWSXJH
            source_type: api_record
            title: 中国历代人物传记资料库：王明（CBDB 296439）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=296439&o=json
            external_identifier: CBDB:296439
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.523Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_WKi2gp1aMEpd772NtPQKTC
        status: active
        display_name: 王明
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_VCAaQ5DVp22ylxK11EL7dz
        subject_person_id: p_JtMRRUVv2cpMBMJ3wWDY61
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_RQeyyQ8v759GU56Ye8JJst
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_BIs3Aer-j8ZA_PJhaqvRVf
          claim_id: c_VCAaQ5DVp22ylxK11EL7dz
          source_id: s_CwX2BhQJDQD4uDR4tmTC7m
          stance: supports
          locator: 嘉靖十四年進士登科錄:一卷，第三甲第三十二名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_CwX2BhQJDQD4uDR4tmTC7m
            source_type: api_record
            title: 中国历代人物传记资料库：王景春（CBDB 296438）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=296438&o=json
            external_identifier: CBDB:296438
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.522Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_JtMRRUVv2cpMBMJ3wWDY61
        status: active
        display_name: 王景春
        merged_into_person_id: null
    - claim:
        id: c_rVJK8iUI9ZbdSlty9ysXwt
        subject_person_id: p_YooVVt33STAedzoaznvNWg
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_RQeyyQ8v759GU56Ye8JJst
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_5H2H1XJDxtgJs8Mw8uS8Xz
          claim_id: c_rVJK8iUI9ZbdSlty9ysXwt
          source_id: s_bFcyPXCiqLHh7CapMJztWH
          stance: supports
          locator: 嘉靖十四年進士登科錄:一卷，第三甲第三十二名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_bFcyPXCiqLHh7CapMJztWH
            source_type: api_record
            title: 中国历代人物传记资料库：王軏（CBDB 296437）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=296437&o=json
            external_identifier: CBDB:296437
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.522Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_YooVVt33STAedzoaznvNWg
        status: active
        display_name: 王軏
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王崇冠

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王崇冠（生于1508年），史料所见人物。本项目依据《中国历代人物传记资料库：王崇冠（CBDB 202901）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| birth.date | 1508年 | accepted |
| name.primary | 王崇冠 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_WKi2gp1aMEpd772NtPQKTC | 王明 | accepted |
| ancestors | p_JtMRRUVv2cpMBMJ3wWDY61 | 王景春 | accepted |
| ancestors | p_YooVVt33STAedzoaznvNWg | 王軏 | accepted |

## 外部来源

- [中国历代人物传记资料库：王崇冠（CBDB 202901）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=202901&o=json)
- [中国历代人物传记资料库：王景春（CBDB 296438）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=296438&o=json)
- [中国历代人物传记资料库：王明（CBDB 296439）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=296439&o=json)
- [中国历代人物传记资料库：王軏（CBDB 296437）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=296437&o=json)
