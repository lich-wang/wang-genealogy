---
schema: wang-person/v1
id: p_GrUu3zghDRVSzoU1NxbQux
status: active
merged_into: null
display_name: 王宗耀
cbdb_id: 71815
revision: 3
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_mDi79XnfNNgCYdUa5kU3wG
        subject_person_id: p_GrUu3zghDRVSzoU1NxbQux
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王宗耀（1782年—1841年），史料所见人物。本项目依据《中国历代人物传记资料库：王宗耀（CBDB 71815）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_jFJAzBMVShI5lYW2pGojON
          claim_id: c_mDi79XnfNNgCYdUa5kU3wG
          source_id: s_LKHPB8NGhzsCtGP9SQQs5e
          stance: supports
          locator: CBDB:71815
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_LKHPB8NGhzsCtGP9SQQs5e
            source_type: api_record
            title: 中国历代人物传记资料库：王宗耀（CBDB 71815）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=71815&o=json
            external_identifier: CBDB:71815
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:36.789Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_PdfnUTAajgYAS3M2wBHxcn
        subject_person_id: p_GrUu3zghDRVSzoU1NxbQux
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1782年
            calendar_note: CBDB 生年字段，精度：年
            earliest: 1782-01-01
            latest: 1782-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_B3t9EhBVxqB3wqFfoANiTs
          claim_id: c_PdfnUTAajgYAS3M2wBHxcn
          source_id: s_LKHPB8NGhzsCtGP9SQQs5e
          stance: supports
          locator: CBDB:71815
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（201-300）｜历史性依据：CBDB 卒年 = 1841
          source:
            id: s_LKHPB8NGhzsCtGP9SQQs5e
            source_type: api_record
            title: 中国历代人物传记资料库：王宗耀（CBDB 71815）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=71815&o=json
            external_identifier: CBDB:71815
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:36.789Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_vijjy1132WsFhR7nNRGY27
        subject_person_id: p_GrUu3zghDRVSzoU1NxbQux
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1841年
            calendar_note: CBDB 卒年字段，精度：年
            earliest: 1841-01-01
            latest: 1841-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_zLomsE24SCQ31FGL3rNEJc
          claim_id: c_vijjy1132WsFhR7nNRGY27
          source_id: s_LKHPB8NGhzsCtGP9SQQs5e
          stance: supports
          locator: CBDB:71815
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（201-300）｜历史性依据：CBDB 卒年 = 1841
          source:
            id: s_LKHPB8NGhzsCtGP9SQQs5e
            source_type: api_record
            title: 中国历代人物传记资料库：王宗耀（CBDB 71815）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=71815&o=json
            external_identifier: CBDB:71815
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:36.789Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_fTboLKwP4M8pxDbwcasRw5
        subject_person_id: p_GrUu3zghDRVSzoU1NxbQux
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王宗耀
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_mN8Hm4N8hZFotKEqHmj3hv
          claim_id: c_fTboLKwP4M8pxDbwcasRw5
          source_id: s_LKHPB8NGhzsCtGP9SQQs5e
          stance: supports
          locator: CBDB:71815
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（201-300）｜历史性依据：CBDB 卒年 = 1841
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_VMWscnMHNDKv9SjyFhz1vv
        subject_person_id: p_gWeQqYSDEj93QiAPCoQxmK
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_GrUu3zghDRVSzoU1NxbQux
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_ux_bxbLtqMZ-_7p39mYktP
          claim_id: c_VMWscnMHNDKv9SjyFhz1vv
          source_id: s_4cM4ZihPu3KAUqfAFWA7ZV
          stance: supports
          locator: 鄞縣志，lgid=243606：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_4cM4ZihPu3KAUqfAFWA7ZV
            source_type: api_record
            title: 中国历代人物传记资料库：王予彰（CBDB 694148）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=694148&o=json
            external_identifier: CBDB:694148
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:06.516Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_gWeQqYSDEj93QiAPCoQxmK
        status: active
        display_name: 王予彰
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_lSQznjYAdFG_E2zU1aY_uK
        subject_person_id: p_ixapZhQmmSDAS1angZKZYU
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_GrUu3zghDRVSzoU1NxbQux
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_JWZhwOZte8Bo5T-W3MkKpt
          claim_id: c_lSQznjYAdFG_E2zU1aY_uK
          source_id: s_hPA2wXFn6CM42W4D5FLhFz
          stance: supports
          locator: 鄞縣志，lgid=243606：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_hPA2wXFn6CM42W4D5FLhFz
            source_type: api_record
            title: 中国历代人物传记资料库：王士霖（CBDB 694147）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=694147&o=json
            external_identifier: CBDB:694147
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:06.515Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_ixapZhQmmSDAS1angZKZYU
        status: active
        display_name: 王士霖
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王宗耀

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王宗耀（1782年—1841年），史料所见人物。本项目依据《中国历代人物传记资料库：王宗耀（CBDB 71815）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| birth.date | 1782年 | accepted |
| death.date | 1841年 | accepted |
| name.primary | 王宗耀 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_gWeQqYSDEj93QiAPCoQxmK | 王予彰 | accepted |
| ancestors | p_ixapZhQmmSDAS1angZKZYU | 王士霖 | accepted |

## 外部来源

- [中国历代人物传记资料库：王士霖（CBDB 694147）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=694147&o=json)
- [中国历代人物传记资料库：王予彰（CBDB 694148）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=694148&o=json)
- [中国历代人物传记资料库：王宗耀（CBDB 71815）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=71815&o=json)
