---
schema: wang-person/v1
id: p_wV4tfa6rqeec47q9qXtyoE
status: active
merged_into: null
display_name: 王庸世
cbdb_id: 526180
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_6JKzdKbQyAoKaN3kqA1iiM
        subject_person_id: p_wV4tfa6rqeec47q9qXtyoE
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王庸世，史料所见人物。本项目依据《中国历代人物传记资料库：王庸世（CBDB 526180）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_6J0TLKKVEEScyvpPtPVPL0
          claim_id: c_6JKzdKbQyAoKaN3kqA1iiM
          source_id: s_NzpWEdSF1sqaodxcDrqZDb
          stance: supports
          locator: CBDB:526180
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_NzpWEdSF1sqaodxcDrqZDb
            source_type: api_record
            title: 中国历代人物传记资料库：王庸世（CBDB 526180）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=526180&o=json
            external_identifier: CBDB:526180
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:18:17.868Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_RMWKGjHvxkFapZxk4mPRfo
        subject_person_id: p_wV4tfa6rqeec47q9qXtyoE
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王庸世
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_mqr63YcgP9rEaAkaAHwzbo
          claim_id: c_RMWKGjHvxkFapZxk4mPRfo
          source_id: s_NzpWEdSF1sqaodxcDrqZDb
          stance: supports
          locator: CBDB:526180
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（7001-7100）｜历史性依据：CBDB 朝代 = 明
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_zG8fioHhDKJfcMS3isXQMa
        subject_person_id: p_wV4tfa6rqeec47q9qXtyoE
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_qC31C42hD4UK4m3NDzMPZB
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_cbLUuD6FvjGU5EPw6iKK0t
          claim_id: c_zG8fioHhDKJfcMS3isXQMa
          source_id: s_NzpWEdSF1sqaodxcDrqZDb
          stance: supports
          locator: 人名權威資料（中央研究院歷史語言研究所），12015：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_NzpWEdSF1sqaodxcDrqZDb
            source_type: api_record
            title: 中国历代人物传记资料库：王庸世（CBDB 526180）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=526180&o=json
            external_identifier: CBDB:526180
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:18:17.868Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_qC31C42hD4UK4m3NDzMPZB
        status: active
        display_name: 王暐
        merged_into_person_id: null
  other: []
---

# 王庸世

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王庸世，史料所见人物。本项目依据《中国历代人物传记资料库：王庸世（CBDB 526180）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 王庸世 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_qC31C42hD4UK4m3NDzMPZB | 王暐 | accepted |

## 外部来源

- [中国历代人物传记资料库：王庸世（CBDB 526180）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=526180&o=json)
