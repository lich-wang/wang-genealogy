---
schema: wang-person/v1
id: p_vtVH2A2Vaj9Tb5xnzYiPZD
status: active
merged_into: null
display_name: 王經
revision: 3
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_JcZXTrkUpNFC5tZS2BL3Uk
        subject_person_id: p_vtVH2A2Vaj9Tb5xnzYiPZD
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王經
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_yCkzVtfuGQVCHgKFqGynPP
          claim_id: c_JcZXTrkUpNFC5tZS2BL3Uk
          source_id: s_6kHmSgoQaVS4PftNxYZCu9
          stance: supports
          locator: CBDB:325692
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（325692）
          source: &a1
            id: s_6kHmSgoQaVS4PftNxYZCu9
            source_type: api_record
            title: 中国历代人物传记资料库：王經（CBDB 325692）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=325692&o=json
            external_identifier: CBDB:325692
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:20.202Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_LH39ki2wV4hLqcfKXqvbkB
        subject_person_id: p_vtVH2A2Vaj9Tb5xnzYiPZD
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王經，明人物。嘉靖三十八年進士，籍贯定遠。（中国历代人物传记资料库 CBDB 325692）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_xKdpbENIdtlTmjlMBCcNP0
          claim_id: c_LH39ki2wV4hLqcfKXqvbkB
          source_id: s_6kHmSgoQaVS4PftNxYZCu9
          stance: supports
          locator: CBDB:325692
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_dnBJD1JjMUCzHPcelj2dPg
        subject_person_id: p_Py4o4MSaFdQ83M1B2MuZ1Y
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_vtVH2A2Vaj9Tb5xnzYiPZD
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_nCkn0atdJhBS9z3I3Mq9GE
          claim_id: c_dnBJD1JjMUCzHPcelj2dPg
          source_id: s_WsptdqE7iXkAQ7WBlpJBMh
          stance: supports
          locator: CBDB：兄弟 王楫（204909）之父／母 王鶴
          quotation: null
          interpretation_note: 由兄弟关系推断：王經 与 王楫 为同胞（CBDB 记「弟」），王楫 之父／母即 王經 之父／母。
          source:
            id: s_WsptdqE7iXkAQ7WBlpJBMh
            source_type: api_record
            title: 中国历代人物传记资料库：王經（CBDB 325692）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=325692&o=json
            external_identifier: CBDB:325692
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_Py4o4MSaFdQ83M1B2MuZ1Y
        status: active
        display_name: 王鶴
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_Qt3DywPscrbosDkhLYQP3k
        subject_person_id: p_vtVH2A2Vaj9Tb5xnzYiPZD
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_zZwvDUs3YgX6S9BozMt7RC
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_BgyWTuWStHvHa8MSw_TvDo
          claim_id: c_Qt3DywPscrbosDkhLYQP3k
          source_id: s_WsptdqE7iXkAQ7WBlpJBMh
          stance: supports
          locator: CBDB 亲属：弟（KinPerson 204909 王楫）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_WsptdqE7iXkAQ7WBlpJBMh
            source_type: api_record
            title: 中国历代人物传记资料库：王經（CBDB 325692）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=325692&o=json
            external_identifier: CBDB:325692
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_zZwvDUs3YgX6S9BozMt7RC
        status: active
        display_name: 王楫
        merged_into_person_id: null
---

# 王經

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王經 | accepted |
| bio.summary | 王經，明人物。嘉靖三十八年進士，籍贯定遠。（中国历代人物传记资料库 CBDB 325692） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_Py4o4MSaFdQ83M1B2MuZ1Y | 王鶴 | accepted |
| other | p_zZwvDUs3YgX6S9BozMt7RC | 王楫 | accepted |

## 外部来源

- [中国历代人物传记资料库：王經（CBDB 325692）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=325692&o=json)
