---
schema: wang-person/v1
id: p_knpq7rxcxzyhPa2D2LZHK7
status: active
merged_into: null
display_name: 王繼業
revision: 3
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_iiNKGnUyz6m4VJPdWosXc2
        subject_person_id: p_knpq7rxcxzyhPa2D2LZHK7
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王繼業
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_hJ8zJ3kUNuHPWkn4Dp7yCx
          claim_id: c_iiNKGnUyz6m4VJPdWosXc2
          source_id: s_TP3NtDep2QATCmJF6C9ZSk
          stance: supports
          locator: CBDB:334040
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（334040）
          source: &a1
            id: s_TP3NtDep2QATCmJF6C9ZSk
            source_type: api_record
            title: 中国历代人物传记资料库：王繼業（CBDB 334040）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=334040&o=json
            external_identifier: CBDB:334040
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:20.430Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_sxDPDyBhYYrAGf8CxRjnFJ
        subject_person_id: p_knpq7rxcxzyhPa2D2LZHK7
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王繼業，明人物。隆慶二年進士，籍贯寧夏衛。（中国历代人物传记资料库 CBDB 334040）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_tULabcRO9iY7I-pLcro2Yp
          claim_id: c_sxDPDyBhYYrAGf8CxRjnFJ
          source_id: s_TP3NtDep2QATCmJF6C9ZSk
          stance: supports
          locator: CBDB:334040
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_DRzUGfrlBLxZ7zkJoDRke3
        subject_person_id: p_PyEHQmuoeFWGbQR7iXh3pQ
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_knpq7rxcxzyhPa2D2LZHK7
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_KU1Rup8dWh7i8GKIluNyWq
          claim_id: c_DRzUGfrlBLxZ7zkJoDRke3
          source_id: s_qxEw2t22Rvqu-4y-tSbVuT
          stance: supports
          locator: CBDB：兄弟 王繼祖（205485）之父／母 王鑑
          quotation: null
          interpretation_note: 由兄弟关系推断：王繼業 与 王繼祖 为同胞（CBDB 记「兄」），王繼祖 之父／母即 王繼業 之父／母。
          source:
            id: s_qxEw2t22Rvqu-4y-tSbVuT
            source_type: api_record
            title: 中国历代人物传记资料库：王繼業（CBDB 334040）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=334040&o=json
            external_identifier: CBDB:334040
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_PyEHQmuoeFWGbQR7iXh3pQ
        status: active
        display_name: 王鑑
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_BLej4XS_X1hc73v7YR4Ngw
        subject_person_id: p_Q5jiewEf7qr9FT4CBgkaYV
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_knpq7rxcxzyhPa2D2LZHK7
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs__Vw-87BIq-GptBKB-9nVI3
          claim_id: c_BLej4XS_X1hc73v7YR4Ngw
          source_id: s_qxEw2t22Rvqu-4y-tSbVuT
          stance: supports
          locator: CBDB 亲属：兄（KinPerson 205485 王繼祖）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_qxEw2t22Rvqu-4y-tSbVuT
            source_type: api_record
            title: 中国历代人物传记资料库：王繼業（CBDB 334040）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=334040&o=json
            external_identifier: CBDB:334040
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_Q5jiewEf7qr9FT4CBgkaYV
        status: active
        display_name: 王繼祖
        merged_into_person_id: null
---

# 王繼業

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王繼業 | accepted |
| bio.summary | 王繼業，明人物。隆慶二年進士，籍贯寧夏衛。（中国历代人物传记资料库 CBDB 334040） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_PyEHQmuoeFWGbQR7iXh3pQ | 王鑑 | accepted |
| other | p_Q5jiewEf7qr9FT4CBgkaYV | 王繼祖 | accepted |

## 外部来源

- [中国历代人物传记资料库：王繼業（CBDB 334040）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=334040&o=json)
