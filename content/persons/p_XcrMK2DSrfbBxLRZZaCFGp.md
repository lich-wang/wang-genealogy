---
schema: wang-person/v1
id: p_XcrMK2DSrfbBxLRZZaCFGp
status: active
merged_into: null
display_name: 王玉燕
cbdb_id: 70032
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_qRnqiEnjR42mFKBxciBkZG
        subject_person_id: p_XcrMK2DSrfbBxLRZZaCFGp
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王玉燕，史料所见人物。本项目依据《中国历代人物传记资料库：王玉燕（CBDB 70032）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_VcfccIsuIXXj1UFHkS3cWO
          claim_id: c_qRnqiEnjR42mFKBxciBkZG
          source_id: s_BupXzPmgC823DUQsPRB31g
          stance: supports
          locator: CBDB:70032
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_BupXzPmgC823DUQsPRB31g
            source_type: api_record
            title: 中国历代人物传记资料库：王玉燕（CBDB 70032）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=70032&o=json
            external_identifier: CBDB:70032
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:48.831Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_Zd7TLkv8gJ7k5rGgw9JKqW
        subject_person_id: p_XcrMK2DSrfbBxLRZZaCFGp
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王玉燕
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_PxF9YJUHfJwgJiepi3855U
          claim_id: c_Zd7TLkv8gJ7k5rGgw9JKqW
          source_id: s_BupXzPmgC823DUQsPRB31g
          stance: supports
          locator: CBDB:70032
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（1501-1600）｜历史性依据：CBDB 朝代 = 清
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_tX6Gi57YC5HhFTflzU_s60
        subject_person_id: p_QQ5XhNR32ojWXjM6ctDChw
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_XcrMK2DSrfbBxLRZZaCFGp
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Gck74-ITc9_AmV0Kj8SqbU
          claim_id: c_tX6Gi57YC5HhFTflzU_s60
          source_id: s_BupXzPmgC823DUQsPRB31g
          stance: supports
          locator: "明清婦女著作數據庫，MQWW PoetID #1352：祖父"
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_BupXzPmgC823DUQsPRB31g
            source_type: api_record
            title: 中国历代人物传记资料库：王玉燕（CBDB 70032）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=70032&o=json
            external_identifier: CBDB:70032
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:48.831Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_QQ5XhNR32ojWXjM6ctDChw
        status: active
        display_name: 王文治
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王玉燕

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王玉燕，史料所见人物。本项目依据《中国历代人物传记资料库：王玉燕（CBDB 70032）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 王玉燕 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| ancestors | p_QQ5XhNR32ojWXjM6ctDChw | 王文治 | accepted |

## 外部来源

- [中国历代人物传记资料库：王玉燕（CBDB 70032）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=70032&o=json)
