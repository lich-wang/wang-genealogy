---
schema: wang-person/v1
id: p_3PfS7skGHqAqRHAbm2V2U1
status: active
merged_into: null
display_name: 王潀
revision: 3
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_Fpk2iBN464B7HATgz5wDYn
        subject_person_id: p_3PfS7skGHqAqRHAbm2V2U1
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王潀
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_BQurNrMLk8WQ11nSpo4C8j
          claim_id: c_Fpk2iBN464B7HATgz5wDYn
          source_id: s_xWzMVakEh9Kot93QNTCQub
          stance: supports
          locator: CBDB:19901
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（19901）
          source: &a1
            id: s_xWzMVakEh9Kot93QNTCQub
            source_type: api_record
            title: 中国历代人物传记资料库：王潀（CBDB 19901）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=19901&o=json
            external_identifier: CBDB:19901
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.753Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_MUAgJQvruQq5qyca2mqDiQ
        subject_person_id: p_3PfS7skGHqAqRHAbm2V2U1
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王潀，吳越人物。籍贯奉化。（中国历代人物传记资料库 CBDB 19901）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_6xY2LTLybEWhhLF-iDOWv9
          claim_id: c_MUAgJQvruQq5qyca2mqDiQ
          source_id: s_xWzMVakEh9Kot93QNTCQub
          stance: supports
          locator: CBDB:19901
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_REQ7Dc0MF3Jd771hRDLiCn
        subject_person_id: p_3PfS7skGHqAqRHAbm2V2U1
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_2tQQbiMGFszWkvDF8QvDzV
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_8gyr0WhDz-XTioAdWspHrI
          claim_id: c_REQ7Dc0MF3Jd771hRDLiCn
          source_id: s_RrEjtT4UQdbCjSbh6msTjs
          stance: supports
          locator: CBDB 双向互证（父 王潀 ⇄ 子 王紳）
          quotation: null
          interpretation_note: Luna 在线复核：CBDB 底本未知，经 Wikidata 双向核对后采纳。
          source:
            id: s_RrEjtT4UQdbCjSbh6msTjs
            source_type: api_record
            title: 中国历代人物传记资料库：王紳（CBDB 19902）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=19902&o=json
            external_identifier: CBDB:19902
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.753Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_2tQQbiMGFszWkvDF8QvDzV
        status: active
        display_name: 王紳
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_1seRZ769MStvjNgIY9hZsd
        subject_person_id: p_3PfS7skGHqAqRHAbm2V2U1
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_4TVBjHmwXE3LUJhK31cFAM
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_lApejNdzbGLOB6RDZ0iubF
          claim_id: c_1seRZ769MStvjNgIY9hZsd
          source_id: s_-X6iER2IHK1a3ZRtM0YURm
          stance: supports
          locator: CBDB 亲属：兄（KinPerson 19901 王潀）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_-X6iER2IHK1a3ZRtM0YURm
            source_type: api_record
            title: 中国历代人物传记资料库：王儉（CBDB 19900）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=19900&o=json
            external_identifier: CBDB:19900
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_4TVBjHmwXE3LUJhK31cFAM
        status: active
        display_name: 王儉
        merged_into_person_id: null
---

# 王潀

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王潀 | accepted |
| bio.summary | 王潀，吳越人物。籍贯奉化。（中国历代人物传记资料库 CBDB 19901） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_2tQQbiMGFszWkvDF8QvDzV | 王紳 | accepted |
| other | p_4TVBjHmwXE3LUJhK31cFAM | 王儉 | accepted |

## 外部来源

- [中国历代人物传记资料库：王潀（CBDB 19901）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=19901&o=json)
- [中国历代人物传记资料库：王儉（CBDB 19900）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=19900&o=json)
- [中国历代人物传记资料库：王紳（CBDB 19902）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=19902&o=json)
