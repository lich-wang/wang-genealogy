---
schema: wang-person/v1
id: p_4uVxtmBQ2hs7QQmciEP2jw
status: active
merged_into: null
display_name: 王行淹
cbdb_id: 139326
revision: 3
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_CQGSxvVEK1EeoJz9DwGdjk
        subject_person_id: p_4uVxtmBQ2hs7QQmciEP2jw
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王行淹（625年—686年），史料所见人物。本项目依据《中国历代人物传记资料库：王行淹（CBDB 139326）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_NqyAH8zERunamlqS7bNZ9V
          claim_id: c_CQGSxvVEK1EeoJz9DwGdjk
          source_id: s_tT7unHFKFRd8PdHV8LWLSx
          stance: supports
          locator: CBDB:139326
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_tT7unHFKFRd8PdHV8LWLSx
            source_type: api_record
            title: 中国历代人物传记资料库：王行淹（CBDB 139326）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=139326&o=json
            external_identifier: CBDB:139326
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_QEvKvjJKd58x2R3bMgC9yn
        subject_person_id: p_4uVxtmBQ2hs7QQmciEP2jw
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 625年
            calendar_note: CBDB 生年字段，精度：年
            earliest: 0625-01-01
            latest: 0625-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_22pHnNrruzSZyJQRnc6SMY
          claim_id: c_QEvKvjJKd58x2R3bMgC9yn
          source_id: s_tT7unHFKFRd8PdHV8LWLSx
          stance: supports
          locator: CBDB:139326
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 686
          source:
            id: s_tT7unHFKFRd8PdHV8LWLSx
            source_type: api_record
            title: 中国历代人物传记资料库：王行淹（CBDB 139326）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=139326&o=json
            external_identifier: CBDB:139326
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_hbK1PEDv28x44UEL9FP1vu
        subject_person_id: p_4uVxtmBQ2hs7QQmciEP2jw
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 686年
            calendar_note: CBDB 卒年字段，精度：年
            earliest: 0686-01-01
            latest: 0686-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_igEz3jSbM58nMeWxhmAn8c
          claim_id: c_hbK1PEDv28x44UEL9FP1vu
          source_id: s_tT7unHFKFRd8PdHV8LWLSx
          stance: supports
          locator: CBDB:139326
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 686
          source:
            id: s_tT7unHFKFRd8PdHV8LWLSx
            source_type: api_record
            title: 中国历代人物传记资料库：王行淹（CBDB 139326）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=139326&o=json
            external_identifier: CBDB:139326
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_8FKN5NczbN3DS7o9gr732U
        subject_person_id: p_4uVxtmBQ2hs7QQmciEP2jw
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王行淹
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_haFtytwHRgbxLw5dyzwxEb
          claim_id: c_8FKN5NczbN3DS7o9gr732U
          source_id: s_tT7unHFKFRd8PdHV8LWLSx
          stance: supports
          locator: CBDB:139326
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 686
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_ucI6-q8ySgcXDpsqNXM4Cl
        subject_person_id: p_gDCt2tXVPQF2GNSNKe6eEJ
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_4uVxtmBQ2hs7QQmciEP2jw
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_YRNIR0AUToRAJqD8HvqbKB
          claim_id: c_ucI6-q8ySgcXDpsqNXM4Cl
          source_id: s_D2ATp6nmPFy57uP4N5xMW3
          stance: supports
          locator: 唐代墓誌匯編:二卷，Chuigong 21：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_D2ATp6nmPFy57uP4N5xMW3
            source_type: api_record
            title: 中国历代人物传记资料库：王琰（CBDB 149248）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=149248&o=json
            external_identifier: CBDB:149248
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.781Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_gDCt2tXVPQF2GNSNKe6eEJ
        status: active
        display_name: 王琰
        merged_into_person_id: null
  children:
    - claim:
        id: c_VrcATc48e0DLAdWuuP-KYL
        subject_person_id: p_4uVxtmBQ2hs7QQmciEP2jw
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_wnT45ErE7EjdM2LZBqc24i
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_ikn-KG-xgEQ0W-D8uPV9F4
          claim_id: c_VrcATc48e0DLAdWuuP-KYL
          source_id: s_VAPooGgdUSEbBfz6QHfYeD
          stance: supports
          locator: 唐代墓誌匯編:二卷，Chuigong 21：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_VAPooGgdUSEbBfz6QHfYeD
            source_type: api_record
            title: 中国历代人物传记资料库：王懷素（CBDB 149249）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=149249&o=json
            external_identifier: CBDB:149249
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:53.368Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_wnT45ErE7EjdM2LZBqc24i
        status: active
        display_name: 王懷素
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王行淹

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王行淹（625年—686年），史料所见人物。本项目依据《中国历代人物传记资料库：王行淹（CBDB 139326）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| birth.date | 625年 | accepted |
| death.date | 686年 | accepted |
| name.primary | 王行淹 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_gDCt2tXVPQF2GNSNKe6eEJ | 王琰 | accepted |
| children | p_wnT45ErE7EjdM2LZBqc24i | 王懷素 | accepted |

## 外部来源

- [中国历代人物传记资料库：王懷素（CBDB 149249）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=149249&o=json)
- [中国历代人物传记资料库：王行淹（CBDB 139326）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=139326&o=json)
- [中国历代人物传记资料库：王琰（CBDB 149248）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=149248&o=json)
