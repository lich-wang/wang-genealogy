---
schema: wang-person/v1
id: p_6x2bqhUrScPeQEfMXQdYP2
status: active
merged_into: null
display_name: 王況
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_tKCQFArHkoTRU6yvV38A2g
        subject_person_id: p_6x2bqhUrScPeQEfMXQdYP2
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王況
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_A3vqAQjGxMap6HaSZFyUxE
          claim_id: c_tKCQFArHkoTRU6yvV38A2g
          source_id: s_DHVfmGS6GaLxNDM679KKEF
          stance: supports
          locator: CBDB:697042
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（697042）
          source: &a1
            id: s_DHVfmGS6GaLxNDM679KKEF
            source_type: api_record
            title: 中国历代人物传记资料库：王況（CBDB 697042）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=697042&o=json
            external_identifier: CBDB:697042
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:27.714Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_UCuH8dutqAnLFiMzEKsRmm
        subject_person_id: p_6x2bqhUrScPeQEfMXQdYP2
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王況，史料所见人物。本项目依据《中国历代人物传记资料库：王況（CBDB 697042）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_aF9cYqoYNzM5sVDihVT0_M
          claim_id: c_UCuH8dutqAnLFiMzEKsRmm
          source_id: s_DHVfmGS6GaLxNDM679KKEF
          stance: supports
          locator: CBDB:697042
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_8c4yehLPJbHs0IPmfR8G8o
        subject_person_id: p_LKzDYAPDmX1Dcm3kwWWk7w
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_6x2bqhUrScPeQEfMXQdYP2
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_3YmDaSq-pvrBf5AAp8Zyc8
          claim_id: c_8c4yehLPJbHs0IPmfR8G8o
          source_id: s_DHVfmGS6GaLxNDM679KKEF
          stance: supports
          locator: 新出宋代墓志碑刻輯錄，159 王九言墓志：祖父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_LKzDYAPDmX1Dcm3kwWWk7w
        status: active
        display_name: 王九言
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王況

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王況 | accepted |
| bio.summary | 王況，史料所见人物。本项目依据《中国历代人物传记资料库：王況（CBDB 697042）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| ancestors | p_LKzDYAPDmX1Dcm3kwWWk7w | 王九言 | accepted |

## 外部来源

- [中国历代人物传记资料库：王況（CBDB 697042）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=697042&o=json)
