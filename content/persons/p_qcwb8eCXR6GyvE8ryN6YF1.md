---
schema: wang-person/v1
id: p_qcwb8eCXR6GyvE8ryN6YF1
status: active
merged_into: null
display_name: 王覿
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_1CeBaAwhdCfUyiQV1uXrx1
        subject_person_id: p_qcwb8eCXR6GyvE8ryN6YF1
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王覿
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_T2cMYLcFcWe1MKhBGckXQe
          claim_id: c_1CeBaAwhdCfUyiQV1uXrx1
          source_id: s_SDsZgLqBM9JtYL99b9djLF
          stance: supports
          locator: CBDB:1894
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（1894）
          source: &a1
            id: s_SDsZgLqBM9JtYL99b9djLF
            source_type: api_record
            title: 中国历代人物传记资料库：王覿（CBDB 1894）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=1894&o=json
            external_identifier: CBDB:1894
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.378Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_SyPCJ5Ae11mn999CYu8Ep3
        subject_person_id: p_qcwb8eCXR6GyvE8ryN6YF1
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王覿，宋人物。籍贯如皋，身份为良吏;循吏、入元祐黨籍者，入仕進士，曾任發運使、尚書省戶部侍郎、三公。（中国历代人物传记资料库 CBDB 1894）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_KBPJWwj2tccbixS28Rp9i7
          claim_id: c_SyPCJ5Ae11mn999CYu8Ep3
          source_id: s_SDsZgLqBM9JtYL99b9djLF
          stance: supports
          locator: CBDB:1894
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_JQ8n53VfFB8eraNIyRhOTo
        subject_person_id: p_JEp5M3BLJajMXRjvLCUuVy
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_qcwb8eCXR6GyvE8ryN6YF1
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Y7yDLw_Yrp5voKr6unD7ym
          claim_id: c_JQ8n53VfFB8eraNIyRhOTo
          source_id: s_SDsZgLqBM9JtYL99b9djLF
          stance: supports
          locator: CBDB 双向互证（父 王惟熙 ⇄ 子 王覿）
          quotation: null
          interpretation_note: Luna 在线复核：CBDB 底本未知，经 Wikidata 双向核对后采纳。
          source: *a1
      object_person:
        id: p_JEp5M3BLJajMXRjvLCUuVy
        status: active
        display_name: 王惟熙
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王覿

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王覿 | accepted |
| bio.summary | 王覿，宋人物。籍贯如皋，身份为良吏;循吏、入元祐黨籍者，入仕進士，曾任發運使、尚書省戶部侍郎、三公。（中国历代人物传记资料库 CBDB 1894） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_JEp5M3BLJajMXRjvLCUuVy | 王惟熙 | accepted |

## 外部来源

- [中国历代人物传记资料库：王覿（CBDB 1894）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=1894&o=json)
