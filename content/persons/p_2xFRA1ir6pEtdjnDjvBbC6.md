---
schema: wang-person/v1
id: p_2xFRA1ir6pEtdjnDjvBbC6
status: active
merged_into: null
display_name: 王小僧
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_Qg1F1aCCLy36GFNPoSBp5f
        subject_person_id: p_2xFRA1ir6pEtdjnDjvBbC6
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王小僧
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_si8vZP5a1kC6wsCYUnLHP8
          claim_id: c_Qg1F1aCCLy36GFNPoSBp5f
          source_id: s_43XPoWwhmSJ8x1EwKdw7WJ
          stance: supports
          locator: CBDB:191747
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（191747）
          source: &a1
            id: s_43XPoWwhmSJ8x1EwKdw7WJ
            source_type: api_record
            title: 中国历代人物传记资料库：王小僧（CBDB 191747）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=191747&o=json
            external_identifier: CBDB:191747
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:04.284Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_1LDxdmqVqHUekt45n8LQD2
        subject_person_id: p_2xFRA1ir6pEtdjnDjvBbC6
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 920年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_zgHxLGjzKn5JDicMgwQrap
          claim_id: c_1LDxdmqVqHUekt45n8LQD2
          source_id: s_43XPoWwhmSJ8x1EwKdw7WJ
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_XBGY6SBZrH9EJv54KePEXX
        subject_person_id: p_2xFRA1ir6pEtdjnDjvBbC6
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王小僧（卒于920年），唐人物。籍贯上黨。（中国历代人物传记资料库 CBDB 191747）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_VbpqUASgFOEFLkwWI2wFh5
          claim_id: c_XBGY6SBZrH9EJv54KePEXX
          source_id: s_43XPoWwhmSJ8x1EwKdw7WJ
          stance: supports
          locator: CBDB:191747
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_1_zJ7jMSH6UV1yM0AhevtD
        subject_person_id: p_4mfqFZAqaNrXQSde1B1tdh
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_2xFRA1ir6pEtdjnDjvBbC6
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_oXgA2jRXiSuaTdZT4HXeWj
          claim_id: c_1_zJ7jMSH6UV1yM0AhevtD
          source_id: s_43XPoWwhmSJ8x1EwKdw7WJ
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_4mfqFZAqaNrXQSde1B1tdh
        status: active
        display_name: 王公政
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王小僧

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王小僧 | accepted |
| death.date | 920年 | accepted |
| bio.summary | 王小僧（卒于920年），唐人物。籍贯上黨。（中国历代人物传记资料库 CBDB 191747） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_4mfqFZAqaNrXQSde1B1tdh | 王公政 | accepted |

## 外部来源

- [中国历代人物传记资料库：王小僧（CBDB 191747）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=191747&o=json)
