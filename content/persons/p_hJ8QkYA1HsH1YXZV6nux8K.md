---
schema: wang-person/v1
id: p_hJ8QkYA1HsH1YXZV6nux8K
status: active
merged_into: null
display_name: 汪穀
revision: 1
cbdb_id: 79238
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_cP-RM-AanKHUHeL7T1dclm
        subject_person_id: p_hJ8QkYA1HsH1YXZV6nux8K
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 汪穀（1754—1821），清人物。籍贯休寧。（中国历代人物传记资料库 CBDB 79238）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_qr5fZv_6opJK4iH-5Z_zbZ
          claim_id: c_cP-RM-AanKHUHeL7T1dclm
          source_id: s_-YlcUcMpfNsDOljs2uvC75
          stance: supports
          locator: CBDB:79238
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_-YlcUcMpfNsDOljs2uvC75
            source_type: api_record
            title: 中国历代人物传记资料库：汪穀（CBDB 79238）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=79238&o=json
            external_identifier: CBDB:79238
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_Z5QUjkZdIp3OqI18DFSEPq
        subject_person_id: p_hJ8QkYA1HsH1YXZV6nux8K
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 汪穀
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_eNCJf-o3h-FlwWcjsfukzU
          claim_id: c_Z5QUjkZdIp3OqI18DFSEPq
          source_id: s_-YlcUcMpfNsDOljs2uvC75
          stance: supports
          locator: CBDB:79238
          quotation: null
          interpretation_note: CBDB 明确记录的王碧珠配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_RGq9A1jTixTpFbhw90Geav
        subject_person_id: p_JstbVeQ72pNAP4UCgVSeHP
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_hJ8QkYA1HsH1YXZV6nux8K
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_amOlKtHVMV1c82TeAQJCSP
          claim_id: c_RGq9A1jTixTpFbhw90Geav
          source_id: s_-YlcUcMpfNsDOljs2uvC75
          stance: supports
          locator: "明清婦女著作數據庫，MQWW PoetID #1895：丈夫"
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_JstbVeQ72pNAP4UCgVSeHP
        status: active
        display_name: 王碧珠
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 汪穀

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 汪穀（1754—1821），清人物。籍贯休寧。（中国历代人物传记资料库 CBDB 79238） | accepted |
| name.primary | 汪穀 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_JstbVeQ72pNAP4UCgVSeHP | 王碧珠 | accepted |

## 外部来源

- [中国历代人物传记资料库：汪穀（CBDB 79238）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=79238&o=json)
