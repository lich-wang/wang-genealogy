---
schema: wang-person/v1
id: p_hB8z42Z7wm2scy2oshAB2E
status: active
merged_into: null
display_name: 王之枚
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_pQEG1kAvd6eJPoBxjB7fMZ
        subject_person_id: p_hB8z42Z7wm2scy2oshAB2E
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王之枚
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_xPgN6GovCJbK2giuHDcsap
          claim_id: c_pQEG1kAvd6eJPoBxjB7fMZ
          source_id: s_Pnhxmm7fb7RuEkN3HPyK65
          stance: supports
          locator: CBDB:694297
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（694297）
          source: &a1
            id: s_Pnhxmm7fb7RuEkN3HPyK65
            source_type: api_record
            title: 中国历代人物传记资料库：王之枚（CBDB 694297）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=694297&o=json
            external_identifier: CBDB:694297
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:06.520Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_WYgJRQ8E2JyfSrPF4CeVnp
        subject_person_id: p_hB8z42Z7wm2scy2oshAB2E
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: "王之枚，清人物。籍贯海鹽，入仕貢生: 納貢(例貢,增貢,捐貢)，曾任同知。（中国历代人物传记资料库 CBDB 694297）"
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_uwcTa2hY0wkej-emFKv-vU
          claim_id: c_WYgJRQ8E2JyfSrPF4CeVnp
          source_id: s_Pnhxmm7fb7RuEkN3HPyK65
          stance: supports
          locator: CBDB:694297
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_1k1q6sA1XOcK4rPZ5wbCME
        subject_person_id: p_qzRDrqjW6BFLYhMP3xVCXP
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_hB8z42Z7wm2scy2oshAB2E
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_KktJS5s7uvlfq4sx2xSEmc
          claim_id: c_1k1q6sA1XOcK4rPZ5wbCME
          source_id: s_Pnhxmm7fb7RuEkN3HPyK65
          stance: supports
          locator: "海鹽縣志: 二十二卷(光緒)，lgid=294038：祖父"
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_qzRDrqjW6BFLYhMP3xVCXP
        status: active
        display_name: 王尙志
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王之枚

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王之枚 | accepted |
| bio.summary | 王之枚，清人物。籍贯海鹽，入仕貢生: 納貢(例貢,增貢,捐貢)，曾任同知。（中国历代人物传记资料库 CBDB 694297） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| ancestors | p_qzRDrqjW6BFLYhMP3xVCXP | 王尙志 | accepted |

## 外部来源

- [中国历代人物传记资料库：王之枚（CBDB 694297）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=694297&o=json)
