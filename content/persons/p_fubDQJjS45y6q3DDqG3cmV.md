---
schema: wang-person/v1
id: p_fubDQJjS45y6q3DDqG3cmV
status: active
merged_into: null
display_name: 王谟
cbdb_id: 313121
revision: 3
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_zcmPpQnWL1hzAguaZ7SHas
        subject_person_id: p_fubDQJjS45y6q3DDqG3cmV
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王谟，明人物。CBDB 记录其曾任吏部文選清吏司郎中、奉直大夫。中国历代人物传记资料库（CBDB）以人物编号 313121 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_ukJH3ynZN3HF9zxw7aKGGa
          claim_id: c_zcmPpQnWL1hzAguaZ7SHas
          source_id: s_bMvr9FFr13GEoAwQLCdtrk
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_bMvr9FFr13GEoAwQLCdtrk
            source_type: api_record
            title: 维基数据：王谟（Q45613879）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45613879
            external_identifier: Q45613879
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:34.983Z
            metadata_json: null
        - id: cs_jwDxU5MaIbUUMFX90yMtqo
          claim_id: c_zcmPpQnWL1hzAguaZ7SHas
          source_id: s_k2BKXhddiWDeQJng65taJ9
          stance: supports
          locator: CBDB:313121
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_k2BKXhddiWDeQJng65taJ9
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王謨（313121）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=313121&o=json
            external_identifier: CBDB:313121
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-05T01:59:35.196Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_KYDiLzzQCNN6Htper4n5R4
        subject_person_id: p_fubDQJjS45y6q3DDqG3cmV
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王谟
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_AHzgdHUARrCN5LVWMCbvCF
          claim_id: c_KYDiLzzQCNN6Htper4n5R4
          source_id: s_bMvr9FFr13GEoAwQLCdtrk
          stance: supports
          locator: Q45613879
          quotation: null
          interpretation_note: null
          source:
            id: s_bMvr9FFr13GEoAwQLCdtrk
            source_type: api_record
            title: 维基数据：王谟（Q45613879）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45613879
            external_identifier: Q45613879
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:34.983Z
            metadata_json: null
        - id: cs_CQJXB6m55TLjQxJJPi1kUN
          claim_id: c_KYDiLzzQCNN6Htper4n5R4
          source_id: s_k2BKXhddiWDeQJng65taJ9
          stance: supports
          locator: Q45613879
          quotation: null
          interpretation_note: null
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_tpv3sBQFJ2GxL44ky4M12Q
        subject_person_id: p_YdbFSMV9s3AQb86u7EkdDN
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_fubDQJjS45y6q3DDqG3cmV
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_GzAFh2nwn2JdaE5fSPY4sx
          claim_id: c_tpv3sBQFJ2GxL44ky4M12Q
          source_id: s_bMvr9FFr13GEoAwQLCdtrk
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_bMvr9FFr13GEoAwQLCdtrk
            source_type: api_record
            title: 维基数据：王谟（Q45613879）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45613879
            external_identifier: Q45613879
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:34.983Z
            metadata_json: null
        - id: cs_JoBtNJaaeCFfUNJcnjLyy9
          claim_id: c_tpv3sBQFJ2GxL44ky4M12Q
          source_id: s_M6Jx2ULhfF3afm41LXVGft
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_M6Jx2ULhfF3afm41LXVGft
            source_type: api_record
            title: 维基数据：王臣（Q45613816）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45613816
            external_identifier: Q45613816
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:07:00.338Z
            metadata_json: null
      object_person:
        id: p_YdbFSMV9s3AQb86u7EkdDN
        status: active
        display_name: 王臣
        merged_into_person_id: null
  children:
    - claim:
        id: c_FVGRX6T94g7TZFUvdiQz6j
        subject_person_id: p_fubDQJjS45y6q3DDqG3cmV
        claim_kind: relationship
        predicate: kinship.parent_of
        object_person_id: p_Uu3urkECTiPumEtH6sBydY
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_CE2SrWrnKXZqwNHG4QJeUQ
          claim_id: c_FVGRX6T94g7TZFUvdiQz6j
          source_id: s_EmS3UhLTK9Hc6EoC789D6G
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_EmS3UhLTK9Hc6EoC789D6G
            source_type: api_record
            title: 维基数据：王邦瑞（Q15909508）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15909508
            external_identifier: Q15909508
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:46:37.936Z
            metadata_json: null
        - id: cs_SeY5qDB23dtMYok5QcM4Vr
          claim_id: c_FVGRX6T94g7TZFUvdiQz6j
          source_id: s_bMvr9FFr13GEoAwQLCdtrk
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_bMvr9FFr13GEoAwQLCdtrk
            source_type: api_record
            title: 维基数据：王谟（Q45613879）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45613879
            external_identifier: Q45613879
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:34.983Z
            metadata_json: null
      object_person:
        id: p_Uu3urkECTiPumEtH6sBydY
        status: active
        display_name: 王邦瑞
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_BykQ0eCszU8lx8RqVOYAKM
        subject_person_id: p_fubDQJjS45y6q3DDqG3cmV
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_KbE3VYwBQk3rLuTGnVB2me
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_zpuvwi4RHSkNWpiUsTWb4T
          claim_id: c_BykQ0eCszU8lx8RqVOYAKM
          source_id: s_k2BKXhddiWDeQJng65taJ9
          stance: supports
          locator: 嘉靖二十九年進士登科錄:一卷，第三甲第四十四名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_KbE3VYwBQk3rLuTGnVB2me
        status: active
        display_name: 王正国
        merged_into_person_id: null
  other: []
---

# 王谟

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王谟，明人物。CBDB 记录其曾任吏部文選清吏司郎中、奉直大夫。中国历代人物传记资料库（CBDB）以人物编号 313121 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。 | accepted |
| name.primary | 王谟 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_YdbFSMV9s3AQb86u7EkdDN | 王臣 | accepted |
| children | p_Uu3urkECTiPumEtH6sBydY | 王邦瑞 | accepted |
| descendants | p_KbE3VYwBQk3rLuTGnVB2me | 王正国 | accepted |

## 外部来源

- [维基数据：王邦瑞（Q15909508）](https://www.wikidata.org/wiki/Q15909508)
- [维基数据：王臣（Q45613816）](https://www.wikidata.org/wiki/Q45613816)
- [维基数据：王谟（Q45613879）](https://www.wikidata.org/wiki/Q45613879)
- [CBDB 中国历代人物传记资料库：王謨（313121）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=313121&o=json)
