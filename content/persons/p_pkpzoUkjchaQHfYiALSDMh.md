---
schema: wang-person/v1
id: p_pkpzoUkjchaQHfYiALSDMh
status: active
merged_into: null
display_name: 雒氏
revision: 1
cbdb_id: 222278
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_F3WZ57F1q4ZK4XG8QNSGWK
        subject_person_id: p_pkpzoUkjchaQHfYiALSDMh
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 雒氏，明人物。萬曆十一年進士。（中国历代人物传记资料库 CBDB 222278）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_GdpxHLZD-6Xb8xY2rXb1aE
          claim_id: c_F3WZ57F1q4ZK4XG8QNSGWK
          source_id: s_tkdotZfBkD62uhEpJ1eUE6
          stance: supports
          locator: CBDB:222278
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_tkdotZfBkD62uhEpJ1eUE6
            source_type: api_record
            title: 中国历代人物传记资料库：雒氏(王紹先妻)（CBDB 222278）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=222278&o=json
            external_identifier: CBDB:222278
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_4lZ1vPlcREH1veWVmZO0b9
        subject_person_id: p_pkpzoUkjchaQHfYiALSDMh
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 雒氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_yChW2yxu5Zv5h13fAy2aPe
          claim_id: c_4lZ1vPlcREH1veWVmZO0b9
          source_id: s_tkdotZfBkD62uhEpJ1eUE6
          stance: supports
          locator: CBDB:222278
          quotation: null
          interpretation_note: CBDB 明确记录的王紹先配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_zCjkz9NhzQmMeLdUCyUQ5Z
        subject_person_id: p_stumq3LKUX83aY7FDSYoQ9
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_pkpzoUkjchaQHfYiALSDMh
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_4RoEq_KkvhDAJS-0jgiWzy
          claim_id: c_zCjkz9NhzQmMeLdUCyUQ5Z
          source_id: s_tkdotZfBkD62uhEpJ1eUE6
          stance: supports
          locator: 萬曆十一年進士登科錄:一卷，第二甲第十三名：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_stumq3LKUX83aY7FDSYoQ9
        status: active
        display_name: 王紹先
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 雒氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 雒氏，明人物。萬曆十一年進士。（中国历代人物传记资料库 CBDB 222278） | accepted |
| name.primary | 雒氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_stumq3LKUX83aY7FDSYoQ9 | 王紹先 | accepted |

## 外部来源

- [中国历代人物传记资料库：雒氏(王紹先妻)（CBDB 222278）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=222278&o=json)
