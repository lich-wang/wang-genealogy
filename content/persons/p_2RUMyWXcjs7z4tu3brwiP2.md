---
schema: wang-person/v1
id: p_2RUMyWXcjs7z4tu3brwiP2
status: active
merged_into: null
display_name: 王琳
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_TcCM6TwrQto35yrbCrZG2w
        subject_person_id: p_2RUMyWXcjs7z4tu3brwiP2
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王琳
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_kos9AZSWJSQx8pSvh5wHP9
          claim_id: c_TcCM6TwrQto35yrbCrZG2w
          source_id: s_1vK55fGdquAnGgvgiRUmGW
          stance: supports
          locator: CBDB:25610
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（25610）
          source: &a1
            id: s_1vK55fGdquAnGgvgiRUmGW
            source_type: api_record
            title: 中国历代人物传记资料库：王琳（CBDB 25610）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25610&o=json
            external_identifier: CBDB:25610
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.915Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_Zo5vthWnqhjF3WZzUS94kQ
        subject_person_id: p_2RUMyWXcjs7z4tu3brwiP2
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王琳，宋人物。籍贯廬陵，入仕進士。（中国历代人物传记资料库 CBDB 25610）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_spX48rM2jous-jGWtoTpZ9
          claim_id: c_Zo5vthWnqhjF3WZzUS94kQ
          source_id: s_1vK55fGdquAnGgvgiRUmGW
          stance: supports
          locator: CBDB:25610
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_ozVQP2x4nHeR61kfTDxk83
        subject_person_id: p_pkGx6M29iSBcCWS4PfHNkN
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_2RUMyWXcjs7z4tu3brwiP2
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_e7aiforoT7j4D8E__eQ8t4
          claim_id: c_ozVQP2x4nHeR61kfTDxk83
          source_id: s_eK1FsbK_JNUVYNZ3VFIsgF
          stance: supports
          locator: CBDB 亲属：父（KinPerson 25609）
          quotation: null
          interpretation_note: CBDB 直接记录的父／母关系。
          source:
            id: s_eK1FsbK_JNUVYNZ3VFIsgF
            source_type: api_record
            title: 中国历代人物传记资料库：王琳（CBDB 25610）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25610&o=json
            external_identifier: CBDB:25610
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_pkGx6M29iSBcCWS4PfHNkN
        status: active
        display_name: 王英臣
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王琳

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王琳 | accepted |
| bio.summary | 王琳，宋人物。籍贯廬陵，入仕進士。（中国历代人物传记资料库 CBDB 25610） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_pkGx6M29iSBcCWS4PfHNkN | 王英臣 | accepted |

## 外部来源

- [中国历代人物传记资料库：王琳（CBDB 25610）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25610&o=json)
