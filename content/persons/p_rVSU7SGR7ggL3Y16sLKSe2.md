---
schema: wang-person/v1
id: p_rVSU7SGR7ggL3Y16sLKSe2
status: active
merged_into: null
display_name: 王褒
revision: 1
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_agXNLUrCsWsqNBajW3nhv1
        subject_person_id: p_rVSU7SGR7ggL3Y16sLKSe2
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王褒（513年—576年），南北朝人物。CBDB 记录其籍贯记录为京兆府，曾任司空。中国历代人物传记资料库（CBDB）以人物编号 157867 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_7x31pA8njDYEyB7L6r9qr8
          claim_id: c_agXNLUrCsWsqNBajW3nhv1
          source_id: s_uYCbjkL9HT7KCAR6K4SZ9c
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_uYCbjkL9HT7KCAR6K4SZ9c
            source_type: api_record
            title: 维基数据：王褒（Q10416850）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q10416850
            external_identifier: Q10416850
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:39:53.590Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%A4%92_(%E5%8D%97%E5%8C%97%E6%9C%9D)
        - id: cs_A42R7PrzW9hid-VbIaGcv8
          claim_id: c_agXNLUrCsWsqNBajW3nhv1
          source_id: s_hiHG6cvQPHs7S7JeSAtB6D
          stance: supports
          locator: CBDB:157867
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_hiHG6cvQPHs7S7JeSAtB6D
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王裒（157867）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=157867&o=json
            external_identifier: CBDB:157867
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T03:39:53.727Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_94iiDwAuGxA7Q8xFjFwfuX
        subject_person_id: p_rVSU7SGR7ggL3Y16sLKSe2
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 513年
            calendar_note: 维基数据 P569 结构化日期，精度：年
            earliest: 0513-01-01
            latest: 0513-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_3MRdFGn74LKRaM3egiFVt4
          claim_id: c_94iiDwAuGxA7Q8xFjFwfuX
          source_id: s_uYCbjkL9HT7KCAR6K4SZ9c
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_uYCbjkL9HT7KCAR6K4SZ9c
            source_type: api_record
            title: 维基数据：王褒（Q10416850）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q10416850
            external_identifier: Q10416850
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:39:53.590Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%A4%92_(%E5%8D%97%E5%8C%97%E6%9C%9D)
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_JJ6hontnedL8j6bevCSPHP
        subject_person_id: p_rVSU7SGR7ggL3Y16sLKSe2
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 576年
            calendar_note: 维基数据 P570 结构化日期，精度：年
            earliest: 0576-01-01
            latest: 0576-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_GZryvk6FCLB2zUznEYYXbL
          claim_id: c_JJ6hontnedL8j6bevCSPHP
          source_id: s_uYCbjkL9HT7KCAR6K4SZ9c
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_uYCbjkL9HT7KCAR6K4SZ9c
            source_type: api_record
            title: 维基数据：王褒（Q10416850）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q10416850
            external_identifier: Q10416850
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:39:53.590Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%A4%92_(%E5%8D%97%E5%8C%97%E6%9C%9D)
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_Q4ana666yE6NyMikDsttTY
        subject_person_id: p_rVSU7SGR7ggL3Y16sLKSe2
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王褒
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_6eubgBAmcyFRKRjcjnMe36
          claim_id: c_Q4ana666yE6NyMikDsttTY
          source_id: s_hiHG6cvQPHs7S7JeSAtB6D
          stance: supports
          locator: Q10416850
          quotation: null
          interpretation_note: null
          source: *a1
        - id: cs_dcLXDeQAs5BAEPEeZmLDMr
          claim_id: c_Q4ana666yE6NyMikDsttTY
          source_id: s_uYCbjkL9HT7KCAR6K4SZ9c
          stance: supports
          locator: Q10416850
          quotation: null
          interpretation_note: null
          source:
            id: s_uYCbjkL9HT7KCAR6K4SZ9c
            source_type: api_record
            title: 维基数据：王褒（Q10416850）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q10416850
            external_identifier: Q10416850
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:39:53.590Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%A4%92_(%E5%8D%97%E5%8C%97%E6%9C%9D)
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_NeB1bE3iNaSS27st52NPQA
        subject_person_id: p_p6NDwA85ehBi6FPpFUWBiZ
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_rVSU7SGR7ggL3Y16sLKSe2
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_B3mM6fSzNNkd4m3f1ufBR7
          claim_id: c_NeB1bE3iNaSS27st52NPQA
          source_id: s_2PBtrEh1zDAZWj4V8jNPMP
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_2PBtrEh1zDAZWj4V8jNPMP
            source_type: api_record
            title: 维基数据：王规（Q10416918）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q10416918
            external_identifier: Q10416918
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:31:30.337Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%A6%8F_(%E5%8D%97%E6%9C%9D)
        - id: cs_ndGUcNFocLSAoC6s6AhC58
          claim_id: c_NeB1bE3iNaSS27st52NPQA
          source_id: s_uYCbjkL9HT7KCAR6K4SZ9c
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_uYCbjkL9HT7KCAR6K4SZ9c
            source_type: api_record
            title: 维基数据：王褒（Q10416850）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q10416850
            external_identifier: Q10416850
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:39:53.590Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%A4%92_(%E5%8D%97%E5%8C%97%E6%9C%9D)
        - id: cs_spp7q43XCQg6PK8WuWqfgY
          claim_id: c_NeB1bE3iNaSS27st52NPQA
          source_id: s_MGBoig692jprej3f8uiDi2
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王规 → 王褒
          interpretation_note: null
          source:
            id: s_MGBoig692jprej3f8uiDi2
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:58:59.171Z
            metadata_json: null
        - id: cs_77Bp6k3wnp4vzQHHExEAkP
          claim_id: c_NeB1bE3iNaSS27st52NPQA
          source_id: s_xEBYVW3xjFzLnp9P7pHZ2L
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王规 → 王褒
          interpretation_note: null
          source:
            id: s_xEBYVW3xjFzLnp9P7pHZ2L
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:02:22.297Z
            metadata_json: null
        - id: cs_SkEEL4rFWFgb6389oCEdGC
          claim_id: c_NeB1bE3iNaSS27st52NPQA
          source_id: s_XmK4uXKaoBAKcwvzzk8Dzr
          stance: supports
          locator: 条文：条文识读（生）
          quotation: 生褒，褒字子淵，後周光祿大夫、石泉康侯。
          interpretation_note: null
          source:
            id: s_XmK4uXKaoBAKcwvzzk8Dzr
            source_type: book
            title: 中文维基文库：新唐書/卷072中
            creator: null
            publisher: 维基文库
            published_at_text: null
            canonical_url: https://zh.wikisource.org/wiki/%E6%96%B0%E5%94%90%E6%9B%B8%2F%E5%8D%B7072%E4%B8%AD
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T10:12:55.026Z
            metadata_json: null
      object_person:
        id: p_p6NDwA85ehBi6FPpFUWBiZ
        status: active
        display_name: 王规
        merged_into_person_id: null
  children:
    - claim:
        id: c_yBLH2a9XKQayy3iW24MKgM
        subject_person_id: p_rVSU7SGR7ggL3Y16sLKSe2
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_TDm8AW3FGV4iqKti7zSPt9
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_8kMFg3uJE8bYz5Xectb7ww
          claim_id: c_yBLH2a9XKQayy3iW24MKgM
          source_id: s_hiHG6cvQPHs7S7JeSAtB6D
          stance: supports
          locator: 亲属关系：子
          quotation: null
          interpretation_note: CBDB 注明此条来源：唐五代人物傳記與社會網絡資料庫(1.0版)
          source:
            id: s_hiHG6cvQPHs7S7JeSAtB6D
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王裒（157867）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=157867&o=json
            external_identifier: CBDB:157867
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T03:39:53.727Z
            metadata_json: null
        - id: cs_T9Vu1LcF5LiaKiUC41BQYG
          claim_id: c_yBLH2a9XKQayy3iW24MKgM
          source_id: s_uYCbjkL9HT7KCAR6K4SZ9c
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_uYCbjkL9HT7KCAR6K4SZ9c
            source_type: api_record
            title: 维基数据：王褒（Q10416850）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q10416850
            external_identifier: Q10416850
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:39:53.590Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%A4%92_(%E5%8D%97%E5%8C%97%E6%9C%9D)
        - id: cs_2nxu4367rNq1v4Eewu2vAV
          claim_id: c_yBLH2a9XKQayy3iW24MKgM
          source_id: s_ygACJZVSZz5JfhGZSbgin6
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_ygACJZVSZz5JfhGZSbgin6
            source_type: api_record
            title: 维基数据：王某（Q45489166）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45489166
            external_identifier: Q45489166
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:59:56.263Z
            metadata_json: null
        - id: cs_rtYSycVLSXN5xEJ4qbbBND
          claim_id: c_yBLH2a9XKQayy3iW24MKgM
          source_id: s_S6a4xDfApii643Q4gS3eph
          stance: supports
          locator: 亲属关系：父
          quotation: null
          interpretation_note: CBDB 注明此条来源：唐五代人物傳記與社會網絡資料庫(1.0版)
          source:
            id: s_S6a4xDfApii643Q4gS3eph
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王某（193614）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=193614&o=json
            external_identifier: CBDB:193614
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T03:59:56.587Z
            metadata_json: null
      object_person:
        id: p_TDm8AW3FGV4iqKti7zSPt9
        status: active
        display_name: 王某
        merged_into_person_id: null
    - claim:
        id: c_TCp87FxBxYEt6rqkVozKHj
        subject_person_id: p_rVSU7SGR7ggL3Y16sLKSe2
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_XCeFwG9tJ3CxQQZAaDRPw2
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_1tLPAF13zWhV1Z8NPVe1wj
          claim_id: c_TCp87FxBxYEt6rqkVozKHj
          source_id: s_uYCbjkL9HT7KCAR6K4SZ9c
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_uYCbjkL9HT7KCAR6K4SZ9c
            source_type: api_record
            title: 维基数据：王褒（Q10416850）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q10416850
            external_identifier: Q10416850
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:39:53.590Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E8%A4%92_(%E5%8D%97%E5%8C%97%E6%9C%9D)
        - id: cs_jXMv5tKaL6tPkLxhHCMgMK
          claim_id: c_TCp87FxBxYEt6rqkVozKHj
          source_id: s_hiHG6cvQPHs7S7JeSAtB6D
          stance: supports
          locator: 亲属关系：子
          quotation: null
          interpretation_note: CBDB 注明此条来源：唐五代人物傳記與社會網絡資料庫(1.0版)
          source:
            id: s_hiHG6cvQPHs7S7JeSAtB6D
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王裒（157867）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=157867&o=json
            external_identifier: CBDB:157867
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T03:39:53.727Z
            metadata_json: null
        - id: cs_pEm6U7Xd62KjGzW4pCQsok
          claim_id: c_TCp87FxBxYEt6rqkVozKHj
          source_id: s_H38AfivXbhKVQWcQyxqL5W
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_H38AfivXbhKVQWcQyxqL5W
            source_type: api_record
            title: 维基数据：王鼒（Q45566627）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45566627
            external_identifier: Q45566627
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:59:56.263Z
            metadata_json: null
        - id: cs_o74Non54BPvMbmQXMxkFAf
          claim_id: c_TCp87FxBxYEt6rqkVozKHj
          source_id: s_Wac8zdUyM6Acgp31ubAQga
          stance: supports
          locator: 亲属关系：父
          quotation: null
          interpretation_note: CBDB 注明此条来源：唐五代人物傳記與社會網絡資料庫(1.0版)
          source:
            id: s_Wac8zdUyM6Acgp31ubAQga
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王鼒（154143）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=154143&o=json
            external_identifier: CBDB:154143
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T03:59:56.601Z
            metadata_json: null
        - id: cs_2dPY3z6Fo5XHbaY31nhRY2
          claim_id: c_TCp87FxBxYEt6rqkVozKHj
          source_id: s_xEBYVW3xjFzLnp9P7pHZ2L
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王褒 → 王鼒
          interpretation_note: null
          source:
            id: s_xEBYVW3xjFzLnp9P7pHZ2L
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:02:22.297Z
            metadata_json: null
        - id: cs_8P33pQ7hzxZmfgoYHKN52C
          claim_id: c_TCp87FxBxYEt6rqkVozKHj
          source_id: s_XmK4uXKaoBAKcwvzzk8Dzr
          stance: supports
          locator: 条文：条文识读（生）
          quotation: 生鼒，字玉鉉，隋安都通守、石泉明威侯。
          interpretation_note: null
          source:
            id: s_XmK4uXKaoBAKcwvzzk8Dzr
            source_type: book
            title: 中文维基文库：新唐書/卷072中
            creator: null
            publisher: 维基文库
            published_at_text: null
            canonical_url: https://zh.wikisource.org/wiki/%E6%96%B0%E5%94%90%E6%9B%B8%2F%E5%8D%B7072%E4%B8%AD
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T10:12:55.026Z
            metadata_json: null
      object_person:
        id: p_XCeFwG9tJ3CxQQZAaDRPw2
        status: active
        display_name: 王鼒
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王褒

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王褒（513年—576年），南北朝人物。CBDB 记录其籍贯记录为京兆府，曾任司空。中国历代人物传记资料库（CBDB）以人物编号 157867 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。 | accepted |
| birth.date | 513年 | accepted |
| death.date | 576年 | accepted |
| name.primary | 王褒 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_p6NDwA85ehBi6FPpFUWBiZ | 王规 | accepted |
| children | p_TDm8AW3FGV4iqKti7zSPt9 | 王某 | accepted |
| children | p_XCeFwG9tJ3CxQQZAaDRPw2 | 王鼒 | accepted |

## 外部来源

- [维基数据：王褒（Q10416850）](https://www.wikidata.org/wiki/Q10416850)
- [维基数据：王规（Q10416918）](https://www.wikidata.org/wiki/Q10416918)
- [维基数据：王某（Q45489166）](https://www.wikidata.org/wiki/Q45489166)
- [维基数据：王鼒（Q45566627）](https://www.wikidata.org/wiki/Q45566627)
- [中文维基百科：琅邪王氏世系圖](https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96)
- [中文维基文库：新唐書/卷072中](https://zh.wikisource.org/wiki/%E6%96%B0%E5%94%90%E6%9B%B8%2F%E5%8D%B7072%E4%B8%AD)
- [CBDB 中国历代人物传记资料库：王某（193614）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=193614&o=json)
- [CBDB 中国历代人物传记资料库：王裒（157867）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=157867&o=json)
- [CBDB 中国历代人物传记资料库：王鼒（154143）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=154143&o=json)
