---
schema: wang-person/v1
id: p_J3P2fTe8ds7r3dVB3L7trB
status: active
merged_into: null
display_name: 王兆豸
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_kWhGtRhq5tso9djgD7RQF7
        subject_person_id: p_J3P2fTe8ds7r3dVB3L7trB
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王兆豸
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_RigxNK52vQYeDusKjUW7ve
          claim_id: c_kWhGtRhq5tso9djgD7RQF7
          source_id: s_Srs8kbRMxxDaDN7FRM7y2D
          stance: supports
          locator: CBDB:697655
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（697655）
          source: &a1
            id: s_Srs8kbRMxxDaDN7FRM7y2D
            source_type: api_record
            title: 中国历代人物传记资料库：王兆豸（CBDB 697655）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=697655&o=json
            external_identifier: CBDB:697655
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:06.561Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_8kciNyMH9Vb1ZXCBqa763Z
        subject_person_id: p_J3P2fTe8ds7r3dVB3L7trB
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王兆豸，清人物。籍贯鄞縣，身份为孝子/孝女。（中国历代人物传记资料库 CBDB 697655）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_QOe_dEsH17Y1g_fjlqhmtS
          claim_id: c_8kciNyMH9Vb1ZXCBqa763Z
          source_id: s_Srs8kbRMxxDaDN7FRM7y2D
          stance: supports
          locator: CBDB:697655
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_nkvtQ0jtv0KoRUjTt-r1y7
        subject_person_id: p_LwtPo2cTAEJShD7Q9nLDW7
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_J3P2fTe8ds7r3dVB3L7trB
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_jmTh7bhG4DcKb9LFlPCEG4
          claim_id: c_nkvtQ0jtv0KoRUjTt-r1y7
          source_id: s_Srs8kbRMxxDaDN7FRM7y2D
          stance: supports
          locator: 鄞縣志，lgid=243410：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_LwtPo2cTAEJShD7Q9nLDW7
        status: active
        display_name: 王纘爵
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王兆豸

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王兆豸 | accepted |
| bio.summary | 王兆豸，清人物。籍贯鄞縣，身份为孝子/孝女。（中国历代人物传记资料库 CBDB 697655） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_LwtPo2cTAEJShD7Q9nLDW7 | 王纘爵 | accepted |

## 外部来源

- [中国历代人物传记资料库：王兆豸（CBDB 697655）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=697655&o=json)
