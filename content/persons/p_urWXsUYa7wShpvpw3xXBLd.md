---
schema: wang-person/v1
id: p_urWXsUYa7wShpvpw3xXBLd
status: active
merged_into: null
display_name: 成氏
revision: 1
cbdb_id: 147374
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_wqGpC_uzmv-Ym6JVpSplmI
        subject_person_id: p_urWXsUYa7wShpvpw3xXBLd
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 成氏，史料所见人物。本项目依据《中国历代人物传记资料库：成氏(王立妻)（CBDB 147374）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_OwTVAO0xS_U1M7qdQyaRtx
          claim_id: c_wqGpC_uzmv-Ym6JVpSplmI
          source_id: s_Oqbe_Udu8NXiYQuPUpqZ5G
          stance: supports
          locator: CBDB:147374
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_Oqbe_Udu8NXiYQuPUpqZ5G
            source_type: api_record
            title: 中国历代人物传记资料库：成氏(王立妻)（CBDB 147374）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=147374&o=json
            external_identifier: CBDB:147374
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_1KyxWokizYAvizdJWwXD1u
        subject_person_id: p_urWXsUYa7wShpvpw3xXBLd
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 成氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_cgV3JltWhoggb-IMssM5js
          claim_id: c_1KyxWokizYAvizdJWwXD1u
          source_id: s_Oqbe_Udu8NXiYQuPUpqZ5G
          stance: supports
          locator: CBDB:147374
          quotation: null
          interpretation_note: CBDB 明确记录的王立配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_23TT4Tmo4ZM312VzpUHra_
        subject_person_id: p_qC26MJFPMNzaqqf4nfwP6q
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_urWXsUYa7wShpvpw3xXBLd
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_shX7G5w_V9re1ey1O8R_I8
          claim_id: c_23TT4Tmo4ZM312VzpUHra_
          source_id: s_Oqbe_Udu8NXiYQuPUpqZ5G
          stance: supports
          locator: 唐代墓誌匯編:二卷，Xianqing 39：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_qC26MJFPMNzaqqf4nfwP6q
        status: active
        display_name: 王立
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 成氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 成氏，史料所见人物。本项目依据《中国历代人物传记资料库：成氏(王立妻)（CBDB 147374）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 成氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_qC26MJFPMNzaqqf4nfwP6q | 王立 | accepted |

## 外部来源

- [中国历代人物传记资料库：成氏(王立妻)（CBDB 147374）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=147374&o=json)
