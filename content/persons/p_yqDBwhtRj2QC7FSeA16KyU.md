---
schema: wang-person/v1
id: p_yqDBwhtRj2QC7FSeA16KyU
status: active
merged_into: null
display_name: 周氏
revision: 1
cbdb_id: 215742
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_2RtiqehWz_ljUjO0lCDJbw
        subject_person_id: p_yqDBwhtRj2QC7FSeA16KyU
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 周氏，明人物。萬曆五年進士。（中国历代人物传记资料库 CBDB 215742）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_4VVpqfpmgU0wHmvWER1ElZ
          claim_id: c_2RtiqehWz_ljUjO0lCDJbw
          source_id: s_QdMc--RVD8j2YT2Y_KYttI
          stance: supports
          locator: CBDB:215742
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_QdMc--RVD8j2YT2Y_KYttI
            source_type: api_record
            title: 中国历代人物传记资料库：周氏(王豫妻)（CBDB 215742）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=215742&o=json
            external_identifier: CBDB:215742
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_BIDQMMd6tsFPqBNMZqJcCy
        subject_person_id: p_yqDBwhtRj2QC7FSeA16KyU
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 周氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_tqWQce79Oe36WGE9UyDcaP
          claim_id: c_BIDQMMd6tsFPqBNMZqJcCy
          source_id: s_QdMc--RVD8j2YT2Y_KYttI
          stance: supports
          locator: CBDB:215742
          quotation: null
          interpretation_note: CBDB 明确记录的王豫配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_hmVAAtgzKs4OomX1bMieHr
        subject_person_id: p_dJSjDNjDHVG2RbXzuMWW8y
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_yqDBwhtRj2QC7FSeA16KyU
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_RYtpUl31lWNNwZ1PIaqDlc
          claim_id: c_hmVAAtgzKs4OomX1bMieHr
          source_id: s_QdMc--RVD8j2YT2Y_KYttI
          stance: supports
          locator: 萬曆五年進士登科錄:一卷，第二甲第二十六名：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_dJSjDNjDHVG2RbXzuMWW8y
        status: active
        display_name: 王豫
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 周氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 周氏，明人物。萬曆五年進士。（中国历代人物传记资料库 CBDB 215742） | accepted |
| name.primary | 周氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_dJSjDNjDHVG2RbXzuMWW8y | 王豫 | accepted |

## 外部来源

- [中国历代人物传记资料库：周氏(王豫妻)（CBDB 215742）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=215742&o=json)
