---
schema: wang-person/v1
id: p_KYWRsH6nLq7aw1qYYBCMoD
status: active
merged_into: null
display_name: 來氏
revision: 1
cbdb_id: 570987
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_zTcVRCf3R9iL7eDG8y1Cel
        subject_person_id: p_KYWRsH6nLq7aw1qYYBCMoD
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 來氏，清人物。籍贯蕭山，身份为貞婦/節婦。（中国历代人物传记资料库 CBDB 570987）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_P6Dtv6YJ6BdGqkDNKVKyzR
          claim_id: c_zTcVRCf3R9iL7eDG8y1Cel
          source_id: s_OrlMK8w1bUtyvVv_BlXXCr
          stance: supports
          locator: CBDB:570987
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_OrlMK8w1bUtyvVv_BlXXCr
            source_type: api_record
            title: 中国历代人物传记资料库：來氏(王德新妻)（CBDB 570987）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=570987&o=json
            external_identifier: CBDB:570987
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_jnV2oTp9XDsUe-WRpiJ03u
        subject_person_id: p_KYWRsH6nLq7aw1qYYBCMoD
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 來氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_e97dl-1w9aaoqZJazR8rnO
          claim_id: c_jnV2oTp9XDsUe-WRpiJ03u
          source_id: s_OrlMK8w1bUtyvVv_BlXXCr
          stance: supports
          locator: CBDB:570987
          quotation: null
          interpretation_note: CBDB 明确记录的王德新配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_4Jkts8CK0BhKen0pt-ef6X
        subject_person_id: p_bqEoSPF37A64nsJ8LA87oR
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_KYWRsH6nLq7aw1qYYBCMoD
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_1N9hVbgCTytamjk5X38t3H
          claim_id: c_4Jkts8CK0BhKen0pt-ef6X
          source_id: s_OrlMK8w1bUtyvVv_BlXXCr
          stance: supports
          locator: 紹興府志:八十卷，lgid=316871：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_bqEoSPF37A64nsJ8LA87oR
        status: active
        display_name: 王德新
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 來氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 來氏，清人物。籍贯蕭山，身份为貞婦/節婦。（中国历代人物传记资料库 CBDB 570987） | accepted |
| name.primary | 來氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_bqEoSPF37A64nsJ8LA87oR | 王德新 | accepted |

## 外部来源

- [中国历代人物传记资料库：來氏(王德新妻)（CBDB 570987）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=570987&o=json)
