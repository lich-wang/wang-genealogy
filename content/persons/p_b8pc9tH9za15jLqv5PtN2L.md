---
schema: wang-person/v1
id: p_b8pc9tH9za15jLqv5PtN2L
status: active
merged_into: null
display_name: 封氏
revision: 1
cbdb_id: 247522
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_9-E1TTF1GlpOwpnbdH4Syg
        subject_person_id: p_b8pc9tH9za15jLqv5PtN2L
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 封氏，明人物。成化八年進士。（中国历代人物传记资料库 CBDB 247522）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_QPbRIPwJwUBce4RlRfHMiC
          claim_id: c_9-E1TTF1GlpOwpnbdH4Syg
          source_id: s_6wBUr9SpqamMXRopTByYIW
          stance: supports
          locator: CBDB:247522
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_6wBUr9SpqamMXRopTByYIW
            source_type: api_record
            title: 中国历代人物传记资料库：封氏(王佐妻)（CBDB 247522）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=247522&o=json
            external_identifier: CBDB:247522
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_uc661qd_vEZ43xLG6Jy3sM
        subject_person_id: p_b8pc9tH9za15jLqv5PtN2L
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 封氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_79uy2X39qMXvNOylQ25BJE
          claim_id: c_uc661qd_vEZ43xLG6Jy3sM
          source_id: s_6wBUr9SpqamMXRopTByYIW
          stance: supports
          locator: CBDB:247522
          quotation: null
          interpretation_note: CBDB 明确记录的王佐配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_tkap6j4lylP77vXfiu4Un4
        subject_person_id: p_VUBpMzfYH8LCVFm8M3HgXc
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_b8pc9tH9za15jLqv5PtN2L
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_aORqv_D4lWYPqY6vSGh_SK
          claim_id: c_tkap6j4lylP77vXfiu4Un4
          source_id: s_6wBUr9SpqamMXRopTByYIW
          stance: supports
          locator: 成化八年進士登科錄:一卷，第三甲第一百六十四名：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_VUBpMzfYH8LCVFm8M3HgXc
        status: active
        display_name: 王佐
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 封氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 封氏，明人物。成化八年進士。（中国历代人物传记资料库 CBDB 247522） | accepted |
| name.primary | 封氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_VUBpMzfYH8LCVFm8M3HgXc | 王佐 | accepted |

## 外部来源

- [中国历代人物传记资料库：封氏(王佐妻)（CBDB 247522）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=247522&o=json)
