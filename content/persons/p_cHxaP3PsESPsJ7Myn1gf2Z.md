---
schema: wang-person/v1
id: p_cHxaP3PsESPsJ7Myn1gf2Z
status: active
merged_into: null
display_name: 王彥常
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_686VT1Gfy9DaJ3dDzqfzxC
        subject_person_id: p_cHxaP3PsESPsJ7Myn1gf2Z
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王彥常
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_534RDGY5ic9c4NpD48pgK5
          claim_id: c_686VT1Gfy9DaJ3dDzqfzxC
          source_id: s_5Di7uM3HUuhXWfmFj2GGho
          stance: supports
          locator: CBDB:275612
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（275612）
          source: &a1
            id: s_5Di7uM3HUuhXWfmFj2GGho
            source_type: api_record
            title: 中国历代人物传记资料库：王彥常（CBDB 275612）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=275612&o=json
            external_identifier: CBDB:275612
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.928Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_bmHctyXA47dmGwHkFxqreQ
        subject_person_id: p_cHxaP3PsESPsJ7Myn1gf2Z
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王彥常，明人物。弘治十八年進士。（中国历代人物传记资料库 CBDB 275612）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_NYwD5unICVYu1ai3MGLqcH
          claim_id: c_bmHctyXA47dmGwHkFxqreQ
          source_id: s_5Di7uM3HUuhXWfmFj2GGho
          stance: supports
          locator: CBDB:275612
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_r00iaImOfEL-8haYsrgby-
        subject_person_id: p_cHxaP3PsESPsJ7Myn1gf2Z
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_hgNMo84ydGRxnNtLqHjNYL
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_RwXsEa_eOR3q8blE78G78s
          claim_id: c_r00iaImOfEL-8haYsrgby-
          source_id: s_5Di7uM3HUuhXWfmFj2GGho
          stance: supports
          locator: 弘治十八年進士登科錄:一卷，第三甲第一百九十七名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_hgNMo84ydGRxnNtLqHjNYL
        status: active
        display_name: 王光佐
        merged_into_person_id: null
  other: []
---

# 王彥常

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王彥常 | accepted |
| bio.summary | 王彥常，明人物。弘治十八年進士。（中国历代人物传记资料库 CBDB 275612） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_hgNMo84ydGRxnNtLqHjNYL | 王光佐 | accepted |

## 外部来源

- [中国历代人物传记资料库：王彥常（CBDB 275612）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=275612&o=json)
