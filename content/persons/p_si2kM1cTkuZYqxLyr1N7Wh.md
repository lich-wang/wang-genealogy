---
schema: wang-person/v1
id: p_si2kM1cTkuZYqxLyr1N7Wh
status: active
merged_into: null
display_name: 王士林
cbdb_id: 141245
revision: 4
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_S2fVs6JQdFmwHdbTAHnoWz
        subject_person_id: p_si2kM1cTkuZYqxLyr1N7Wh
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王士林（740年—781年），唐人物。中国历代人物传记资料库（CBDB）以人物编号 141245 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 5
      sources:
        - id: cs_-Ao-fDyTqyUtb8SEqTOGk-
          claim_id: c_S2fVs6JQdFmwHdbTAHnoWz
          source_id: s_2H8NEczuENQJaM31Wf8qDP
          stance: supports
          locator: CBDB:141245
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source:
            id: s_2H8NEczuENQJaM31Wf8qDP
            source_type: api_record
            title: 中国历代人物传记资料库：王士林（CBDB 141245）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=141245&o=json
            external_identifier: CBDB:141245
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_3RkGSEAUDb68P7PPjSL3LV
        subject_person_id: p_si2kM1cTkuZYqxLyr1N7Wh
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 740年
            calendar_note: CBDB 生年字段，精度：年
            earliest: 0740-01-01
            latest: 0740-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_urSKwZd553Hy6qHnGRvZ4n
          claim_id: c_3RkGSEAUDb68P7PPjSL3LV
          source_id: s_2H8NEczuENQJaM31Wf8qDP
          stance: supports
          locator: CBDB:141245
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 781
          source:
            id: s_2H8NEczuENQJaM31Wf8qDP
            source_type: api_record
            title: 中国历代人物传记资料库：王士林（CBDB 141245）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=141245&o=json
            external_identifier: CBDB:141245
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_dLqhKyhGAWLEdPSTvNeedD
        subject_person_id: p_si2kM1cTkuZYqxLyr1N7Wh
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 781年
            calendar_note: CBDB 卒年字段，精度：年
            earliest: 0781-01-01
            latest: 0781-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_eowerhPDNxgi48fgpDQ2ot
          claim_id: c_dLqhKyhGAWLEdPSTvNeedD
          source_id: s_2H8NEczuENQJaM31Wf8qDP
          stance: supports
          locator: CBDB:141245
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 781
          source:
            id: s_2H8NEczuENQJaM31Wf8qDP
            source_type: api_record
            title: 中国历代人物传记资料库：王士林（CBDB 141245）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=141245&o=json
            external_identifier: CBDB:141245
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_CnuAEtS15YSUqsDs7DBMde
        subject_person_id: p_si2kM1cTkuZYqxLyr1N7Wh
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王士林
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_2g9XynnSeLZXH2NEVJc3jH
          claim_id: c_CnuAEtS15YSUqsDs7DBMde
          source_id: s_2H8NEczuENQJaM31Wf8qDP
          stance: supports
          locator: CBDB:141245
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（401-500）｜历史性依据：CBDB 卒年 = 781
          source:
            id: s_2H8NEczuENQJaM31Wf8qDP
            source_type: api_record
            title: 中国历代人物传记资料库：王士林（CBDB 141245）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=141245&o=json
            external_identifier: CBDB:141245
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:39.240Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_TbOxASNZ86LCT8eAjGRqUR
        subject_person_id: p_2R9S6o8WgY6gPoDdLpNtgP
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_si2kM1cTkuZYqxLyr1N7Wh
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_j9nLW3M2EfK8Xs7YDW1Y24
          claim_id: c_TbOxASNZ86LCT8eAjGRqUR
          source_id: s_6XLPGeN77uiVbFMu8xP1ux
          stance: supports
          locator: 唐代墓誌匯編:二卷，Jianzhong 14：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_6XLPGeN77uiVbFMu8xP1ux
            source_type: api_record
            title: 中国历代人物传记资料库：王業（CBDB 155341）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=155341&o=json
            external_identifier: CBDB:155341
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.875Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_2R9S6o8WgY6gPoDdLpNtgP
        status: active
        display_name: 王業
        merged_into_person_id: null
  children:
    - claim:
        id: c_akIFE8_ZPqJ0apWa5nMejc
        subject_person_id: p_si2kM1cTkuZYqxLyr1N7Wh
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_BU7YRUBoRcsCK2SCC3Ban2
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_0_h2vUvwqhy82c5ydBMk6S
          claim_id: c_akIFE8_ZPqJ0apWa5nMejc
          source_id: s_ev1cuCAKVQW4LDSCraSc15
          stance: supports
          locator: 唐代墓誌匯編:二卷，Jianzhong 14：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_ev1cuCAKVQW4LDSCraSc15
            source_type: api_record
            title: 中国历代人物传记资料库：王革（CBDB 155343）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=155343&o=json
            external_identifier: CBDB:155343
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.877Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_BU7YRUBoRcsCK2SCC3Ban2
        status: active
        display_name: 王革
        merged_into_person_id: null
    - claim:
        id: c__YUh1ddq5fet1E8frligmb
        subject_person_id: p_si2kM1cTkuZYqxLyr1N7Wh
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_DBh6Vf8z7K4qE3hwcGPBbo
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_9zZ8_pz_E08bCaiW_rG5T-
          claim_id: c__YUh1ddq5fet1E8frligmb
          source_id: s_ygSK85SQ5KK3Y5yAwK2z41
          stance: supports
          locator: 唐代墓誌匯編:二卷，Jianzhong 14：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_ygSK85SQ5KK3Y5yAwK2z41
            source_type: api_record
            title: 中国历代人物传记资料库：王萃（CBDB 155342）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=155342&o=json
            external_identifier: CBDB:155342
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.876Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_DBh6Vf8z7K4qE3hwcGPBbo
        status: active
        display_name: 王萃
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王士林

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王士林（740年—781年），唐人物。中国历代人物传记资料库（CBDB）以人物编号 141245 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。 | accepted |
| birth.date | 740年 | accepted |
| death.date | 781年 | accepted |
| name.primary | 王士林 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_2R9S6o8WgY6gPoDdLpNtgP | 王業 | accepted |
| children | p_BU7YRUBoRcsCK2SCC3Ban2 | 王革 | accepted |
| children | p_DBh6Vf8z7K4qE3hwcGPBbo | 王萃 | accepted |

## 外部来源

- [中国历代人物传记资料库：王萃（CBDB 155342）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=155342&o=json)
- [中国历代人物传记资料库：王革（CBDB 155343）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=155343&o=json)
- [中国历代人物传记资料库：王士林（CBDB 141245）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=141245&o=json)
- [中国历代人物传记资料库：王業（CBDB 155341）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=155341&o=json)
