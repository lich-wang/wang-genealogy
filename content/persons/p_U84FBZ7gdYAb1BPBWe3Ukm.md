---
schema: wang-person/v1
id: p_U84FBZ7gdYAb1BPBWe3Ukm
status: active
merged_into: null
display_name: 王廷蘭
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_R6o45Wiz517XMYFrijMHuk
        subject_person_id: p_U84FBZ7gdYAb1BPBWe3Ukm
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王廷蘭
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_qqs2AjphFZagwcFJGJJL2R
          claim_id: c_R6o45Wiz517XMYFrijMHuk
          source_id: s_gCqPfS7xLAogpBsQFYxi5k
          stance: supports
          locator: CBDB:572480
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（572480）
          source: &a1
            id: s_gCqPfS7xLAogpBsQFYxi5k
            source_type: api_record
            title: 中国历代人物传记资料库：王廷蘭（CBDB 572480）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=572480&o=json
            external_identifier: CBDB:572480
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:25.888Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_4zVBKcv6mtreNP2qMTB5py
        subject_person_id: p_U84FBZ7gdYAb1BPBWe3Ukm
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王廷蘭，明人物。曾任知縣。（中国历代人物传记资料库 CBDB 572480）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_DsvGuC3Ql-lMd98aXhYBF1
          claim_id: c_4zVBKcv6mtreNP2qMTB5py
          source_id: s_gCqPfS7xLAogpBsQFYxi5k
          stance: supports
          locator: CBDB:572480
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_jjllQqXVm7nxyyqjNKnYMd
        subject_person_id: p_qLd4Pf4cDMmmQ6kBWSiJ8T
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_U84FBZ7gdYAb1BPBWe3Ukm
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Yonf-9MoYqzx3Q8Oq7daka
          claim_id: c_jjllQqXVm7nxyyqjNKnYMd
          source_id: s_gCqPfS7xLAogpBsQFYxi5k
          stance: supports
          locator: (道光)徽州府志:十六卷卷首一卷，lgid=1160764：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_qLd4Pf4cDMmmQ6kBWSiJ8T
        status: active
        display_name: 王壽
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王廷蘭

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王廷蘭 | accepted |
| bio.summary | 王廷蘭，明人物。曾任知縣。（中国历代人物传记资料库 CBDB 572480） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_qLd4Pf4cDMmmQ6kBWSiJ8T | 王壽 | accepted |

## 外部来源

- [中国历代人物传记资料库：王廷蘭（CBDB 572480）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=572480&o=json)
