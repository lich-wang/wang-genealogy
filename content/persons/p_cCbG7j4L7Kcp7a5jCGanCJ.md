---
schema: wang-person/v1
id: p_cCbG7j4L7Kcp7a5jCGanCJ
status: active
merged_into: null
display_name: 王廷濟
revision: 3
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_d8jPuB65rn1ZZsQQ55j6ER
        subject_person_id: p_cCbG7j4L7Kcp7a5jCGanCJ
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王廷濟
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_SZ4hTUC2uoiv1VMDq7LuL7
          claim_id: c_d8jPuB65rn1ZZsQQ55j6ER
          source_id: s_hAVkJNoYUNAshPyNz7zPAe
          stance: supports
          locator: CBDB:530909
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（530909）
          source: &a1
            id: s_hAVkJNoYUNAshPyNz7zPAe
            source_type: api_record
            title: 中国历代人物传记资料库：王廷濟（CBDB 530909）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=530909&o=json
            external_identifier: CBDB:530909
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:04.356Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_wBEPACMHZxoADWvt9u6b46
        subject_person_id: p_cCbG7j4L7Kcp7a5jCGanCJ
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王廷濟，清人物。籍贯武功。（中国历代人物传记资料库 CBDB 530909）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_hiLMnNBOWx4KbCH59ZbGO2
          claim_id: c_wBEPACMHZxoADWvt9u6b46
          source_id: s_hAVkJNoYUNAshPyNz7zPAe
          stance: supports
          locator: CBDB:530909
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_nyxxtFxTaXqoMOKRTNt3tJ
        subject_person_id: p_KTF8TNqVhiVNBbLepsymBT
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_cCbG7j4L7Kcp7a5jCGanCJ
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_g0lY-TKRtMFUpb8s3iaIlm
          claim_id: c_nyxxtFxTaXqoMOKRTNt3tJ
          source_id: s_hAVkJNoYUNAshPyNz7zPAe
          stance: supports
          locator: 武功縣後志，lgid=1045879：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_KTF8TNqVhiVNBbLepsymBT
        status: active
        display_name: 王錦
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_yH4gq7YLJvsgndUGAFEkna
        subject_person_id: p_cCbG7j4L7Kcp7a5jCGanCJ
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_QGhRE5bGsTgAQf4AguGCV1
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_lKiWtHUShQS8HrBisD9BpT
          claim_id: c_yH4gq7YLJvsgndUGAFEkna
          source_id: s_Y63A0_9FTv0dC50kK1Jdb4
          stance: supports
          locator: 武功縣後志，lgid=1045879：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_Y63A0_9FTv0dC50kK1Jdb4
            source_type: api_record
            title: 中国历代人物传记资料库：孫氏(王廷濟妻)（CBDB 530910）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=530910&o=json
            external_identifier: CBDB:530910
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_QGhRE5bGsTgAQf4AguGCV1
        status: active
        display_name: 孫氏
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 王廷濟

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王廷濟 | accepted |
| bio.summary | 王廷濟，清人物。籍贯武功。（中国历代人物传记资料库 CBDB 530909） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_KTF8TNqVhiVNBbLepsymBT | 王錦 | accepted |
| spouses | p_QGhRE5bGsTgAQf4AguGCV1 | 孫氏 | accepted |

## 外部来源

- [中国历代人物传记资料库：孫氏(王廷濟妻)（CBDB 530910）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=530910&o=json)
- [中国历代人物传记资料库：王廷濟（CBDB 530909）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=530909&o=json)
