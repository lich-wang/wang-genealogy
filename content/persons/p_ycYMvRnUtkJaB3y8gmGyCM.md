---
schema: wang-person/v1
id: p_ycYMvRnUtkJaB3y8gmGyCM
status: active
merged_into: null
display_name: 王鑑之
cbdb_id: 126887
revision: 3
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_SNoBLZVx1AcrKnJzV1EMux
        subject_person_id: p_ycYMvRnUtkJaB3y8gmGyCM
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王鑑之（1440年—1519年），明人物。中国历代人物传记资料库（CBDB）以人物编号 126887 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 5
      sources:
        - id: cs_tMDETwH2U4sNuULqz04LLT
          claim_id: c_SNoBLZVx1AcrKnJzV1EMux
          source_id: s_yVFwpMn5jWhWM5FcMtUb81
          stance: supports
          locator: CBDB:126887
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source:
            id: s_yVFwpMn5jWhWM5FcMtUb81
            source_type: api_record
            title: 中国历代人物传记资料库：王鑑之（CBDB 126887）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=126887&o=json
            external_identifier: CBDB:126887
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:37.980Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_nBzSs1L12CFcQHaDrNgtYt
        subject_person_id: p_ycYMvRnUtkJaB3y8gmGyCM
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1440年
            calendar_note: CBDB 生年字段，精度：年
            earliest: 1440-01-01
            latest: 1440-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_7DJ8q37H1gxehxLz2tQiE3
          claim_id: c_nBzSs1L12CFcQHaDrNgtYt
          source_id: s_yVFwpMn5jWhWM5FcMtUb81
          stance: supports
          locator: CBDB:126887
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（301-400）｜历史性依据：CBDB 卒年 = 1519
          source:
            id: s_yVFwpMn5jWhWM5FcMtUb81
            source_type: api_record
            title: 中国历代人物传记资料库：王鑑之（CBDB 126887）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=126887&o=json
            external_identifier: CBDB:126887
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:37.980Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_z8tNZ5XePAmgEVL76oDQd8
        subject_person_id: p_ycYMvRnUtkJaB3y8gmGyCM
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1519年
            calendar_note: CBDB 卒年字段，精度：年
            earliest: 1519-01-01
            latest: 1519-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_SsFmDZEWLnv2iv8M4Up328
          claim_id: c_z8tNZ5XePAmgEVL76oDQd8
          source_id: s_yVFwpMn5jWhWM5FcMtUb81
          stance: supports
          locator: CBDB:126887
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（301-400）｜历史性依据：CBDB 卒年 = 1519
          source:
            id: s_yVFwpMn5jWhWM5FcMtUb81
            source_type: api_record
            title: 中国历代人物传记资料库：王鑑之（CBDB 126887）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=126887&o=json
            external_identifier: CBDB:126887
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:37.980Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_7VgpT7WTJF2KcDg2mLWM5K
        subject_person_id: p_ycYMvRnUtkJaB3y8gmGyCM
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王鑑之
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_PnupLTrZ3jwCRtLWYFjXVr
          claim_id: c_7VgpT7WTJF2KcDg2mLWM5K
          source_id: s_yVFwpMn5jWhWM5FcMtUb81
          stance: supports
          locator: CBDB:126887
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（301-400）｜历史性依据：CBDB 卒年 = 1519
          source:
            id: s_yVFwpMn5jWhWM5FcMtUb81
            source_type: api_record
            title: 中国历代人物传记资料库：王鑑之（CBDB 126887）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=126887&o=json
            external_identifier: CBDB:126887
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:37.980Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_Wi_C3KIUCt343_Sf4G1io3
        subject_person_id: p_X8pnsgqtNiK4YaNNve2N13
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_ycYMvRnUtkJaB3y8gmGyCM
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_eZQy6U6kG3A0JO5ctH2pkU
          claim_id: c_Wi_C3KIUCt343_Sf4G1io3
          source_id: s_RTBEdN2CvaEbLVcFC3nV9a
          stance: supports
          locator: 成化十四年進士登科錄:一卷，第三甲第一百六十二名：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_RTBEdN2CvaEbLVcFC3nV9a
            source_type: api_record
            title: 中国历代人物传记资料库：王詵（CBDB 253203）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=253203&o=json
            external_identifier: CBDB:253203
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.295Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_X8pnsgqtNiK4YaNNve2N13
        status: active
        display_name: 王詵
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_B5TT8KixiDaCnWhgfdlTP8
        subject_person_id: p_orojH38je49BQs4M93nrDn
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_ycYMvRnUtkJaB3y8gmGyCM
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_1-EVlWhoefsU8e9XBqoczV
          claim_id: c_B5TT8KixiDaCnWhgfdlTP8
          source_id: s_Ri3yCoC5JgQUbhMQjCewAg
          stance: supports
          locator: 成化十四年進士登科錄:一卷，第三甲第一百六十二名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_Ri3yCoC5JgQUbhMQjCewAg
            source_type: api_record
            title: 中国历代人物传记资料库：王壽（CBDB 253200）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=253200&o=json
            external_identifier: CBDB:253200
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:18.294Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_orojH38je49BQs4M93nrDn
        status: active
        display_name: 王壽
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王鑑之

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王鑑之（1440年—1519年），明人物。中国历代人物传记资料库（CBDB）以人物编号 126887 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。 | accepted |
| birth.date | 1440年 | accepted |
| death.date | 1519年 | accepted |
| name.primary | 王鑑之 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_X8pnsgqtNiK4YaNNve2N13 | 王詵 | accepted |
| ancestors | p_orojH38je49BQs4M93nrDn | 王壽 | accepted |

## 外部来源

- [中国历代人物传记资料库：王鑑之（CBDB 126887）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=126887&o=json)
- [中国历代人物传记资料库：王詵（CBDB 253203）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=253203&o=json)
- [中国历代人物传记资料库：王壽（CBDB 253200）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=253200&o=json)
