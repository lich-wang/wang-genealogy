---
schema: wang-person/v1
id: p_QEExoTUPC65FDzcepDMC86
status: active
merged_into: null
display_name: 王銳
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_sAAD5YHRQHFypp4fUZLqX7
        subject_person_id: p_QEExoTUPC65FDzcepDMC86
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王銳
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_AtML9LRLWJVmTQqeGANLDp
          claim_id: c_sAAD5YHRQHFypp4fUZLqX7
          source_id: s_cmmMgAqt2Fefd6yY6tSWRS
          stance: supports
          locator: CBDB:29196
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（29196）
          source: &a1
            id: s_cmmMgAqt2Fefd6yY6tSWRS
            source_type: api_record
            title: 中国历代人物传记资料库：王銳（CBDB 29196）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=29196&o=json
            external_identifier: CBDB:29196
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.990Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_CUNAs97jaQKs7V2FBPQPtu
        subject_person_id: p_QEExoTUPC65FDzcepDMC86
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王銳，元人物。籍贯寧晉。（中国历代人物传记资料库 CBDB 29196）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_mPXg-RhHZf3v6zoUiCf1mf
          claim_id: c_CUNAs97jaQKs7V2FBPQPtu
          source_id: s_cmmMgAqt2Fefd6yY6tSWRS
          stance: supports
          locator: CBDB:29196
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_xAGMIkV42S3H2LxbQa0oaJ
        subject_person_id: p_DRV1HsEBatah9fyUGYScze
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_QEExoTUPC65FDzcepDMC86
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_f3g1IrNbmSGoWb8yhFb_iy
          claim_id: c_xAGMIkV42S3H2LxbQa0oaJ
          source_id: s_OiTHMrTXsYanZpuhDspKdj
          stance: supports
          locator: CBDB 亲属：父（KinPerson 29140）
          quotation: null
          interpretation_note: CBDB 直接记录的父／母关系。
          source:
            id: s_OiTHMrTXsYanZpuhDspKdj
            source_type: api_record
            title: 中国历代人物传记资料库：王銳（CBDB 29196）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=29196&o=json
            external_identifier: CBDB:29196
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_DRV1HsEBatah9fyUGYScze
        status: active
        display_name: 王忱
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王銳

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王銳 | accepted |
| bio.summary | 王銳，元人物。籍贯寧晉。（中国历代人物传记资料库 CBDB 29196） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_DRV1HsEBatah9fyUGYScze | 王忱 | accepted |

## 外部来源

- [中国历代人物传记资料库：王銳（CBDB 29196）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=29196&o=json)
