---
schema: wang-person/v1
id: p_Bn1ZVcnd33H3XNU3Lai8Qn
status: active
merged_into: null
display_name: 王禮
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_LY41F9xqB6VjyBAaLKGaTG
        subject_person_id: p_Bn1ZVcnd33H3XNU3Lai8Qn
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王禮
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_YGFde7NwRpNMiVhhDeG5Km
          claim_id: c_LY41F9xqB6VjyBAaLKGaTG
          source_id: s_AV6nNYCCCV7F972GfKyb6G
          stance: supports
          locator: CBDB:280975
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（280975）
          source: &a1
            id: s_AV6nNYCCCV7F972GfKyb6G
            source_type: api_record
            title: 中国历代人物传记资料库：王禮（CBDB 280975）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=280975&o=json
            external_identifier: CBDB:280975
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.031Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_9dDLcLmWVaqbHjnFpeCtmF
        subject_person_id: p_Bn1ZVcnd33H3XNU3Lai8Qn
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王禮，明人物。景泰五年進士，籍贯懷柔，曾任府知事、海防同知。（中国历代人物传记资料库 CBDB 280975）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_OZsb1R1AsopXHnImLB0LKb
          claim_id: c_9dDLcLmWVaqbHjnFpeCtmF
          source_id: s_AV6nNYCCCV7F972GfKyb6G
          stance: supports
          locator: CBDB:280975
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_8LoE5eMoOW77cJdgQFtX1f
        subject_person_id: p_Bn1ZVcnd33H3XNU3Lai8Qn
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_VSNhYvRvJzPMQs1xDCfb9J
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Qls34JaVRZD8xcuw3O-6UR
          claim_id: c_8LoE5eMoOW77cJdgQFtX1f
          source_id: s_AV6nNYCCCV7F972GfKyb6G
          stance: supports
          locator: 景泰五年進士登科錄:一卷，第三甲第二百零八名：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_VSNhYvRvJzPMQs1xDCfb9J
        status: active
        display_name: 王詔
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王禮

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王禮 | accepted |
| bio.summary | 王禮，明人物。景泰五年進士，籍贯懷柔，曾任府知事、海防同知。（中国历代人物传记资料库 CBDB 280975） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_VSNhYvRvJzPMQs1xDCfb9J | 王詔 | accepted |

## 外部来源

- [中国历代人物传记资料库：王禮（CBDB 280975）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=280975&o=json)
