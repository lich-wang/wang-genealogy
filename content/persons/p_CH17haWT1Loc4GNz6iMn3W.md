---
schema: wang-person/v1
id: p_CH17haWT1Loc4GNz6iMn3W
status: active
merged_into: null
display_name: 曹某
revision: 1
cbdb_id: 150598
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_7uXI0sqlZ2BqHOzhPSGgJq
        subject_person_id: p_CH17haWT1Loc4GNz6iMn3W
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 曹某，史料所见人物。本项目依据《中国历代人物传记资料库：曹某(王三娘夫)（CBDB 150598）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Q6kLw7dUC4VB-ku3f4hp6r
          claim_id: c_7uXI0sqlZ2BqHOzhPSGgJq
          source_id: s_k4PL66DkaPYPD9kMf777hC
          stance: supports
          locator: CBDB:150598
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_k4PL66DkaPYPD9kMf777hC
            source_type: api_record
            title: 中国历代人物传记资料库：曹某(王三娘夫)（CBDB 150598）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=150598&o=json
            external_identifier: CBDB:150598
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_A7yPo4Sqo1S9n0YbIMLjza
        subject_person_id: p_CH17haWT1Loc4GNz6iMn3W
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 曹某
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_a3ETdnqvCSUTcRhIblBP4d
          claim_id: c_A7yPo4Sqo1S9n0YbIMLjza
          source_id: s_k4PL66DkaPYPD9kMf777hC
          stance: supports
          locator: CBDB:150598
          quotation: null
          interpretation_note: CBDB 明确记录的王三娘配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_cRaw-k161SwDjwdwUnPz85
        subject_person_id: p_4XvEffiKBLHcAB3UhE6GM2
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_CH17haWT1Loc4GNz6iMn3W
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_sUtFx9oldvA5oSzwYToxV9
          claim_id: c_cRaw-k161SwDjwdwUnPz85
          source_id: s_k4PL66DkaPYPD9kMf777hC
          stance: supports
          locator: 唐代墓誌匯編:二卷，Shenlong 23：丈夫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_4XvEffiKBLHcAB3UhE6GM2
        status: active
        display_name: 王三娘
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 曹某

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 曹某，史料所见人物。本项目依据《中国历代人物传记资料库：曹某(王三娘夫)（CBDB 150598）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 曹某 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_4XvEffiKBLHcAB3UhE6GM2 | 王三娘 | accepted |

## 外部来源

- [中国历代人物传记资料库：曹某(王三娘夫)（CBDB 150598）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=150598&o=json)
