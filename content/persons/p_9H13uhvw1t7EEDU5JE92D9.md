---
schema: wang-person/v1
id: p_9H13uhvw1t7EEDU5JE92D9
status: active
merged_into: null
display_name: 王在明
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_e2DCSY1RaG5HYhpNoRXYoB
        subject_person_id: p_9H13uhvw1t7EEDU5JE92D9
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王在明
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_zmwppeEMLiFGKJa2Wd8b8M
          claim_id: c_e2DCSY1RaG5HYhpNoRXYoB
          source_id: s_BQprRgv39qvp6Ah1vTgw7H
          stance: supports
          locator: CBDB:294430
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（294430）
          source: &a1
            id: s_BQprRgv39qvp6Ah1vTgw7H
            source_type: api_record
            title: 中国历代人物传记资料库：王在明（CBDB 294430）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=294430&o=json
            external_identifier: CBDB:294430
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.454Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_FW3p4uBBojaPjzmBJjKosb
        subject_person_id: p_9H13uhvw1t7EEDU5JE92D9
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王在明，明人物。嘉靖十一年進士，曾任知縣。（中国历代人物传记资料库 CBDB 294430）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_Uu5LXhbqgsYIzmqyY7-yDe
          claim_id: c_FW3p4uBBojaPjzmBJjKosb
          source_id: s_BQprRgv39qvp6Ah1vTgw7H
          stance: supports
          locator: CBDB:294430
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_h-rXTXreEj5GoCF5ObF9DA
        subject_person_id: p_9H13uhvw1t7EEDU5JE92D9
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_TdovnQSDHPgtdb6L5QzgtZ
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_XxuIb24zXOatU9tvbN60if
          claim_id: c_h-rXTXreEj5GoCF5ObF9DA
          source_id: s_BQprRgv39qvp6Ah1vTgw7H
          stance: supports
          locator: 嘉靖十一年進士登科錄:一卷，第三甲第一百九十八名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_TdovnQSDHPgtdb6L5QzgtZ
        status: active
        display_name: 王梴
        merged_into_person_id: null
  other: []
---

# 王在明

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王在明 | accepted |
| bio.summary | 王在明，明人物。嘉靖十一年進士，曾任知縣。（中国历代人物传记资料库 CBDB 294430） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_TdovnQSDHPgtdb6L5QzgtZ | 王梴 | accepted |

## 外部来源

- [中国历代人物传记资料库：王在明（CBDB 294430）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=294430&o=json)
