---
schema: wang-person/v1
id: p_LHKoMbFGzi2LiWjocEU5qY
status: active
merged_into: null
display_name: 王㻊
cbdb_id: 255547
revision: 3
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_gBD4bSXTotHVxhgAD6oNoZ
        subject_person_id: p_LHKoMbFGzi2LiWjocEU5qY
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王㻊，明人物。景泰五年進士，籍贯閩縣。（中国历代人物传记资料库 CBDB 255547）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 7
      sources:
        - id: cs_kJ_LSKSV8buwrO2Y2zOT1o
          claim_id: c_gBD4bSXTotHVxhgAD6oNoZ
          source_id: s_KqFVevUdGcEPGM3H2mpjfU
          stance: supports
          locator: CBDB:255547
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_KqFVevUdGcEPGM3H2mpjfU
            source_type: api_record
            title: 中国历代人物传记资料库：王㻊（CBDB 255547）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=255547&o=json
            external_identifier: CBDB:255547
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:17:00.581Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_S9AJsSjRrPDsNdUDcE24PD
        subject_person_id: p_LHKoMbFGzi2LiWjocEU5qY
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王㻊
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_es5GDE8PHGTnzfTMibcvar
          claim_id: c_S9AJsSjRrPDsNdUDcE24PD
          source_id: s_KqFVevUdGcEPGM3H2mpjfU
          stance: supports
          locator: CBDB:255547
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（3001-3100）｜历史性依据：CBDB 朝代 = 明
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_5aE_I5keNoSHqEc25bWIo8
        subject_person_id: p_xVTypDWdUQ3wcoicLvKHN5
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_LHKoMbFGzi2LiWjocEU5qY
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_fuLDyjmPzbpDTvj2EAJ4-Q
          claim_id: c_5aE_I5keNoSHqEc25bWIo8
          source_id: s_sV1TTLf5r2wFwh1LUQ2_dz
          stance: supports
          locator: CBDB：兄弟 王衡（126831）之父／母 王穆
          quotation: null
          interpretation_note: 由兄弟关系推断：王㻊 与 王衡 为同胞（CBDB 记「兄」），王衡 之父／母即 王㻊 之父／母。
          source:
            id: s_sV1TTLf5r2wFwh1LUQ2_dz
            source_type: api_record
            title: 中国历代人物传记资料库：王㻊（CBDB 255547）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=255547&o=json
            external_identifier: CBDB:255547
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_xVTypDWdUQ3wcoicLvKHN5
        status: active
        display_name: 王穆
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_ZBIs9BGZvAsMeC5KiMy31u
        subject_person_id: p_KL6pD2p9SR6r7j4F89boFc
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_LHKoMbFGzi2LiWjocEU5qY
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_qLD2gkTSZfsTq7qAD3675C
          claim_id: c_ZBIs9BGZvAsMeC5KiMy31u
          source_id: s_sV1TTLf5r2wFwh1LUQ2_dz
          stance: supports
          locator: CBDB 亲属：兄（KinPerson 126831 王衡）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_sV1TTLf5r2wFwh1LUQ2_dz
            source_type: api_record
            title: 中国历代人物传记资料库：王㻊（CBDB 255547）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=255547&o=json
            external_identifier: CBDB:255547
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_KL6pD2p9SR6r7j4F89boFc
        status: active
        display_name: 王衡
        merged_into_person_id: null
---

# 王㻊

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王㻊，明人物。景泰五年進士，籍贯閩縣。（中国历代人物传记资料库 CBDB 255547） | accepted |
| name.primary | 王㻊 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_xVTypDWdUQ3wcoicLvKHN5 | 王穆 | accepted |
| other | p_KL6pD2p9SR6r7j4F89boFc | 王衡 | accepted |

## 外部来源

- [中国历代人物传记资料库：王㻊（CBDB 255547）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=255547&o=json)
