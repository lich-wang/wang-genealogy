---
schema: wang-person/v1
id: p_An1eB7p5nNk1QDqNJqiSDj
status: active
merged_into: null
display_name: 孫氏
revision: 1
cbdb_id: 5365
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_5NYCC8TgaTb_z_A3QVKTI7
        subject_person_id: p_An1eB7p5nNk1QDqNJqiSDj
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 孫氏（1103—1175），宋人物。籍贯石埭，入仕封贈，曾任郡夫人。（中国历代人物传记资料库 CBDB 5365）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_FZ1lltMbnmiQVQ9M3bSenE
          claim_id: c_5NYCC8TgaTb_z_A3QVKTI7
          source_id: s_E5Jpx280VFkf4M9FFhXCyW
          stance: supports
          locator: CBDB:5365
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_E5Jpx280VFkf4M9FFhXCyW
            source_type: api_record
            title: 中国历代人物传记资料库：孫氏(王鎡妻)（CBDB 5365）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=5365&o=json
            external_identifier: CBDB:5365
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_U0SxDPAAi6eqP--0t9-j34
        subject_person_id: p_An1eB7p5nNk1QDqNJqiSDj
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 孫氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_ZXREHk-CD2Jq4gZQ3Uc6uR
          claim_id: c_U0SxDPAAi6eqP--0t9-j34
          source_id: s_E5Jpx280VFkf4M9FFhXCyW
          stance: supports
          locator: CBDB:5365
          quotation: null
          interpretation_note: CBDB 明确记录的王鎡配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_TO5JduJzodJV77d9quJPCi
        subject_person_id: p_zGcbQEDDUm5Y71RMcWUTgh
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_An1eB7p5nNk1QDqNJqiSDj
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_WLiE9Kbq3j3fkrXAQSsKIo
          claim_id: c_TO5JduJzodJV77d9quJPCi
          source_id: s_E5Jpx280VFkf4M9FFhXCyW
          stance: supports
          locator: 宋人傳記資料索引(電子版)，1278;1279：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_zGcbQEDDUm5Y71RMcWUTgh
        status: active
        display_name: 王鎡
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 孫氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 孫氏（1103—1175），宋人物。籍贯石埭，入仕封贈，曾任郡夫人。（中国历代人物传记资料库 CBDB 5365） | accepted |
| name.primary | 孫氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_zGcbQEDDUm5Y71RMcWUTgh | 王鎡 | accepted |

## 外部来源

- [中国历代人物传记资料库：孫氏(王鎡妻)（CBDB 5365）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=5365&o=json)
