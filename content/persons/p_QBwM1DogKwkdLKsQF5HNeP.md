---
schema: wang-person/v1
id: p_QBwM1DogKwkdLKsQF5HNeP
status: active
merged_into: null
display_name: 王懋枝
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_EXg46PRC6vvA5xn8udJ9C3
        subject_person_id: p_QBwM1DogKwkdLKsQF5HNeP
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王懋枝
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_X4LueRLGzoPA1PbC7SN7kC
          claim_id: c_EXg46PRC6vvA5xn8udJ9C3
          source_id: s_FTPLH5PEEJXJAJ29iJ5g8H
          stance: supports
          locator: CBDB:696652
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（696652）
          source: &a1
            id: s_FTPLH5PEEJXJAJ29iJ5g8H
            source_type: api_record
            title: 中国历代人物传记资料库：王懋枝（CBDB 696652）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=696652&o=json
            external_identifier: CBDB:696652
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:06.551Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_63wbxxpPV1ni4QHF5mRk6t
        subject_person_id: p_QBwM1DogKwkdLKsQF5HNeP
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: "王懋枝，清人物。籍贯仙居，身份为良吏;循吏、義民/義官，入仕貢生: 恩貢，曾任知縣。（中国历代人物传记资料库 CBDB 696652）"
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_MlxpLzW0mxDu7BkSE-R1KC
          claim_id: c_63wbxxpPV1ni4QHF5mRk6t
          source_id: s_FTPLH5PEEJXJAJ29iJ5g8H
          stance: supports
          locator: CBDB:696652
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_FNpyX-KaHUQUol6UHHwaJE
        subject_person_id: p_yYAPAC96fgCvfvGxbcFpmK
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_QBwM1DogKwkdLKsQF5HNeP
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_YNWCrN13JgOo7b-qhoy9zG
          claim_id: c_FNpyX-KaHUQUol6UHHwaJE
          source_id: s_FTPLH5PEEJXJAJ29iJ5g8H
          stance: supports
          locator: 仙居志，lgid=356678：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_yYAPAC96fgCvfvGxbcFpmK
        status: active
        display_name: 王汝瑚
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王懋枝

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王懋枝 | accepted |
| bio.summary | 王懋枝，清人物。籍贯仙居，身份为良吏;循吏、義民/義官，入仕貢生: 恩貢，曾任知縣。（中国历代人物传记资料库 CBDB 696652） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_yYAPAC96fgCvfvGxbcFpmK | 王汝瑚 | accepted |

## 外部来源

- [中国历代人物传记资料库：王懋枝（CBDB 696652）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=696652&o=json)
