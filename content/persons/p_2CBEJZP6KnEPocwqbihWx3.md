---
schema: wang-person/v1
id: p_2CBEJZP6KnEPocwqbihWx3
status: active
merged_into: null
display_name: 王蒙
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_U4B8KWBr2Kp4QVmdZmrC3S
        subject_person_id: p_2CBEJZP6KnEPocwqbihWx3
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王蒙
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_Gdv2JrfUseNHd62t6hiVEW
          claim_id: c_U4B8KWBr2Kp4QVmdZmrC3S
          source_id: s_RmaebJj8faNFAEyMCTLkY8
          stance: supports
          locator: CBDB:100923
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（100923）
          source: &a1
            id: s_RmaebJj8faNFAEyMCTLkY8
            source_type: api_record
            title: 中国历代人物传记资料库：王蒙（CBDB 100923）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=100923&o=json
            external_identifier: CBDB:100923
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:14.570Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_DNkBrRQH51FXTtTRqrBFJm
        subject_person_id: p_2CBEJZP6KnEPocwqbihWx3
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1308年
            calendar_note: CBDB 生年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_WuJ7boHwhGjY15gcs8yYga
          claim_id: c_DNkBrRQH51FXTtTRqrBFJm
          source_id: s_RmaebJj8faNFAEyMCTLkY8
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 生年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_fvfrEWE5KPA7JKAkK1NVds
        subject_person_id: p_2CBEJZP6KnEPocwqbihWx3
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1385年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_eSH4fnN7LywjcKpd6wKMBa
          claim_id: c_fvfrEWE5KPA7JKAkK1NVds
          source_id: s_RmaebJj8faNFAEyMCTLkY8
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_PfwAJNzpJudGXWzJuNuczY
        subject_person_id: p_2CBEJZP6KnEPocwqbihWx3
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王蒙（1308年—1385年），元人物。籍贯湖州路，身份为畫家，曾任理問所理問、知州。（中国历代人物传记资料库 CBDB 100923）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_aHm9Uwnvbao_AQ-cp-yGS1
          claim_id: c_PfwAJNzpJudGXWzJuNuczY
          source_id: s_RmaebJj8faNFAEyMCTLkY8
          stance: supports
          locator: CBDB:100923
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_-TLCzEwIpFfapOdukculk3
        subject_person_id: p_RFUdJSKn4VGECrkxCnE2tK
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_2CBEJZP6KnEPocwqbihWx3
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_KaipQs9Cb_RfvFJogK9UDw
          claim_id: c_-TLCzEwIpFfapOdukculk3
          source_id: s_RmaebJj8faNFAEyMCTLkY8
          stance: supports
          locator: 元人傳記資料索引，1178：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_RFUdJSKn4VGECrkxCnE2tK
        status: active
        display_name: 王國器
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王蒙

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王蒙 | accepted |
| birth.date | 1308年 | accepted |
| death.date | 1385年 | accepted |
| bio.summary | 王蒙（1308年—1385年），元人物。籍贯湖州路，身份为畫家，曾任理問所理問、知州。（中国历代人物传记资料库 CBDB 100923） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_RFUdJSKn4VGECrkxCnE2tK | 王國器 | accepted |

## 外部来源

- [中国历代人物传记资料库：王蒙（CBDB 100923）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=100923&o=json)
