---
schema: wang-person/v1
id: p_1zf7uM3w8Zewvcp6HeXLFy
status: active
merged_into: null
display_name: 許季蘭
revision: 1
cbdb_id: 118584
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_d-rxEggkcmKo2UiHS2K7f-
        subject_person_id: p_1zf7uM3w8Zewvcp6HeXLFy
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 許季蘭，史料所见人物。本项目依据《中国历代人物传记资料库：許季蘭（CBDB 118584）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_aw_0QnGRnmSNlR6Sw35ETc
          claim_id: c_d-rxEggkcmKo2UiHS2K7f-
          source_id: s_BdvTpe82KsSCRNNyG16yfm
          stance: supports
          locator: CBDB:118584
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_BdvTpe82KsSCRNNyG16yfm
            source_type: api_record
            title: 中国历代人物传记资料库：許季蘭（CBDB 118584）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=118584&o=json
            external_identifier: CBDB:118584
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_0XXfOB7S4KNXldcBAwBUoG
        subject_person_id: p_1zf7uM3w8Zewvcp6HeXLFy
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 許季蘭
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_quqyGKfhnWqhd2NqtNkkRy
          claim_id: c_0XXfOB7S4KNXldcBAwBUoG
          source_id: s_BdvTpe82KsSCRNNyG16yfm
          stance: supports
          locator: CBDB:118584
          quotation: null
          interpretation_note: CBDB 明确记录的王修文配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_5_rKpu0dpRYeLS-QENn9TA
        subject_person_id: p_XjupuoZtM7GbiaQbn194GQ
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_1zf7uM3w8Zewvcp6HeXLFy
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_-yoZ0AvvTcUM1OPQ6FEu0A
          claim_id: c_5_rKpu0dpRYeLS-QENn9TA
          source_id: s_BdvTpe82KsSCRNNyG16yfm
          stance: supports
          locator: "明清婦女著作數據庫，MQWW PoetID #2657, HuWenKai #560：妻子"
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_XjupuoZtM7GbiaQbn194GQ
        status: active
        display_name: 王修文
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 許季蘭

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 許季蘭，史料所见人物。本项目依据《中国历代人物传记资料库：許季蘭（CBDB 118584）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 許季蘭 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_XjupuoZtM7GbiaQbn194GQ | 王修文 | accepted |

## 外部来源

- [中国历代人物传记资料库：許季蘭（CBDB 118584）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=118584&o=json)
