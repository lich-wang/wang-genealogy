---
schema: wang-person/v1
id: p_ETdwCm54JGjgAZiDmfGBaU
status: active
merged_into: null
display_name: 王偉
revision: 4
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_BstM8SH9HnUQrrBoNvPNAm
        subject_person_id: p_ETdwCm54JGjgAZiDmfGBaU
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王偉
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_oAccqrvc7ESnQGd8U8ady2
          claim_id: c_BstM8SH9HnUQrrBoNvPNAm
          source_id: s_2CqNomGUNbdb9NCR8WAhFY
          stance: supports
          locator: CBDB:439386
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（439386）
          source: &a1
            id: s_2CqNomGUNbdb9NCR8WAhFY
            source_type: api_record
            title: 中国历代人物传记资料库：王偉（CBDB 439386）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=439386&o=json
            external_identifier: CBDB:439386
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:22.094Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_LLfCgWY3wBC4LbgHBW7P11
        subject_person_id: p_ETdwCm54JGjgAZiDmfGBaU
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1591年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_YPVzRDjEjFxoSCv6N4FMCg
          claim_id: c_LLfCgWY3wBC4LbgHBW7P11
          source_id: s_2CqNomGUNbdb9NCR8WAhFY
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_N3QyZHk87xD32Q2zti1BhH
        subject_person_id: p_ETdwCm54JGjgAZiDmfGBaU
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王偉（卒于1591年），明人物。籍贯餘姚，曾任文思院副使、衛指揮使。（中国历代人物传记资料库 CBDB 439386）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_ziG-fTJ8BF-37DyIOLdetB
          claim_id: c_N3QyZHk87xD32Q2zti1BhH
          source_id: s_2CqNomGUNbdb9NCR8WAhFY
          stance: supports
          locator: CBDB:439386
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_J0lu5S2QLpv04pG9uZZX94
        subject_person_id: p_67QPdbGBEAuDLU32GCV69J
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_ETdwCm54JGjgAZiDmfGBaU
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_FFzvrIjIDG609GPclDJhbW
          claim_id: c_J0lu5S2QLpv04pG9uZZX94
          source_id: s_izjCM7Xjq7AiXpp61KmMyD
          stance: supports
          locator: 紹興府志:八十卷，lgid=316700：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_izjCM7Xjq7AiXpp61KmMyD
            source_type: api_record
            title: 中国历代人物传记资料库：王正（CBDB 563487）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=563487&o=json
            external_identifier: CBDB:563487
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:25.801Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_67QPdbGBEAuDLU32GCV69J
        status: active
        display_name: 王正
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_Xt8z03dY3okm-36YC8A__L
        subject_person_id: p_K9QEgp6JSKaffEKcTo7xe2
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_ETdwCm54JGjgAZiDmfGBaU
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_AoxFsz5vQpexaI53aBELNm
          claim_id: c_Xt8z03dY3okm-36YC8A__L
          source_id: s_9kzsHgRKwkNrd268ZDTF6o
          stance: supports
          locator: 紹興府志:八十卷，lgid=316700：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_9kzsHgRKwkNrd268ZDTF6o
            source_type: api_record
            title: 中国历代人物传记资料库：王賢（CBDB 563485）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=563485&o=json
            external_identifier: CBDB:563485
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:25.799Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_K9QEgp6JSKaffEKcTo7xe2
        status: active
        display_name: 王賢
        merged_into_person_id: null
    - claim:
        id: c_hbWtKGTi5OT8uT_nxhXTEm
        subject_person_id: p_XM9QNMLmGKfYJ3GSaLi11d
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_ETdwCm54JGjgAZiDmfGBaU
        generation_count: 4
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_nlyf0kZFtQOr0iA7G5MSGd
          claim_id: c_hbWtKGTi5OT8uT_nxhXTEm
          source_id: s_y3UEciAhJvmsr7XUzLhMNX
          stance: supports
          locator: 紹興府志:八十卷，lgid=316700：玄孫;四世孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_y3UEciAhJvmsr7XUzLhMNX
            source_type: api_record
            title: 中国历代人物传记资料库：王蘊（CBDB 563484）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=563484&o=json
            external_identifier: CBDB:563484
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:25.799Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_XM9QNMLmGKfYJ3GSaLi11d
        status: active
        display_name: 王蘊
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王偉

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王偉 | accepted |
| death.date | 1591年 | accepted |
| bio.summary | 王偉（卒于1591年），明人物。籍贯餘姚，曾任文思院副使、衛指揮使。（中国历代人物传记资料库 CBDB 439386） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_67QPdbGBEAuDLU32GCV69J | 王正 | accepted |
| ancestors | p_K9QEgp6JSKaffEKcTo7xe2 | 王賢 | accepted |
| ancestors | p_XM9QNMLmGKfYJ3GSaLi11d | 王蘊 | accepted |

## 外部来源

- [中国历代人物传记资料库：王偉（CBDB 439386）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=439386&o=json)
- [中国历代人物传记资料库：王賢（CBDB 563485）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=563485&o=json)
- [中国历代人物传记资料库：王蘊（CBDB 563484）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=563484&o=json)
- [中国历代人物传记资料库：王正（CBDB 563487）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=563487&o=json)
