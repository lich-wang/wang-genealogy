---
schema: wang-person/v1
id: p_nQxA1N58Gt4YqoNxaQxPCL
status: active
merged_into: null
display_name: 王鋗
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_xpgXy8uqnFeQU1YWrMRKJR
        subject_person_id: p_nQxA1N58Gt4YqoNxaQxPCL
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王鋗
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_rFoJMYqTSH9Zea9v7KhgN5
          claim_id: c_xpgXy8uqnFeQU1YWrMRKJR
          source_id: s_G9J1xRLo4LHgtz9fqrypbP
          stance: supports
          locator: CBDB:190725
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（190725）
          source: &a1
            id: s_G9J1xRLo4LHgtz9fqrypbP
            source_type: api_record
            title: 中国历代人物传记资料库：王鋗（CBDB 190725）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=190725&o=json
            external_identifier: CBDB:190725
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:16.334Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_8GnXLYQPGeJhfhwALwTGYn
        subject_person_id: p_nQxA1N58Gt4YqoNxaQxPCL
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 780年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_WZuzKTf4MJCTRzyR6vC6cK
          claim_id: c_8GnXLYQPGeJhfhwALwTGYn
          source_id: s_G9J1xRLo4LHgtz9fqrypbP
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_gCRgNvpfByRcwnjJFLpGsk
        subject_person_id: p_nQxA1N58Gt4YqoNxaQxPCL
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王鋗（卒于780年），唐人物。籍贯洛陽，曾任州刺史。（中国历代人物传记资料库 CBDB 190725）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_5CMcXLlXvYh9Zaf7zZ6pAp
          claim_id: c_gCRgNvpfByRcwnjJFLpGsk
          source_id: s_G9J1xRLo4LHgtz9fqrypbP
          stance: supports
          locator: CBDB:190725
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_o7HaH854jB_HMhvVnt9ngV
        subject_person_id: p_nQxA1N58Gt4YqoNxaQxPCL
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_nxBAZr28DeuBqx5PPMgaj5
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_ZwM77QNfgNJDmUCH7jyyW7
          claim_id: c_o7HaH854jB_HMhvVnt9ngV
          source_id: s_G9J1xRLo4LHgtz9fqrypbP
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_nxBAZr28DeuBqx5PPMgaj5
        status: active
        display_name: 王汭
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王鋗

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王鋗 | accepted |
| death.date | 780年 | accepted |
| bio.summary | 王鋗（卒于780年），唐人物。籍贯洛陽，曾任州刺史。（中国历代人物传记资料库 CBDB 190725） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_nxBAZr28DeuBqx5PPMgaj5 | 王汭 | accepted |

## 外部来源

- [中国历代人物传记资料库：王鋗（CBDB 190725）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=190725&o=json)
