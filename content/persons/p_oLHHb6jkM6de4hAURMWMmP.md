---
schema: wang-person/v1
id: p_oLHHb6jkM6de4hAURMWMmP
status: active
merged_into: null
display_name: 王綸
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_FTtCiEB9siS4H1YYdsKfDN
        subject_person_id: p_oLHHb6jkM6de4hAURMWMmP
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王綸
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_8v5P2GTcH6SZomTZSZpsYk
          claim_id: c_FTtCiEB9siS4H1YYdsKfDN
          source_id: s_QAWPGKjTx4y3o5Ayoi8ijG
          stance: supports
          locator: CBDB:305999
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（305999）
          source: &a1
            id: s_QAWPGKjTx4y3o5Ayoi8ijG
            source_type: api_record
            title: 中国历代人物传记资料库：王綸（CBDB 305999）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=305999&o=json
            external_identifier: CBDB:305999
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.749Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_EHJExhHKUDkLQq9FKVMuYj
        subject_person_id: p_oLHHb6jkM6de4hAURMWMmP
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王綸，明人物。嘉靖二十三年進士。（中国历代人物传记资料库 CBDB 305999）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_rxyGxkyBt8z2FP-u4RPXxB
          claim_id: c_EHJExhHKUDkLQq9FKVMuYj
          source_id: s_QAWPGKjTx4y3o5Ayoi8ijG
          stance: supports
          locator: CBDB:305999
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_WQ93-2zlGqqXGWlWV4uQzZ
        subject_person_id: p_oLHHb6jkM6de4hAURMWMmP
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_Znen7jNez4DsQZ8W2U88iW
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_t6fx9Ktpve1u_KXTy0xuH8
          claim_id: c_WQ93-2zlGqqXGWlWV4uQzZ
          source_id: s_QAWPGKjTx4y3o5Ayoi8ijG
          stance: supports
          locator: 嘉靖二十三年登科錄:一卷，第二甲第七十三名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_Znen7jNez4DsQZ8W2U88iW
        status: active
        display_name: 王會
        merged_into_person_id: null
  other: []
---

# 王綸

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王綸 | accepted |
| bio.summary | 王綸，明人物。嘉靖二十三年進士。（中国历代人物传记资料库 CBDB 305999） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_Znen7jNez4DsQZ8W2U88iW | 王會 | accepted |

## 外部来源

- [中国历代人物传记资料库：王綸（CBDB 305999）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=305999&o=json)
