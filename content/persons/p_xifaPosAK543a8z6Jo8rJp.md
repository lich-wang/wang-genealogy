---
schema: wang-person/v1
id: p_xifaPosAK543a8z6Jo8rJp
status: active
merged_into: null
display_name: 張氏
revision: 1
cbdb_id: 578853
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_6_d0LNwmiZV4HwIVjYYB-V
        subject_person_id: p_xifaPosAK543a8z6Jo8rJp
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 張氏，明人物。籍贯息縣。（中国历代人物传记资料库 CBDB 578853）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_qfAxalBIBz561___rDRkCV
          claim_id: c_6_d0LNwmiZV4HwIVjYYB-V
          source_id: s_NiPzvS1USboNWxs1KW5jYW
          stance: supports
          locator: CBDB:578853
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_NiPzvS1USboNWxs1KW5jYW
            source_type: api_record
            title: 中国历代人物传记资料库：張氏(王六妻)（CBDB 578853）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=578853&o=json
            external_identifier: CBDB:578853
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_O7MYmy_HQhMwpelbA0WSri
        subject_person_id: p_xifaPosAK543a8z6Jo8rJp
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 張氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_cs_03XqbDHopXcvKU3dOhS
          claim_id: c_O7MYmy_HQhMwpelbA0WSri
          source_id: s_NiPzvS1USboNWxs1KW5jYW
          stance: supports
          locator: CBDB:578853
          quotation: null
          interpretation_note: CBDB 明确记录的王六配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_AZJ7rd0b9RxfPLfkJTmQi1
        subject_person_id: p_P16ktcYxGZ96UYk84jaM8A
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_xifaPosAK543a8z6Jo8rJp
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_DzHHkiVX1abPeC71UUEEDN
          claim_id: c_AZJ7rd0b9RxfPLfkJTmQi1
          source_id: s_NiPzvS1USboNWxs1KW5jYW
          stance: supports
          locator: (康熙)息縣續志，lgid= 870146-870147：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_P16ktcYxGZ96UYk84jaM8A
        status: active
        display_name: 王六
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 張氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 張氏，明人物。籍贯息縣。（中国历代人物传记资料库 CBDB 578853） | accepted |
| name.primary | 張氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_P16ktcYxGZ96UYk84jaM8A | 王六 | accepted |

## 外部来源

- [中国历代人物传记资料库：張氏(王六妻)（CBDB 578853）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=578853&o=json)
