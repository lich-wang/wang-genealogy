---
schema: wang-person/v1
id: p_zkfNFf6VZ8candj6SdsGh8
status: active
merged_into: null
display_name: 王彬
revision: 3
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_qVZq3ENq5TA3x7CyAoCALE
        subject_person_id: p_zkfNFf6VZ8candj6SdsGh8
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王彬
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_VnHKUwHf2YHauJnHzXQCCy
          claim_id: c_qVZq3ENq5TA3x7CyAoCALE
          source_id: s_gHbN1t5KgUtafF6ATDgJaS
          stance: supports
          locator: CBDB:283314
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（283314）
          source: &a1
            id: s_gHbN1t5KgUtafF6ATDgJaS
            source_type: api_record
            title: 中国历代人物传记资料库：王彬（CBDB 283314）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=283314&o=json
            external_identifier: CBDB:283314
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.133Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_9GyQQnEU1qwWCLReBh2QSE
        subject_person_id: p_zkfNFf6VZ8candj6SdsGh8
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王彬，明人物。正德十六年進士，籍贯鄞縣。（中国历代人物传记资料库 CBDB 283314）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_LtC2xUUyCCkuy0_2oBqiXf
          claim_id: c_9GyQQnEU1qwWCLReBh2QSE
          source_id: s_gHbN1t5KgUtafF6ATDgJaS
          stance: supports
          locator: CBDB:283314
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_h3xepD0KDsKfc1fvSrqVET
        subject_person_id: p_b9S8xbPufiFgxPKXwMLGKV
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_zkfNFf6VZ8candj6SdsGh8
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Su6SMwPyftSLQ4VTiKI6qI
          claim_id: c_h3xepD0KDsKfc1fvSrqVET
          source_id: s_zPJpdvqGGGAMd0AziLLRey
          stance: supports
          locator: CBDB：兄弟 王相（68072）之父／母 王瀚
          quotation: null
          interpretation_note: 由兄弟关系推断：王彬 与 王相 为同胞（CBDB 记「兄」），王相 之父／母即 王彬 之父／母。
          source:
            id: s_zPJpdvqGGGAMd0AziLLRey
            source_type: api_record
            title: 中国历代人物传记资料库：王彬（CBDB 283314）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=283314&o=json
            external_identifier: CBDB:283314
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_b9S8xbPufiFgxPKXwMLGKV
        status: active
        display_name: 王瀚
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other:
    - claim:
        id: c_itNd3yzS52XvyFmoRA3ybh
        subject_person_id: p_tkFi7K6ReQ4K325KrqZnAe
        claim_kind: relationship
        predicate: kinship.sibling_of
        object_person_id: p_zkfNFf6VZ8candj6SdsGh8
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Q2U5sgTogKxU6EA3ATFx_L
          claim_id: c_itNd3yzS52XvyFmoRA3ybh
          source_id: s_zPJpdvqGGGAMd0AziLLRey
          stance: supports
          locator: CBDB 亲属：兄（KinPerson 68072 王相）
          quotation: null
          interpretation_note: CBDB 记录的同辈兄弟姊妹关系，按字典序规范化存储。
          source:
            id: s_zPJpdvqGGGAMd0AziLLRey
            source_type: api_record
            title: 中国历代人物传记资料库：王彬（CBDB 283314）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=283314&o=json
            external_identifier: CBDB:283314
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-14T15:51:37.715Z
            metadata_json: null
      object_person:
        id: p_tkFi7K6ReQ4K325KrqZnAe
        status: active
        display_name: 王相
        merged_into_person_id: null
---

# 王彬

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王彬 | accepted |
| bio.summary | 王彬，明人物。正德十六年進士，籍贯鄞縣。（中国历代人物传记资料库 CBDB 283314） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_b9S8xbPufiFgxPKXwMLGKV | 王瀚 | accepted |
| other | p_tkFi7K6ReQ4K325KrqZnAe | 王相 | accepted |

## 外部来源

- [中国历代人物传记资料库：王彬（CBDB 283314）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=283314&o=json)
