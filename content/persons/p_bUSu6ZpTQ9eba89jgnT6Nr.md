---
schema: wang-person/v1
id: p_bUSu6ZpTQ9eba89jgnT6Nr
status: active
merged_into: null
display_name: 潘氏
revision: 1
cbdb_id: 111536
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_stSynBKeInaBR541sOFtfy
        subject_person_id: p_bUSu6ZpTQ9eba89jgnT6Nr
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 潘氏，史料所见人物。本项目依据《中国历代人物传记资料库：潘氏(王相妻)（CBDB 111536）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs__XeeQkDCGlkTdIVK_RR9qi
          claim_id: c_stSynBKeInaBR541sOFtfy
          source_id: s_UjwqKmhbM_wkL4VP9zU_yz
          stance: supports
          locator: CBDB:111536
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_UjwqKmhbM_wkL4VP9zU_yz
            source_type: api_record
            title: 中国历代人物传记资料库：潘氏(王相妻)（CBDB 111536）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=111536&o=json
            external_identifier: CBDB:111536
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_WxShKZj7gH2tuGQDboKAVB
        subject_person_id: p_bUSu6ZpTQ9eba89jgnT6Nr
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 潘氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_1s_ZYSSZKh4jZy3KY8ZeMx
          claim_id: c_WxShKZj7gH2tuGQDboKAVB
          source_id: s_UjwqKmhbM_wkL4VP9zU_yz
          stance: supports
          locator: CBDB:111536
          quotation: null
          interpretation_note: CBDB 明确记录的王相配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_qRnAvXkLYzV9r0n-kUh2nA
        subject_person_id: p_VwzQHupke6i4zHhf3Hgb9p
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_bUSu6ZpTQ9eba89jgnT6Nr
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_tkbxmHjaVdP14aNk60eEaX
          claim_id: c_qRnAvXkLYzV9r0n-kUh2nA
          source_id: s_UjwqKmhbM_wkL4VP9zU_yz
          stance: supports
          locator: 元人傳記資料索引，12330：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_VwzQHupke6i4zHhf3Hgb9p
        status: active
        display_name: 王相
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 潘氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 潘氏，史料所见人物。本项目依据《中国历代人物传记资料库：潘氏(王相妻)（CBDB 111536）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 潘氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_VwzQHupke6i4zHhf3Hgb9p | 王相 | accepted |

## 外部来源

- [中国历代人物传记资料库：潘氏(王相妻)（CBDB 111536）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=111536&o=json)
