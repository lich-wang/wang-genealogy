---
schema: wang-person/v1
id: p_gkPpiEzsNPGEaJ8ZMcrkuN
status: active
merged_into: null
display_name: 王孝傑
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_VzYArCV569HqHEaAcNtAsQ
        subject_person_id: p_gkPpiEzsNPGEaJ8ZMcrkuN
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王孝傑
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_r6WbVNMAXmYmb1CdD99AxY
          claim_id: c_VzYArCV569HqHEaAcNtAsQ
          source_id: s_EKMKrLvVviV2guyQPDzWMJ
          stance: supports
          locator: CBDB:21974
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（21974）
          source: &a1
            id: s_EKMKrLvVviV2guyQPDzWMJ
            source_type: api_record
            title: 中国历代人物传记资料库：王孝傑（CBDB 21974）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=21974&o=json
            external_identifier: CBDB:21974
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.785Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_TEhFdGa1eAzQvpgyF781E5
        subject_person_id: p_gkPpiEzsNPGEaJ8ZMcrkuN
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王孝傑，宋人物。籍贯饒陽。（中国历代人物传记资料库 CBDB 21974）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_Vi3OY2A3Yp9AnAHt3T5pww
          claim_id: c_TEhFdGa1eAzQvpgyF781E5
          source_id: s_EKMKrLvVviV2guyQPDzWMJ
          stance: supports
          locator: CBDB:21974
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_O5MatSHKJPRJddeI_0nh34
        subject_person_id: p_NuMx6iWWSXV7F6yKDTFjP8
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_gkPpiEzsNPGEaJ8ZMcrkuN
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_dEQ7v-2oPNfxx2E7AU5kmQ
          claim_id: c_O5MatSHKJPRJddeI_0nh34
          source_id: s_EKMKrLvVviV2guyQPDzWMJ
          stance: supports
          locator: CBDB 双向互证（父 王濟 ⇄ 子 王孝傑）
          quotation: null
          interpretation_note: Luna 在线复核：CBDB 底本未知，经 Wikidata 双向核对后采纳。
          source: *a1
      object_person:
        id: p_NuMx6iWWSXV7F6yKDTFjP8
        status: active
        display_name: 王濟
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王孝傑

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王孝傑 | accepted |
| bio.summary | 王孝傑，宋人物。籍贯饒陽。（中国历代人物传记资料库 CBDB 21974） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_NuMx6iWWSXV7F6yKDTFjP8 | 王濟 | accepted |

## 外部来源

- [中国历代人物传记资料库：王孝傑（CBDB 21974）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=21974&o=json)
