---
schema: wang-person/v1
id: p_GU3s4118GxG2LN8CLNr6d3
status: active
merged_into: null
display_name: 王之栻
cbdb_id: 519659
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_r1nuevBp2UJZx2yrjdoTtC
        subject_person_id: p_GU3s4118GxG2LN8CLNr6d3
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王之栻（卒于1644年），史料所见人物。本项目依据《中国历代人物传记资料库：王之栻（CBDB 519659）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_sn5YIHdeYcIRYGXT1C8Ooz
          claim_id: c_r1nuevBp2UJZx2yrjdoTtC
          source_id: s_cdjeWtbDKRYdormNgVSx1J
          stance: supports
          locator: CBDB:519659
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_cdjeWtbDKRYdormNgVSx1J
            source_type: api_record
            title: 中国历代人物传记资料库：王之栻（CBDB 519659）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=519659&o=json
            external_identifier: CBDB:519659
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:40.348Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_9uJ7gFQVtP16qWCJcFeL4A
        subject_person_id: p_GU3s4118GxG2LN8CLNr6d3
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 1644年
            calendar_note: CBDB 卒年字段，精度：年
            earliest: 1644-01-01
            latest: 1644-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_UyqPF1YQHAi1hjVJWiW5pu
          claim_id: c_9uJ7gFQVtP16qWCJcFeL4A
          source_id: s_cdjeWtbDKRYdormNgVSx1J
          stance: supports
          locator: CBDB:519659
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（501-600）｜历史性依据：CBDB 卒年 = 1644
          source:
            id: s_cdjeWtbDKRYdormNgVSx1J
            source_type: api_record
            title: 中国历代人物传记资料库：王之栻（CBDB 519659）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=519659&o=json
            external_identifier: CBDB:519659
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:40.348Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_3g1Cxxka5MyosmteJXUy7d
        subject_person_id: p_GU3s4118GxG2LN8CLNr6d3
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王之栻
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_hQQNooNSeEa5UZr8iUbnue
          claim_id: c_3g1Cxxka5MyosmteJXUy7d
          source_id: s_cdjeWtbDKRYdormNgVSx1J
          stance: supports
          locator: CBDB:519659
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（501-600）｜历史性依据：CBDB 卒年 = 1644
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_klopVtnYaG4qNMNl1fUAyq
        subject_person_id: p_5zJGY1xRx85XhFi5qyGy8X
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_GU3s4118GxG2LN8CLNr6d3
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_wuGjAxVD1thGRZ8otYBH84
          claim_id: c_klopVtnYaG4qNMNl1fUAyq
          source_id: s_cdjeWtbDKRYdormNgVSx1J
          stance: supports
          locator: 武進陽湖縣志，lgid=152332：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_cdjeWtbDKRYdormNgVSx1J
            source_type: api_record
            title: 中国历代人物传记资料库：王之栻（CBDB 519659）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=519659&o=json
            external_identifier: CBDB:519659
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:40.348Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_5zJGY1xRx85XhFi5qyGy8X
        status: active
        display_name: 王章
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王之栻

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王之栻（卒于1644年），史料所见人物。本项目依据《中国历代人物传记资料库：王之栻（CBDB 519659）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| death.date | 1644年 | accepted |
| name.primary | 王之栻 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_5zJGY1xRx85XhFi5qyGy8X | 王章 | accepted |

## 外部来源

- [中国历代人物传记资料库：王之栻（CBDB 519659）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=519659&o=json)
