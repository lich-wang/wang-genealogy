---
schema: wang-person/v1
id: p_6BymFd7sw6j7GAmg7erqPj
status: active
merged_into: null
display_name: 王智兴
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_RuDUH7ThxKN8HNysDsYXjB
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王智兴（758年—836年），唐人物。CBDB 记录其籍贯记录为懷州，身份包括詩人，曾任節度副使、節度使。中国历代人物传记资料库（CBDB）以人物编号 92090 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_aHtowQ1aacJr7Qgq9Rmp6q
          claim_id: c_RuDUH7ThxKN8HNysDsYXjB
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source: &a2
            id: s_YtiuJ4x6oyDJbCvjY9nQK7
            source_type: api_record
            title: 维基数据：王智兴（Q7967776）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7967776
            external_identifier: Q7967776
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:24.494Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%99%BA%E8%88%88
        - id: cs_6qespEI0B0BJ50J91amgnJ
          claim_id: c_RuDUH7ThxKN8HNysDsYXjB
          source_id: s_wkTaSHWKF3X87qQ4xAXoWJ
          stance: supports
          locator: CBDB:92090
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_wkTaSHWKF3X87qQ4xAXoWJ
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王智興（92090）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=92090&o=json
            external_identifier: CBDB:92090
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-05T01:59:24.638Z
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_9rG2Lagv13A9unjNJqUhW3
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 758年
            calendar_note: 维基数据 P569 结构化日期，精度：年
            earliest: 0758-01-01
            latest: 0758-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_zHVDcXe8zaLVQa7jYgYVBq
          claim_id: c_9rG2Lagv13A9unjNJqUhW3
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_YtiuJ4x6oyDJbCvjY9nQK7
            source_type: api_record
            title: 维基数据：王智兴（Q7967776）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7967776
            external_identifier: Q7967776
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:24.494Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%99%BA%E8%88%88
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_ZF8ggg77ttCv2GLzYyxNRd
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 836年
            calendar_note: 维基数据 P570 结构化日期，精度：年
            earliest: 0836-01-01
            latest: 0836-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_R6z7718cXrwF4ezJLzEqBq
          claim_id: c_ZF8ggg77ttCv2GLzYyxNRd
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_YtiuJ4x6oyDJbCvjY9nQK7
            source_type: api_record
            title: 维基数据：王智兴（Q7967776）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7967776
            external_identifier: Q7967776
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:24.494Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%99%BA%E8%88%88
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_ucKDYsh9ALE7dj9h1Xw3gf
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王智兴
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_k2zEqPRwHwwsW45thvRx1W
          claim_id: c_ucKDYsh9ALE7dj9h1Xw3gf
          source_id: s_wkTaSHWKF3X87qQ4xAXoWJ
          stance: supports
          locator: Q7967776
          quotation: null
          interpretation_note: null
          source: *a1
        - id: cs_qf67wEhjfGDnB27TVXv63y
          claim_id: c_ucKDYsh9ALE7dj9h1Xw3gf
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: Q7967776
          quotation: null
          interpretation_note: null
          source:
            id: s_YtiuJ4x6oyDJbCvjY9nQK7
            source_type: api_record
            title: 维基数据：王智兴（Q7967776）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7967776
            external_identifier: Q7967776
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:24.494Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%99%BA%E8%88%88
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_eVprui53fbC5CpcQeLhfFK
        subject_person_id: p_mLR2LwV5BkQKiVBVi28oBG
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_6BymFd7sw6j7GAmg7erqPj
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_sXrcCSsNtq6CDVcGsiCHk6
          claim_id: c_eVprui53fbC5CpcQeLhfFK
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source: *a2
        - id: cs_KAH1x6dcCHss8k1aXNi2sc
          claim_id: c_eVprui53fbC5CpcQeLhfFK
          source_id: s_5qmAs78nS9KmbmEN8TQgud
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_5qmAs78nS9KmbmEN8TQgud
            source_type: api_record
            title: 维基数据：王缙（Q45603209）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45603209
            external_identifier: Q45603209
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:40.541Z
            metadata_json: null
        - id: cs_gKcUuYpmbBGexoMzBh4rhD
          claim_id: c_eVprui53fbC5CpcQeLhfFK
          source_id: s_RPpZiHDWSHwgmepChP8AMg
          stance: supports
          locator: 亲属关系：子
          quotation: null
          interpretation_note: CBDB 注明此条来源：唐五代人物傳記與社會網絡資料庫(1.0版)
          source:
            id: s_RPpZiHDWSHwgmepChP8AMg
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王縉（184938）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=184938&o=json
            external_identifier: CBDB:184938
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-05T02:06:40.685Z
            metadata_json: null
      object_person:
        id: p_mLR2LwV5BkQKiVBVi28oBG
        status: active
        display_name: 王缙
        merged_into_person_id: null
  children:
    - claim:
        id: c_536TAk2iV2zhS8quJuep4j
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: relationship
        predicate: kinship.parent_of
        object_person_id: p_d5wrqNKm2UtzkniGD7eLTC
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_MP5MUnAox7eCj3SiREZ3gD
          claim_id: c_536TAk2iV2zhS8quJuep4j
          source_id: s_ro1QX8Adonbc7TpcZqY3Cc
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_ro1QX8Adonbc7TpcZqY3Cc
            source_type: api_record
            title: 维基数据：王晏平（Q15901012）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15901012
            external_identifier: Q15901012
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:46:30.188Z
            metadata_json: null
        - id: cs_b5JBKEm91WtJWCmv6KCBTX
          claim_id: c_536TAk2iV2zhS8quJuep4j
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_YtiuJ4x6oyDJbCvjY9nQK7
            source_type: api_record
            title: 维基数据：王智兴（Q7967776）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7967776
            external_identifier: Q7967776
            license_code: CC0-1.0
            accessed_at: 2026-09-05T01:59:24.494Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%99%BA%E8%88%88
      object_person:
        id: p_d5wrqNKm2UtzkniGD7eLTC
        status: active
        display_name: 王晏平
        merged_into_person_id: null
    - claim:
        id: c_yERBvf92HxFK8KzHTfhQmQ
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_HgVy44kEGbjeLMZt16FcbN
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_7pbo63HMpaMgmYYD3MdUSv
          claim_id: c_yERBvf92HxFK8KzHTfhQmQ
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source: *a2
        - id: cs_Fg4FvgpPz5x66bFGTPFCi6
          claim_id: c_yERBvf92HxFK8KzHTfhQmQ
          source_id: s_QFwegxSRo2mTANEr9BMeM8
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_QFwegxSRo2mTANEr9BMeM8
            source_type: api_record
            title: 维基数据：王宰（Q7967755）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7967755
            external_identifier: Q7967755
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:22.442Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%AE%B0_(%E8%8A%82%E5%BA%A6%E4%BD%BF)
      object_person:
        id: p_HgVy44kEGbjeLMZt16FcbN
        status: active
        display_name: 王宰
        merged_into_person_id: null
    - claim:
        id: c_wJe1Ms67ndSnbsHv3bJwT8
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_5Drssue4KRkK7sPyMEaQ5G
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_5LLkBE7LUQ5q8wSLNsqCSE
          claim_id: c_wJe1Ms67ndSnbsHv3bJwT8
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source: *a2
        - id: cs_p21rmv4TCgCVQoGqS2oA5B
          claim_id: c_wJe1Ms67ndSnbsHv3bJwT8
          source_id: s_PxZ6xSiD3QFbbKsp31BkGr
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_PxZ6xSiD3QFbbKsp31BkGr
            source_type: api_record
            title: 维基数据：王晏皋（Q45603470）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45603470
            external_identifier: Q45603470
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:40.541Z
            metadata_json: null
      object_person:
        id: p_5Drssue4KRkK7sPyMEaQ5G
        status: active
        display_name: 王晏皋
        merged_into_person_id: null
    - claim:
        id: c_sXn8d6QyevbFAstEWTjrZC
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_iVXKWi9f5gNuEVGc79EzBF
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_8dCaCHvE76S94LX6jPnxKF
          claim_id: c_sXn8d6QyevbFAstEWTjrZC
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source: *a2
        - id: cs_w9aAxAcvMPDKHv2geMWVtt
          claim_id: c_sXn8d6QyevbFAstEWTjrZC
          source_id: s_64XeMFRrpqodXsZKs2NNK9
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_64XeMFRrpqodXsZKs2NNK9
            source_type: api_record
            title: 维基数据：王晏宝（Q45603535）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45603535
            external_identifier: Q45603535
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:40.541Z
            metadata_json: null
      object_person:
        id: p_iVXKWi9f5gNuEVGc79EzBF
        status: active
        display_name: 王晏宝
        merged_into_person_id: null
    - claim:
        id: c_UzGjYXXG4oefKRQ8MJD8QA
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_L9wgabBW76ENxwGEnK8E56
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_cryJ7Y4ijTQgV6X9fSwPbc
          claim_id: c_UzGjYXXG4oefKRQ8MJD8QA
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source: *a2
        - id: cs_r6X9LYDLhB58z1VPSkp9ia
          claim_id: c_UzGjYXXG4oefKRQ8MJD8QA
          source_id: s_M6pT5SJYio7TACr5zCkNku
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_M6pT5SJYio7TACr5zCkNku
            source_type: api_record
            title: 维基数据：王晏恭（Q45603597）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45603597
            external_identifier: Q45603597
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:42.360Z
            metadata_json: null
      object_person:
        id: p_L9wgabBW76ENxwGEnK8E56
        status: active
        display_name: 王晏恭
        merged_into_person_id: null
    - claim:
        id: c_1rpGMfr87ggrG5rCXueQLK
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_o59djP1edkTV42VCk3GSN1
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_XSQHGRfpJAgwNxUK9XLB6h
          claim_id: c_1rpGMfr87ggrG5rCXueQLK
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source: *a2
        - id: cs_EVSeBoRL4KJ15YGQRv1m54
          claim_id: c_1rpGMfr87ggrG5rCXueQLK
          source_id: s_dXC5zvYcXBqVgQbw4AHiux
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_dXC5zvYcXBqVgQbw4AHiux
            source_type: api_record
            title: 维基数据：王晏逸（Q45603663）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45603663
            external_identifier: Q45603663
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:42.360Z
            metadata_json: null
      object_person:
        id: p_o59djP1edkTV42VCk3GSN1
        status: active
        display_name: 王晏逸
        merged_into_person_id: null
    - claim:
        id: c_aRnEy98Ss7TLaDNajYmE41
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_8ptoyYXrGsuZM3gms5pv3F
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_4AoByNgWRb4Yj6np31LJk5
          claim_id: c_aRnEy98Ss7TLaDNajYmE41
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source: *a2
        - id: cs_DpXFvysBFHeDYCHo7nLz8W
          claim_id: c_aRnEy98Ss7TLaDNajYmE41
          source_id: s_CFmM6hwyzMR4e2AUk9sQEA
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_CFmM6hwyzMR4e2AUk9sQEA
            source_type: api_record
            title: 维基数据：王晏深（Q45603730）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45603730
            external_identifier: Q45603730
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:42.360Z
            metadata_json: null
      object_person:
        id: p_8ptoyYXrGsuZM3gms5pv3F
        status: active
        display_name: 王晏深
        merged_into_person_id: null
    - claim:
        id: c_Mrz8jY49Dq82f4Ao56pKUK
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_hrNsSN4wVj9jcL4eC2GhRW
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_7mBpFnDk47ye1HeHQHWsHL
          claim_id: c_Mrz8jY49Dq82f4Ao56pKUK
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source: *a2
        - id: cs_Hgg5oSQQ3FkWMx4VnhEDWD
          claim_id: c_Mrz8jY49Dq82f4Ao56pKUK
          source_id: s_NbnvMKFf1XpDayAMaJZZwv
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_NbnvMKFf1XpDayAMaJZZwv
            source_type: api_record
            title: 维基数据：王晏斌（Q45603793）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45603793
            external_identifier: Q45603793
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:42.360Z
            metadata_json: null
      object_person:
        id: p_hrNsSN4wVj9jcL4eC2GhRW
        status: active
        display_name: 王晏斌
        merged_into_person_id: null
    - claim:
        id: c_5rNvdZFkprG56UKdQqDnjm
        subject_person_id: p_6BymFd7sw6j7GAmg7erqPj
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_Un1qCeghV1uPA98yeCmoYH
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_DRf7kLxnPNDq4uvQfSkzzZ
          claim_id: c_5rNvdZFkprG56UKdQqDnjm
          source_id: s_YtiuJ4x6oyDJbCvjY9nQK7
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source: *a2
        - id: cs_sHoyT2PH3P8ems1Lbear5v
          claim_id: c_5rNvdZFkprG56UKdQqDnjm
          source_id: s_5rxPhLzwuoKL3pvKpB4AUq
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_5rxPhLzwuoKL3pvKpB4AUq
            source_type: api_record
            title: 维基数据：王晏韬（Q45603856）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q45603856
            external_identifier: Q45603856
            license_code: CC0-1.0
            accessed_at: 2026-09-05T02:06:44.307Z
            metadata_json: null
      object_person:
        id: p_Un1qCeghV1uPA98yeCmoYH
        status: active
        display_name: 王晏韬
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王智兴

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王智兴（758年—836年），唐人物。CBDB 记录其籍贯记录为懷州，身份包括詩人，曾任節度副使、節度使。中国历代人物传记资料库（CBDB）以人物编号 92090 收录其独立传记记录；未列明的生平细节仍待可靠史料补充。 | accepted |
| birth.date | 758年 | accepted |
| death.date | 836年 | accepted |
| name.primary | 王智兴 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_mLR2LwV5BkQKiVBVi28oBG | 王缙 | accepted |
| children | p_d5wrqNKm2UtzkniGD7eLTC | 王晏平 | accepted |
| children | p_HgVy44kEGbjeLMZt16FcbN | 王宰 | accepted |
| children | p_5Drssue4KRkK7sPyMEaQ5G | 王晏皋 | accepted |
| children | p_iVXKWi9f5gNuEVGc79EzBF | 王晏宝 | accepted |
| children | p_L9wgabBW76ENxwGEnK8E56 | 王晏恭 | accepted |
| children | p_o59djP1edkTV42VCk3GSN1 | 王晏逸 | accepted |
| children | p_8ptoyYXrGsuZM3gms5pv3F | 王晏深 | accepted |
| children | p_hrNsSN4wVj9jcL4eC2GhRW | 王晏斌 | accepted |
| children | p_Un1qCeghV1uPA98yeCmoYH | 王晏韬 | accepted |

## 外部来源

- [维基数据：王缙（Q45603209）](https://www.wikidata.org/wiki/Q45603209)
- [维基数据：王晏宝（Q45603535）](https://www.wikidata.org/wiki/Q45603535)
- [维基数据：王晏斌（Q45603793）](https://www.wikidata.org/wiki/Q45603793)
- [维基数据：王晏皋（Q45603470）](https://www.wikidata.org/wiki/Q45603470)
- [维基数据：王晏恭（Q45603597）](https://www.wikidata.org/wiki/Q45603597)
- [维基数据：王晏平（Q15901012）](https://www.wikidata.org/wiki/Q15901012)
- [维基数据：王晏深（Q45603730）](https://www.wikidata.org/wiki/Q45603730)
- [维基数据：王晏韬（Q45603856）](https://www.wikidata.org/wiki/Q45603856)
- [维基数据：王晏逸（Q45603663）](https://www.wikidata.org/wiki/Q45603663)
- [维基数据：王宰（Q7967755）](https://www.wikidata.org/wiki/Q7967755)
- [维基数据：王智兴（Q7967776）](https://www.wikidata.org/wiki/Q7967776)
- [CBDB 中国历代人物传记资料库：王縉（184938）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=184938&o=json)
- [CBDB 中国历代人物传记资料库：王智興（92090）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=92090&o=json)
