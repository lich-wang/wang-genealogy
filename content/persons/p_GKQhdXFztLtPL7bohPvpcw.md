---
schema: wang-person/v1
id: p_GKQhdXFztLtPL7bohPvpcw
status: active
merged_into: null
display_name: 王猛
revision: 3
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_7C8JYJ7FykkdcmafJAS85b
        subject_person_id: p_GKQhdXFztLtPL7bohPvpcw
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王猛
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_87mmAj9bfPZTTxQ2tE5Bje
          claim_id: c_7C8JYJ7FykkdcmafJAS85b
          source_id: s_hjv4vhKBr5cWXCtBcBEfSR
          stance: supports
          locator: CBDB:175662
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（175662）
          source: &a1
            id: s_hjv4vhKBr5cWXCtBcBEfSR
            source_type: api_record
            title: 中国历代人物传记资料库：王猛（CBDB 175662）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175662&o=json
            external_identifier: CBDB:175662
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:16.048Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_e43xbA4JscxXTmKgPYrogR
        subject_person_id: p_GKQhdXFztLtPL7bohPvpcw
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 552年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_fmuDRHFt1LzDr3QCUevg6H
          claim_id: c_e43xbA4JscxXTmKgPYrogR
          source_id: s_hjv4vhKBr5cWXCtBcBEfSR
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_NaGLePAYFtSLDfcc6Dh15b
        subject_person_id: p_GKQhdXFztLtPL7bohPvpcw
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王猛（卒于552年），南北朝人物。籍贯琅琊，曾任州刺史。（中国历代人物传记资料库 CBDB 175662）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_RNOwFmxc1hbySJjma9VXLO
          claim_id: c_NaGLePAYFtSLDfcc6Dh15b
          source_id: s_hjv4vhKBr5cWXCtBcBEfSR
          stance: supports
          locator: CBDB:175662
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_D6g9DavvXR46WMhJCnRmWA
        subject_person_id: p_GKQhdXFztLtPL7bohPvpcw
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_bKjdjLf8x3GHpbMR6Z5dP8
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_eFWoGA8kgvcFjY_wNYy695
          claim_id: c_D6g9DavvXR46WMhJCnRmWA
          source_id: s_u6q6Kmo7hHqqGKn8wgPP6G
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_u6q6Kmo7hHqqGKn8wgPP6G
            source_type: api_record
            title: 中国历代人物传记资料库：王纊（CBDB 175664）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175664&o=json
            external_identifier: CBDB:175664
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-08T23:54:04.243Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_bKjdjLf8x3GHpbMR6Z5dP8
        status: active
        display_name: 王纊
        merged_into_person_id: null
    - claim:
        id: c_98q4Yjufpd99xkFsxt20De
        subject_person_id: p_GKQhdXFztLtPL7bohPvpcw
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_Ge7FEkGdRYL2w8reRpexBN
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_1RWFprHnqiJyuAntW8SB00
          claim_id: c_98q4Yjufpd99xkFsxt20De
          source_id: s_eFXLCWnjE1pmP89LGYRHj2
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source:
            id: s_eFXLCWnjE1pmP89LGYRHj2
            source_type: api_record
            title: 中国历代人物传记资料库：王繕（CBDB 175663）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175663&o=json
            external_identifier: CBDB:175663
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:16.049Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person:
        id: p_Ge7FEkGdRYL2w8reRpexBN
        status: active
        display_name: 王繕
        merged_into_person_id: null
    - claim:
        id: c_u0LFAwSk2Qcr3DNvo6biR6
        subject_person_id: p_GKQhdXFztLtPL7bohPvpcw
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_oVGxgeqgB52Zexa9NQq2Wt
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_jqGKAZaZ89pR0C8Z5M6tii
          claim_id: c_u0LFAwSk2Qcr3DNvo6biR6
          source_id: s_hjv4vhKBr5cWXCtBcBEfSR
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_oVGxgeqgB52Zexa9NQq2Wt
        status: active
        display_name: 王續
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王猛

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王猛 | accepted |
| death.date | 552年 | accepted |
| bio.summary | 王猛（卒于552年），南北朝人物。籍贯琅琊，曾任州刺史。（中国历代人物传记资料库 CBDB 175662） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_bKjdjLf8x3GHpbMR6Z5dP8 | 王纊 | accepted |
| children | p_Ge7FEkGdRYL2w8reRpexBN | 王繕 | accepted |
| children | p_oVGxgeqgB52Zexa9NQq2Wt | 王續 | accepted |

## 外部来源

- [中国历代人物传记资料库：王纊（CBDB 175664）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175664&o=json)
- [中国历代人物传记资料库：王猛（CBDB 175662）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175662&o=json)
- [中国历代人物传记资料库：王繕（CBDB 175663）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175663&o=json)
