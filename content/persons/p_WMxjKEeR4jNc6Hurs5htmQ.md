---
schema: wang-person/v1
id: p_WMxjKEeR4jNc6Hurs5htmQ
status: active
merged_into: null
display_name: 王輔
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_uQkk47Wfsub5rYFoKX5hc3
        subject_person_id: p_WMxjKEeR4jNc6Hurs5htmQ
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王輔
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_LLDa9PF26Q48WGHMmMJ3o7
          claim_id: c_uQkk47Wfsub5rYFoKX5hc3
          source_id: s_JjTNW9ngBNw21W8aLCQf2b
          stance: supports
          locator: CBDB:27384
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（27384）
          source: &a1
            id: s_JjTNW9ngBNw21W8aLCQf2b
            source_type: api_record
            title: 中国历代人物传记资料库：王輔（CBDB 27384）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=27384&o=json
            external_identifier: CBDB:27384
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.956Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_xJy1NFCmZ25WXYa2AvUTKG
        subject_person_id: p_WMxjKEeR4jNc6Hurs5htmQ
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王輔，宋人物。籍贯德安，入仕進士。（中国历代人物传记资料库 CBDB 27384）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_5m6tfpEGEtvzYdOTa8PL-F
          claim_id: c_xJy1NFCmZ25WXYa2AvUTKG
          source_id: s_JjTNW9ngBNw21W8aLCQf2b
          stance: supports
          locator: CBDB:27384
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_XKOEWu8ruV_Mojm9GqjBTa
        subject_person_id: p_xzPfgDfv5g3iErqsXbyN9W
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_WMxjKEeR4jNc6Hurs5htmQ
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_bfGAW71aMDotgaPpApAFxj
          claim_id: c_XKOEWu8ruV_Mojm9GqjBTa
          source_id: s_JjTNW9ngBNw21W8aLCQf2b
          stance: supports
          locator: CBDB 双向互证（父 王延 ⇄ 子 王輔）
          quotation: null
          interpretation_note: Luna 在线复核：CBDB 底本未知，经 Wikidata 双向核对后采纳。
          source: *a1
      object_person:
        id: p_xzPfgDfv5g3iErqsXbyN9W
        status: active
        display_name: 王延
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王輔

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王輔 | accepted |
| bio.summary | 王輔，宋人物。籍贯德安，入仕進士。（中国历代人物传记资料库 CBDB 27384） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_xzPfgDfv5g3iErqsXbyN9W | 王延 | accepted |

## 外部来源

- [中国历代人物传记资料库：王輔（CBDB 27384）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=27384&o=json)
