---
schema: wang-person/v1
id: p_ma56j3xFL2iP3FzuSAfpHk
status: active
merged_into: null
display_name: 王政君
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_W6MiPS9CLMpWL2CxrkTRu4
        subject_person_id: p_ma56j3xFL2iP3FzuSAfpHk
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 汉元帝皇后、汉成帝生母，为王莽姑母。身居后位六十余年，享年约八十四岁，是中国历史上在位最久的皇后之一。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_pu134NqN6ym7t46ZQc23J5
          claim_id: c_W6MiPS9CLMpWL2CxrkTRu4
          source_id: s_FD9JyVHjRLEPiULBeuVyR5
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_FD9JyVHjRLEPiULBeuVyR5
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王政君
            external_identifier: Q701379
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_GB6avFch9NXemiTUAXG4UJ
        subject_person_id: p_ma56j3xFL2iP3FzuSAfpHk
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 前71年（本始三年）
            calendar_note: null
            earliest: null
            latest: null
            precision: unknown
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_v1kRzJ6brDo7962vAoe1Mg
          claim_id: c_GB6avFch9NXemiTUAXG4UJ
          source_id: s_FD9JyVHjRLEPiULBeuVyR5
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_FD9JyVHjRLEPiULBeuVyR5
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王政君
            external_identifier: Q701379
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_aXjSJbHKma7yQvk5xYCKt9
        subject_person_id: p_ma56j3xFL2iP3FzuSAfpHk
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 13年2月3日
            calendar_note: null
            earliest: null
            latest: null
            precision: unknown
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_ECstKteuRN4HUbYyWJtBzm
          claim_id: c_aXjSJbHKma7yQvk5xYCKt9
          source_id: s_FD9JyVHjRLEPiULBeuVyR5
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_FD9JyVHjRLEPiULBeuVyR5
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王政君
            external_identifier: Q701379
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: name.alias
    recommended:
      claim:
        id: c_A7D89hiLa4uqp1kBXiCGpG
        subject_person_id: p_ma56j3xFL2iP3FzuSAfpHk
        claim_kind: property
        predicate: name.alias
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: Wang Zhengjun
          language: zh-Latn-pinyin
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_YmhGV2Kry52vFHLNYLnVuC
          claim_id: c_A7D89hiLa4uqp1kBXiCGpG
          source_id: s_FD9JyVHjRLEPiULBeuVyR5
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_FD9JyVHjRLEPiULBeuVyR5
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王政君
            external_identifier: Q701379
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_SgAs57881U9NjZDsnyNBU4
        subject_person_id: p_ma56j3xFL2iP3FzuSAfpHk
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王政君
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_3keSkyEHG4SSKiX2B3tGzv
          claim_id: c_SgAs57881U9NjZDsnyNBU4
          source_id: s_FD9JyVHjRLEPiULBeuVyR5
          stance: supports
          locator: Q701379
          quotation: null
          interpretation_note: null
          source:
            id: s_FD9JyVHjRLEPiULBeuVyR5
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王政君
            external_identifier: Q701379
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: place.origin
    recommended:
      claim:
        id: c_9svZPVWELaBbu7SC57qAD1
        subject_person_id: p_ma56j3xFL2iP3FzuSAfpHk
        claim_kind: property
        predicate: place.origin
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 魏郡元城县（今河北省大名县）
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_62GZyviadLsFGGih811JXk
          claim_id: c_9svZPVWELaBbu7SC57qAD1
          source_id: s_FD9JyVHjRLEPiULBeuVyR5
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_FD9JyVHjRLEPiULBeuVyR5
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王政君
            external_identifier: Q701379
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_7rTFPgia1feLQZcny9nrR1
        subject_person_id: p_xN7tAHKk6W68mdWgNxHCZz
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_ma56j3xFL2iP3FzuSAfpHk
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_cK1hDQVVhY6byj7usMB334
          claim_id: c_7rTFPgia1feLQZcny9nrR1
          source_id: s_FD9JyVHjRLEPiULBeuVyR5
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_FD9JyVHjRLEPiULBeuVyR5
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王政君
            external_identifier: Q701379
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_yAJk8tcH4VH9fYeodAwRKX
          claim_id: c_7rTFPgia1feLQZcny9nrR1
          source_id: s_C7hWM7fhFjjaRFXePJV6V9
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_C7hWM7fhFjjaRFXePJV6V9
            source_type: api_record
            title: 維基數據：王政君（Q701379）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q701379
            external_identifier: Q701379
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:52.776Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%94%BF%E5%90%9B
        - id: cs_gZvanfxX6Cg7YwbQ53L1Bs
          claim_id: c_7rTFPgia1feLQZcny9nrR1
          source_id: s_6FVNxhGNZGZenpLdtfzxFq
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_6FVNxhGNZGZenpLdtfzxFq
            source_type: api_record
            title: 維基數據：王禁（Q2979420）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q2979420
            external_identifier: Q2979420
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:27.787Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%A6%81
        - id: cs_eoLnss1QDeywWrFJZgKe8F
          claim_id: c_7rTFPgia1feLQZcny9nrR1
          source_id: s_eShAADUfLgra9oAH2NVuaE
          stance: supports
          locator: 条文：父
          quotation: 父王禁，母李親
          interpretation_note: null
          source:
            id: s_eShAADUfLgra9oAH2NVuaE
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%94%BF%E5%90%9B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:25:13.064Z
            metadata_json: null
        - id: cs_JCnnGGQ4Ma26JMkmfVf6Kr
          claim_id: c_7rTFPgia1feLQZcny9nrR1
          source_id: s_fdwDmmP75ocP2wH96WxNK5
          stance: supports
          locator: 条文：父
          quotation: 父王禁，母李親
          interpretation_note: null
          source:
            id: s_fdwDmmP75ocP2wH96WxNK5
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%94%BF%E5%90%9B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:54:28.935Z
            metadata_json: null
      object_person:
        id: p_xN7tAHKk6W68mdWgNxHCZz
        status: active
        display_name: 王禁
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_mW8gjKsdHMpsehtNyQBLRw
        subject_person_id: p_ma56j3xFL2iP3FzuSAfpHk
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_uKaMLxEP9pJu7gJCicb8eT
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 4
      sources:
        - id: cs_TGxT96RM3jG1qHhY7vUjME
          claim_id: c_mW8gjKsdHMpsehtNyQBLRw
          source_id: s_C7hWM7fhFjjaRFXePJV6V9
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_C7hWM7fhFjjaRFXePJV6V9
            source_type: api_record
            title: 維基數據：王政君（Q701379）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q701379
            external_identifier: Q701379
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:52.776Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%94%BF%E5%90%9B
        - id: cs_VDZmbGFtBzWi7XP9vD985f
          claim_id: c_mW8gjKsdHMpsehtNyQBLRw
          source_id: s_WLN13BPzEGAu7khaiim8UW
          stance: supports
          locator: 亲属关系：丈夫
          quotation: null
          interpretation_note: CBDB 注明此条来源：明清婦女著作數據庫
          source:
            id: s_WLN13BPzEGAu7khaiim8UW
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王政君（135028）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=135028&o=json
            external_identifier: CBDB:135028
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:16.614Z
            metadata_json: null
        - id: cs_opmVvevFDVjQe6ERuFLzsD
          claim_id: c_mW8gjKsdHMpsehtNyQBLRw
          source_id: s_YeMvVQSCqJeSwAtk2nP7kb
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_YeMvVQSCqJeSwAtk2nP7kb
            source_type: api_record
            title: 维基数据：汉元帝（Q7232）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q7232
            external_identifier: Q7232
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:48:36.997Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E6%B1%89%E5%85%83%E5%B8%9D
        - id: cs_i8b8tR24qwRwJtcMEUURWj
          claim_id: c_mW8gjKsdHMpsehtNyQBLRw
          source_id: s_Qb4QPKxAESFyxeAB2NtXJm
          stance: supports
          locator: 亲属关系：妻子
          quotation: null
          interpretation_note: CBDB 注明此条来源：明清婦女著作數據庫
          source:
            id: s_Qb4QPKxAESFyxeAB2NtXJm
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：劉奭（135026）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=135026&o=json
            external_identifier: CBDB:135026
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:48:37.139Z
            metadata_json: null
        - id: cs_Ux7CjwVUuMx9Ti4kGr16Mj
          claim_id: c_mW8gjKsdHMpsehtNyQBLRw
          source_id: s_eShAADUfLgra9oAH2NVuaE
          stance: supports
          locator: 条文：信息框 夫
          quotation: 高宗孝元皇帝劉奭
          interpretation_note: null
          source:
            id: s_eShAADUfLgra9oAH2NVuaE
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%94%BF%E5%90%9B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:25:13.064Z
            metadata_json: null
        - id: cs_TShTvejSxmyhxaBwT79Exn
          claim_id: c_mW8gjKsdHMpsehtNyQBLRw
          source_id: s_fdwDmmP75ocP2wH96WxNK5
          stance: supports
          locator: 条文：信息框 夫
          quotation: 高宗孝元皇帝劉奭
          interpretation_note: null
          source:
            id: s_fdwDmmP75ocP2wH96WxNK5
            source_type: website
            title: 中文维基百科：王政君
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%94%BF%E5%90%9B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:54:28.935Z
            metadata_json: null
      object_person:
        id: p_uKaMLxEP9pJu7gJCicb8eT
        status: active
        display_name: 劉奭
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 王政君

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 汉元帝皇后、汉成帝生母，为王莽姑母。身居后位六十余年，享年约八十四岁，是中国历史上在位最久的皇后之一。 | accepted |
| birth.date | 前71年（本始三年） | accepted |
| death.date | 13年2月3日 | accepted |
| name.alias | Wang Zhengjun | accepted |
| name.primary | 王政君 | accepted |
| place.origin | 魏郡元城县（今河北省大名县） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_xN7tAHKk6W68mdWgNxHCZz | 王禁 | accepted |
| spouses | p_uKaMLxEP9pJu7gJCicb8eT | 劉奭 | accepted |

## 外部来源

- [维基数据：汉元帝（Q7232）](https://www.wikidata.org/wiki/Q7232)
- [維基數據：王禁（Q2979420）](https://www.wikidata.org/wiki/Q2979420)
- [維基數據：王政君（Q701379）](https://www.wikidata.org/wiki/Q701379)
- [中文维基百科：王政君](https://zh.wikipedia.org/wiki/王政君)
- [中文维基百科：王政君](https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%94%BF%E5%90%9B)
- [CBDB 中国历代人物传记资料库：劉奭（135026）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=135026&o=json)
- [CBDB 中国历代人物传记资料库：王政君（135028）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=135028&o=json)
