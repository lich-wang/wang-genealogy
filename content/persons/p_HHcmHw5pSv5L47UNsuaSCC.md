---
schema: wang-person/v1
id: p_HHcmHw5pSv5L47UNsuaSCC
status: active
merged_into: null
display_name: 王藹
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_cqx524a7L7heQCttYA6pq4
        subject_person_id: p_HHcmHw5pSv5L47UNsuaSCC
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王藹
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_mi4YVynyaQa7M4zGYo6LFq
          claim_id: c_cqx524a7L7heQCttYA6pq4
          source_id: s_5NpUbDuQPXWq4a97FALFaP
          stance: supports
          locator: CBDB:175736
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（175736）
          source: &a1
            id: s_5NpUbDuQPXWq4a97FALFaP
            source_type: api_record
            title: 中国历代人物传记资料库：王藹（CBDB 175736）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175736&o=json
            external_identifier: CBDB:175736
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:16.089Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_P8XfR3MDttTUgvUVCuer9b
        subject_person_id: p_HHcmHw5pSv5L47UNsuaSCC
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 843年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_9DkRfZ2Yicqtpxpc3XW3A9
          claim_id: c_P8XfR3MDttTUgvUVCuer9b
          source_id: s_5NpUbDuQPXWq4a97FALFaP
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_FpM6vHanVD4wkh5gWCqNZt
        subject_person_id: p_HHcmHw5pSv5L47UNsuaSCC
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王藹（卒于843年），唐人物。籍贯北芒山。（中国历代人物传记资料库 CBDB 175736）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_vdbbTPPlsM5MuQo29AjlLz
          claim_id: c_FpM6vHanVD4wkh5gWCqNZt
          source_id: s_5NpUbDuQPXWq4a97FALFaP
          stance: supports
          locator: CBDB:175736
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_SsrbjEWqxD10xWU743XdfQ
        subject_person_id: p_wZLsyUPWJEtvvYr3vt2i9j
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_HHcmHw5pSv5L47UNsuaSCC
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_qI9Wa1nRp359_2o-yApEB3
          claim_id: c_SsrbjEWqxD10xWU743XdfQ
          source_id: s_5NpUbDuQPXWq4a97FALFaP
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_wZLsyUPWJEtvvYr3vt2i9j
        status: active
        display_name: 王華
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王藹

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王藹 | accepted |
| death.date | 843年 | accepted |
| bio.summary | 王藹（卒于843年），唐人物。籍贯北芒山。（中国历代人物传记资料库 CBDB 175736） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_wZLsyUPWJEtvvYr3vt2i9j | 王華 | accepted |

## 外部来源

- [中国历代人物传记资料库：王藹（CBDB 175736）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175736&o=json)
