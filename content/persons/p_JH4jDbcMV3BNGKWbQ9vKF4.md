---
schema: wang-person/v1
id: p_JH4jDbcMV3BNGKWbQ9vKF4
status: active
merged_into: null
display_name: 王銳
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_1BGepzZNs1YDujJzWvAZvQ
        subject_person_id: p_JH4jDbcMV3BNGKWbQ9vKF4
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王銳
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_A6YTCHhH8oX4z9QZAAvWEZ
          claim_id: c_1BGepzZNs1YDujJzWvAZvQ
          source_id: s_zK3XJnZokR4EFPfZGjURLH
          stance: supports
          locator: CBDB:326816
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（326816）
          source: &a1
            id: s_zK3XJnZokR4EFPfZGjURLH
            source_type: api_record
            title: 中国历代人物传记资料库：王銳（CBDB 326816）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=326816&o=json
            external_identifier: CBDB:326816
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:20.229Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_Cks3Q93SEjByujJ9EjHajE
        subject_person_id: p_JH4jDbcMV3BNGKWbQ9vKF4
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王銳，明人物。嘉靖四十一年進士。（中国历代人物传记资料库 CBDB 326816）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_U6P4kGeV9Qz2VbNqKvRqxS
          claim_id: c_Cks3Q93SEjByujJ9EjHajE
          source_id: s_zK3XJnZokR4EFPfZGjURLH
          stance: supports
          locator: CBDB:326816
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_56qZCzU3RwCNYU2YzG_8Rf
        subject_person_id: p_JH4jDbcMV3BNGKWbQ9vKF4
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_LNAKBPYt9PQCz44835GtLD
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_C7yVq2MFq8BsabJKBEt-ra
          claim_id: c_56qZCzU3RwCNYU2YzG_8Rf
          source_id: s_zK3XJnZokR4EFPfZGjURLH
          stance: supports
          locator: 嘉靖四十一年進士登科錄:一卷，第二甲第五十七名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_LNAKBPYt9PQCz44835GtLD
        status: active
        display_name: 王續之
        merged_into_person_id: null
  other: []
---

# 王銳

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王銳 | accepted |
| bio.summary | 王銳，明人物。嘉靖四十一年進士。（中国历代人物传记资料库 CBDB 326816） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_LNAKBPYt9PQCz44835GtLD | 王續之 | accepted |

## 外部来源

- [中国历代人物传记资料库：王銳（CBDB 326816）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=326816&o=json)
