---
schema: wang-person/v1
id: p_32BLPDKJangJT5tNVMnCFe
status: active
merged_into: null
display_name: 王永福
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_CRFH3eGvJwNPBSNHW7kCDH
        subject_person_id: p_32BLPDKJangJT5tNVMnCFe
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王永福
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_2AJ7V6sojsDhdefVXEHtt7
          claim_id: c_CRFH3eGvJwNPBSNHW7kCDH
          source_id: s_K6zqDpk8r6QfRuJW6sAPAY
          stance: supports
          locator: CBDB:241135
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（241135）
          source: &a1
            id: s_K6zqDpk8r6QfRuJW6sAPAY
            source_type: api_record
            title: 中国历代人物传记资料库：王永福（CBDB 241135）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=241135&o=json
            external_identifier: CBDB:241135
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:17.944Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_zNxwrBKzFq8PMYL4Rb8Egz
        subject_person_id: p_32BLPDKJangJT5tNVMnCFe
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王永福，明人物。正統十三年進士。（中国历代人物传记资料库 CBDB 241135）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_AZLHtcpGMRcJJuK4aAU3ss
          claim_id: c_zNxwrBKzFq8PMYL4Rb8Egz
          source_id: s_K6zqDpk8r6QfRuJW6sAPAY
          stance: supports
          locator: CBDB:241135
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_NUSWzDf21GganExuYVFbz9
        subject_person_id: p_32BLPDKJangJT5tNVMnCFe
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_bF6WY3wgJWVigFgpBLaV3H
        generation_count: 3
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_3nEb8WaeZEP1HENqOkkFdX
          claim_id: c_NUSWzDf21GganExuYVFbz9
          source_id: s_K6zqDpk8r6QfRuJW6sAPAY
          stance: supports
          locator: 正統十三年進士登科錄:一卷，第三甲第五十九名：曾孫; 重孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_bF6WY3wgJWVigFgpBLaV3H
        status: active
        display_name: 王璽
        merged_into_person_id: null
  other: []
---

# 王永福

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王永福 | accepted |
| bio.summary | 王永福，明人物。正統十三年進士。（中国历代人物传记资料库 CBDB 241135） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_bF6WY3wgJWVigFgpBLaV3H | 王璽 | accepted |

## 外部来源

- [中国历代人物传记资料库：王永福（CBDB 241135）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=241135&o=json)
