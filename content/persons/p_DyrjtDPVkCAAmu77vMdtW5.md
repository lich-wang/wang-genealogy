---
schema: wang-person/v1
id: p_DyrjtDPVkCAAmu77vMdtW5
status: active
merged_into: null
display_name: 王鑣
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_jevbMrGXY34gYYoQyiE9QN
        subject_person_id: p_DyrjtDPVkCAAmu77vMdtW5
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王鑣
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_8N5FNxLon8ygWx8bZaNeUX
          claim_id: c_jevbMrGXY34gYYoQyiE9QN
          source_id: s_AzcrSuHMHpRocLLoZPiMEs
          stance: supports
          locator: CBDB:175712
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（175712）
          source: &a1
            id: s_AzcrSuHMHpRocLLoZPiMEs
            source_type: api_record
            title: 中国历代人物传记资料库：王鑣（CBDB 175712）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175712&o=json
            external_identifier: CBDB:175712
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:16.074Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_yRJxcpU7msiLEsiy4PCBmm
        subject_person_id: p_DyrjtDPVkCAAmu77vMdtW5
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 917年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Rjaatph3JRG51onEqW3htz
          claim_id: c_yRJxcpU7msiLEsiy4PCBmm
          source_id: s_AzcrSuHMHpRocLLoZPiMEs
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_JYGNhifLGPhNFin5cRoa7j
        subject_person_id: p_DyrjtDPVkCAAmu77vMdtW5
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王鑣（卒于917年），唐人物。籍贯宣州。（中国历代人物传记资料库 CBDB 175712）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_JjqYuKjjcRwr3S6ylrrkki
          claim_id: c_JYGNhifLGPhNFin5cRoa7j
          source_id: s_AzcrSuHMHpRocLLoZPiMEs
          stance: supports
          locator: CBDB:175712
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_ZMNSwdTQb7XHDfkSgEsYFd
        subject_person_id: p_HYw3h3gd2SPmfVE18GBoDH
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_DyrjtDPVkCAAmu77vMdtW5
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_NeUPqvG1Ns1J7DugQD2hNi
          claim_id: c_ZMNSwdTQb7XHDfkSgEsYFd
          source_id: s_AzcrSuHMHpRocLLoZPiMEs
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：父
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_HYw3h3gd2SPmfVE18GBoDH
        status: active
        display_name: 王凝
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王鑣

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王鑣 | accepted |
| death.date | 917年 | accepted |
| bio.summary | 王鑣（卒于917年），唐人物。籍贯宣州。（中国历代人物传记资料库 CBDB 175712） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_HYw3h3gd2SPmfVE18GBoDH | 王凝 | accepted |

## 外部来源

- [中国历代人物传记资料库：王鑣（CBDB 175712）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=175712&o=json)
