---
schema: wang-person/v1
id: p_CDzewbvc57C3MHsFxzq2nG
status: active
merged_into: null
display_name: 王羲之
revision: 6
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_NwRX1HwK1dRpK3BL9E6jC5
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 东晋书法家，官至右军将军、会稽内史，世称「王右军」，被尊为「书圣」。所书《兰亭集序》被誉为「天下第一行书」。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_28RwjoP21DQVLHHku41dcn
          claim_id: c_NwRX1HwK1dRpK3BL9E6jC5
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_nsQPZ59fW2cf8C3uW8rAsD
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 约303年
            calendar_note: null
            earliest: 0298-01-01
            latest: 0308-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_dKnyB8NG7NyeUN3Z6w4ifa
          claim_id: c_nsQPZ59fW2cf8C3uW8rAsD
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_T4S25SCKi2NmZy2FF6RZzy
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 约361年
            calendar_note: null
            earliest: 0356-01-01
            latest: 0366-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_pCKRGJ9MzsujgmZNxkpvx3
          claim_id: c_T4S25SCKi2NmZy2FF6RZzy
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: historical.period
    recommended:
      claim:
        id: c_DHe4QpyxohAxJqrV2NhatG
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: property
        predicate: historical.period
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 东晋
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_AAyqMJZQjWmUKn31VsDXLg
          claim_id: c_DHe4QpyxohAxJqrV2NhatG
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: 人物简介中的朝代／时期表述
          quotation: null
          interpretation_note: 从同一来源支持的简介摘取：东晋书法家，官至右军将军、会稽内史，世称「王右军」，被尊为「书圣」。所书《兰亭集序》被誉为「天下第一行书」。
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: name.alias
    recommended:
      claim:
        id: c_9EgaAW9DwAWatYEMrd6TwC
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: property
        predicate: name.alias
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: Wang Xizhi
          language: zh-Latn-pinyin
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_cMyvjBXnqyXNDXDj3YAHkV
          claim_id: c_9EgaAW9DwAWatYEMrd6TwC
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_6Jkf27hGJNXMDNpKUHKrSG
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王羲之
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_13AScejw2rEXSUEP2QcMrh
          claim_id: c_6Jkf27hGJNXMDNpKUHKrSG
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: Q378564
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
  - predicate: place.origin
    recommended:
      claim:
        id: c_VjFt1ZJv26n2rd2mAgCfPt
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: property
        predicate: place.origin
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 琅琊临沂（今山东临沂）
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_3e5F19JCXfpq2yDxyNeiw6
          claim_id: c_VjFt1ZJv26n2rd2mAgCfPt
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_qtzy21f4DyDGHZDEKFn8uN
        subject_person_id: p_svvmSXELHxu1dgtCxnAmuM
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_CDzewbvc57C3MHsFxzq2nG
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_psm7oQXYyYRb599Rf98uvy
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_emFD4dG96sXcGpNr96MCWz
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_emFD4dG96sXcGpNr96MCWz
            source_type: website
            title: 中文维基百科：王曠
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王曠
            external_identifier: Q22814790
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_vFimTL8QYzNkT3f4B6n2x1
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_mgxHFeBfrj7VDRDVH9F5MV
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_5V2XJ8Xzw7QY26fNr8MXSS
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_5V2XJ8Xzw7QY26fNr8MXSS
            source_type: api_record
            title: 維基數據：王曠（Q22814790）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q22814790
            external_identifier: Q22814790
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:35.742Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%9B%A0
        - id: cs_QZgHgGLP6YDHvWpDLLFk27
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_drh2iw4DqLNxZsshh35unL
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_drh2iw4DqLNxZsshh35unL
            source_type: api_record
            title: 維基數據：王羲之（Q378564）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q378564
            external_identifier: Q378564
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:36.856Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%BE%B2%E4%B9%8B
        - id: cs_RUECvYihvZY3CaAqS6odRs
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_xUKpvE7ee83xcSwLSANkBp
          stance: supports
          locator: 亲属关系：長子
          quotation: null
          interpretation_note: CBDB 注明此条来源：未知
          source:
            id: s_xUKpvE7ee83xcSwLSANkBp
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王曠（25792）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25792&o=json
            external_identifier: CBDB:25792
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:23.558Z
            metadata_json: null
        - id: cs_pSYwoo5DLWu5RBPN2V1Y1A
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_jXCmapPkdPs7qEEHCCvXxR
          stance: supports
          locator: 亲属关系：父
          quotation: null
          interpretation_note: CBDB 注明此条来源：未知
          source:
            id: s_jXCmapPkdPs7qEEHCCvXxR
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王羲之（25793）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25793&o=json
            external_identifier: CBDB:25793
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:23.693Z
            metadata_json: null
        - id: cs_8nhiiQYqGy7ZXLjjAFabv7
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_EuhopAKPrGRPYfhK6LWGqg
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_EuhopAKPrGRPYfhK6LWGqg
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-24T12:08:06.893Z
            metadata_json: null
        - id: cs_vL2Fi8ZfUjukE325Y33BcF
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_UT2fnx3qtzoieQ2DR3uegK
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_UT2fnx3qtzoieQ2DR3uegK
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-24T12:12:42.934Z
            metadata_json: null
        - id: cs_ySTbLrH8zmCLU4eM6XWcoz
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_Hu48xKLWbTYqemmfCh8psU
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_Hu48xKLWbTYqemmfCh8psU
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T01:50:46.620Z
            metadata_json: null
        - id: cs_dtEWeTEHi5mHeNuvTw4N1z
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_am7Fw6MoGrjn6MQaPht7Mr
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_am7Fw6MoGrjn6MQaPht7Mr
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:24:33.641Z
            metadata_json: null
        - id: cs_LPf4Jw1GyDBsQcwMMfQJLV
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_MGBoig692jprej3f8uiDi2
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王曠 → 王羲之
          interpretation_note: null
          source:
            id: s_MGBoig692jprej3f8uiDi2
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:58:59.171Z
            metadata_json: null
        - id: cs_E5u6tSnBNyq97pJEN4qvjL
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_xEBYVW3xjFzLnp9P7pHZ2L
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王曠 → 王羲之
          interpretation_note: null
          source:
            id: s_xEBYVW3xjFzLnp9P7pHZ2L
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:02:22.297Z
            metadata_json: null
        - id: cs_tvMRyPxNqz2xbqchXmDC5Y
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_2Vkz5GQzEDauSJDkgPos6m
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_2Vkz5GQzEDauSJDkgPos6m
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:22:13.683Z
            metadata_json: null
        - id: cs_R4xYJ9e3j5u8HthRKr3FTn
          claim_id: c_qtzy21f4DyDGHZDEKFn8uN
          source_id: s_WUGDBD9YgdJyH7S1ZJAkW2
          stance: supports
          locator: 条文：世系缩进（王览家族）
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_WUGDBD9YgdJyH7S1ZJAkW2
            source_type: website
            title: 中文维基百科：琅邪王氏
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:53:57.308Z
            metadata_json: null
      object_person:
        id: p_svvmSXELHxu1dgtCxnAmuM
        status: active
        display_name: 王曠
        merged_into_person_id: null
  children:
    - claim:
        id: c_NPGJfNi9xwRGUzUcNB6BHV
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_9kvwNC8EwFdL41MBGhg7HG
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_M4qVXVAqmh9s7WA22N8o6k
          claim_id: c_NPGJfNi9xwRGUzUcNB6BHV
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_nVcfB1JPTY48FEHmQiecNC
          claim_id: c_NPGJfNi9xwRGUzUcNB6BHV
          source_id: s_95dF5c3fH2rDR5sHrw8xuZ
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_95dF5c3fH2rDR5sHrw8xuZ
            source_type: website
            title: 中文维基百科：王獻之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王獻之
            external_identifier: Q559425
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_MBUfVLXRPoyLdbHm78jEk5
          claim_id: c_NPGJfNi9xwRGUzUcNB6BHV
          source_id: s_drh2iw4DqLNxZsshh35unL
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_drh2iw4DqLNxZsshh35unL
            source_type: api_record
            title: 維基數據：王羲之（Q378564）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q378564
            external_identifier: Q378564
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:36.856Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%BE%B2%E4%B9%8B
        - id: cs_6bf7Lg5DMU2BqdioGnt9zS
          claim_id: c_NPGJfNi9xwRGUzUcNB6BHV
          source_id: s_dypc2T3zk15WKQ7XHyCLp1
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_dypc2T3zk15WKQ7XHyCLp1
            source_type: api_record
            title: 維基數據：王獻之（Q559425）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q559425
            external_identifier: Q559425
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:47.659Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%8C%AE%E4%B9%8B
        - id: cs_EWw2zGRpN47pJCtJCQx1Np
          claim_id: c_NPGJfNi9xwRGUzUcNB6BHV
          source_id: s_jXCmapPkdPs7qEEHCCvXxR
          stance: supports
          locator: 亲属关系：子
          quotation: null
          interpretation_note: CBDB 注明此条来源：明清婦女著作數據庫
          source:
            id: s_jXCmapPkdPs7qEEHCCvXxR
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王羲之（25793）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25793&o=json
            external_identifier: CBDB:25793
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:23.693Z
            metadata_json: null
        - id: cs_CJMxufRBFw5bHo8KsfnVc7
          claim_id: c_NPGJfNi9xwRGUzUcNB6BHV
          source_id: s_nKpuUhtxqdaxgT45pDQfEo
          stance: supports
          locator: 亲属关系：父
          quotation: null
          interpretation_note: CBDB 注明此条来源：明清婦女著作數據庫
          source:
            id: s_nKpuUhtxqdaxgT45pDQfEo
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王獻之（135441）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=135441&o=json
            external_identifier: CBDB:135441
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:24.791Z
            metadata_json: null
      object_person:
        id: p_9kvwNC8EwFdL41MBGhg7HG
        status: active
        display_name: 王獻之
        merged_into_person_id: null
    - claim:
        id: c_nED1XSaKBkwLQsGhHmMxXy
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_AuDnp3tfDdGY8Do4f11kBs
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_iYKuV9yhArJHCSHCEtS5kp
          claim_id: c_nED1XSaKBkwLQsGhHmMxXy
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_Kn2DvmV3JNuZ5uaE263N4z
          claim_id: c_nED1XSaKBkwLQsGhHmMxXy
          source_id: s_drh2iw4DqLNxZsshh35unL
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_drh2iw4DqLNxZsshh35unL
            source_type: api_record
            title: 維基數據：王羲之（Q378564）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q378564
            external_identifier: Q378564
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:36.856Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%BE%B2%E4%B9%8B
        - id: cs_j8ecURKPeR4JvUy8oDVNRc
          claim_id: c_nED1XSaKBkwLQsGhHmMxXy
          source_id: s_zPYxEpH8gHaJEAKuT5R6M3
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_zPYxEpH8gHaJEAKuT5R6M3
            source_type: api_record
            title: 維基數據：王徽之（Q15928529）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15928529
            external_identifier: Q15928529
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:13.674Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%BE%BD%E4%B9%8B
        - id: cs_4YVpTkn8isykwzPLTzMQfP
          claim_id: c_nED1XSaKBkwLQsGhHmMxXy
          source_id: s_jXCmapPkdPs7qEEHCCvXxR
          stance: supports
          locator: 亲属关系：三子
          quotation: null
          interpretation_note: CBDB 注明此条来源：未知
          source:
            id: s_jXCmapPkdPs7qEEHCCvXxR
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王羲之（25793）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25793&o=json
            external_identifier: CBDB:25793
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:23.693Z
            metadata_json: null
        - id: cs_6cAirWAA5SeH6yp5N1g82G
          claim_id: c_nED1XSaKBkwLQsGhHmMxXy
          source_id: s_jjP9BB54GcuGB17tTLWJ4N
          stance: supports
          locator: 亲属关系：父
          quotation: null
          interpretation_note: CBDB 注明此条来源：未知
          source:
            id: s_jjP9BB54GcuGB17tTLWJ4N
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王徽之（25794）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25794&o=json
            external_identifier: CBDB:25794
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:43.684Z
            metadata_json: null
      object_person:
        id: p_AuDnp3tfDdGY8Do4f11kBs
        status: active
        display_name: 王徽之
        merged_into_person_id: null
    - claim:
        id: c_4KzwV6dg7kUByFtJzu4xmE
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_euM8vAaSVjgAC2CHZ5oZsj
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_nB9LMEsJ5k3gK89sM3PdEV
          claim_id: c_4KzwV6dg7kUByFtJzu4xmE
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_brTVPFXj47QSDp6bCHGQjj
          claim_id: c_4KzwV6dg7kUByFtJzu4xmE
          source_id: s_drh2iw4DqLNxZsshh35unL
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_drh2iw4DqLNxZsshh35unL
            source_type: api_record
            title: 維基數據：王羲之（Q378564）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q378564
            external_identifier: Q378564
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:36.856Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%BE%B2%E4%B9%8B
        - id: cs_9TY1zBjLmhZKZP11TkZ7uj
          claim_id: c_4KzwV6dg7kUByFtJzu4xmE
          source_id: s_4id4JzG8i2kMoebswpshNH
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_4id4JzG8i2kMoebswpshNH
            source_type: api_record
            title: 維基數據：王凝之（Q16077602）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q16077602
            external_identifier: Q16077602
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:15.077Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%87%9D%E4%B9%8B
        - id: cs_2c6R6w53jGBtqoPPK5zMZG
          claim_id: c_4KzwV6dg7kUByFtJzu4xmE
          source_id: s_jXCmapPkdPs7qEEHCCvXxR
          stance: supports
          locator: 亲属关系：子
          quotation: null
          interpretation_note: CBDB 注明此条来源：明清婦女著作數據庫
          source:
            id: s_jXCmapPkdPs7qEEHCCvXxR
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王羲之（25793）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25793&o=json
            external_identifier: CBDB:25793
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:23.693Z
            metadata_json: null
        - id: cs_JQZoz4b5qsPuLArmqscrFy
          claim_id: c_4KzwV6dg7kUByFtJzu4xmE
          source_id: s_pcjTTg5jeEhc9P41TxkCQn
          stance: supports
          locator: 亲属关系：父
          quotation: null
          interpretation_note: CBDB 注明此条来源：明清婦女著作數據庫
          source:
            id: s_pcjTTg5jeEhc9P41TxkCQn
            source_type: api_record
            title: CBDB 中国历代人物传记资料库：王凝之（134837）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=134837&o=json
            external_identifier: CBDB:134837
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-08-24T02:50:49.488Z
            metadata_json: null
        - id: cs_WEXFD9dLAUp7JsSWArTs3q
          claim_id: c_4KzwV6dg7kUByFtJzu4xmE
          source_id: s_xk1dN6HNsFfXqNNRpdJ3N6
          stance: supports
          locator: 条文：之子/之女
          quotation: 王凝之{{BD|334年||399年||CatIdx=W王}}，字叔平，琅邪郡临沂县（今山东省临沂市）人，东晋書法家王羲之次子，亦知名書法家，工草書、隸書
          interpretation_note: null
          source:
            id: s_xk1dN6HNsFfXqNNRpdJ3N6
            source_type: website
            title: 中文维基百科：王凝之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%87%9D%E4%B9%8B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:25:17.205Z
            metadata_json: null
      object_person:
        id: p_euM8vAaSVjgAC2CHZ5oZsj
        status: active
        display_name: 王凝之
        merged_into_person_id: null
    - claim:
        id: c_kG6umm78AqYrQH34fAWVcK
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_DgGDrDAArgrEURaMevB7Aa
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_LcUzpb2xzacVv2BruJvXdK
          claim_id: c_kG6umm78AqYrQH34fAWVcK
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_K66WQYwQhxGG2GoLhht42E
          claim_id: c_kG6umm78AqYrQH34fAWVcK
          source_id: s_drh2iw4DqLNxZsshh35unL
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_drh2iw4DqLNxZsshh35unL
            source_type: api_record
            title: 維基數據：王羲之（Q378564）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q378564
            external_identifier: Q378564
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:36.856Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%BE%B2%E4%B9%8B
        - id: cs_9rSaUowSj4kqmoSFBFWWLW
          claim_id: c_kG6umm78AqYrQH34fAWVcK
          source_id: s_AFb1FHSuYVfBKL5AZw46X3
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_AFb1FHSuYVfBKL5AZw46X3
            source_type: api_record
            title: 維基數據：王孟姜（Q16905978）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q16905978
            external_identifier: Q16905978
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:16.383Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%AD%9F%E5%A7%9C
        - id: cs_KwpHZJMR4RyAJNeBazd6kB
          claim_id: c_kG6umm78AqYrQH34fAWVcK
          source_id: s_Fr4L5WsE8VDkLidLwtgLDG
          stance: supports
          locator: 条文：条文识读（父）
          quotation: 王羲之與郗璿的獨女
          interpretation_note: null
          source:
            id: s_Fr4L5WsE8VDkLidLwtgLDG
            source_type: website
            title: 中文维基百科：王孟姜
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%AD%9F%E5%A7%9C
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:26:59.152Z
            metadata_json: null
        - id: cs_UQbmSp1u19SsZy9qDLcBQJ
          claim_id: c_kG6umm78AqYrQH34fAWVcK
          source_id: s_obU3sWETpVMu1YP1TPQiEC
          stance: supports
          locator: 条文：条文识读（父）
          quotation: 王羲之与郗璿的独女
          interpretation_note: null
          source:
            id: s_obU3sWETpVMu1YP1TPQiEC
            source_type: website
            title: 中文维基百科：王孟姜
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%AD%9F%E5%A7%9C
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:53:08.503Z
            metadata_json: null
      object_person:
        id: p_DgGDrDAArgrEURaMevB7Aa
        status: active
        display_name: 王孟姜
        merged_into_person_id: null
    - claim:
        id: c_W4KKoq2cKvfv1Q3hs5NnVg
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_eM19JSXJmo5wpa7Pt2ESoj
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_moBeY3owzkvEyQ13KuyxHs
          claim_id: c_W4KKoq2cKvfv1Q3hs5NnVg
          source_id: s_U3ygJC89TMNzm9Wt2sMRb4
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_U3ygJC89TMNzm9Wt2sMRb4
            source_type: website
            title: 中文维基百科：王羲之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/王羲之
            external_identifier: Q378564
            license_code: CC-BY-SA-4.0
            accessed_at: null
            metadata_json: null
        - id: cs_LipE3g6yxdy3M3bA8ewrmu
          claim_id: c_W4KKoq2cKvfv1Q3hs5NnVg
          source_id: s_drh2iw4DqLNxZsshh35unL
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_drh2iw4DqLNxZsshh35unL
            source_type: api_record
            title: 維基數據：王羲之（Q378564）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q378564
            external_identifier: Q378564
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:36.856Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%BE%B2%E4%B9%8B
        - id: cs_JU4FAHPVq5AQq8PcD7i7wP
          claim_id: c_W4KKoq2cKvfv1Q3hs5NnVg
          source_id: s_MxvzFWtBwwLm7du9qJ2Nxv
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_MxvzFWtBwwLm7du9qJ2Nxv
            source_type: api_record
            title: 維基數據：王玄之（Q47182758）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q47182758
            external_identifier: Q47182758
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:17.360Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%8E%84%E4%B9%8B
        - id: cs_u9WDF5LrVZnNV9EwFsmc48
          claim_id: c_W4KKoq2cKvfv1Q3hs5NnVg
          source_id: s_em2Ve68RuaqtB9qnBt9r8f
          stance: supports
          locator: 条文：条文识读（父）
          quotation: 東晉書法家王羲之長子
          interpretation_note: null
          source:
            id: s_em2Ve68RuaqtB9qnBt9r8f
            source_type: website
            title: 中文维基百科：王玄之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%8E%84%E4%B9%8B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:25:18.047Z
            metadata_json: null
        - id: cs_Ughk9v2ran7QX7Jrfg2KeH
          claim_id: c_W4KKoq2cKvfv1Q3hs5NnVg
          source_id: s_9M8a6WLDE9DdMBjd4t59JB
          stance: supports
          locator: 条文：条文识读（父）
          quotation: 東晉書法家王羲之長子
          interpretation_note: null
          source:
            id: s_9M8a6WLDE9DdMBjd4t59JB
            source_type: website
            title: 中文维基百科：王玄之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%8E%84%E4%B9%8B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:54:41.566Z
            metadata_json: null
      object_person:
        id: p_eM19JSXJmo5wpa7Pt2ESoj
        status: active
        display_name: 王玄之
        merged_into_person_id: null
    - claim:
        id: c_eNnoFT9kqCktAhd18PCZyD
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: relationship
        predicate: kinship.parent_of
        object_person_id: p_WJAVudN6i44AynGRG2Trts
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_7e1dJ1xkD7BX8Dx27DMNRq
          claim_id: c_eNnoFT9kqCktAhd18PCZyD
          source_id: s_gpBGTn4QVgA3DggBymgxcG
          stance: supports
          locator: 卷八十·羲之子操之
          quotation: 羲之子 操之……操之字子重，歷侍中、尚書、豫章太守。
          interpretation_note: null
          source:
            id: s_gpBGTn4QVgA3DggBymgxcG
            source_type: book
            title: 中文维基文库：晉書/卷080
            creator: null
            publisher: 维基文库
            published_at_text: null
            canonical_url: https://zh.wikisource.org/wiki/%E6%99%89%E6%9B%B8/%E5%8D%B7080
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-31T10:37:52.156Z
            metadata_json: null
        - id: cs_gDsHJY7kgc8ea9bSRBC2UP
          claim_id: c_eNnoFT9kqCktAhd18PCZyD
          source_id: s_rSLEZBeju7xqmzfSNtMzEY
          stance: supports
          locator: 王羲之条·家庭
          quotation: 他有7个儿子，即玄之、凝之、涣之、肃之、徽之、操之、献之。
          interpretation_note: null
          source:
            id: s_rSLEZBeju7xqmzfSNtMzEY
            source_type: gazetteer
            title: 山东地情档案：《琅邪王氏文化志·王羲之》
            creator: 《琅邪王氏文化志》编纂机构
            publisher: 山东地情档案
            published_at_text: null
            canonical_url: https://shandong-chorography.org/database/bg/section/12/article/102/
            external_identifier: null
            license_code: unknown
            accessed_at: 2026-08-31T10:37:52.298Z
            metadata_json: null
      object_person:
        id: p_WJAVudN6i44AynGRG2Trts
        status: active
        display_name: 王操之
        merged_into_person_id: null
    - claim:
        id: c_x278MPRPyLFe7jJ3XC8yhi
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: relationship
        predicate: kinship.parent_of
        object_person_id: p_qbvqP1MVbkrnJ6f5KWy4vt
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_WtD1B5AFYHL4Kg8FJ1TZCo
          claim_id: c_x278MPRPyLFe7jJ3XC8yhi
          source_id: s_rSLEZBeju7xqmzfSNtMzEY
          stance: supports
          locator: 王羲之条·家庭
          quotation: 他有7个儿子，即玄之、凝之、涣之、肃之、徽之、操之、献之。
          interpretation_note: null
          source:
            id: s_rSLEZBeju7xqmzfSNtMzEY
            source_type: gazetteer
            title: 山东地情档案：《琅邪王氏文化志·王羲之》
            creator: 《琅邪王氏文化志》编纂机构
            publisher: 山东地情档案
            published_at_text: null
            canonical_url: https://shandong-chorography.org/database/bg/section/12/article/102/
            external_identifier: null
            license_code: unknown
            accessed_at: 2026-08-31T10:37:52.298Z
            metadata_json: null
      object_person:
        id: p_qbvqP1MVbkrnJ6f5KWy4vt
        status: active
        display_name: 王肃之
        merged_into_person_id: null
    - claim:
        id: c_KvTwNHajNbnAUgm5RT3KxN
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: relationship
        predicate: kinship.parent_of
        object_person_id: p_tMh82RBuYVD7P2Gou14ptK
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_LU9Aq4nT1gQEi7g23JEfLE
          claim_id: c_KvTwNHajNbnAUgm5RT3KxN
          source_id: s_rSLEZBeju7xqmzfSNtMzEY
          stance: supports
          locator: 王羲之条·家庭
          quotation: 他有7个儿子，即玄之、凝之、涣之、肃之、徽之、操之、献之。
          interpretation_note: null
          source:
            id: s_rSLEZBeju7xqmzfSNtMzEY
            source_type: gazetteer
            title: 山东地情档案：《琅邪王氏文化志·王羲之》
            creator: 《琅邪王氏文化志》编纂机构
            publisher: 山东地情档案
            published_at_text: null
            canonical_url: https://shandong-chorography.org/database/bg/section/12/article/102/
            external_identifier: null
            license_code: unknown
            accessed_at: 2026-08-31T10:37:52.298Z
            metadata_json: null
      object_person:
        id: p_tMh82RBuYVD7P2Gou14ptK
        status: active
        display_name: 王涣之
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_cbFuce5ABHe82KJMY8AuEP
        subject_person_id: p_6QD3NR9CRnR5uih3d3rCn1
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_CDzewbvc57C3MHsFxzq2nG
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 4
      sources:
        - id: cs_iU3Q9dPPv71QRjLBWLE4gE
          claim_id: c_cbFuce5ABHe82KJMY8AuEP
          source_id: s_drh2iw4DqLNxZsshh35unL
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_drh2iw4DqLNxZsshh35unL
            source_type: api_record
            title: 維基數據：王羲之（Q378564）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q378564
            external_identifier: Q378564
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:14:36.856Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%BE%B2%E4%B9%8B
        - id: cs_uK7qSpu7F4DbTy54LHK1YE
          claim_id: c_cbFuce5ABHe82KJMY8AuEP
          source_id: s_Du3n4QsfJxZGhMcMVwNB6b
          stance: supports
          locator: P26（配偶）
          quotation: null
          interpretation_note: null
          source:
            id: s_Du3n4QsfJxZGhMcMVwNB6b
            source_type: api_record
            title: 維基數據：郗璿（Q15905263）
            creator: 維基數據貢獻者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q15905263
            external_identifier: Q15905263
            license_code: CC0-1.0
            accessed_at: 2026-08-24T02:10:18.669Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E9%83%97%E7%92%BF
        - id: cs_iBPMA9C7ei9JsJuqFwTd6H
          claim_id: c_cbFuce5ABHe82KJMY8AuEP
          source_id: s_mFVNchy7qqMj3FBHzehGX7
          stance: supports
          locator: 条文：信息框 夫
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_mFVNchy7qqMj3FBHzehGX7
            source_type: website
            title: 中文维基百科：郗璿
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E9%83%97%E7%92%BF
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-24T12:08:13.592Z
            metadata_json: null
        - id: cs_z7qqsZ14xh97DvQLm52tCz
          claim_id: c_cbFuce5ABHe82KJMY8AuEP
          source_id: s_UJq8keM5CswFW29PuRAEQ3
          stance: supports
          locator: 条文：信息框 夫
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_UJq8keM5CswFW29PuRAEQ3
            source_type: website
            title: 中文维基百科：郗璿
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E9%83%97%E7%92%BF
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-24T12:13:23.937Z
            metadata_json: null
        - id: cs_GPPj9Ki6kznQA4HNcToLVS
          claim_id: c_cbFuce5ABHe82KJMY8AuEP
          source_id: s_U2iqU9PQH3qa2r1AVu3wmD
          stance: supports
          locator: 条文：信息框 夫
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_U2iqU9PQH3qa2r1AVu3wmD
            source_type: website
            title: 中文维基百科：郗璿
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E9%83%97%E7%92%BF
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T01:51:25.299Z
            metadata_json: null
        - id: cs_DCLJs8H9EB4L6PPjHLno6F
          claim_id: c_cbFuce5ABHe82KJMY8AuEP
          source_id: s_Yx6e8nfArAikWv9PP6qUNh
          stance: supports
          locator: 条文：信息框 夫
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_Yx6e8nfArAikWv9PP6qUNh
            source_type: website
            title: 中文维基百科：郗璿
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E9%83%97%E7%92%BF
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:25:18.643Z
            metadata_json: null
        - id: cs_3MfRTVYtkkCx7YMNfj8Hoe
          claim_id: c_cbFuce5ABHe82KJMY8AuEP
          source_id: s_aFXoLup774oRFpDMhDywv1
          stance: supports
          locator: 条文：信息框 夫
          quotation: 王羲之
          interpretation_note: null
          source:
            id: s_aFXoLup774oRFpDMhDywv1
            source_type: website
            title: 中文维基百科：郗璿
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E9%83%97%E7%92%BF
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:55:33.089Z
            metadata_json: null
      object_person:
        id: p_6QD3NR9CRnR5uih3d3rCn1
        status: active
        display_name: 郗璿
        merged_into_person_id: null
  ancestors: []
  descendants:
    - claim:
        id: c_6tTt9xguMVBeYptNYZJJdq
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_pqebTttw9MCLeZnfbrPfZD
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_D2bPDz63LFoMEA8DUQHKAS
          claim_id: c_6tTt9xguMVBeYptNYZJJdq
          source_id: s_Fri9oi811gDmnFFhiKmnjp
          stance: supports
          locator: 条文：条文识读（祖）（2世）
          quotation: 祖父王羲之、父王獻之都是中國古代書法家
          interpretation_note: null
          source:
            id: s_Fri9oi811gDmnFFhiKmnjp
            source_type: website
            title: 中文维基百科：王神愛
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%A5%9E%E6%84%9B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:25:19.678Z
            metadata_json: null
        - id: cs_d5W6PpuQgJR2ffhb4fZz58
          claim_id: c_6tTt9xguMVBeYptNYZJJdq
          source_id: s_JNE7MkSy5Ect4YFvmFvnKh
          stance: supports
          locator: 条文：条文识读（祖父）（2世）
          quotation: 祖父王羲之、父王献之都是中国古代书法家
          interpretation_note: null
          source:
            id: s_JNE7MkSy5Ect4YFvmFvnKh
            source_type: website
            title: 中文维基百科：王神愛
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%A5%9E%E6%84%9B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:54:46.989Z
            metadata_json: null
      object_person:
        id: p_pqebTttw9MCLeZnfbrPfZD
        status: active
        display_name: 王神爱
        merged_into_person_id: null
    - claim:
        id: c_42YcSfVsWP7PhyAQ792WL6
        subject_person_id: p_CDzewbvc57C3MHsFxzq2nG
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_eG5miLGWzg6mphbBUuSbNA
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_e8qhD93CeppBiw1yp4jB1v
          claim_id: c_42YcSfVsWP7PhyAQ792WL6
          source_id: s_LzPqCZP5CSutS296cDkJQV
          stance: supports
          locator: 条文：条文识读（祖）（2世）
          quotation: 王桢之（），字公榦，琅邪郡临沂县（今山东省临沂市）人，王羲之之孙，王徽之之子
          interpretation_note: null
          source:
            id: s_LzPqCZP5CSutS296cDkJQV
            source_type: website
            title: 中文维基百科：王桢之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%A1%A2%E4%B9%8B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:25:37.129Z
            metadata_json: null
        - id: cs_chMXnMoPRYiP4QxWiNZZtr
          claim_id: c_42YcSfVsWP7PhyAQ792WL6
          source_id: s_bM5XXwFKQ8UA6nGHJqzdku
          stance: supports
          locator: 条文：条文识读（ancestor）（2世）
          quotation: 王羲之之孙，王徽之之子
          interpretation_note: null
          source:
            id: s_bM5XXwFKQ8UA6nGHJqzdku
            source_type: website
            title: 中文维基百科：王桢之
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%A1%A2%E4%B9%8B
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:54:33.799Z
            metadata_json: null
      object_person:
        id: p_eG5miLGWzg6mphbBUuSbNA
        status: active
        display_name: 王桢之
        merged_into_person_id: null
  other: []
---

# 王羲之

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 东晋书法家，官至右军将军、会稽内史，世称「王右军」，被尊为「书圣」。所书《兰亭集序》被誉为「天下第一行书」。 | accepted |
| birth.date | 约303年 | accepted |
| death.date | 约361年 | accepted |
| historical.period | 东晋 | accepted |
| name.alias | Wang Xizhi | accepted |
| name.primary | 王羲之 | accepted |
| place.origin | 琅琊临沂（今山东临沂） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_svvmSXELHxu1dgtCxnAmuM | 王曠 | accepted |
| children | p_9kvwNC8EwFdL41MBGhg7HG | 王獻之 | accepted |
| children | p_AuDnp3tfDdGY8Do4f11kBs | 王徽之 | accepted |
| children | p_euM8vAaSVjgAC2CHZ5oZsj | 王凝之 | accepted |
| children | p_DgGDrDAArgrEURaMevB7Aa | 王孟姜 | accepted |
| children | p_eM19JSXJmo5wpa7Pt2ESoj | 王玄之 | accepted |
| children | p_WJAVudN6i44AynGRG2Trts | 王操之 | accepted |
| children | p_qbvqP1MVbkrnJ6f5KWy4vt | 王肃之 | accepted |
| children | p_tMh82RBuYVD7P2Gou14ptK | 王涣之 | accepted |
| spouses | p_6QD3NR9CRnR5uih3d3rCn1 | 郗璿 | accepted |
| descendants | p_pqebTttw9MCLeZnfbrPfZD | 王神爱 | accepted |
| descendants | p_eG5miLGWzg6mphbBUuSbNA | 王桢之 | accepted |

## 外部来源

- [山东地情档案：《琅邪王氏文化志·王羲之》](https://shandong-chorography.org/database/bg/section/12/article/102/)
- [維基數據：王徽之（Q15928529）](https://www.wikidata.org/wiki/Q15928529)
- [維基數據：王曠（Q22814790）](https://www.wikidata.org/wiki/Q22814790)
- [維基數據：王孟姜（Q16905978）](https://www.wikidata.org/wiki/Q16905978)
- [維基數據：王凝之（Q16077602）](https://www.wikidata.org/wiki/Q16077602)
- [維基數據：王羲之（Q378564）](https://www.wikidata.org/wiki/Q378564)
- [維基數據：王獻之（Q559425）](https://www.wikidata.org/wiki/Q559425)
- [維基數據：王玄之（Q47182758）](https://www.wikidata.org/wiki/Q47182758)
- [維基數據：郗璿（Q15905263）](https://www.wikidata.org/wiki/Q15905263)
- [中文维基百科：琅邪王氏](https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F)
- [中文维基百科：琅邪王氏世系圖](https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96)
- [中文维基百科：王曠](https://zh.wikipedia.org/wiki/王曠)
- [中文维基百科：王孟姜](https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%AD%9F%E5%A7%9C)
- [中文维基百科：王凝之](https://zh.wikipedia.org/wiki/%E7%8E%8B%E5%87%9D%E4%B9%8B)
- [中文维基百科：王神愛](https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%A5%9E%E6%84%9B)
- [中文维基百科：王羲之](https://zh.wikipedia.org/wiki/王羲之)
- [中文维基百科：王獻之](https://zh.wikipedia.org/wiki/王獻之)
- [中文维基百科：王玄之](https://zh.wikipedia.org/wiki/%E7%8E%8B%E7%8E%84%E4%B9%8B)
- [中文维基百科：王桢之](https://zh.wikipedia.org/wiki/%E7%8E%8B%E6%A1%A2%E4%B9%8B)
- [中文维基百科：郗璿](https://zh.wikipedia.org/wiki/%E9%83%97%E7%92%BF)
- [中文维基文库：晉書/卷080](https://zh.wikisource.org/wiki/%E6%99%89%E6%9B%B8/%E5%8D%B7080)
- [CBDB 中国历代人物传记资料库：王徽之（25794）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25794&o=json)
- [CBDB 中国历代人物传记资料库：王曠（25792）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25792&o=json)
- [CBDB 中国历代人物传记资料库：王凝之（134837）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=134837&o=json)
- [CBDB 中国历代人物传记资料库：王羲之（25793）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=25793&o=json)
- [CBDB 中国历代人物传记资料库：王獻之（135441）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=135441&o=json)
