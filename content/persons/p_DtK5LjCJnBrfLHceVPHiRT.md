---
schema: wang-person/v1
id: p_DtK5LjCJnBrfLHceVPHiRT
status: active
merged_into: null
display_name: 符氏
revision: 1
cbdb_id: 573071
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_v_Pm23xVSfs7E78wkfgs35
        subject_person_id: p_DtK5LjCJnBrfLHceVPHiRT
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 符氏，史料所见人物。本项目依据《中国历代人物传记资料库：符氏(符昭愿女)（CBDB 573071）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_hG5YsulOvTHyt2hAgp5BZT
          claim_id: c_v_Pm23xVSfs7E78wkfgs35
          source_id: s_F_8IKkG0EOD6SdFIVc-U0O
          stance: supports
          locator: CBDB:573071
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_F_8IKkG0EOD6SdFIVc-U0O
            source_type: api_record
            title: 中国历代人物传记资料库：符氏(符昭愿女)（CBDB 573071）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=573071&o=json
            external_identifier: CBDB:573071
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_Gj8juakHAxvFwVYQPnTrD4
        subject_person_id: p_DtK5LjCJnBrfLHceVPHiRT
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 符氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_pd19TteQvMVDo16bUShUFo
          claim_id: c_Gj8juakHAxvFwVYQPnTrD4
          source_id: s_F_8IKkG0EOD6SdFIVc-U0O
          stance: supports
          locator: CBDB:573071
          quotation: null
          interpretation_note: CBDB 明确记录的王希逸配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_iC5KUwetaAtrSOLTzgrgAc
        subject_person_id: p_eGwj2kV1qTWNvDXuDqBhQ5
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_DtK5LjCJnBrfLHceVPHiRT
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_OMybS1GUSL6ySozaX73c_x
          claim_id: c_iC5KUwetaAtrSOLTzgrgAc
          source_id: s_F_8IKkG0EOD6SdFIVc-U0O
          stance: supports
          locator: CBDB 双向互证（妻子 符氏(符昭愿女)）
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_eGwj2kV1qTWNvDXuDqBhQ5
        status: active
        display_name: 王希逸
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 符氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 符氏，史料所见人物。本项目依据《中国历代人物传记资料库：符氏(符昭愿女)（CBDB 573071）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 符氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_eGwj2kV1qTWNvDXuDqBhQ5 | 王希逸 | accepted |

## 外部来源

- [中国历代人物传记资料库：符氏(符昭愿女)（CBDB 573071）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=573071&o=json)
