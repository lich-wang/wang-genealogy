---
schema: wang-person/v1
id: p_PdRkzyCE8dbVGJM6pbU2vZ
status: active
merged_into: null
display_name: 王涔
cbdb_id: 21997
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_HxhdXD7y1RZqN4DiQH8RWC
        subject_person_id: p_PdRkzyCE8dbVGJM6pbU2vZ
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王涔，史料所见人物。本项目依据《中国历代人物传记资料库：王涔（CBDB 21997）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_aQiZMm76nTGMaEX97m5rUP
          claim_id: c_HxhdXD7y1RZqN4DiQH8RWC
          source_id: s_En2QAJqXHpgKT9bFsEAJtR
          stance: supports
          locator: CBDB:21997
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_En2QAJqXHpgKT9bFsEAJtR
            source_type: api_record
            title: 中国历代人物传记资料库：王涔（CBDB 21997）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=21997&o=json
            external_identifier: CBDB:21997
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:44.071Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_uB8Bk9vt5xgTzfHv6ADszM
        subject_person_id: p_PdRkzyCE8dbVGJM6pbU2vZ
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王涔
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_J4qGv6XPv1Q9Bx1hxr7r5H
          claim_id: c_uB8Bk9vt5xgTzfHv6ADszM
          source_id: s_En2QAJqXHpgKT9bFsEAJtR
          stance: supports
          locator: CBDB:21997
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（901-1000）｜历史性依据：CBDB 朝代 = 宋
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_8IbtLoJiSdS4x4wLJ6-Z4N
        subject_person_id: p_M9sDetT5acG1nDTAxFe7as
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_PdRkzyCE8dbVGJM6pbU2vZ
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_KeAVq_7MqSioFgGI_z7vPc
          claim_id: c_8IbtLoJiSdS4x4wLJ6-Z4N
          source_id: s_En2QAJqXHpgKT9bFsEAJtR
          stance: supports
          locator: CBDB 双向互证（祖父 王之望 ⇄ 孫 王涔）
          quotation: null
          interpretation_note: Luna 在线复核：CBDB 底本未知，经 Wikidata 双向核对后采纳。
          source:
            id: s_En2QAJqXHpgKT9bFsEAJtR
            source_type: api_record
            title: 中国历代人物传记资料库：王涔（CBDB 21997）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=21997&o=json
            external_identifier: CBDB:21997
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:44.071Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_M9sDetT5acG1nDTAxFe7as
        status: active
        display_name: 王之望
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王涔

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王涔，史料所见人物。本项目依据《中国历代人物传记资料库：王涔（CBDB 21997）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 王涔 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| ancestors | p_M9sDetT5acG1nDTAxFe7as | 王之望 | accepted |

## 外部来源

- [中国历代人物传记资料库：王涔（CBDB 21997）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=21997&o=json)
