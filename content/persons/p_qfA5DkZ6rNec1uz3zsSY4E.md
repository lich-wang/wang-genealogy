---
schema: wang-person/v1
id: p_qfA5DkZ6rNec1uz3zsSY4E
status: active
merged_into: null
display_name: 崔某
revision: 1
cbdb_id: 185800
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_nXIbbcev7ZAwmmW4_8zv7x
        subject_person_id: p_qfA5DkZ6rNec1uz3zsSY4E
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 崔某，史料所见人物。本项目依据《中国历代人物传记资料库：崔某（CBDB 185800）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_um1dYgLtT2fiTlqKOHmVFi
          claim_id: c_nXIbbcev7ZAwmmW4_8zv7x
          source_id: s_-NEFVtNMi38UEblrS6Qe2U
          stance: supports
          locator: CBDB:185800
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_-NEFVtNMi38UEblrS6Qe2U
            source_type: api_record
            title: 中国历代人物传记资料库：崔某（CBDB 185800）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=185800&o=json
            external_identifier: CBDB:185800
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_Iw87ASJ4b-6oW7xrbMahmL
        subject_person_id: p_qfA5DkZ6rNec1uz3zsSY4E
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 崔某
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_-iAReCjNd7B_v-pE9udz-M
          claim_id: c_Iw87ASJ4b-6oW7xrbMahmL
          source_id: s_-NEFVtNMi38UEblrS6Qe2U
          stance: supports
          locator: CBDB:185800
          quotation: null
          interpretation_note: CBDB 明确记录的王流謙配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_2KNjHQJyhfUuq09mdE2RYx
        subject_person_id: p_VwwzPsbTezpZTDDq8p6cuf
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_qfA5DkZ6rNec1uz3zsSY4E
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_XYfIdEIDu6ricJ7Swlu0lm
          claim_id: c_2KNjHQJyhfUuq09mdE2RYx
          source_id: s_-NEFVtNMi38UEblrS6Qe2U
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：丈夫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_VwwzPsbTezpZTDDq8p6cuf
        status: active
        display_name: 王流謙
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 崔某

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 崔某，史料所见人物。本项目依据《中国历代人物传记资料库：崔某（CBDB 185800）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 崔某 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_VwwzPsbTezpZTDDq8p6cuf | 王流謙 | accepted |

## 外部来源

- [中国历代人物传记资料库：崔某（CBDB 185800）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=185800&o=json)
