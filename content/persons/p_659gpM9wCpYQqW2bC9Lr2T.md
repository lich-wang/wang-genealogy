---
schema: wang-person/v1
id: p_659gpM9wCpYQqW2bC9Lr2T
status: active
merged_into: null
display_name: 王福榮
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_dyFs38E3orfhhqXruqnBWm
        subject_person_id: p_659gpM9wCpYQqW2bC9Lr2T
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王福榮
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_zBaLbRzjr2X54z1HCiw9Yg
          claim_id: c_dyFs38E3orfhhqXruqnBWm
          source_id: s_1pb5X8kEN9TPVKRZ9FnArJ
          stance: supports
          locator: CBDB:318696
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（318696）
          source: &a1
            id: s_1pb5X8kEN9TPVKRZ9FnArJ
            source_type: api_record
            title: 中国历代人物传记资料库：王福榮（CBDB 318696）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=318696&o=json
            external_identifier: CBDB:318696
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:20.012Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_qgyobmyb799fuYWKmi1xb2
        subject_person_id: p_659gpM9wCpYQqW2bC9Lr2T
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王福榮，明人物。天順四年進士，曾任倉副使。（中国历代人物传记资料库 CBDB 318696）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_3IAbfm6sM2Oq11hx-FWA5r
          claim_id: c_qgyobmyb799fuYWKmi1xb2
          source_id: s_1pb5X8kEN9TPVKRZ9FnArJ
          stance: supports
          locator: CBDB:318696
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants:
    - claim:
        id: c_ia0VeTq6TIBY3BDOhK8GkH
        subject_person_id: p_659gpM9wCpYQqW2bC9Lr2T
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_FXTUX1KirgmPw4nA2wx9vP
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_by3bcwNh2dXn1ywhH9Bx9f
          claim_id: c_ia0VeTq6TIBY3BDOhK8GkH
          source_id: s_1pb5X8kEN9TPVKRZ9FnArJ
          stance: supports
          locator: 天順四年進士登科錄:一卷，第三甲第四十五名：孫
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_FXTUX1KirgmPw4nA2wx9vP
        status: active
        display_name: 王範
        merged_into_person_id: null
  other: []
---

# 王福榮

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王福榮 | accepted |
| bio.summary | 王福榮，明人物。天順四年進士，曾任倉副使。（中国历代人物传记资料库 CBDB 318696） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| descendants | p_FXTUX1KirgmPw4nA2wx9vP | 王範 | accepted |

## 外部来源

- [中国历代人物传记资料库：王福榮（CBDB 318696）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=318696&o=json)
