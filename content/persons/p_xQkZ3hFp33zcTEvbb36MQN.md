---
schema: wang-person/v1
id: p_xQkZ3hFp33zcTEvbb36MQN
status: active
merged_into: null
display_name: 蘇氏
revision: 1
cbdb_id: 683896
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_Fu1sqJ_fw6Kdfa8J8qBHz9
        subject_person_id: p_xQkZ3hFp33zcTEvbb36MQN
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 蘇氏，宋人物。籍贯長子。（中国历代人物传记资料库 CBDB 683896）
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_BC2jFy5Frso0WKSCIBdMbN
          claim_id: c_Fu1sqJ_fw6Kdfa8J8qBHz9
          source_id: s_QeEms9U0yT6vr-xkMRV_l2
          stance: supports
          locator: CBDB:683896
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_QeEms9U0yT6vr-xkMRV_l2
            source_type: api_record
            title: 中国历代人物传记资料库：蘇氏(王茂欽妻)（CBDB 683896）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=683896&o=json
            external_identifier: CBDB:683896
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_9fCKJul4_ZZk61K4RpTVz3
        subject_person_id: p_xQkZ3hFp33zcTEvbb36MQN
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 蘇氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_7pn4RRgH94uilsPMP7vIYm
          claim_id: c_9fCKJul4_ZZk61K4RpTVz3
          source_id: s_QeEms9U0yT6vr-xkMRV_l2
          stance: supports
          locator: CBDB:683896
          quotation: null
          interpretation_note: CBDB 明确记录的王茂欽配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_H4XFBlsvoetSI6T9tZeSNF
        subject_person_id: p_UPHfkGqotrdZukuBX5GD3d
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_xQkZ3hFp33zcTEvbb36MQN
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_RATrbnvtpvFPAUJ9Y9NUU5
          claim_id: c_H4XFBlsvoetSI6T9tZeSNF
          source_id: s_QeEms9U0yT6vr-xkMRV_l2
          stance: supports
          locator: 新出宋代墓志碑刻輯錄，王章、王虔、王珣墓誌：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_UPHfkGqotrdZukuBX5GD3d
        status: active
        display_name: 王茂欽
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 蘇氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 蘇氏，宋人物。籍贯長子。（中国历代人物传记资料库 CBDB 683896） | accepted |
| name.primary | 蘇氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_UPHfkGqotrdZukuBX5GD3d | 王茂欽 | accepted |

## 外部来源

- [中国历代人物传记资料库：蘇氏(王茂欽妻)（CBDB 683896）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=683896&o=json)
