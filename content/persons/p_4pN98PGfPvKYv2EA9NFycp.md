---
schema: wang-person/v1
id: p_4pN98PGfPvKYv2EA9NFycp
status: active
merged_into: null
display_name: 石錦繡
revision: 1
cbdb_id: 120523
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_hf9YGYt4nsKBMQBcQMaQki
        subject_person_id: p_4pN98PGfPvKYv2EA9NFycp
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 石錦繡，清人物。籍贯會稽。（中国历代人物传记资料库 CBDB 120523）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_i05cmwEsDNQWmNLUoXKVYb
          claim_id: c_hf9YGYt4nsKBMQBcQMaQki
          source_id: s_7uvfJLwHWWPfTzPcZkWo9q
          stance: supports
          locator: CBDB:120523
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_7uvfJLwHWWPfTzPcZkWo9q
            source_type: api_record
            title: 中国历代人物传记资料库：石錦繡（CBDB 120523）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=120523&o=json
            external_identifier: CBDB:120523
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_FtzP8AFU4HGDReK-5KFZ9B
        subject_person_id: p_4pN98PGfPvKYv2EA9NFycp
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 石錦繡
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_WaS6GVxA1G4Qk4dfQlp_UR
          claim_id: c_FtzP8AFU4HGDReK-5KFZ9B
          source_id: s_7uvfJLwHWWPfTzPcZkWo9q
          stance: supports
          locator: CBDB:120523
          quotation: null
          interpretation_note: CBDB 明确记录的王長治配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_sntbmXPTlZBaj8aKtBqSFl
        subject_person_id: p_pG5WKV3i6pashNpAeCVJkC
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_4pN98PGfPvKYv2EA9NFycp
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_XzgioDpe8NoBq29afyNa8V
          claim_id: c_sntbmXPTlZBaj8aKtBqSFl
          source_id: s_7uvfJLwHWWPfTzPcZkWo9q
          stance: supports
          locator: "明清婦女著作數據庫，MQWW PoetID #3656, HuWenKai #270：妻子"
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_pG5WKV3i6pashNpAeCVJkC
        status: active
        display_name: 王長治
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 石錦繡

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 石錦繡，清人物。籍贯會稽。（中国历代人物传记资料库 CBDB 120523） | accepted |
| name.primary | 石錦繡 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_pG5WKV3i6pashNpAeCVJkC | 王長治 | accepted |

## 外部来源

- [中国历代人物传记资料库：石錦繡（CBDB 120523）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=120523&o=json)
