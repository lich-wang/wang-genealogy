---
schema: wang-person/v1
id: p_uCC1DMixuQpxcRtk8o9HAJ
status: active
merged_into: null
display_name: 曹氏
revision: 1
cbdb_id: 385776
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_5qikBo_iO8qc8D7U33Qgqw
        subject_person_id: p_uCC1DMixuQpxcRtk8o9HAJ
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 曹氏，宋人物。曾任淑人。（中国历代人物传记资料库 CBDB 385776）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_rlGIB40OnSpFDIOPaQ2pMa
          claim_id: c_5qikBo_iO8qc8D7U33Qgqw
          source_id: s_GcHjL2yt-y82NmEAhfQuzq
          stance: supports
          locator: CBDB:385776
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_GcHjL2yt-y82NmEAhfQuzq
            source_type: api_record
            title: 中国历代人物传记资料库：曹氏(王師心妻)（CBDB 385776）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=385776&o=json
            external_identifier: CBDB:385776
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_hBzzUhw9fadASLqr2YkMdF
        subject_person_id: p_uCC1DMixuQpxcRtk8o9HAJ
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 曹氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_4leI3X6bK1-RDCwfb2bJKs
          claim_id: c_hBzzUhw9fadASLqr2YkMdF
          source_id: s_GcHjL2yt-y82NmEAhfQuzq
          stance: supports
          locator: CBDB:385776
          quotation: null
          interpretation_note: CBDB 明确记录的王師心配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_v9dsnmSBcscCs_t8s_mz7u
        subject_person_id: p_bkMbqmq6U23rFXaG17Pxzk
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_uCC1DMixuQpxcRtk8o9HAJ
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_O1zpfJqYwDjINQOLPfl9ul
          claim_id: c_v9dsnmSBcscCs_t8s_mz7u
          source_id: s_GcHjL2yt-y82NmEAhfQuzq
          stance: supports
          locator: 全宋文，卷 4781：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_bkMbqmq6U23rFXaG17Pxzk
        status: active
        display_name: 王師心
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 曹氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 曹氏，宋人物。曾任淑人。（中国历代人物传记资料库 CBDB 385776） | accepted |
| name.primary | 曹氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_bkMbqmq6U23rFXaG17Pxzk | 王師心 | accepted |

## 外部来源

- [中国历代人物传记资料库：曹氏(王師心妻)（CBDB 385776）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=385776&o=json)
