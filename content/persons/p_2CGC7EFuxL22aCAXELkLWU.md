---
schema: wang-person/v1
id: p_2CGC7EFuxL22aCAXELkLWU
status: active
merged_into: null
display_name: 王天章
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_QpTcdQkY1Cd2sLwYPhAwSQ
        subject_person_id: p_2CGC7EFuxL22aCAXELkLWU
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王天章
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_T3R9SE9KmjQVYMT6WHL9jK
          claim_id: c_QpTcdQkY1Cd2sLwYPhAwSQ
          source_id: s_qWXG3agM4JN8JWq7pjV7NJ
          stance: supports
          locator: CBDB:316215
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（316215）
          source: &a1
            id: s_qWXG3agM4JN8JWq7pjV7NJ
            source_type: api_record
            title: 中国历代人物传记资料库：王天章（CBDB 316215）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=316215&o=json
            external_identifier: CBDB:316215
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:19.958Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_wHTaSwnt3u2YjJWzY83zN7
        subject_person_id: p_2CGC7EFuxL22aCAXELkLWU
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王天章，明人物。嘉靖三十二年進士，籍贯松滋。（中国历代人物传记资料库 CBDB 316215）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_RrXlc_IQX8p3ivgnT-pdSM
          claim_id: c_wHTaSwnt3u2YjJWzY83zN7
          source_id: s_qWXG3agM4JN8JWq7pjV7NJ
          stance: supports
          locator: CBDB:316215
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_cejZRHXmTHP8h0E-hb2-ib
        subject_person_id: p_2CGC7EFuxL22aCAXELkLWU
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_cUV1C65FDffcbStFoe34YE
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_rIhEXenEQMiQJldXTtLLxF
          claim_id: c_cejZRHXmTHP8h0E-hb2-ib
          source_id: s_qWXG3agM4JN8JWq7pjV7NJ
          stance: supports
          locator: 嘉靖三十二年進士登科錄:一卷，第三甲第十九名：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_cUV1C65FDffcbStFoe34YE
        status: active
        display_name: 王其勤
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王天章

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王天章 | accepted |
| bio.summary | 王天章，明人物。嘉靖三十二年進士，籍贯松滋。（中国历代人物传记资料库 CBDB 316215） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_cUV1C65FDffcbStFoe34YE | 王其勤 | accepted |

## 外部来源

- [中国历代人物传记资料库：王天章（CBDB 316215）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=316215&o=json)
