---
schema: wang-person/v1
id: p_cGef58rhE7bXm1CRCgR3Qt
status: active
merged_into: null
display_name: 王正綱
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_EXf6NW7PgE3234JJJpUFMA
        subject_person_id: p_cGef58rhE7bXm1CRCgR3Qt
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王正綱
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_1gqS6qUEJurCSEMiwALze1
          claim_id: c_EXf6NW7PgE3234JJJpUFMA
          source_id: s_Qx8rkcxqMcWQr7ib8KhZC7
          stance: supports
          locator: CBDB:27696
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（27696）
          source: &a1
            id: s_Qx8rkcxqMcWQr7ib8KhZC7
            source_type: api_record
            title: 中国历代人物传记资料库：王正綱（CBDB 27696）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=27696&o=json
            external_identifier: CBDB:27696
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:11.958Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_dRmFJbJeGXfTpN6UtD1Uko
        subject_person_id: p_cGef58rhE7bXm1CRCgR3Qt
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王正綱，宋人物。籍贯如皋，入仕進士。（中国历代人物传记资料库 CBDB 27696）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_gsEtcyLHNR1l_BPYUhp72A
          claim_id: c_dRmFJbJeGXfTpN6UtD1Uko
          source_id: s_Qx8rkcxqMcWQr7ib8KhZC7
          stance: supports
          locator: CBDB:27696
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_cIfcyARpE6nykoKPQhfrXm
        subject_person_id: p_JEp5M3BLJajMXRjvLCUuVy
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_cGef58rhE7bXm1CRCgR3Qt
        generation_count: 4
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs__xYysvfNEWbxJfMwPXjtyz
          claim_id: c_cIfcyARpE6nykoKPQhfrXm
          source_id: s_Qx8rkcxqMcWQr7ib8KhZC7
          stance: supports
          locator: CBDB 双向互证（高祖;四世祖 王惟熙 ⇄ 玄孫;四世孫 王正綱）
          quotation: null
          interpretation_note: Luna 在线复核：CBDB 底本未知，经 Wikidata 双向核对后采纳。
          source: *a1
      object_person:
        id: p_JEp5M3BLJajMXRjvLCUuVy
        status: active
        display_name: 王惟熙
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王正綱

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王正綱 | accepted |
| bio.summary | 王正綱，宋人物。籍贯如皋，入仕進士。（中国历代人物传记资料库 CBDB 27696） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| ancestors | p_JEp5M3BLJajMXRjvLCUuVy | 王惟熙 | accepted |

## 外部来源

- [中国历代人物传记资料库：王正綱（CBDB 27696）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=27696&o=json)
