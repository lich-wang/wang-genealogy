---
schema: wang-person/v1
id: p_bihchq7ihoPQFxDPNdJBwa
status: active
merged_into: null
display_name: 王庭老
cbdb_id: 1887
revision: 2
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_AHBLPJ8QFHSG68DkE8hExA
        subject_person_id: p_bihchq7ihoPQFxDPNdJBwa
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王庭老，史料所见人物。本项目依据《中国历代人物传记资料库：王庭老（CBDB 1887）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 6
      sources:
        - id: cs_ZJrRvyT-xDjMjIZMlbdLaH
          claim_id: c_AHBLPJ8QFHSG68DkE8hExA
          source_id: s_L28svGF5thkJ5X4U3Vkr5B
          stance: supports
          locator: CBDB:1887
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_L28svGF5thkJ5X4U3Vkr5B
            source_type: api_record
            title: 中国历代人物传记资料库：王庭老（CBDB 1887）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=1887&o=json
            external_identifier: CBDB:1887
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:43.132Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_SqCSQ1XRRstwzpkoh56Ui5
        subject_person_id: p_bihchq7ihoPQFxDPNdJBwa
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王庭老
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_Jid7fzeENJC7E3bZ2BEVZa
          claim_id: c_SqCSQ1XRRstwzpkoh56Ui5
          source_id: s_L28svGF5thkJ5X4U3Vkr5B
          stance: supports
          locator: CBDB:1887
          quotation: null
          interpretation_note: CBDB 2026-08-29 Luna 审核通过人物批量导入（801-900）｜历史性依据：CBDB 朝代 = 宋
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_RT014RBxjOJGGPf_Xy_jNS
        subject_person_id: p_MTk1zakKvoc4HW2SfG5Ese
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_bihchq7ihoPQFxDPNdJBwa
        generation_count: 2
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_W9BaaxX10Hg-6J_j-4AKUc
          claim_id: c_RT014RBxjOJGGPf_Xy_jNS
          source_id: s_L28svGF5thkJ5X4U3Vkr5B
          stance: supports
          locator: CBDB 双向互证（祖父 王瀆 ⇄ 孫 王庭老）
          quotation: null
          interpretation_note: Luna 在线复核：CBDB 底本未知，经 Wikidata 双向核对后采纳。
          source:
            id: s_L28svGF5thkJ5X4U3Vkr5B
            source_type: api_record
            title: 中国历代人物传记资料库：王庭老（CBDB 1887）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            published_at_text: null
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=1887&o=json
            external_identifier: CBDB:1887
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-06T01:16:43.132Z
            metadata_json:
              source_release: CBDB 2026-08-29
      object_person:
        id: p_MTk1zakKvoc4HW2SfG5Ese
        status: active
        display_name: 王渎
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王庭老

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王庭老，史料所见人物。本项目依据《中国历代人物传记资料库：王庭老（CBDB 1887）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 王庭老 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| ancestors | p_MTk1zakKvoc4HW2SfG5Ese | 王渎 | accepted |

## 外部来源

- [中国历代人物传记资料库：王庭老（CBDB 1887）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=1887&o=json)
