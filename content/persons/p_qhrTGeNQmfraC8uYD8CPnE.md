---
schema: wang-person/v1
id: p_qhrTGeNQmfraC8uYD8CPnE
status: active
merged_into: null
display_name: 許飛雲
revision: 1
cbdb_id: 56214
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c__77RB0q77htm7kNByXzQ5M
        subject_person_id: p_qhrTGeNQmfraC8uYD8CPnE
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 許飛雲，清人物。籍贯吳縣。（中国历代人物传记资料库 CBDB 56214）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_i0zp_XlgauMAxGa7kifx8K
          claim_id: c__77RB0q77htm7kNByXzQ5M
          source_id: s_kIkqfgseAIGMsd4we3kbCm
          stance: supports
          locator: CBDB:56214
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: &a1
            id: s_kIkqfgseAIGMsd4we3kbCm
            source_type: api_record
            title: 中国历代人物传记资料库：許飛雲（CBDB 56214）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=56214&o=json
            external_identifier: CBDB:56214
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_p9XcxV9b2yMFOsCdGi7m0p
        subject_person_id: p_qhrTGeNQmfraC8uYD8CPnE
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 許飛雲
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_TklftWyzIHzx0Nyzc4oFgz
          claim_id: c_p9XcxV9b2yMFOsCdGi7m0p
          source_id: s_kIkqfgseAIGMsd4we3kbCm
          stance: supports
          locator: CBDB:56214
          quotation: null
          interpretation_note: CBDB 明确记录的王又溟配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_Ha51gw34a84Dy1U7eb6ZdX
        subject_person_id: p_ygW4DDucdKk8PecoLSKG3k
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_qhrTGeNQmfraC8uYD8CPnE
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_Fon7hlyzvufBKDxQljQNJw
          claim_id: c_Ha51gw34a84Dy1U7eb6ZdX
          source_id: s_kIkqfgseAIGMsd4we3kbCm
          stance: supports
          locator: "明清婦女著作數據庫，MQWW PoetID #662, HuWenKai #561：妻子"
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_ygW4DDucdKk8PecoLSKG3k
        status: active
        display_name: 王又溟
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 許飛雲

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 許飛雲，清人物。籍贯吳縣。（中国历代人物传记资料库 CBDB 56214） | accepted |
| name.primary | 許飛雲 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_ygW4DDucdKk8PecoLSKG3k | 王又溟 | accepted |

## 外部来源

- [中国历代人物传记资料库：許飛雲（CBDB 56214）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=56214&o=json)
