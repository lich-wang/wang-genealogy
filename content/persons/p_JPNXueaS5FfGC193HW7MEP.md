---
schema: wang-person/v1
id: p_JPNXueaS5FfGC193HW7MEP
status: active
merged_into: null
display_name: 劉氏
revision: 1
cbdb_id: 148705
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_km6_RhUu3bdy9IA2ODCaiU
        subject_person_id: p_JPNXueaS5FfGC193HW7MEP
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉氏，史料所见人物。本项目依据《中国历代人物传记资料库：劉氏(王儉妻)（CBDB 148705）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_W28z6hgda-o6PS0G39MjNK
          claim_id: c_km6_RhUu3bdy9IA2ODCaiU
          source_id: s_rf1z7rROowQUAVDNN1KRVv
          stance: supports
          locator: CBDB:148705
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 source。
          source: &a1
            id: s_rf1z7rROowQUAVDNN1KRVv
            source_type: api_record
            title: 中国历代人物传记资料库：劉氏(王儉妻)（CBDB 148705）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=148705&o=json
            external_identifier: CBDB:148705
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-11T06:08:33.163Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_SXEVW-TFxi_ZeGg603nThU
        subject_person_id: p_JPNXueaS5FfGC193HW7MEP
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 劉氏
          language: zh-Hant
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_PVPk-LKg-VhYQAZhBvl2d5
          claim_id: c_SXEVW-TFxi_ZeGg603nThU
          source_id: s_rf1z7rROowQUAVDNN1KRVv
          stance: supports
          locator: CBDB:148705
          quotation: null
          interpretation_note: CBDB 明确记录的王儉配偶
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses:
    - claim:
        id: c_42CgxMRKZ9-6K3caQAqYWN
        subject_person_id: p_eiYNhoFHbSoqLPmP4X3HPw
        claim_kind: relationship
        predicate: kinship.spouse_of
        object_person_id: p_JPNXueaS5FfGC193HW7MEP
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_XVGiF13YMVtPzfbrpr8V9j
          claim_id: c_42CgxMRKZ9-6K3caQAqYWN
          source_id: s_rf1z7rROowQUAVDNN1KRVv
          stance: supports
          locator: 唐代墓誌匯編:二卷，Xianheng 93：妻子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_eiYNhoFHbSoqLPmP4X3HPw
        status: active
        display_name: 王儉
        merged_into_person_id: null
  ancestors: []
  descendants: []
  other: []
---

# 劉氏

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 劉氏，史料所见人物。本项目依据《中国历代人物传记资料库：劉氏(王儉妻)（CBDB 148705）》所载的独立记录收录其姓名；目前可核实的信息仍较有限，生卒年代、籍贯与具体经历有待更多可靠来源补充。 | accepted |
| name.primary | 劉氏 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| spouses | p_eiYNhoFHbSoqLPmP4X3HPw | 王儉 | accepted |

## 外部来源

- [中国历代人物传记资料库：劉氏(王儉妻)（CBDB 148705）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=148705&o=json)
