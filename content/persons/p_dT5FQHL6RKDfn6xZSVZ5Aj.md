---
schema: wang-person/v1
id: p_dT5FQHL6RKDfn6xZSVZ5Aj
status: active
merged_into: null
display_name: 王寵
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_EEjaBL1ECN26NRMNpyasNj
        subject_person_id: p_dT5FQHL6RKDfn6xZSVZ5Aj
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王寵
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_5T9NMStsZZA9myTC82V5Ep
          claim_id: c_EEjaBL1ECN26NRMNpyasNj
          source_id: s_2Z1tc71teL5KbYAKYiGQDT
          stance: supports
          locator: CBDB:148000
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（148000）
          source: &a1
            id: s_2Z1tc71teL5KbYAKYiGQDT
            source_type: api_record
            title: 中国历代人物传记资料库：王寵（CBDB 148000）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=148000&o=json
            external_identifier: CBDB:148000
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:15.749Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_4v6VoS8R7JYEBTWhkGQXjF
        subject_person_id: p_dT5FQHL6RKDfn6xZSVZ5Aj
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王寵，唐人物。籍贯北市。（中国历代人物传记资料库 CBDB 148000）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_Xho_rUjnCvdxZ2N-dvt81s
          claim_id: c_4v6VoS8R7JYEBTWhkGQXjF
          source_id: s_2Z1tc71teL5KbYAKYiGQDT
          stance: supports
          locator: CBDB:148000
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_QuokLtJ3_rTXt1aEmRgkZp
        subject_person_id: p_dT5FQHL6RKDfn6xZSVZ5Aj
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_H4W2Q8y4hDuLu6zz75hDFe
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_94_OeJYooEE5rTv6RdjrDQ
          claim_id: c_QuokLtJ3_rTXt1aEmRgkZp
          source_id: s_2Z1tc71teL5KbYAKYiGQDT
          stance: supports
          locator: 唐代墓誌匯編:二卷，Linde 24：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_H4W2Q8y4hDuLu6zz75hDFe
        status: active
        display_name: 王達
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王寵

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王寵 | accepted |
| bio.summary | 王寵，唐人物。籍贯北市。（中国历代人物传记资料库 CBDB 148000） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_H4W2Q8y4hDuLu6zz75hDFe | 王達 | accepted |

## 外部来源

- [中国历代人物传记资料库：王寵（CBDB 148000）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=148000&o=json)
