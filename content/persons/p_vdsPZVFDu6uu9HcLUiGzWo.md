---
schema: wang-person/v1
id: p_vdsPZVFDu6uu9HcLUiGzWo
status: active
merged_into: null
display_name: 王亮
revision: 1
properties:
  - predicate: bio.summary
    recommended:
      claim:
        id: c_GykMMcddR72iZv2B3ZrzZY
        subject_person_id: p_vdsPZVFDu6uu9HcLUiGzWo
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王亮（5世纪—510年），琅邪临沂人，南朝人。维基数据以独立条目 Q11572903 收录该人物；当前资料页据此确认其身份，其他生平细节仍待可靠史料补充。
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 5
      sources:
        - id: cs_mTzag5-x5xJu543HvAQkLy
          claim_id: c_GykMMcddR72iZv2B3ZrzZY
          source_id: s_LPRKxSnkjAdksMP14r5Wkj
          stance: supports
          locator: Q11572903
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 wikidata。
          source: &a1
            id: s_LPRKxSnkjAdksMP14r5Wkj
            source_type: api_record
            title: 维基数据：王亮（Q11572903）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q11572903
            external_identifier: Q11572903
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:25:29.260Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E4%BA%AE_(%E5%8D%97%E6%9C%9D)
      object_person: null
    alternatives: []
  - predicate: birth.date
    recommended:
      claim:
        id: c_5xbKjwv4ejC1yzcteCi3vG
        subject_person_id: p_vdsPZVFDu6uu9HcLUiGzWo
        claim_kind: property
        predicate: birth.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 5世纪
            calendar_note: 维基数据 P569 结构化日期，精度：世纪
            earliest: 0401-01-01
            latest: 0500-12-31
            precision: century
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_4rrnJ2DsrC2TfqsuzsDNvg
          claim_id: c_5xbKjwv4ejC1yzcteCi3vG
          source_id: s_LPRKxSnkjAdksMP14r5Wkj
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_LPRKxSnkjAdksMP14r5Wkj
            source_type: api_record
            title: 维基数据：王亮（Q11572903）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q11572903
            external_identifier: Q11572903
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:25:29.260Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E4%BA%AE_(%E5%8D%97%E6%9C%9D)
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_k4EwgDgVQDojQzK6bEKoQZ
        subject_person_id: p_vdsPZVFDu6uu9HcLUiGzWo
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 510年
            calendar_note: 维基数据 P570 结构化日期，精度：年
            earliest: 0510-01-01
            latest: 0510-12-31
            precision: year
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_sBZCjgWoNnShGH2phBiepw
          claim_id: c_k4EwgDgVQDojQzK6bEKoQZ
          source_id: s_LPRKxSnkjAdksMP14r5Wkj
          stance: supports
          locator: null
          quotation: null
          interpretation_note: null
          source:
            id: s_LPRKxSnkjAdksMP14r5Wkj
            source_type: api_record
            title: 维基数据：王亮（Q11572903）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q11572903
            external_identifier: Q11572903
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:25:29.260Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E4%BA%AE_(%E5%8D%97%E6%9C%9D)
      object_person: null
    alternatives: []
  - predicate: name.primary
    recommended:
      claim:
        id: c_My3a9G3JkjKKQvtLm92CLe
        subject_person_id: p_vdsPZVFDu6uu9HcLUiGzWo
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王亮
          language: zh-Hans
        status: accepted
        confidence: high
        current_revision: 2
      sources:
        - id: cs_9Bw1kE6jFKnumPE628EbpJ
          claim_id: c_My3a9G3JkjKKQvtLm92CLe
          source_id: s_LPRKxSnkjAdksMP14r5Wkj
          stance: supports
          locator: Q11572903
          quotation: null
          interpretation_note: null
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents:
    - claim:
        id: c_FKFHFFSeirfeVoBDWSS3Eb
        subject_person_id: p_48BB9G5rFGngD1z7yugBB9
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_vdsPZVFDu6uu9HcLUiGzWo
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 3
      sources:
        - id: cs_yxWx4GW2A7pgmLDFYx8Q4H
          claim_id: c_FKFHFFSeirfeVoBDWSS3Eb
          source_id: s_8k1YrcbGt4jZKeJcY98o6y
          stance: supports
          locator: P40（子女）
          quotation: null
          interpretation_note: null
          source:
            id: s_8k1YrcbGt4jZKeJcY98o6y
            source_type: api_record
            title: 维基数据：王攸（Q22814746）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q22814746
            external_identifier: Q22814746
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:17:56.458Z
            metadata_json: null
        - id: cs_5YKJCjrb9n73CAPaepuQTv
          claim_id: c_FKFHFFSeirfeVoBDWSS3Eb
          source_id: s_LPRKxSnkjAdksMP14r5Wkj
          stance: supports
          locator: P22（父）
          quotation: null
          interpretation_note: null
          source:
            id: s_LPRKxSnkjAdksMP14r5Wkj
            source_type: api_record
            title: 维基数据：王亮（Q11572903）
            creator: 维基数据贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://www.wikidata.org/wiki/Q11572903
            external_identifier: Q11572903
            license_code: CC0-1.0
            accessed_at: 2026-08-24T03:25:29.260Z
            metadata_json:
              zh_wikipedia: https://zh.wikipedia.org/wiki/%E7%8E%8B%E4%BA%AE_(%E5%8D%97%E6%9C%9D)
        - id: cs_WhMaFa8y69k9PG8fWED8tB
          claim_id: c_FKFHFFSeirfeVoBDWSS3Eb
          source_id: s_MGBoig692jprej3f8uiDi2
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王攸 → 王亮
          interpretation_note: null
          source:
            id: s_MGBoig692jprej3f8uiDi2
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:58:59.171Z
            metadata_json: null
        - id: cs_pFATyPVferTtG4XCyuQa9d
          claim_id: c_FKFHFFSeirfeVoBDWSS3Eb
          source_id: s_xEBYVW3xjFzLnp9P7pHZ2L
          stance: supports
          locator: 条文：世系圖
          quotation: 琅邪王氏世系圖 世系圖：王攸 → 王亮
          interpretation_note: null
          source:
            id: s_xEBYVW3xjFzLnp9P7pHZ2L
            source_type: website
            title: 中文维基百科：琅邪王氏世系圖
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T03:02:22.297Z
            metadata_json: null
        - id: cs_cL4rH54mTnCPdFgqbVQd23
          claim_id: c_FKFHFFSeirfeVoBDWSS3Eb
          source_id: s_GJbx2t6HVns7rAQw4jZtDx
          stance: supports
          locator: 条文：条文识读（父）
          quotation: 父王攸，曾任南朝宋太宰中郎，赠给事黄门侍郎。
          interpretation_note: null
          source:
            id: s_GJbx2t6HVns7rAQw4jZtDx
            source_type: website
            title: 中文维基百科：王亮 (南朝)
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E4%BA%AE_(%E5%8D%97%E6%9C%9D)
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:53:52.061Z
            metadata_json: null
      object_person:
        id: p_48BB9G5rFGngD1z7yugBB9
        status: active
        display_name: 王攸
        merged_into_person_id: null
  children: []
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors:
    - claim:
        id: c_HKc6FXxA9E8oCVrt716re8
        subject_person_id: p_Xbn65bXVofZnsi76cqAQCP
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_vdsPZVFDu6uu9HcLUiGzWo
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_zS8caZoovAYcvwHfrTqk8J
          claim_id: c_HKc6FXxA9E8oCVrt716re8
          source_id: s_njkP3TkbVR716749ZHKECt
          stance: supports
          locator: 条文：世孙（六世）
          quotation: 王亮（{{bd|5世纪||510年|}}），字奉叔，琅邪临沂人，南朝人，晋丞相王导的六世孙
          interpretation_note: null
          source:
            id: s_njkP3TkbVR716749ZHKECt
            source_type: website
            title: 中文维基百科：王亮 (南朝)
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E4%BA%AE_(%E5%8D%97%E6%9C%9D)
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T02:26:10.712Z
            metadata_json: null
        - id: cs_ZMnGNpTzKsQj57bpS4Gnqp
          claim_id: c_HKc6FXxA9E8oCVrt716re8
          source_id: s_GJbx2t6HVns7rAQw4jZtDx
          stance: supports
          locator: 条文：世孙（六世）
          quotation: 王亮，字奉叔，琅邪临沂人，南朝人，晋丞相王导的六世孙
          interpretation_note: null
          source:
            id: s_GJbx2t6HVns7rAQw4jZtDx
            source_type: website
            title: 中文维基百科：王亮 (南朝)
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E4%BA%AE_(%E5%8D%97%E6%9C%9D)
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:53:52.061Z
            metadata_json: null
      object_person:
        id: p_Xbn65bXVofZnsi76cqAQCP
        status: active
        display_name: 王導
        merged_into_person_id: null
    - claim:
        id: c_rY87Say8tGVodobqh8Dgsy
        subject_person_id: p_zzsFuAEs6JJ4EJkXaf48Mn
        claim_kind: relationship
        predicate: kinship.ancestor_of
        object_person_id: p_vdsPZVFDu6uu9HcLUiGzWo
        generation_count: null
        parent_role: null
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_qG86o5aB89dT6NmRbGREbC
          claim_id: c_rY87Say8tGVodobqh8Dgsy
          source_id: s_GJbx2t6HVns7rAQw4jZtDx
          stance: supports
          locator: 条文：条文识读（祖）（2世）
          quotation: 祖父王偃，曾任职南朝宋右光禄大夫，赠开府仪同三司。
          interpretation_note: null
          source:
            id: s_GJbx2t6HVns7rAQw4jZtDx
            source_type: website
            title: 中文维基百科：王亮 (南朝)
            creator: 维基百科贡献者
            publisher: Wikimedia Foundation
            published_at_text: null
            canonical_url: https://zh.wikipedia.org/wiki/%E7%8E%8B%E4%BA%AE_(%E5%8D%97%E6%9C%9D)
            external_identifier: null
            license_code: CC-BY-SA-4.0
            accessed_at: 2026-08-25T09:53:52.061Z
            metadata_json: null
      object_person:
        id: p_zzsFuAEs6JJ4EJkXaf48Mn
        status: active
        display_name: 王偃
        merged_into_person_id: null
  descendants: []
  other: []
---

# 王亮

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| bio.summary | 王亮（5世纪—510年），琅邪临沂人，南朝人。维基数据以独立条目 Q11572903 收录该人物；当前资料页据此确认其身份，其他生平细节仍待可靠史料补充。 | accepted |
| birth.date | 5世纪 | accepted |
| death.date | 510年 | accepted |
| name.primary | 王亮 | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| parents | p_48BB9G5rFGngD1z7yugBB9 | 王攸 | accepted |
| ancestors | p_Xbn65bXVofZnsi76cqAQCP | 王導 | accepted |
| ancestors | p_zzsFuAEs6JJ4EJkXaf48Mn | 王偃 | accepted |

## 外部来源

- [维基数据：王亮（Q11572903）](https://www.wikidata.org/wiki/Q11572903)
- [维基数据：王攸（Q22814746）](https://www.wikidata.org/wiki/Q22814746)
- [中文维基百科：琅邪王氏世系圖](https://zh.wikipedia.org/wiki/%E7%90%85%E9%82%AA%E7%8E%8B%E6%B0%8F%E4%B8%96%E7%B3%BB%E5%9C%96)
- [中文维基百科：王亮 (南朝)](https://zh.wikipedia.org/wiki/%E7%8E%8B%E4%BA%AE_(%E5%8D%97%E6%9C%9D))
