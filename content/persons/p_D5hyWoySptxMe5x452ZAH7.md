---
schema: wang-person/v1
id: p_D5hyWoySptxMe5x452ZAH7
status: active
merged_into: null
display_name: 王遷
revision: 2
properties:
  - predicate: name.primary
    recommended:
      claim:
        id: c_VxWt4zMF2juvpSF8hAG18j
        subject_person_id: p_D5hyWoySptxMe5x452ZAH7
        claim_kind: property
        predicate: name.primary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王遷
          language: zh-Hant
        status: accepted
        confidence: high
        current_revision: 1
      sources:
        - id: cs_NJBQqt2EMwbzaCc2MKmzie
          claim_id: c_VxWt4zMF2juvpSF8hAG18j
          source_id: s_jRuBtnPHzmBYrY3HNazUgn
          stance: supports
          locator: CBDB:169212
          quotation: null
          interpretation_note: CBDB 明确记录的王姓历史人物（169212）
          source: &a1
            id: s_jRuBtnPHzmBYrY3HNazUgn
            source_type: api_record
            title: 中国历代人物传记资料库：王遷（CBDB 169212）
            creator: China Biographical Database (CBDB)
            publisher: Harvard University / Academia Sinica / Peking University
            canonical_url: https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=169212&o=json
            external_identifier: CBDB:169212
            license_code: CC-BY-NC-SA-4.0
            accessed_at: 2026-09-09T00:03:16.012Z
            metadata_json:
              source_release: CBDB 2026-09-05
      object_person: null
    alternatives: []
  - predicate: death.date
    recommended:
      claim:
        id: c_mnH7bSDWR4JRNU8FLEy29f
        subject_person_id: p_D5hyWoySptxMe5x452ZAH7
        claim_kind: property
        predicate: death.date
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          date:
            original_text: 871年
            calendar_note: CBDB 卒年字段，精度：年
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_uUi5Nk5K65aX6Qf4h9hJja
          claim_id: c_mnH7bSDWR4JRNU8FLEy29f
          source_id: s_jRuBtnPHzmBYrY3HNazUgn
          stance: supports
          locator: null
          quotation: null
          interpretation_note: CBDB 卒年
          source: *a1
      object_person: null
    alternatives: []
  - predicate: bio.summary
    recommended:
      claim:
        id: c_zRsYap72VEuAcdMA42X9He
        subject_person_id: p_D5hyWoySptxMe5x452ZAH7
        claim_kind: property
        predicate: bio.summary
        object_person_id: null
        generation_count: null
        parent_role: null
        value_json:
          text: 王遷（卒于871年），唐人物。籍贯元城。（中国历代人物传记资料库 CBDB 169212）
          language: zh-Hans
        status: accepted
        confidence: medium
        current_revision: 2
      sources:
        - id: cs_UBPmtR9ZfV4T-V3oOzJh0a
          claim_id: c_zRsYap72VEuAcdMA42X9He
          source_id: s_jRuBtnPHzmBYrY3HNazUgn
          stance: supports
          locator: CBDB:169212
          quotation: null
          interpretation_note: 自动补全人物基本介绍；身份由稳定外部标识锁定，信息来源层级为 cbdb。
          source: *a1
      object_person: null
    alternatives: []
relationships:
  parents: []
  children:
    - claim:
        id: c_m26t-A9P-AC5fR24_0RrKR
        subject_person_id: p_D5hyWoySptxMe5x452ZAH7
        claim_kind: relationship
        predicate: kinship.father_of
        object_person_id: p_LZoYNNpmSAN5mhNmAGrUeH
        generation_count: null
        parent_role: father
        value_json: null
        status: accepted
        confidence: medium
        current_revision: 1
      sources:
        - id: cs_HS9axN1I8Qlpbvy6eojp2R
          claim_id: c_m26t-A9P-AC5fR24_0RrKR
          source_id: s_jRuBtnPHzmBYrY3HNazUgn
          stance: supports
          locator: 唐五代人物傳記與社會網絡資料庫(1.0版)：子
          quotation: null
          interpretation_note: Luna 在线复核 CBDB 实时 API 后补全；关系端点按 CBDB ID 唯一映射。
          source: *a1
      object_person:
        id: p_LZoYNNpmSAN5mhNmAGrUeH
        status: active
        display_name: 王楚
        merged_into_person_id: null
  adoptive_parents: []
  adoptive_children: []
  spouses: []
  ancestors: []
  descendants: []
  other: []
---

# 王遷

> 本文件是该人物的权威数据页。修改 front matter 并提交 Pull Request；正文用于人工阅读。

## 基本信息

| 字段 | 内容 | 状态 |
| --- | --- | --- |
| name.primary | 王遷 | accepted |
| death.date | 871年 | accepted |
| bio.summary | 王遷（卒于871年），唐人物。籍贯元城。（中国历代人物传记资料库 CBDB 169212） | accepted |

## 关联关系

| 关系 | 人物 ID | 姓名 | 状态 |
| --- | --- | --- | --- |
| children | p_LZoYNNpmSAN5mhNmAGrUeH | 王楚 | accepted |

## 外部来源

- [中国历代人物传记资料库：王遷（CBDB 169212）](https://cbdb.fas.harvard.edu/cbdbapi/person.php?id=169212&o=json)
