...
- 配偶边只在两端都在本次切片内时返回。
- 匿名 `scope=all` 直接读取最近发布的只读公开快照，避免每次展开全局树都扫描 D1；登录用户仍读取实时数据。普通亲缘切片在 D1 暂时不可用时也会退回快照。响应头 `X-Wang-Data-Source: public-tree-snapshot` 明示来源；快照只含本端点原有的公开人物、关系与引用字段，不含账号、草稿或其他主张正文。
