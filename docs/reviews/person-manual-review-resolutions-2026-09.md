...
## 七、全量待办批处理（2026-09-11）

对 35,711 人审核产生的 6,157 条“待人工”项，按类别上网核验并批量处理：

### 7.1 底本未知的亲属关系（1,678 条）

逐一取端点自身 CBDB 实时记录，查验是否存在反向亲属条目（父↔子、祖↔孙、曾祖↔曾孙、妻↔夫）互证：

- **采纳 642 条**（双向互证成立，已双端物化，locator 标注“CBDB 双向互证”）；
- 611 条此前已存在，无需重复；
- **411 条无反向互证**，缺乏可证底本，留待进一步史料，不物化；
- 14 条为抓取/解析等杂项。

### 7.2 配偶端点（946 条）

- 按 CBDB 实时记录新建最小配偶端点 **940 个**（`name.primary` + `kinship.spouse_of` 双向，仅当 CBDB 双向互证或具名底本成立）；
- 为 6 条已在库的配偶补上缺失的婚姻边。
...
- **2,299 条为误判**：该人物自身已有关联（多为先前批次建立），启发式把本人当作“同名他人”；
- **11 条为真重复**：源为纯 CBDB、无关系，目标为同名同父的既有人物，已全部合并入目标：
  - 王承元 `p_253L…`→`p_qCFGm…`、王裁 `p_2WPrv…`→`p_awcmjv…`、王敬直 `p_EG3J…`/`p_N6Fb…`→`p_H3Gcv…`、王象之 `p_F6q8…`→`p_KvSA…`、王世隆 `p_P7pK…`→`p_yr8f…`、王澤 `p_WJnZ…`→`p_vhZh…`、王直方 `p_WxUE…`→`p_Nm2i…`、王珏 `p_x23W…`→`p_bgBM…`、王師震 `p_XCsY…`→`p_Qcou…`、王正 `p_yq7j…`→`p_zgLS…`。

### 7.4 无 CBDB 身份（1,223 条）

这些人物无 CBDB 权威号，无法经 CBDB 核验；其 Wikidata 亲属 QID 绝大多数不在本库（端点缺失），无法建立关系边，故维持原状，不属 CBDB 审核范围。
