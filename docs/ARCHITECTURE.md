...
发布包同时携带由 D1 导出生成的**公开家族树只读快照**。正常请求仍以 D1 为权威；仅当亲缘端点读取失败时，Worker 才按人物公共 ID 返回所属连通分量的快照。生成器只选择 `active`／`merged` 人物及公开关系 DTO 所需字段，明确排除用户、候选草稿和其他主张正文。该降级层保证临时额度或区域故障时仍可浏览家族树，不参与任何写入。
